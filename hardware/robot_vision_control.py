...
class VisionGuidedController:
    ...
    def _initialize_real_detector(self):
        ...
        try:
            ...
            for detector_name, cascade_path in cascade_paths.items():
                if os.path.exists(cascade_path):
                    ...
                    if not cascade.empty():
                        ...
                        self.logger.info(
                            f"成功加载 {detector_name} 检测器: {cascade_path}"
                        )
                    else:
                        self.logger.warning(
                            f"加载 {detector_name} 检测器失败: 分类器为空"
                        )
                else:
            ...
            if not self.real_detector:
                self.logger.warning(
                    "未成功加载任何真实对象检测器，真实对象检测功能将不可用"
                )
                self.config["use_real_object_detection"] = False
            else:
                self.logger.info(
                    f"真实对象检测器初始化成功，已加载 {len(self.real_detector)} 个检测器"
                )
    ...
    def _start_processing_threads(self):
        ...
        if self.voice_enabled:
            voice_thread = threading.Thread(
                target=self._voice_listening_loop, daemon=True, name="VoiceListening"
            )
            voice_thread.start()
        ...
        command_thread = threading.Thread(
            target=self._command_processing_loop, daemon=True, name="CommandProcessing"
        )
    ...
    def process_visual_frame(self, image_data: np.ndarray) -> List[VisualObject]:
        ...
        try:
            ...
            # 仅使用真实对象检测，不采用任何降级机制
            if (
                self.config.get("use_real_object_detection", False)
                and self.real_detector
            ):
            ...
            for detector_name, cascade in self.real_detector.items():
                ...
                for i, (x, y, w, h) in enumerate(detections):
                    ...
                    if detector_name == "face":
                        class_name = "person"
                        confidence = (
                            0.7 + (w * h) / (gray.shape[0] * gray.shape[1]) * 0.3
                        )
    ...
    def _cleanup_old_history(self):
        ...
        for obj_id in list(self.object_history.keys()):
            ...
            self.object_history[obj_id] = [
                obj
                for obj in self.object_history[obj_id]
        ...
        try:
            ...
            with sr.Microphone() as source:
                ...
                audio = self.voice_recognizer.listen(
                    source, timeout=self.config["voice_timeout"]
                )
    ...
    def _parse_voice_command(self, text: str) -> Optional[VoiceCommand]:
        ...
        # 移动命令
        if any(
            word in text_lower for word in ["向前", "前进", "forward", "go forward"]
        ):
            return VoiceCommand(
                ...
                text=text,
                confidence=0.8,
                parameters={"direction": "forward", "distance": 0.5, "speed": 0.3},
            )

        elif any(
            word in text_lower for word in ["向后", "后退", "backward", "go backward"]
        ):
            return VoiceCommand(
                command_id=command_id,
                command_type=VoiceCommandType.MOVEMENT,
                text=text,
                confidence=0.8,
                parameters={"direction": "backward", "distance": 0.3, "speed": 0.2},
            )
            ...
            return VoiceCommand(
                ...
                text=text,
                confidence=0.8,
                parameters={"direction": "left", "angle": 45.0, "speed": 0.3},
            )

        elif any(
            word in text_lower for word in ["向右", "右转", "turn right", "right"]
        ):
            return VoiceCommand(
                command_id=command_id,
                command_type=VoiceCommandType.MOVEMENT,
                text=text,
                confidence=0.8,
                parameters={"direction": "right", "angle": 45.0, "speed": 0.3},
            )
        ...
        # 手势命令
            ...
            return VoiceCommand(
                ...
                text=text,
                confidence=0.7,
                parameters={"gesture": "wave", "hand": "right", "duration": 2.0},
            )
            ...
            return VoiceCommand(
                ...
                text=text,
                confidence=0.9,
                parameters={"gesture": "stand", "duration": 3.0},
            )
        ...
        # 导航命令
            ...
            return VoiceCommand(
                ...
                text=text,
                confidence=0.6,
                parameters={"action": "go_to", "target": "unknown"},
            )
        ...
        # 默认命令
            ...
            return VoiceCommand(
                ...
                text=text,
                confidence=0.3,
                parameters={"action": "process_text", "text": text},
            )
    ...
    def execute_voice_command(self, command: VoiceCommand) -> bool:
        ...
        try:
            self.logger.info(
                f"执行语音命令: {command.text} (类型: {command.command_type.value})"
            )
        ...
        if self.robot_controller and hasattr(self.robot_controller, "walk_forward"):
            try:
                if direction == "forward":
                    ...
                    success = self.robot_controller.walk_forward(
                        steps=steps, step_length=0.1
                    )
                    if success:
                    ...
                    if hasattr(self.robot_controller, "walk_backward"):
                        steps = max(1, int(distance / 0.1))
                        success = self.robot_controller.walk_backward(
                            steps=steps, step_length=0.1
                        )
                        if success:
        ...
        if gesture == "wave":
            ...
            if self.robot_controller and hasattr(self.robot_controller, "set_pose"):
                try:
                    ...
                    success = self.robot_controller.set_pose(
                        default_pose, duration=duration
                    )
                    if success:
    ...
    def execute_visual_task(
        self, task_type: VisionTaskType, target_object: Optional[str] = None
    ) -> bool:
        """执行视觉任务
        ...
        if target_object:
            for obj_id, obj in self.tracked_objects.items():
                if target_object in obj.class_name.lower():
                    ...
                    self.logger.info(
                        f"导航到 {obj.class_name}: 转向角度 {turn_angle:.1f} 度"
                    )
    ...
    def _execute_visual_manipulation(self, target_object: Optional[str]) -> bool:
        ...
        for obj_id, obj in self.tracked_objects.items():
            if target_object in obj.class_name.lower():
                ...
                self.logger.info(
                    f"操作 {                         obj.class_name}: 位置 ({                         object_center_x:.0f}, {                         object_center_y:.0f})"
                )

                # 这里应该调用实际的机器人操作控制
                # 例如：self.hardware_interface.reach_to_position(object_center_x,
                # object_center_y)
...
def create_vision_guided_controller(
...
def test_vision_guided_control():
    ...
    try:
        ...
        test_commands = ["向前走", "向右转", "挥手打招呼", "去桌子那边"]

        for cmd_text in test_commands:
            command = controller._parse_voice_command(cmd_text)
            if command:
                print(
                    f"命令: '{cmd_text}' -> 类型: {command.command_type.value}, 参数: {command.parameters}"
                )
            else:
