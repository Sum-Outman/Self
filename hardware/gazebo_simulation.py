...
class GazeboSimulation(HardwareInterface):
    ...
    def __init__(
        ...
        self._interface_type = "gazebo_simulation"
        ...
        self._sensor_update_interval = 0.1  # 100ms
        self._stop_sensor_thread = threading.Event()
        
        # 检查roslibpy可用性
        self._check_roslibpy_availability()
        
        self.logger.info(f"Gazebo仿真环境初始化完成: {robot_model}, 世界: {gazebo_world}")
    
    def _check_roslibpy_availability(self) -> None:
        """检查roslibpy库是否可用"""
        try:
            import roslibpy  # type: ignore
            
            # 测试基本功能
            test_ros = hasattr(roslibpy, 'Ros')
            test_topic = hasattr(roslibpy, 'Topic')
            test_service = hasattr(roslibpy, 'Service')
            
            if test_ros and test_topic and test_service:
                self.roslibpy_available = True
                self.logger.info("roslibpy库可用")
            else:
                self.roslibpy_available = False
                self.logger.warning("roslibpy库导入成功但功能不完整")
        
        except ImportError as e:
            self.roslibpy_available = False
            self.logger.warning(f"roslibpy库不可用: {e}")
        except Exception as e:
            self.roslibpy_available = False
            self.logger.warning(f"检查roslibpy可用性时出错: {e}")
    ...
    def connect(self) -> bool:
        ...
        if not self.roslibpy_available:
            ...
            try:
                ...
                self.logger.warning(f"检查roslibpy可用性失败: {e}")
            
            if not self.roslibpy_available:
                self.logger.error("roslibpy不可用，无法连接Gazebo仿真环境")
                # 提供安装指导但不直接报错，允许系统继续运行
                self.logger.info("请安装roslibpy库: pip install roslibpy")
                return False
        ...
        try:
            ...
            try:
                # 尝试调用Gazebo服务
                get_world_properties = Service(
                    self.ros_client,
                    "/gazebo/get_world_properties",
                    "gazebo_msgs/GetWorldProperties",
                )
                get_world_properties.call()
            ...
            if self.gazebo_available:
                ...
                self.logger.warning("Gazebo服务不可用，使用纯仿真模式")
                # 初始化仿真关节状态（无Gazebo物理引擎）
                self._initialize_joint_states()
                self._initialize_sensors()
                ...
                self.connected = True
                self.logger.info("Gazebo仿真环境连接成功（纯仿真模式）")
                return True

        except Exception as e:
            self.logger.error(f"连接Gazebo仿真环境失败: {e}")
            # 记录错误但不抛出异常，允许系统继续运行其他功能
            self.logger.warning("Gazebo仿真连接失败，系统将继续运行其他功能")
            return False
    ...
    def _connect_simulation_only(self) -> bool:
        ...
        提供纯模拟模式支持。
        """
        self.logger.info("连接纯模拟模式（无Gazebo物理引擎）")
    ...
    def _load_gazebo_world(self):
        ...
        try:
            ...
            # 调用Gazebo服务加载世界
            load_world = Service(
                self.ros_client, "/gazebo/load_world", "gazebo_msgs/LoadWorld"
            )
    ...
    def _load_robot_model(self):
        ...
        try:
            ...
            # 调用Gazebo服务生成机器人
            spawn_model = Service(
                self.ros_client, "/gazebo/spawn_urdf_model", "gazebo_msgs/SpawnModel"
            )

            # 完整版）
            urdf_content = f"""             <?xml version="1.0"?>             <robot name="{self.robot_model}">                 <link name="base_link">                     <inertial>                         <mass value="10.0"/>                         <origin xyz="0 0 0.5"/>                         <inertia ixx="0.1" ixy="0" ixz="0" iyy="0.1" iyz="0" izz="0.1"/>                     </inertial>                     <visual>                         <origin xyz="0 0 0.5"/>                         <geometry>                             <box size="0.2 0.2 1.0"/>                         </geometry>                         <material name="blue">                             <color rgba="0 0 0.8 1"/>                         </material>                     </visual>                     <collision>                         <origin xyz="0 0 0.5"/>                         <geometry>                             <box size="0.2 0.2 1.0"/>                         </geometry>                     </collision>                 </link>             </robot>             """
    ...
    def _simulation_loop(self):
        ...
        try:
            ...
            joint_state_topic = Topic(
                self.ros_client, "/gazebo/joint_states", "sensor_msgs/JointState"
            )
                    ...
                    for i, joint_name in enumerate(message["name"]):
                        ...
                        if joint:
                            state = JointState(
                                ...
                                velocity=(
                                    message["velocity"][i]
                                    if i < len(message["velocity"])
                                    else None
                                ),
                                torque=(
                                    message["effort"][i]
                                    if i < len(message["effort"])
                                    else None
                                ),
                                temperature=None,
                                voltage=None,
    ...
    def plan_path(
        ...
        try:
            self.logger.info(
                f"Gazebo路径规划: {start_position} -> {goal_position}, 算法: {algorithm}"
            )
            ...
            # Gazebo环境中的路径规划
            return self._plan_path_in_gazebo(
                start_position, goal_position, algorithm, grid_size, max_iterations
            )
    ...
    def _plan_path_in_gazebo(
        ...
        self.logger.info(
            f"Gazebo路径规划完成: 路径长度={path_length:.2f}, 时间={computation_time:.3f}s"
        )
    ...
    def _simulate_gazebo_path_planning(
        self, start_position: List[float], goal_position: List[float], algorithm: str
    ) -> Dict[str, Any]:
    ...
    def move_to_position(
        self, target_position: List[float], speed: float = 0.1
    ) -> bool:
        """移动Gazebo机器人到指定位置
        ...
        try:
            self.logger.info(
                f"移动Gazebo机器人到位置: {target_position}, 速度: {speed} m/s"
            )
            ...
            if not path_result.get("success", False):
                self.logger.error(
                    f"Gazebo路径规划失败: {path_result.get('message', '未知错误')}"
                )
                return False
