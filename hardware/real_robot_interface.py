...
class NAOqiRobotInterface(HardwareInterface):
    ...
    def get_interface_info(self) -> Dict[str, Any]:
        ...
        naoqi_info = {
            ...
            "joint_count": (
                len(self._joint_mapping) if hasattr(self, "_joint_mapping") else 0
            ),
            "sensor_cache_size": (
                len(self._sensor_cache) if hasattr(self, "_sensor_cache") else 0
            ),
    ...
    def connect(self) -> bool:
        ...
        try:
            logger.info(
                f"尝试连接到 {                     self.config.robot_type.value} 机器人: {                     self.config.host}:{                     self.config.port}"
            )
            ...
            if proxy_success_count == len(critical_proxies):
                ...
                logger.info(
                    f"部分连接成功，{proxy_success_count}/{len(critical_proxies)} 个关键代理可用"
                )
            ...
            logger.info(
                f"机器人连接{'完全' if not self._partial_connection else '部分'}成功，准备就绪"
            )
            ...
            if self._connected:
                if available_count == total_count:
                    ...
                    health_status["message"] = (
                        f"部分硬件组件可用 ({available_count}/{total_count})"
                    )
                ...
                if available_count > 0:
                    ...
                    health_status["message"] = (
                        f"部分硬件代理可用但未连接 ({available_count}/{total_count})"
                    )
                    health_status["partial_hardware"] = True
                else:
    ...
    def _start_sensor_thread(self):
        ...
        self._connection_thread = threading.Thread(
            target=sensor_collection, daemon=True
        )
    ...
    def _start_safety_monitor(self):
        ...
        self._safety_monitor_thread = threading.Thread(
            target=safety_monitor, daemon=True
        )
        ...
        with self._cache_lock:
            try:
                ...
                if self.memory_proxy is not None:
                    try:
                        # NAOqi v2.8+支持
                        joint_temperatures = self.memory_proxy.getData(
                            "Device/SubDeviceList/Temperature/Sensor/Value"
                        )
                    except Exception as e:
                        # 根据项目要求"不采用任何降级处理，直接报错"
                        error_message = (
    ...
    def _check_safety(self):
        ...
        try:
            ...
            if self.memory_proxy is not None:
                try:
                    # 获取关节电流
                    currents = self.memory_proxy.getData(
                        "Device/SubDeviceList/ElectricCurrent/Sensor/Value"
                    )
                    if isinstance(currents, dict):
                        for joint_name, current in currents.items():
                            if current > 2.0:  # 电流阈值（安培）
                                logger.warning(
                                    f"关节 {joint_name} 电流过高: {current}A"
                                )
                                if current > 3.0:
                                    self.emergency_stop()
                                    return
            ...
            if self.memory_proxy is not None:
                try:
                    battery_level = self.memory_proxy.getData(
    ...
    def get_joint_state(self, joint: RobotJoint) -> Optional[JointState]:
        ...
        try:
            ...
            with self._cache_lock:
                ...
                temp = (
                    joint_temperatures.get(joint_name)
                    if isinstance(joint_temperatures, dict)
                    else None
                )
        ...
        try:
            ...
            with self._cache_lock:
                ...
                for robot_joint, naoqi_name in self._joint_mapping.items():
                    ...
                    temp = (
                        joint_temperatures.get(naoqi_name)
                        if isinstance(joint_temperatures, dict)
                        else None
                    )
...
class ROS2RobotInterface(HardwareInterface):
    ...
    def connect(self) -> bool:
        ...
        try:
            ...
            # 创建ROS2节点
            self.ros2_node = Node(
                f"self_agi_robot_interface_{self.config.robot_type.value}"
            )
            ...
            try:
                self.camera_sub = self.ros2_node.create_subscription(
                    Image, "/camera/color/image_raw", self._camera_callback, 10
                )
    ...
    def disconnect_robot(self, interface_id: str) -> Dict[str, Any]:
        ...
        try:
            with self.lock:
                ...
                return {"success": True, "message": f"已断开接口 {interface_id}"}
...
class UniversalRobotAdapter(HardwareInterface):
    ...
    def __init__(
        self, target_interface: HardwareInterface, robot_config: Dict[str, Any]
    ):
        """
        初始化通用机器人适配器
        ...
        logger.info(
            f"初始化通用机器人适配器，目标接口: {                 target_interface.interface_type}，机器人型号: {                 self.robot_model}"
        )

    def _convert_string_keys_to_robot_joint(
                ...
                if isinstance(key_str, str):
                    ...
                    try:
                        ...
                        for joint in RobotJoint:
                            ...
                            logger.warning(
                                f"无法将字符串 '{key_str}' 转换为RobotJoint枚举，将跳过此键"
                            )
                            continue
        ...
        logger.debug(
            f"将 {len(input_dict)} 个键转换为RobotJoint枚举，成功转换 {len(result)} 个"
        )
        return result
    ...
    def _get_default_geometry_params(self, robot_model: str) -> Dict[str, float]:
        ...
        if "nao" in robot_model_lower:
            ...
            # 未知型号，使用通用人形机器人默认值
            logger.warning(
                f"未知机器人型号 '{robot_model}'，使用通用人形机器人默认尺寸"
            )
            return {
                "upper_arm_length": 0.20,
                "lower_arm_length": 0.20,
    ...
    def get_interface_info(self) -> Dict[str, Any]:
        ...
        joint_mapping_str = {
            k.value if isinstance(k, RobotJoint) else str(k): v
    ...
    def set_joint_position(self, joint: RobotJoint, position: float) -> bool:
        ...
        if joint in self.joint_limits:
            ...
            position = max(
                limits.get("min", -3.14), min(position, limits.get("max", 3.14))
            )
    ...
    def calculate_forward_kinematics(
        ...
        try:
            ...
            self.geometry_params.get("lower_arm_length", 0.15)
            ...
            if end_effector == "right_hand":
                ...
                # 简化运动学计算（基于几何参数）
                # 实际应使用完整的DH参数或URDF模型
                x = shoulder_width / 2 + upper_arm * np.sin(shoulder_pitch) * np.cos(
                    shoulder_roll
                )
