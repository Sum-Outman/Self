...
def get_real_robot_interface_module():
    ...
    if _real_robot_interface_module is None:
        try:
            ...
            _real_robot_interface_module = importlib.import_module(
                ".real_robot_interface", package="hardware"
            )
...
class HardwareInterface(ABC):
    ...
    def get_interface_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "connected": (
                self.is_connected() if hasattr(self, "is_connected") else False
            ),
...
class SerialInterface(HardwareInterface):
    ...
    def set_joint_positions(self, positions: Dict[RobotJoint, float]) -> bool:
        """设置多个关节位置（串口实现）"""
        position_str = " ".join(
            [f"{joint.value}:{pos:.4f}" for joint, pos in positions.items()]
        )
    ...
    def connect(self) -> bool:
        ...
        try:
            ...
            self.joint_publishers["command"] = self.node.create_publisher(
                JointState, f"{self.namespace}/joint_commands", 10
            )
    ...
    def get_sensor_data(self, sensor_type: SensorType) -> Optional[Any]:
        ...
        try:
            ...
            if response_data.get("status") != "success":
                self.logger.error(
                    f"获取传感器数据失败: {response_data.get('error', '未知错误')}"
                )
                return None
            ...
            if sensor_type == SensorType.IMU:
                ...
                try:
                    ...
                    return IMUData(
                        acceleration=np.array(
                            sensor_data["acceleration"], dtype=np.float32
                        ),
                        gyroscope=np.array(sensor_data["gyroscope"], dtype=np.float32),
                        magnetometer=np.array(
                            sensor_data["magnetometer"], dtype=np.float32
                        ),
                        orientation=np.array(
                            sensor_data["orientation"], dtype=np.float32
                        ),
                ...
                if "image_base64" in sensor_data:
                    ...
                    try:
                        ...
                        return CameraData(
                            image=image_array, timestamp=float(sensor_data["timestamp"])
                        )
                    ...
                    self.logger.warning(
                        "摄像头数据格式不支持：既不是image_base64也不是image_array"
                    )
                    return None
            else:
                # 对于其他传感器类型，返回原始数据
...
class HumanoidRobotController:
    ...
    def __init__(
        self, interface: HardwareInterface, enable_advanced_control: bool = False
    ):
        ...
        self.logger.info(
            f"初始化机器人控制器 - 接口类型: {                 interface_info['type']}, 模拟模式: {                 interface_info['simulation']}"
        )
    ...
    def connect(self) -> bool:
        ...
        self.logger.info(
            f"开始连接机器人 - 接口类型: {interface_info['type']}, 模式: {mode_str}"
        )
    ...
    def start_control(self, control_loop: Callable[[], None]):
        ...
        self.control_thread = threading.Thread(
            target=self._control_loop_wrapper, args=(control_loop,), daemon=True
        )
    ...
    def _initialize_advanced_control(self):
        ...
        if ADVANCED_CONTROL_AVAILABLE is None:
            try:
                from .advanced_robot_control import (
                    AdvancedRobotController,
                    ControlAlgorithm,
                    TrajectoryType,
                    ControlParameters,
        ...
        try:
            ...
            # 创建高级控制器
            self.advanced_controller = self._AdvancedRobotController(
                self.enhanced_interface
            )
        ...
        **kwargs,
        ...
        try:
            ...
            if trajectory_type == "linear":
                ...
                trajectory_id = (
                    self.advanced_controller.generate_minimum_jerk_trajectory(
                        joint, start_pos, end_pos, duration
                    )
                )
...
class HardwareManager:
    ...
    def create_pybullet_interface(
        self, name: str = "pybullet", gui_enabled: bool = True, **kwargs
    ) -> bool:
        """创建PyBullet仿真接口
        ...
        try:
            ...
            engine_config = {"gui_enabled": gui_enabled, **kwargs}
            ...
            self.logger.info(
                f"PyBullet仿真接口 {name} 已创建 (使用统一仿真接口, GUI: {gui_enabled})"
            )
            return True
            ...
            try:
                ...
                self.interfaces[name] = simulation
                self.logger.info(
                    f"PyBullet仿真接口 {name} 已创建 (使用原始PyBulletSimulation, GUI: {gui_enabled})"
                )
                return True
    ...
    def create_gazebo_interface(
        ...
        **kwargs,
        ...
        try:
            ...
            self.interfaces[name] = simulation
            self.logger.info(
                f"Gazebo仿真接口 {name} 已创建 (使用统一仿真接口, 世界: {gazebo_world}, 机器人: {robot_model}, GUI: {gui_enabled})"
            )
            return True
            ...
            try:
                ...
                self.interfaces[name] = simulation
                self.logger.info(
                    f"Gazebo仿真接口 {name} 已创建 (使用原始GazeboSimulation, 世界: {gazebo_world}, 机器人: {robot_model}, GUI: {gui_enabled})"
                )
                return True
            except ImportError as e2:
    ...
    def create_ros_interface(
        ...
        try:
            ...
            ros_interface = ROSInterface(
                namespace=namespace, simulation_mode=simulation_mode
            )
    ...
    def create_ros2_interface(
        self, name: str = "ros2_real", config: Dict[str, Any] = None
    ) -> bool:
        """创建真实ROS2机器人接口
    ...
    def create_websocket_interface(
        ...
        try:
            ...
            websocket_interface = WebSocketInterface(
                url=url, simulation_mode=simulation_mode
            )

            # 注册接口
            self.interfaces[name] = websocket_interface
            self.logger.info(
                f"WebSocket接口 {name} 已创建 (URL: {url}, 模拟模式: {simulation_mode})"
            )
            return True
    ...
    def create_i2c_interface(
        ...
        try:
            ...
            i2c_interface = I2CInterface(
                bus=bus, address=address, simulation_mode=simulation_mode
            )

            # 注册接口
            self.interfaces[name] = i2c_interface
            self.logger.info(
                f"I2C接口 {name} 已创建 (总线: {bus}, 地址: 0x{                     address:02x}, 模拟模式: {simulation_mode})"
            )
            return True
    ...
    def create_spi_interface(
        ...
        try:
            ...
            self.logger.info(
                f"SPI接口 {name} 已创建 (总线: {bus}, 设备: {device}, 速度: {max_speed_hz}Hz, 模拟模式: {simulation_mode})"
            )
            return True
    ...
    def create_real_robot_interface(
        self,
        name: str = "real_robot",
        robot_type: str = "nao",
        host: str = "localhost",
        ...
        if not is_real_robot_interface_available():
            self.logger.error(
                "真实机器人硬件接口不可用（请检查real_robot_interface.py导入）"
            )
            return False
        ...
        try:
            ...
            try:
                RobotConnectionConfig = (
                    real_robot_interface_module.RobotConnectionConfig
                )
            ...
            self.interfaces[name] = interface
            self.logger.info(
                f"真实机器人接口 {name} 已创建 (类型: {robot_type}, 主机: {host}:{port}, 连接: {connection_type})"
            )
    ...
    def create_robot_controller(
        self, name: str, interface_name: str, enable_advanced_control: bool = False
    ) -> Optional[HumanoidRobotController]:
        """创建机器人控制器
...
class I2CInterface(HardwareInterface):
    ...
    def __init__(self, bus: int = 1, address: int = 0x68):  # I2C总线编号  # 设备地址
        super().__init__()
        self.bus = bus
    ...
    def connect(self) -> bool:
        ...
        try:
            ...
            self.logger.info(
                f"I2C连接成功 (总线: {self.bus}, 地址: 0x{self.address:02x})"
            )
            self.connected = True
            return True
        except ImportError:
    ...
    def _read_imu_data(self) -> Optional[IMUData]:
        ...
        try:
            ...
            for data_packet in recent_data:
                if (
                    "decode_result" in data_packet
                    and data_packet["decode_result"]["success"]
                ):
                    decoded_data = data_packet["decode_result"]["data"]
                    # 检查是否为IMU数据
                    if isinstance(decoded_data, dict):
                        # 尝试提取IMU数据
                        imu_accel = decoded_data.get(
                            "imu_accelerometer", decoded_data.get("acceleration", None)
                        )
                        imu_gyro = decoded_data.get(
                            "imu_gyroscope", decoded_data.get("gyroscope", None)
                        )
                        ...
                        if all(
                            field is not None
                            for field in [imu_accel, imu_gyro, imu_mag, imu_orientation]
                        ):
                            import time
                            import numpy as np
        ...
        # 真实硬件数据不可用
        self.logger.info(
            "IMU数据：无法获取真实硬件数据，请通过串口数据服务接收实时传感器数据"
        )
        return None
    ...
    def _read_joint_position_data(self) -> Optional[Dict[RobotJoint, JointState]]:
                ...
                if (
                    ...
                    if isinstance(decoded_data, dict):
                        ...
                        for joint in RobotJoint:
                            ...
                            # 获取字段值，不提供默认值
                            position = decoded_data.get(
                                f"{joint_key}_position"
                            ) or decoded_data.get("position")
                            velocity = decoded_data.get(
                                f"{joint_key}_velocity"
                            ) or decoded_data.get("velocity")
                            torque = decoded_data.get(
                                f"{joint_key}_torque"
                            ) or decoded_data.get("torque")
                            temperature = decoded_data.get(
                                f"{joint_key}_temperature"
                            ) or decoded_data.get("temperature")
                            voltage = decoded_data.get(
                                f"{joint_key}_voltage"
                            ) or decoded_data.get("voltage")
                            current = decoded_data.get(
                                f"{joint_key}_current"
                            ) or decoded_data.get("current")
        ...
        # 真实硬件数据不可用
        self.logger.info(
            "关节数据：无法获取真实硬件数据，请通过串口数据服务接收实时关节位置数据"
        )
        return None
...
class SPIInterface(HardwareInterface):
    ...
    def connect(self) -> bool:
        ...
        try:
            ...
            self.logger.info(
                f"SPI连接成功 (总线: {                     self.bus}, 设备: {                     self.device}, 速度: {                     self.max_speed_hz}Hz)"
            )
            self.connected = True
            return True
        except ImportError:
    ...
    def _read_imu_data_spi(self) -> Optional[IMUData]:
            ...
            for data_packet in recent_data:
                if (
                    ...
                    if isinstance(decoded_data, dict):
                        ...
                        protocol = data_packet.get("decode_result", {}).get(
                            "protocol", ""
                        )
                        ...
                        is_spi_data = any(
                            ...
                            or keyword in str(decoded_data).lower()
                        ...
                        if is_spi_data:
                            ...
                            imu_accel = decoded_data.get(
                                "acceleration",
                                decoded_data.get(
                                    "accel", decoded_data.get("imu_accelerometer", None)
                                ),
                            )
                            imu_gyro = decoded_data.get(
                                "gyroscope",
                                decoded_data.get(
                                    "gyro", decoded_data.get("imu_gyroscope", None)
                                ),
                            )
                            imu_mag = decoded_data.get(
                                "magnetometer", decoded_data.get("mag", None)
                            )
                            imu_orientation = decoded_data.get(
                                "orientation", decoded_data.get("euler", None)
                            )
                            ...
                            if imu_accel is not None or imu_gyro is not None:
                                ...
                                return IMUData(
                                    ...
                                    gyroscope=(
                                        ...
                                        else np.array([0.001, 0.002, 0.003])
                                    ),
                                    magnetometer=(
                                        to_array(imu_mag)
        ...
        # 真实硬件数据不可用
        self.logger.info(
            "SPI IMU数据：无法获取真实硬件数据，请通过串口数据服务接收SPI设备转发的实时数据"
        )
        return None

    def _extract_value(
        self, data_dict: Dict[str, Any], keys: List[str], default: Any
    ) -> Any:
        """从字典中提取值
    ...
    def _read_joint_position_data_spi(self) -> Optional[Dict[RobotJoint, JointState]]:
                    ...
                    if isinstance(decoded_data, dict):
                        ...
                        if is_spi_joint_data:
                            ...
                            for joint in RobotJoint:
                                ...
                                # 从数据中提取值
                                position = self._extract_value(
                                    decoded_data, position_keys, 0.0
                                )
                                velocity = self._extract_value(
                                    decoded_data, velocity_keys, 0.0
                                )
                                torque = self._extract_value(
                                    decoded_data, torque_keys, 0.0
                                )
                                temperature = self._extract_value(
                                    decoded_data, temperature_keys, 25.0
                                )
                                voltage = self._extract_value(
                                    decoded_data, voltage_keys, 12.0
                                )
                                current = self._extract_value(
                                    decoded_data, current_keys, 0.1
                                )
        ...
        # 真实硬件数据不可用
        self.logger.info(
            "SPI关节数据：无法获取真实硬件数据，请通过串口数据服务接收SPI编码器转发的实时数据"
        )
        return None
...
class SimulatedRobotInterface(HardwareInterface):
    ...
    def get_sensor_data(self, sensor_type: SensorType) -> Optional[Any]:
        """获取传感器数据（已被禁用）"""
        self.logger.error(
            f"模拟硬件接口已被完全禁用。无法获取传感器数据（类型: {sensor_type.value}）。"
        )
        raise RuntimeError(
            "模拟硬件接口已被完全禁用。请使用真实硬件或物理仿真环境获取传感器数据。"
        )

    def set_joint_position(self, joint: RobotJoint, position: float) -> bool:
        """设置关节位置（已被禁用）"""
        self.logger.error(
            f"模拟硬件接口已被完全禁用。无法设置关节位置（关节: {joint.value}, 位置: {position}）。"
        )
        raise RuntimeError(
            "模拟硬件接口已被完全禁用。请使用真实硬件或物理仿真环境控制关节。"
        )

    def set_joint_positions(self, positions: Dict[RobotJoint, float]) -> bool:
        """设置多个关节位置（已被禁用）"""
        self.logger.error(
            f"模拟硬件接口已被完全禁用。无法设置多个关节位置（关节数: {len(positions)}）。"
        )
        raise RuntimeError(
            "模拟硬件接口已被完全禁用。请使用真实硬件或物理仿真环境控制关节。"
        )
    ...
    def get_joint_state(self, joint: RobotJoint) -> Optional[JointState]:
        """获取关节状态（已被禁用）"""
        self.logger.error(
            f"模拟硬件接口已被完全禁用。无法获取关节状态（关节: {joint.value}）。"
        )
        raise RuntimeError(
            "模拟硬件接口已被完全禁用。请使用真实硬件或物理仿真环境获取关节状态。"
        )
