...
class PyBulletSimulation(HardwareInterface):
    ...
    def __init__(
        ...
        self._interface_type = "pybullet_simulation"
        ...
        # 初始化机器人状态
        self.joint_states = {}  # 关节状态（不带下划线） - 遵循代码规范
        
        # 初始化关节映射和限制
        self.robot_joint_indices = {}  # RobotJoint -> PyBullet关节索引映射
        self.robot_joint_limits = {}   # RobotJoint -> (min, max)限制
        
        # 初始化仿真控制
        self.simulation_running = False
        self.simulation_thread = None
        self._stop_simulation = threading.Event()
        self.simulation_time = 0.0  # 仿真时间
        ...
        # 检查PyBullet可用性
        self._check_pybullet_availability()
        
        self.logger.info(f"PyBullet仿真环境初始化完成: GUI={gui_enabled}, 时间步长={physics_timestep}s")
    
    def _check_pybullet_availability(self) -> None:
        """检查PyBullet库是否可用"""
        try:
            ...
            # 测试基本功能
            test_connect = hasattr(pybullet, 'connect')
            test_urdf = hasattr(pybullet, 'loadURDF')
            test_gravity = hasattr(pybullet, 'setGravity')
            
            if test_connect and test_urdf and test_gravity:
                self.pybullet_available = True
                self.logger.info("PyBullet库可用")
            else:
                self.pybullet_available = False
                self.logger.warning("PyBullet库导入成功但功能不完整")
        
        except ImportError as e:
            self.pybullet_available = False
            self.logger.warning(f"PyBullet库不可用: {e}")
        except Exception as e:
            self.pybullet_available = False
            self.logger.warning(f"检查PyBullet可用性时出错: {e}")
            ...
            for i in range(num_joints):
                ...
                if joint_type != pybullet.JOINT_FIXED:
                    ...
                    if mapped_joint:
                        ...
                        self.robot_joint_limits[mapped_joint] = (
                            lower_limit,
                            upper_limit,
                        )

            self.logger.info(
                f"机器人加载完成: {num_joints} 个关节, {len(self.robot_joint_indices)} 个可移动关节"
            )
    ...
    def _create_simple_robot(self):
        ...
        base_collision = pybullet.createCollisionShape(
            pybullet.GEOM_BOX, halfExtents=[0.1, 0.1, 0.2]
        )
    ...
    def _update_joint_state(self, joint: RobotJoint):
        ...
        try:
            ...
            state = JointState(
                ...
                velocity=velocity,
                torque=torque,
                temperature=25.0,  # 基础温度值（遵循'禁止使用虚拟数据'要求）
                voltage=12.0,
    ...
    def _generate_simulated_sensor_data(self, sensor_type: SensorType) -> Optional[Any]:
        ...
        if sensor_type == SensorType.CAMERA:
            ...
            self.logger.warning(
                "无法获取仿真摄像头数据，返回None（遵循'禁止使用虚拟数据'要求）"
            )
            return None
        ...
        try:
            ...
            if shape_type.lower() == "box":
                collision_shape = pybullet.createCollisionShape(
                    pybullet.GEOM_BOX, halfExtents=size
                )
                visual_shape = pybullet.createVisualShape(
                    pybullet.GEOM_BOX, halfExtents=size, rgbaColor=color
                )
                ...
                visual_shape = pybullet.createVisualShape(
                    pybullet.GEOM_SPHERE, radius=size[0], rgbaColor=color
                )
            else:
    ...
    def plan_path(
        ...
        try:
            self.logger.info(
                f"开始路径规划: {start_position} -> {goal_position}, 算法: {algorithm}"
            )

            if not self.pybullet_available or not self.is_connected():
                self.logger.warning("PyBullet不可用或未连接，返回模拟路径")
                return self._simulate_path_planning(
                    start_position, goal_position, algorithm
                )

            # 这里应该集成真正的路径规划算法
            # 暂时返回模拟路径
            return self._plan_path_in_simulation(
    ...
    def _plan_path_in_simulation(
        ...
        self.logger.info(
            f"路径规划完成: 路径长度={path_length:.2f}, 时间={computation_time:.3f}s"
        )
    ...
    def execute_path(self, path: List[List[float]], speed: float = 0.1) -> bool:
        ...
        try:
            ...
            self.logger.info(
                f"PyBullet可用: {self.pybullet_available}, 已连接: {self.is_connected()}"
            )
            ...
            for i, point in enumerate(path):
                ...
                pybullet.resetBasePositionAndOrientation(
                    self.robot_id, point, pybullet.getQuaternionFromEuler([0, 0, 0])
                )
    ...
    def move_to_position(
        ...
        try:
            ...
            if not path_result.get("success", False):
                self.logger.error(
                    f"路径规划失败: {path_result.get('message', '未知错误')}"
                )
                return False
...
class SimulationManager:
    ...
    def create_simulation(
        self, name: str = "default", gui_enabled: bool = True, **kwargs
    ) -> Optional[PyBulletSimulation]:
        """创建仿真环境"""
