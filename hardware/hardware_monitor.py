...
from .robot_controller import HardwareInterface
...
class HardwareMonitor:
    ...
    def __init__(self, hardware_interface: HardwareInterface):
        ...
        logger.info(
            f"硬件监控器初始化完成，接口类型: {hardware_interface.interface_type}"
        )
    ...
    def start_monitoring(self):
        ...
        self.monitoring_thread = threading.Thread(
            target=self._monitoring_loop, daemon=True, name="HardwareMonitor"
        )
    ...
    def _monitor_connection(self):
        ...
        try:
            ...
            if is_connected:
                ...
                # 计算连接稳定性
                connection_stability = min(
                    100.0, 100.0 - (connection_time / 3600.0)
                )  # 每小时衰减
            ...
            try:
                ...
                if sensor_data:
                    ...
                    if sensor_health < 80.0:
                        status.status = "degraded"
                        status.health_score = max(
                            0.0, status.health_score - (100.0 - sensor_health)
                        )
            ...
            try:
                ...
                if motor_status:
                    ...
                    # 检查电机健康度
                        ...
                        status.status = "degraded"
                        status.health_score = max(
                            0.0, status.health_score - (100.0 - motor_health)
                        )
    ...
    def _perform_health_check(self):
        ...
        try:
            ...
            if component_count > 0:
                ...
                # 记录整体健康状态
                self.component_status["safety"].metrics[
                    "overall_health"
                ] = overall_health
                self.component_status["safety"].metrics[
                    "last_health_check"
        ...
        try:
            ...
            if component in self.component_status:
                ...
                if len(self.component_status[component].errors) > 50:
                    self.component_status[component].errors = self.component_status[
                        component
                    ].errors[-50:]
    ...
    def _execute_safety_rules(self):
        ...
        try:
            # 按优先级排序
            rules = sorted(
                self.safety_rules.values(), key=lambda r: r.priority, reverse=True
            )
