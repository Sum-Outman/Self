...
from .robot_controller import HardwareInterface, NullHardwareInterface
from .real_robot_interface import (
    NAOqiRobotInterface,
...
class HardwareConfig:
    ...
    interface_strategy: str = (
        "auto"  # auto, real_first, simulation_first, real_only, simulation_only
    )
...
class HardwareFactory:
    ...
    def _init_sdk_availability(self) -> None:
        ...
        logger.info(
            f"SDK可用性检测: NAOqi={naoqi_available}, PyBullet={pybullet_available}"
        )
    ...
    def _check_naoqi_availability(self) -> bool:
        ...
        try:
            ...
            logger.warning(
                "NAOqi SDK不可用,NAO机器人功能将不可用(项目要求禁止使用虚拟数据)"
            )
            return False
    ...
    def _check_pybullet_availability(self) -> bool:
        ...
        try:
            ...
            logger.warning(
                "PyBullet不可用,物理仿真功能将不可用(项目要求禁止使用虚拟数据)"
            )
            return False
        except Exception as e:
    ...
    def get_hardware_interface(
        ...
        if strategy == "real_only":
            ...
            logger.info("使用simulation_only策略: 只使用仿真接口")
            interface = self._create_simulation_interface()
        elif strategy == "real_first":
            logger.info("使用real_first策略: 先尝试真实硬件，失败时使用仿真")
            # 先尝试真实硬件
            try:
                interface = self._create_real_interface()
            except Exception as e:
                logger.warning(f"真实硬件接口创建失败，尝试仿真接口: {e}")
                interface = self._create_simulation_interface()
        elif strategy == "simulation_first":
            logger.info("使用simulation_first策略: 先尝试仿真，失败时使用真实硬件")
            # 先尝试仿真
            interface = self._create_simulation_interface()
            # 如果仿真失败或返回无硬件接口，尝试真实硬件
            if isinstance(interface, NullHardwareInterface):
                logger.info("仿真接口返回无硬件接口，尝试真实硬件")
                try:
                    interface = self._create_real_interface()
                except Exception as e:
                    logger.warning(f"真实硬件接口创建失败: {e}")
        else:  # auto
            logger.info("使用auto策略: 自动选择最佳接口")
            # 自动选择：根据SDK可用性决定
            naoqi_available = self._sdk_availability_cache.get("naoqi", False)
            pybullet_available = self._sdk_availability_cache.get("pybullet", False)
            
            if naoqi_available:
                # 如果有NAOqi SDK，尝试真实硬件
                logger.info("检测到NAOqi SDK可用，尝试真实硬件接口")
                try:
                    interface = self._create_real_interface()
                except Exception as e:
                    logger.warning(f"真实硬件接口创建失败，尝试仿真: {e}")
                    if pybullet_available:
                        interface = self._create_simulation_interface()
                    else:
                        interface = NullHardwareInterface()
            elif pybullet_available:
                # 如果有PyBullet，使用仿真
                logger.info("检测到PyBullet可用，使用仿真接口")
                interface = self._create_simulation_interface()
            else:
                # 都不可用，返回无硬件接口
                logger.info("无可用SDK，返回无硬件接口")
                interface = NullHardwareInterface()
            ...
            try:
                ...
                if "components" in health:
                    ...
                    unavailable_count = sum(
                        1 for status in components.values() if status == "unavailable"
                    )
                    if unavailable_count > 0:
                        logger.info(
                            f"检测到 {unavailable_count} 个硬件组件不可用，进入部分硬件模式"
                        )
    ...
    def _create_simulation_interface(self) -> HardwareInterface:
        ...
        此方法提供仿真接口支持。
        """
        logger.info("创建仿真硬件接口")
        
        try:
            # 根据配置选择仿真引擎
            engine = self._config.extra_config.get("simulation_engine", "pybullet")
            gui_enabled = self._config.simulation_gui_enabled
            physics_timestep = self._config.simulation_physics_timestep
            realtime_simulation = self._config.simulation_realtime
            
            if engine.lower() == "pybullet":
                ...
                logger.info(f"创建PyBullet仿真接口: GUI={gui_enabled}")
                return interface
                
            elif engine.lower() == "gazebo":
                from .gazebo_simulation import GazeboSimulation
                
                robot_model = self._config.extra_config.get("robot_model", "humanoid")
                interface = GazeboSimulation(
                    ros_master_uri=self._config.extra_config.get("ros_master_uri", "http://localhost:11311"),
                    gazebo_world=self._config.extra_config.get("gazebo_world", "empty.world"),
                    robot_model=robot_model,
                    gui_enabled=gui_enabled,
                    physics_timestep=physics_timestep,
                )
                
                logger.info(f"创建Gazebo仿真接口: {robot_model}")
                return interface
                
            elif engine.lower() == "unified":
                from .unified_simulation import UnifiedSimulation
                
                interface = UnifiedSimulation(
                    engine=self._config.extra_config.get("unified_engine", "pybullet"),
                    engine_config=self._config.extra_config.get("engine_config", {}),
                    gui_enabled=gui_enabled,
                    physics_timestep=physics_timestep,
                    realtime_simulation=realtime_simulation,
                )
                
                logger.info(f"创建统一仿真接口: 引擎={engine}")
                return interface
                
            else:
                logger.warning(f"不支持的仿真引擎: {engine}, 使用PyBullet作为默认")
                from .simulation import PyBulletSimulation
            ...
            logger.error(f"创建仿真接口失败: {e}")
            # 返回无硬件接口而不是抛出异常
            from .robot_controller import NullHardwareInterface
            return NullHardwareInterface()

    def _create_basic_simulation_interface(self) -> HardwareInterface:
        """创建基础模拟接口
        
        根据用户需求"系统可以在没有硬件条件下单独运行AGI所有功能"，
        提供基础模拟接口支持。
        """
        logger.info("创建基础模拟接口")
        
        # 返回无硬件接口，但标记为仿真模式
        from .robot_controller import NullHardwareInterface
        
        interface = NullHardwareInterface()
        interface._interface_type = "basic_simulation"
        interface._simulation_mode = True
        
        logger.info("基础模拟接口创建完成（无硬件功能）")
        return interface
