...
class ArduinoRobotInterface(SerialInterface):
    ...
    def __init__(
        self, port: str = "COM3", baudrate: int = 115200, simulation_mode: bool = False
    ):
        """
        初始化Arduino机器人接口
        ...
        super().__init__(
            port=port, baudrate=baudrate, simulation_mode=False
        )  # 强制为False
    ...
    def set_joint_position(self, joint: RobotJoint, position: float) -> bool:
        ...
        if joint in self.joint_limits:
            ...
            if position < min_limit or position > max_limit:
                logger.warning(
                    f"关节 {joint.value} 位置 {position} 超出限制 [{min_limit}, {max_limit}]"
                )
    ...
    def perform_safe_movement(
        self, positions: Dict[RobotJoint, float], speed: float = 30.0
    ) -> bool:
        """执行安全运动（逐步移动到目标位置）
...
def run_arduino_example():
    ...
    arduino = ArduinoRobotInterface(
        port=port, baudrate=115200, simulation_mode=simulation_mode
    )
...
void sendSensorData() {
  ...
  Serial.println();}"""
    ...
    firmware_path = os.path.join(
        os.path.dirname(__file__), "arduino_firmware_example.ino"
    )
    try:
