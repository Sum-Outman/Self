...
class RealDataDownloader:
    ...
    def download_dataset(
        ...
        if dataset_name not in self.datasets_config:
            raise ValueError(
                f"未知的数据集: {dataset_name}。可用数据集: {self.list_available_datasets()}"
            )
    ...
    def download_cifar10(self, force_download: bool = False) -> Dict[str, Any]:
        ...
        logger.info(
            f"CIFAR-10数据集下载完成: {len(train_dataset)}训练 + {len(test_dataset)}测试样本"
        )
    ...
    def download_mnist(self, force_download: bool = False) -> Dict[str, Any]:
        ...
        dataset_info = {
            ...
            "total_samples": min(len(train_dataset), 10000)
            + min(len(test_dataset), 2000),
            "image_size": [28, 28],
            "channels": 1,
            "created_at": "2026-04-01",
            "is_real_data": True,
        ...
        logger.info(
            f"MNIST数据集下载完成: {min(len(train_dataset),                                  10000)}训练 + {min(len(test_dataset),                                                   2000)}测试样本"
        )
    ...
    def download_fashion_mnist(self, force_download: bool = False) -> Dict[str, Any]:
        ...
        logger.info(
            f"Fashion-MNIST数据集下载完成: {min(len(train_dataset),                                          10000)}训练 + {min(len(test_dataset),                                                           2000)}测试样本"
        )
    ...
    def prepare_for_self_agi(self, dataset_name: str) -> Dict[str, Any]:
        ...
        logger.info("数据集已准备就绪，可用于Self AGI训练")
...
def main():
    ...
    parser.add_argument("--all", action="store_true", help="下载所有可用的数据集")
    parser.add_argument(
        "--prepare",
        action="store_true",
    ...
    if args.all:
        ...
        for dataset_name, result in results:
            ...
            if result.get("success", False):
                ...
                print(
                    f"    样本数: {                         result.get(                             'train_samples',                             result.get(                                 'samples',                                 'N/A'))}"
                )
    else:
        try:
            if args.prepare:
