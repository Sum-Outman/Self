...
class MultiTaskDataset(Dataset):
    ...
    def add_task_data(self, task_id: str, observations: np.ndarray, targets: np.ndarray):
        """添加任务数据"""
    ...
    def get_task_batch(self, task_id: str, batch_size: int = 32) -> Tuple[np.ndarray, np.ndarray]:
        """获取任务批次数据"""
        if task_id not in self.tasks:
    ...
    def get_multi_task_batch(self, task_ids: List[str], batch_size_per_task: int = 16) -> Dict[str, Tuple[np.ndarray, np.ndarray]]:
        """获取多任务批次数据"""
        batches = {}
        
        for task_id in task_ids:
            if task_id in self.tasks and len(self.tasks[task_id]) > 0:
                observations, targets = self.get_task_batch(task_id, batch_size_per_task)
...
class MultiTaskTrainer:
    ...
    def train_step(self) -> Dict[str, float]:
        ...
        # 获取活动任务
        active_tasks = [task_id for task_id in self.model.task_ids 
        ...
        if self.gradient_surgery and len(task_gradients) > 1:
            if self.surgery_method == "pcgrad":
                ...
                # 更新梯度
                for (task_id, gradients), proj_grad in zip(task_gradients.items(), proj_gradients):
    ...
    def train(self, steps: int = 1000, log_interval: int = 100) -> Dict[str, Any]:
        """训练多任务模型"""
        self.logger.info(f"开始多任务训练: {steps} 步, {len(self.model.task_ids)} 个任务")
        ...
        for step in range(steps):
            ...
            if step % log_interval == 0 and task_losses:
                ...
                self.logger.info(
                    f"步数 {step}/{steps}: " +
                    ", ".join([f"{task_id}={loss:.6f}" for task_id, loss in avg_losses.items()])
                )
    ...
    def adjust_task_weights(self, strategy: str = "equal"):
        ...
        if strategy == "equal":
            ...
            for task_id in self.model.task_ids:
                losses = self.loss_history.get(task_id, [])
                if losses:
                    recent_loss = np.mean(losses[-100:]) if len(losses) >= 100 else np.mean(losses)
                    # 损失越大，权重越小（归一化）
            ...
            # 基于优先级的权重
            max_priority = max(config.priority for config in self.model.task_configs.values())
...
class MultiTaskLearningManager:
    ...
    def train(self,
              ...
              batch_size: int = 32,
              gradient_surgery: bool = True,
              surgery_method: str = "pcgrad") -> Dict[str, Any]:
        """训练多任务模型"""
    ...
    def evaluate_task(self, 
                     ...
                     test_targets: np.ndarray) -> Dict[str, Any]:
        """评估任务性能"""
        if self.model is None:
...
def create_multi_task_manager(task_configs: List[TaskConfig], **kwargs) -> MultiTaskLearningManager:
    """创建多任务学习管理器（工厂函数）"""
