...
class TaskDistribution:
    ...
    def sample_task(self, task_id: Optional[int] = None) -> Dict[str, Any]:
        ...
        task = {
            "task_id": task_id if task_id is not None else random.randint(0, self.num_tasks - 1),
            "task_family": self.task_family,
            "params": {}
        }
            ...
            if isinstance(param_dist, dict) and "type" in param_dist:
                ...
                if dist_type == "uniform":
                    ...
                    if probs and len(probs) == len(values):
                        task["params"][param_name] = random.choices(values, weights=probs)[0]
                    else:
...
class MetaDataset(Dataset):
    ...
    def _generate_data_for_task(self, task: Dict[str, Any], num_samples: int) -> Dict[str, torch.Tensor]:
        """为特定任务生成数据"""
...
class MAML:
    ...
    def adapt(self, 
              ...
              loss_fn: Optional[Callable] = None) -> nn.Module:
        """
        快速适应新任务
    ...
    def _forward_with_weights(self, 
                             inputs: torch.Tensor, 
    ...
    def evaluate(self,
        ...
        # 随机选择评估任务
        eval_indices = random.sample(range(len(meta_dataset)), min(num_tasks, len(meta_dataset)))
...
class MetaLearningManager:
    ...
    def train(self,
              ...
              eval_interval: int = 10) -> Dict[str, Any]:
        """
        训练元学习模型
        ...
        for epoch in range(num_epochs):
            ...
            for meta_batch in dataloader:
                ...
                epoch_losses.append(stats.get("meta_loss", stats.get("avg_task_loss", 0.0)))
            ...
            if epoch % eval_interval == 0 or epoch == num_epochs - 1:
                ...
                if epoch % 10 == 0:
                    self.logger.info(f"轮次 {epoch}/{num_epochs}: 损失={avg_epoch_loss:.6f}")
    ...
    def _save_checkpoint(self, name: str):
        ...
        checkpoint = {
            ...
            'meta_learner_state_dict': self.meta_learner.meta_optimizer.state_dict()
            if hasattr(self.meta_learner, 'meta_optimizer') else None,
            'algorithm': self.algorithm.value,
            'config': self.config
        }
    ...
    def load_checkpoint(self, checkpoint_path: str) -> bool:
        ...
        try:
            ...
            if (hasattr(self.meta_learner, 'meta_optimizer') and 
                checkpoint['meta_learner_state_dict'] is not None):
...
def create_sinusoidal_task_distribution(num_tasks: int = 1000, support_set_size: int = 10, query_set_size: int = 10) -> TaskDistribution:
