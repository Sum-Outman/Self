...
# 导入生成模型
try:
    from models.multimodal.generative_models import TextToImageGenerator, ImageToTextGenerator
...
# 导入真实数据集
try:
    from training.real_multimodal_dataset import RealMultimodalDataset, DataSourceType, create_real_multimodal_dataloader
...
class GenerativeTrainer:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        # 模型配置
        self.model_type = config.get("model_type", "both")  # text_to_image, image_to_text, both
    ...
    def _init_models(self):
        ...
        if self.model_type in ["text_to_image", "both"]:
            self.text_to_image_model = TextToImageGenerator(model_config).to(self.device)
            ...
            self.scheduler_text_to_image = optim.lr_scheduler.ReduceLROnPlateau(
                self.optimizer_text_to_image, 
                mode='min',
                patience=5,
                factor=0.5
            )
            logger.info(f"文本到图像模型初始化完成，参数量: {self._count_parameters(self.text_to_image_model):,}")
        
        # 初始化图像到文本模型
        if self.model_type in ["image_to_text", "both"]:
            self.image_to_text_model = ImageToTextGenerator(model_config).to(self.device)
            ...
            self.scheduler_image_to_text = optim.lr_scheduler.ReduceLROnPlateau(
                self.optimizer_image_to_text, 
                mode='min',
                patience=5,
                factor=0.5
            )
            logger.info(f"图像到文本模型初始化完成，参数量: {self._count_parameters(self.image_to_text_model):,}")
    ...
    def _create_synthetic_dataset(self, num_samples: int = 1000) -> Dataset:
        ...
        class SyntheticDataset(Dataset):
            def __init__(self, num_samples, image_size, vocab_size, max_seq_len):
                ...
                self.categories = ["动物", "食物", "交通工具", "建筑", "自然景观", "人物"]
            ...
            def __getitem__(self, idx):
                ...
                # 生成文本ID序列（完整）
                # 在实际实现中，应使用分词器
                text_ids = torch.randint(1, min(100, self.vocab_size), (self.max_seq_len,))
                text_ids[10:] = 0  # 填充
        ...
        return SyntheticDataset(num_samples, self.image_size, self.text_vocab_size, self.max_seq_len)
    ...
    def train_epoch(self, epoch: int) -> Dict[str, float]:
        ...
        for batch_idx, batch in enumerate(self.train_loader):
            ...
            if self.text_to_image_model:
                ...
                torch.nn.utils.clip_grad_norm_(self.text_to_image_model.parameters(), 1.0)
                self.optimizer_text_to_image.step()
                
                # 记录损失
                epoch_losses["text_to_image_total"] += text_to_image_losses["total_loss"].item()
                epoch_losses["text_to_image_recon"] += text_to_image_losses["recon_loss"].item()
                epoch_losses["text_to_image_kl"] += text_to_image_losses["kl_loss"].item()
            ...
            if self.image_to_text_model:
                ...
                torch.nn.utils.clip_grad_norm_(self.image_to_text_model.parameters(), 1.0)
                self.optimizer_image_to_text.step()
                
                # 记录损失
                epoch_losses["image_to_text"] += image_to_text_loss.item()
                epoch_losses["image_to_text_perplexity"] = image_to_text_loss_dict.get("perplexity", 0).item()
        ...
        # 总损失（用于调度器）
        total_train_loss = avg_losses.get("text_to_image_total", 0) + avg_losses.get("image_to_text", 0)
        avg_losses["total"] = total_train_loss
        
        epoch_time = time.time() - epoch_start_time
        logger.info(f"Epoch {epoch} 完成，时间: {epoch_time:.2f}s，总损失: {total_train_loss:.4f}")
    ...
    def validate(self) -> Dict[str, float]:
        ...
        with torch.no_grad():
            for batch in self.val_loader:
                ...
                if self.text_to_image_model:
                    ...
                    val_losses["text_to_image_total"] += text_to_image_losses["total_loss"].item()
                    val_losses["text_to_image_recon"] += text_to_image_losses["recon_loss"].item()
                    val_losses["text_to_image_kl"] += text_to_image_losses["kl_loss"].item()
                ...
                if self.image_to_text_model:
                    ...
                    val_losses["image_to_text_perplexity"] = image_to_text_loss_dict.get("perplexity", 0).item()
        ...
        # 总验证损失
        total_val_loss = avg_val_losses.get("text_to_image_total", 0) + avg_val_losses.get("image_to_text", 0)
        avg_val_losses["total"] = total_val_loss
        
        logger.info(f"验证损失: Text2Image={avg_val_losses.get('text_to_image_total', 0):.4f}, "
    ...
    def evaluate_generation_quality(self) -> Dict[str, float]:
        ...
        if self.text_to_image_model:
            try:
                ...
                # 计算结构相似性（完整）
                evaluation_results["text_to_image_quality"] = 1.0 / (1.0 + mse_loss.item())
                
                logger.info(f"文本到图像生成质量评估: MSE={mse_loss.item():.4f}, "
    ...
    def train(self):
        ...
        for epoch in range(1, self.num_epochs + 1):
            ...
            self.training_history['val_loss'].append(val_losses.get("total", 0) if epoch % self.eval_interval == 0 else None)
            self.training_history['text_to_image_loss'].append(train_losses.get("text_to_image_total", 0))
            self.training_history['image_to_text_loss'].append(train_losses.get("image_to_text", 0))
            
            current_lr = self.optimizer_text_to_image.param_groups[0]['lr'] if self.text_to_image_model else 0
    ...
    def save_checkpoint(self, filename: str):
        ...
        if self.text_to_image_model:
            checkpoint['text_to_image_state_dict'] = self.text_to_image_model.state_dict()
            checkpoint['text_to_image_optimizer'] = self.optimizer_text_to_image.state_dict()
        
        if self.image_to_text_model:
            checkpoint['image_to_text_state_dict'] = self.image_to_text_model.state_dict()
            checkpoint['image_to_text_optimizer'] = self.optimizer_image_to_text.state_dict()
    ...
    def load_checkpoint(self, checkpoint_path: str):
        ...
        if self.text_to_image_model and 'text_to_image_state_dict' in checkpoint:
            self.text_to_image_model.load_state_dict(checkpoint['text_to_image_state_dict'])
            self.optimizer_text_to_image.load_state_dict(checkpoint['text_to_image_optimizer'])
        
        if self.image_to_text_model and 'image_to_text_state_dict' in checkpoint:
            self.image_to_text_model.load_state_dict(checkpoint['image_to_text_state_dict'])
            self.optimizer_image_to_text.load_state_dict(checkpoint['image_to_text_optimizer'])
    ...
    def generate_text_to_image(self, text_input: str) -> torch.Tensor:
        ...
        # 完整文本编码（在实际应用中应使用分词器）
        # 这里使用随机编码作为示例
        text_ids = torch.randint(1, min(100, self.text_vocab_size), (1, self.max_seq_len))
    ...
    def get_training_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "text_to_image_params": self._count_parameters(self.text_to_image_model) if self.text_to_image_model else 0,
            "image_to_text_params": self._count_parameters(self.image_to_text_model) if self.image_to_text_model else 0,
            "dataset_sizes": {
...
def create_generative_trainer(config: Optional[Dict[str, Any]] = None) -> GenerativeTrainer:
    """创建生成模型训练器（工厂函数）"""
    if config is None:
        config = {
            "model_type": "both",
