...
class WebLearningEngine:
    ...
    def search_learning_resources(
        self, learning_query: LearningQuery
    ) -> List[WebResource]:
        """
        搜索学习资源
        ...
        """
        logger.info(
            f"搜索学习资源: {learning_query.topic}, 关键词: {learning_query.keywords}"
        )
        ...
        for resource in resources:
            try:
                ...
                if fetched_resource:
                    # 评估资源质量
                    fetched_resource.quality_score = self.evaluate_resource_quality(
                        fetched_resource
                    )
    ...
    def _search_duckduckgo(self, learning_query: LearningQuery) -> List[WebResource]:
        ...
        try:
            ...
            search_url = (
                f"{self.search_apis['duckduckgo']['endpoint']}?q={encoded_query}"
            )
    ...
    def evaluate_resource_relevance(
        self, resource: WebResource, learning_query: LearningQuery
    ) -> float:
        """评估资源相关性"""
        relevance_score = 0.0
        ...
        query_terms = [learning_query.topic.lower()] + [
            k.lower() for k in learning_query.keywords
        ]
        ...
        # 3. 来源类型匹配
        if (
            learning_query.resource_types
            and resource.content_type in learning_query.resource_types
        ):
            relevance_score += 0.2
    ...
    def process_resource_for_learning(self, resource: WebResource) -> Dict[str, Any]:
        """将资源处理为学习材料"""
        processed_data = {
        ...
        if resource.content_type == "html" or resource.content_type == "text":
            # 分割为学习单元（段落）
            paragraphs = [
                p.strip() for p in resource.extracted_text.split("\n") if p.strip()
            ]
            ...
            # 分割为章节或页面
            sections = [
                s.strip() for s in resource.extracted_text.split("\n\n") if s.strip()
            ]
        ...
        logger.info(
            f"处理资源完成: {resource.title}, 学习单元: {len(processed_data['learning_units'])}"
        )
...
if __name__ == "__main__":
    ...
    for i, resource in enumerate(resources):
        ...
        print(
            f"   质量: {resource.quality_score:.2f}, 相关性: {resource.relevance_score:.2f}"
        )
