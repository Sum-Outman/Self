...
class TrainingProfile:
    ...
    def compute_memory_requirements(self) -> Dict[str, float]:
        ...
        activation_memory_bytes = self.batch_size * self.sequence_length * self.hidden_size * 4 * self.num_layers * 2
        
        # 总内存需求
        total_memory_bytes = param_memory_bytes + grad_memory_bytes + optimizer_memory_bytes + activation_memory_bytes
...
class ResourcePredictor:
        ...
        try:
            ...
            for historical in self.historical_data:
                ...
                if "memory_gb" in historical.actual_resource_usage:
                    labels_memory.append(math.log10(historical.actual_resource_usage["memory_gb"] + 1))
    ...
    def predict(self, training_profile: TrainingProfile, 
                available_resources: Optional[List[ResourceProfile]] = None,
                optimization_goal: Optional[OptimizationGoal] = None) -> PredictionResult:
        """预测训练资源需求
        ...
        """
        self.logger.info(f"开始预测: 模型参数={training_profile.model_parameters:,}, 数据集大小={training_profile.dataset_size:,}")
        
        # 确定优化目标
        goal = optimization_goal or OptimizationGoal(self.config.get("optimization_goal", OptimizationGoal.BALANCED.value))
        
        # 计算理论需求
        theoretical_requirements = self._compute_theoretical_requirements(training_profile)
        
        # 应用规则库
        rule_based_suggestions = self._apply_rules(training_profile)
        
        # ML预测
        ml_predictions = self._ml_predict(training_profile) if self.config.get("enable_ml_predictions", True) else {}
        ...
        self.logger.info(f"预测完成: 预计时长={result.training_duration_hours:.1f} 小时")
        
        return result
    
    def _compute_theoretical_requirements(self, training_profile: TrainingProfile) -> Dict[str, Any]:
        """计算理论资源需求"""
        requirements = {}
        ...
        checkpoint_storage_gb = model_size_gb * training_profile.checkpoint_frequency * 2  # 包含优化器状态
        ...
        if training_profile.distributed_training:
            ...
            # 每轮同步次数
            syncs_per_epoch = training_profile.dataset_size / training_profile.batch_size
            
            requirements["network_requirements"] = {
                "gradient_sync_gb_per_epoch": grad_sync_gb,
                "total_gradient_sync_gb": grad_sync_gb * syncs_per_epoch * training_profile.epochs,
        ...
        for rule in self.rules["memory_rules"]:
            if rule["condition"](training_profile):
                ...
                requirements["min_memory_gb"] = max(
                    requirements.get("min_memory_gb", 0),
                    rule.get("min_memory_gb", 0)
                )
    ...
    def _ml_predict(self, training_profile: TrainingProfile) -> Dict[str, Any]:
        ...
        try:
            ...
            if "gpu" in self.ml_models:
                predictions["gpu_count"] = int(self.ml_models["gpu"].predict(X_scaled)[0])
    ...
    def _combine_predictions(self, 
        ...
        if "memory_requirements" in theoretical:
            memory_values.append(theoretical["memory_requirements"]["minimum_gpu_memory_gb"])
        ...
        if "storage_requirements" in theoretical:
            combined["resource_requirements"]["storage_gb"] = theoretical["storage_requirements"]["total_gb"]
        
        # 网络需求
        if "network_requirements" in theoretical:
            combined["resource_requirements"]["network_gbps"] = theoretical["network_requirements"]["estimated_bandwidth_requirement_gbps"]
    ...
    def _adjust_for_available_resources(self, 
                                       predictions: Dict[str, Any], 
                                       available_resources: List[ResourceProfile]) -> Dict[str, Any]:
        """根据可用资源调整预测"""
        ...
        for resource in available_resources:
            if resource.resource_type == ResourceType.GPU:
                ...
                available["memory_gb"] = available.get("memory_gb", 0) + resource.quantity
            elif resource.resource_type == ResourceType.STORAGE:
                available["storage_gb"] = available.get("storage_gb", 0) + resource.quantity
            elif resource.resource_type == ResourceType.NETWORK:
                available["network_gbps"] = max(available.get("network_gbps", 0), resource.quantity)
        ...
        if "gpu_count" in requirements:
            if available.get("gpu", 0) < requirements["gpu_count"]:
                bottlenecks.append(f"GPU不足: 需要 {requirements['gpu_count']}，可用 {available.get('gpu', 0)}")
                # 调整训练时间（假设线性缩放）
        ...
        if "memory_gb" in requirements:
            if available.get("memory_gb", 0) < requirements["memory_gb"]:
                bottlenecks.append(f"内存不足: 需要 {requirements['memory_gb']:.1f} GB，可用 {available.get('memory_gb', 0):.1f} GB")
                # 内存不足可能导致OOM，无法训练
        ...
        if "storage_gb" in requirements:
            if available.get("storage_gb", 0) < requirements["storage_gb"]:
                bottlenecks.append(f"存储不足: 需要 {requirements['storage_gb']:.1f} GB，可用 {available.get('storage_gb', 0):.1f} GB")
        
        # 网络检查
        if "network_gbps" in requirements:
            if available.get("network_gbps", 0) < requirements["network_gbps"]:
                bottlenecks.append(f"网络带宽不足: 需要 {requirements['network_gbps']} Gbps，可用 {available.get('network_gbps', 0)} Gbps")
                # 网络瓶颈可能增加训练时间
                if "duration_hours" in adjusted:
    ...
    def _generate_optimization_suggestions(self, 
        ...
        if training_profile.batch_size < 32:
            suggestions.append({
                ...
                "suggestions": [f"增加批量大小到32或更大"],
                "priority": "low",
            })
        
        return suggestions
    
    def _assess_risks(self, training_profile: TrainingProfile, predictions: Dict[str, Any]) -> Dict[str, float]:
        """风险评估"""
        risks = {}
        ...
        if len(self.historical_data) > 0:
            failure_rate = sum(1 for h in self.historical_data if not h.success) / len(self.historical_data)
    ...
    def _generate_alternative_configs(self, 
        ...
        original_gpu_count = predictions.get("resource_requirements", {}).get("gpu_count", 1)
        ...
        if original_gpu_count < 8:
            ...
            alternatives.append({
                ...
                "estimated_cost_change": (alt_duration * alt_gpu_count - original_duration * original_gpu_count) * self.config["resource_costs"]["gpu_hour"],
                "suitability": "minimize_time" if goal == OptimizationGoal.MINIMIZE_TIME else "balanced",
            })
        ...
        if training_profile.batch_size > 32:
            ...
            alternatives.append({
                ...
                "estimated_duration_hours": alt_duration,
                "estimated_cost_change": (alt_duration - original_duration) * original_gpu_count * self.config["resource_costs"]["gpu_hour"],
                "suitability": "minimize_resources" if goal == OptimizationGoal.MINIMIZE_RESOURCES else "balanced",
            })
    ...
    def _calculate_confidence_interval(self, 
                                     training_profile: TrainingProfile,
                                     predictions: Dict[str, Any]) -> Tuple[float, float]:
        """计算置信区间"""
        duration = predictions.get("duration_hours", 0)
        ...
        for historical in self.historical_data:
            if historical.success:
                # 简单相似度计算
                similarity = self._compute_similarity(training_profile, historical.training_profile)
                if similarity > 0.7:
                    # 计算相对误差
                    if "duration_hours" in predictions and historical.actual_duration_hours > 0:
                        error = abs(predictions["duration_hours"] - historical.actual_duration_hours) / historical.actual_duration_hours
                        errors.append(error)
        
        if errors:
            mean_error = statistics.mean(errors)
            std_error = statistics.stdev(errors) if len(errors) > 1 else mean_error * 0.5
    ...
    def _compute_similarity(self, tp1: TrainingProfile, tp2: TrainingProfile) -> float:
        ...
        if tp1.model_parameters > 0 and tp2.model_parameters > 0:
            param_sim = min(tp1.model_parameters, tp2.model_parameters) / max(tp1.model_parameters, tp2.model_parameters)
            similarities.append(param_sim)
        
        # 数据集大小相似度
        if tp1.dataset_size > 0 and tp2.dataset_size > 0:
            data_sim = min(tp1.dataset_size, tp2.dataset_size) / max(tp1.dataset_size, tp2.dataset_size)
            similarities.append(data_sim)
        
        # 批量大小相似度
        if tp1.batch_size > 0 and tp2.batch_size > 0:
            batch_sim = min(tp1.batch_size, tp2.batch_size) / max(tp1.batch_size, tp2.batch_size)
            similarities.append(batch_sim)
        
        # GPU数量相似度
        gpu_sim = min(tp1.num_gpus, tp2.num_gpus) / max(tp1.num_gpus, tp2.num_gpus) if max(tp1.num_gpus, tp2.num_gpus) > 0 else 1.0
...
def get_resource_predictor(config: Optional[Dict[str, Any]] = None) -> ResourcePredictor:
    """获取资源预测器单例
