...
# 导入机器人控制器
try:
    from hardware.robot_controller import JointState
...
class GazeboEnvironmentConfig:
    ...
    joint_names: Optional[List[str]] = (
        None  # 关节名称列表，如果为None则使用joint_0, joint_1, ...
    )
    joint_position_limits: Tuple[float, float] = (
        -np.pi / 2,
        np.pi / 2,
    )  # 关节位置限制（弧度）
    ...
    simulated_acceleration_range: Tuple[float, float] = (
        -0.1,
        0.1,
    )  # 模拟加速度范围 (m/s²)
    simulated_angular_velocity_range: Tuple[float, float] = (
        -0.05,
        0.05,
    )  # 模拟角速度范围 (rad/s)
    simulated_robot_base_position: List[float] = field(
...
class GazeboRobotEnvironment(gym.Env):
    ...
    def __init__(self, config: Optional[GazeboEnvironmentConfig] = None):
        ...
        # 使用配置的基准位置作为初始位置
        self.robot_position: List[float] = (
            self.config.simulated_robot_base_position.copy()
        )
    ...
    def _define_spaces(self):
        ...
        if self.config.include_imu_data:
            obs_dim += (
                GazeboEnvironmentConfig.IMU_DIMENSION
            )  # IMU数据：加速度(3)、角速度(3)、姿态(4)
        ...
        # 目标位置（相对位置）
        obs_dim += (
            GazeboEnvironmentConfig.POSITION_DIMENSION
        )  # 目标相对位置 [dx, dy, dz]
    ...
    def step(self, action: np.ndarray):
        ...
        if self.gazebo is not None:
            try:
                ...
                if self.config.use_real_time_sync:
                    time.sleep(
                        self.config.physics_timestep * self.config.simulation_speed
                    )
            except Exception as e:
        ...
        info = {
            ...
            "distance_to_target": np.linalg.norm(
                self.target_position - np.array(self.robot_position)
            ),
        }
    ...
    def _apply_action(self, action: np.ndarray):
        ...
        if len(action) != self.config.num_joints:
            logger.warning(
                f"动作维度不匹配: 预期{self.config.num_joints}, 实际{len(action)}"
            )
            # 裁剪或填充动作向量
            if len(action) > self.config.num_joints:
                ...
                # 填充零
                action = np.pad(
                    action, (0, self.config.num_joints - len(action)), mode="constant"
                )
            ...
            for i in range(self.config.num_joints):
                ...
                if joint_name not in self.joint_states:
                    self.joint_states[joint_name] = JointState(
                        name=joint_name, position=0.0, velocity=0.0, effort=0.0
                    )
    ...
    def _get_observation(self) -> np.ndarray:
        ...
        if expected_dim != actual_dim:
            ...
            if actual_dim < expected_dim:
                # 填充零
                obs_array = np.pad(
                    obs_array, (0, expected_dim - actual_dim), mode="constant"
                )
            else:
                # 裁剪
    ...
    def _calculate_reward(self) -> float:
        ...
        if self.task_type == RobotTaskType.STAND_UP:
            ...
            # 额外奖励：保持直立（小角度）
            if (
                abs(pitch) < self.config.stability_angle_threshold
                and abs(roll) < self.config.stability_angle_threshold
            ):
            ...
            # 惩罚侧向移动（x轴）和高度变化（z轴）
            lateral_penalty = (
                abs(self.robot_position[0]) * self.config.lateral_penalty_coef
            )
            height_penalty = (
                abs(self.robot_position[2] - self.config.walking_target_height)
                * self.config.walking_height_penalty_coef
            )
            ...
            # 使用指数衰减奖励，越近奖励越高
            distance_reward = self.config.target_distance_reward_coef / (
                distance + self.config.distance_reward_smoothing
            )
        ...
        # 倾斜过大惩罚
        if (
            abs(pitch) > self.config.excessive_tilt_threshold
            or abs(roll) > self.config.excessive_tilt_threshold
        ):
            reward += (
                self.config.fall_penalty * self.config.excessive_tilt_penalty_multiplier
            )
    ...
    def render(self, mode="human"):
        ...
        if mode == "human":
            ...
            if self.gazebo is not None and self.config.gui_enabled:
                ...
                # 打印当前状态
                print(
                    f"Step: {self.current_step}, Reward: {self.episode_reward:.2f}, "
