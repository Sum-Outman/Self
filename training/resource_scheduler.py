...
class ResourceMonitor:
                ...
                availability["gpu"].append(
                    {
                        ...
                        "memory_availability": gpu.get("memory_free", 0)
                        / max(gpu.get("memory_total", 1), 1),
                    }
                )
    ...
    def can_allocate_resources(self, required: Dict[str, float]) -> bool:
        ...
        if required_cpu > 0 and availability.get("cpu", 1.0) < (required_cpu / 100):
            logger.debug(
                f"CPU资源不足: 需要{required_cpu}%，可用{                     availability.get(                         'cpu',                         1.0) * 100:.1f}%"
            )
            return False

        # 检查内存
        required_memory = required.get("memory", 0)
        if required_memory > 0 and availability.get("memory", 1.0) < (
            required_memory / 100
        ):
            logger.debug(
                f"内存资源不足: 需要{required_memory}%，可用{                     availability.get(                         'memory',                         1.0) * 100:.1f}%"
            )
            return False
...
class TaskScheduler:
    ...
    def submit_task(
        ...
        # 生成任务ID
        task_id = (
            f"{task_type.value}_{int(time.time())}_{self.stats['tasks_submitted']}"
        )
        ...
        logger.info(
            f"任务已提交: {task_id} - {description} (优先级: {priority_score:.2f})"
        )
        ...
        for task_id in completed_tasks:
            ...
            if task_thread:
                ...
                if task:
                    ...
                    logger.info(
                        f"任务完成: {task_id} - 执行时间: {execution_time:.2f}秒"
                    )
