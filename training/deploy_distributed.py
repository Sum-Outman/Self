...
class HardwareDetector:
    ...
    def detect_gpus(self) -> List[Dict[str, Any]]:
        ...
        for i in range(num_gpus):
            try:
                ...
                memory_free = (
                    cuda.memory_allocated(i) / (1024**3)
                    if hasattr(cuda, "memory_allocated")
                    else 0
                )
    ...
    def detect_cpu(self) -> Dict[str, Any]:
        ...
        try:
            ...
            cpu_info["memory_available_gb"] = psutil.virtual_memory().available / (
                1024**3
            )
        except ImportError:
...
class DistributedDeployer:
    ...
    def detect_and_configure(self) -> Tuple[DistributedConfig, Dict[str, Any]]:
        ...
        if self.config.mode == "auto":
            ...
            if len(gpu_info) >= 2:
                ...
                self.logger.info(
                    f"自动选择模式: single_node (检测到 {len(gpu_info)} 个GPU)"
                )
        ...
        if gpu_info and not self.config.cpu_only:
            if self.config.gpu_ids:
                ...
                selected_gpus = [
                    gid for gid in self.config.gpu_ids if gid in available_gpus
                ]
        ...
        if self.config.dynamic_batch_size and gpu_info:
            ...
            avg_memory_gb = sum(gpu["memory_total_gb"] for gpu in gpu_info) / len(
                gpu_info
            )
            if avg_memory_gb < 8:  # 小于8GB
                dist_config.batch_size_per_gpu = max(
                    8, dist_config.batch_size_per_gpu // 2
                )
            elif avg_memory_gb > 24:  # 大于24GB
                dist_config.batch_size_per_gpu = min(
                    128, dist_config.batch_size_per_gpu * 2
                )
    ...
    def deploy_single_node(self, dist_config: DistributedConfig) -> bool:
        ...
        try:
            # 创建训练管理器
            create_distributed_training_manager(dist_config)
    ...
    def deploy_master_node(self, dist_config: DistributedConfig) -> bool:
        """部署主节点"""
        self.logger.info(
            f"部署主节点: {self.config.master_addr}:{self.config.master_port}"
        )
        ...
        try:
            ...
            self.logger.info(
                f"Worker启动命令: python deploy_distributed.py --mode worker --master-addr {                     self.config.master_addr}"
            )
    ...
    def launch_training(
        self, model_fn, dataset_fn, train_fn, dist_config: DistributedConfig
    ) -> bool:
        """启动训练"""
    ...
    def _generate_worker_scripts(self, dist_config: DistributedConfig):
        ...
        # 生成Bash脚本
        bash_script = f"""#!/bin/bash # Self AGI 分布式训练Worker启动脚本 # 部署ID: {self.deployment_id} # 生成时间: {time.strftime('%Y-%m-%d %H:%M:%S')}  MASTER_ADDR="{dist_config.master_addr}" MASTER_PORT={dist_config.master_port} WORLD_SIZE={dist_config.world_size}  echo "启动分布式训练Worker..." echo "连接到主节点: $MASTER_ADDR:$MASTER_PORT"  export MASTER_ADDR=$MASTER_ADDR export MASTER_PORT=$MASTER_PORT export WORLD_SIZE=$WORLD_SIZE  # 启动训练 python -m training.distributed_training  echo "Worker启动完成" """
    ...
    def _start_monitoring_service(self, dist_config: DistributedConfig):
        ...
        monitor_script = f"""#!/usr/bin/env python3 # 分布式训练监控服务 import http.server import socketserver import json import time  class MonitoringHandler(http.server.BaseHTTPRequestHandler):     def do_GET(self):         if self.path == '/status':             status = {{                 'deployment_id': '{self.deployment_id}',                 'status': '{self.status}',                 'master_addr': '{dist_config.master_addr}',                 'master_port': {dist_config.master_port},                 'world_size': {dist_config.world_size},                 'timestamp': time.time()             }}              self.send_response(200)             self.send_header('Content-Type', 'application/json')             self.end_headers()             self.wfile.write(json.dumps(status).encode())         else:             self.send_response(404)             self.end_headers()  if __name__ == '__main__':     PORT = {self.config.monitoring_port}      with socketserver.TCPServer(("", PORT), MonitoringHandler) as httpd:         print(f"监控服务运行在端口 {{PORT}}")         httpd.serve_forever() """
...
def main():
    ...
    # 硬件配置
    parser.add_argument(
        "--gpus", type=str, default="", help="使用的GPU ID列表，例如: 0,1,2,3"
    )
    ...
    # 训练配置
    parser.add_argument(
        "--batch-size", type=int, default=32, help="每个GPU的batch size"
    )
    parser.add_argument(
        "--checkpoint-dir",
        type=str,
    ...
    # 资源调度
    parser.add_argument(
        "--no-dynamic-batch", action="store_true", help="禁用动态batch size调整"
    )
    parser.add_argument("--no-amp", action="store_true", help="禁用自动混合精度")

    # 监控配置
    parser.add_argument(
        "--monitoring-port", type=int, default=8080, help="监控服务端口"
    )
    parser.add_argument(
        "--log-level",
        type=str,
    ...
    print(f""" ╔══════════════════════════════════════════════════════╗ ║      Self AGI 分布式训练一键部署工具                ║ ║      版本: 1.0 | 部署模式: {args.mode:12}          ║ ╚══════════════════════════════════════════════════════╝ """)
    ...
    if deploy_config.mode == "single_node":
        ...
        if success:
            print("\n单机多GPU部署完成!")
            print(
                f"配置: {                     dist_config.world_size}个进程, batch size={                     dist_config.batch_size_per_gpu}"
            )
            print("\n接下来:")
        ...
        if success:
            ...
            print(
                f"2. 或执行: python deploy_distributed.py --mode worker --master-addr {                     dist_config.master_addr}"
            )
