...
class MultimodalAGITrainer(AGITrainer):
    ...
    def __init__(
        ...
        # 初始化父类
        super().__init__(model=model, config=training_config, from_scratch=from_scratch)
    ...
    def _create_training_config_from_multimodal(self) -> TrainingConfig:
        ...
        return TrainingConfig(
            ...
            gradient_accumulation_steps=self.multimodal_config.get(
            ...
            checkpoint_dir=self.multimodal_config.get(
    ...
    def get_multimodal_status(self) -> Dict[str, Any]:
        ...
        multimodal_status = {
            ...
            "multimodal_processor_initialized": hasattr(
                self.multimodal_processor, "initialized"
            ),
            "progressive_training_phase": self.multimodal_trainer.progressive_scheduler.current_phase,
            "enabled_tasks": (
                self.multimodal_trainer.loss_fn.tasks
                if hasattr(self.multimodal_trainer.loss_fn, "tasks")
                else []
            ),
        }
...
def create_multimodal_agi_trainer(
    ...
    # 创建多模态AGI训练器
    trainer = MultimodalAGITrainer(multimodal_config=default_config, from_scratch=True)
