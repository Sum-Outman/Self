...
# 自适应损失平衡器
try:
    from .adaptive_loss_balancer import AdaptiveLossBalancer, create_loss_balancer
...
# 导入工业级多模态处理器
try:
    from models.multimodal.processor import MultimodalProcessor, IndustrialTextEncoder, IndustrialVisionEncoder, CrossModalAttention, HierarchicalFusionNetwork
...
# 导入注意力分析器
try:
    from models.multimodal.attention_analyzer import AttentionAnalyzer, AttentionAnalysisResult
...
# 导入真实多模态数据集
try:
    from .real_multimodal_dataset import RealMultimodalDataset, DataSourceType, create_real_multimodal_dataloader
...
class MultimodalDataset(Dataset):
    ...
    def __init__(self, config: Dict[str, Any], mode: str = "train"):
        ...
        # 合成数据已被完全禁用
        raise RuntimeError("合成数据已被完全禁用。MultimodalDataset类不再支持合成数据生成。请使用RealMultimodalDataset类加载真实数据集。")
    ...
    def _generate_text_description(self, category: Dict[str, Any], sample_id: int) -> str:
        """生成文本描述"""
        base_texts = [
    ...
    def _generate_image_features(self, category: Dict[str, Any], sample_id: int) -> torch.Tensor:
        """生成图像特征
    ...
    def _generate_audio_features(self, category: Dict[str, Any], sample_id: int) -> torch.Tensor:
        """生成音频特征
    ...
    def _generate_video_features(self, category: Dict[str, Any], sample_id: int) -> torch.Tensor:
        """生成视频特征
    ...
    def _generate_multitask_labels(self, category: Dict[str, Any], category_idx: int, task_labels: Dict[str, List]) -> Dict[str, Any]:
        """生成多任务标签"""
        labels = {}
        ...
        # 颜色识别标签
        color_to_idx = {color: idx for idx, color in enumerate(task_labels["color_recognition"])}
        labels["color_recognition"] = color_to_idx.get(category["color"], 0)
        
        # 形状识别标签
        shape_to_idx = {shape: idx for idx, shape in enumerate(task_labels["shape_recognition"])}
...
class MultimodalLoss(nn.Module):
    ...
    def __init__(self, tasks: List[str], config: Optional[Dict[str, Any]] = None, multimodal_processor: Optional[Any] = None):
        super().__init__()
    ...
    def forward(self, predictions: Dict[str, torch.Tensor], 
                ...
                task_weights: Optional[Dict[str, float]] = None) -> Tuple[torch.Tensor, Dict[str, float]]:
        """计算多任务损失
                    ...
                    try:
                        ...
                        if task == 'contrastive_learning':
                            ...
                            if self.multimodal_processor is not None:
                                ...
                                if text_features is not None or image_features is not None or audio_features is not None:
                                    try:
                                        ...
                                        if losses:
                                            ...
                                            # 如果没有损失，回退到原始对比损失
                                            task_loss = loss_fn(predictions[task], targets[task], self.temperature)
                                    except Exception as e:
                                        logging.warning(f"使用处理器计算对比损失失败: {e}, 回退到原始对比损失")
                                        task_loss = loss_fn(predictions[task], targets[task], self.temperature)
                                else:
                                    # 模态特征不存在，使用原始对比损失
                                    task_loss = loss_fn(predictions[task], targets[task], self.temperature)
        ...
        if total_loss == 0.0:
            ...
            min_loss = torch.tensor(1e-8, requires_grad=True, 
                                    device=next(predictions.values())[0].device if predictions else 'cpu')
            return min_loss, {"min_loss": 1e-8}
        
        return total_loss, loss_dict
    
    def _contrastive_loss(self, embeddings: torch.Tensor, labels: torch.Tensor, 
        ...
        # 分母：所有样本的相似度之和（排除自身）
        sum_exp = exp_sim.sum(dim=1, keepdim=True) - torch.exp(torch.diag(similarity_matrix)).unsqueeze(1)
    ...
    def set_training_phase(self, phase: str):
        ...
        if phase in valid_phases:
            ...
            logging.warning(f"无效的训练阶段: {phase}, 保持当前阶段: {self.training_phase}")
...
class ProgressiveTrainingScheduler:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.phases = {
            "phase1": {
                ...
                "enabled_tasks": ["text_classification", "image_classification", 
                                 "audio_classification", "video_classification",
                                 "color_recognition", "shape_recognition"],
            ...
            "phase2": {
                ...
                "enabled_tasks": ["text_classification", "image_classification",
                                 "audio_classification", "video_classification",
                                 "cross_modal_matching", "contrastive_learning"],
                "disabled_tasks": [],
            ...
            "phase3": {
                ...
                "enabled_tasks": ["text_classification", "image_classification",
                                 "audio_classification", "video_classification",
                                 "cross_modal_matching", "contrastive_learning",
                                 "masked_language_modeling", "masked_vision_modeling"],
                "disabled_tasks": [],
        ...
        self.total_epochs = sum([phase["duration_weeks"] * 7 for phase in self.phases.values()])  # 假设每天一个epoch
    ...
    def update_progress(self, epoch: int):
        ...
        # 计算总体进度
        total_progress = min(epoch / self.total_epochs, 1.0) if self.total_epochs > 0 else 0.0
        ...
        for phase_name, phase_config in self.phases.items():
            ...
            # 如果是持续阶段，则保持在该阶段
            if phase_duration == 0 and cumulative_weeks <= total_progress * self.total_epochs / 7:
...
class MultimodalTrainer:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.enable_adaptive_loss_balancing = config.get("enable_adaptive_loss_balancing", True)
        if ADAPTIVE_LOSS_BALANCER_AVAILABLE and self.enable_adaptive_loss_balancing:
            ...
            self.logger.info(f"自适应损失平衡器已初始化，策略: {balancer_config['balancing_strategy']}")
        ...
        # 检查点目录
        self.checkpoint_dir = Path(config.get("checkpoint_dir", "checkpoints/multimodal"))
    ...
    def _performance_monitoring_loop(self):
        ...
        while self.monitoring_running:
            try:
                ...
                # 记录到历史
                if len(self.metrics_history["performance_metrics"]) < 1000:  # 限制历史大小
    ...
    def get_performance_report(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "system_info": {
                ...
                "total_memory": psutil.virtual_memory().total if PSUTIL_AVAILABLE else "N/A",
    ...
    def save_checkpoint(self, epoch: int, is_best: bool = False):
        ...
        if isinstance(self.multimodal_processor, (torch.nn.DataParallel, torch.nn.parallel.DistributedDataParallel)):
    ...
    def load_checkpoint(self, checkpoint_path: Union[str, Path], load_optimizer: bool = True):
        """加载训练检查点 - 工业级完整状态恢复"""
        ...
        try:
            ...
            if 'progressive_phase' in checkpoint:
                self.progressive_scheduler.current_phase = checkpoint['progressive_phase']
    ...
    def _setup_device(self) -> torch.device:
        ...
        if use_gpu and torch.cuda.is_available():
            if distributed:
                ...
                self.logger.info(f"分布式训练初始化: 本地rank={local_rank}, 世界大小={world_size}")
            elif len(gpu_ids) > 1:
                # 多GPU数据并行训练
    ...
    def _init_loss_function(self):
        ...
        loss_fn = MultimodalLoss(enabled_tasks, loss_config, multimodal_processor=self.multimodal_processor)
    ...
    def setup_datasets(self):
        ...
        base_config = {
            ...
            "annotations_path": self.config.get("annotations_path", "annotations.jsonl"),
        ...
        if use_real_data and REAL_DATASET_AVAILABLE:
            ...
            try:
                # 创建真实数据集
                self.train_dataset = RealMultimodalDataset(train_config, mode="train", data_source=data_source)
                self.eval_dataset = RealMultimodalDataset(eval_config, mode="eval", data_source=data_source)
                ...
                # 严格禁止回退到合成数据，直接抛出异常
                raise RuntimeError(f"真实数据集不可用: {e}。请确保真实数据集模块正确安装或设置use_real_data=False")
        
        if not use_real_data or not REAL_DATASET_AVAILABLE:
            # 严格禁止合成数据：无论use_real_data设置如何，都要求真实数据集可用
            raise RuntimeError("合成数据已被完全禁用。必须使用真实数据集进行训练。请确保真实数据集模块正确安装并设置use_real_data=True")
    ...
    def setup_dataloaders(self, batch_size: Optional[int] = None):
        ...
        self.logger.info(f"训练批次大小: {batch_size}, 训练批次数量: {len(self.train_loader)}")
    ...
    def train_epoch(self, epoch: int) -> Dict[str, float]:
        ...
        for batch_idx, batch in enumerate(self.train_loader):
            ...
            with torch.cuda.amp.autocast(enabled=self.scaler is not None):
                ...
                if self.adaptive_loss_balancer is not None:
                    ...
                    # batch_task_losses格式: {"task_name_loss": loss_value, "task_name_weight": weight_value}
                    task_loss_dict = {}
                    for key, value in batch_task_losses.items():
                        if key.endswith('_loss'):
                            task_name = key.replace('_loss', '')
                            # 确保任务在平衡器的任务列表中
                            if hasattr(self.adaptive_loss_balancer, 'task_metrics') and task_name in self.adaptive_loss_balancer.task_metrics:
                                task_loss_dict[task_name] = torch.tensor(value, device=self.device, requires_grad=True)
                    
                    # 如果有任务损失，使用平衡器重新计算总损失
                    if task_loss_dict:
                        try:
                            # 使用平衡器计算加权总损失
                            balanced_loss = self.adaptive_loss_balancer.compute_weighted_loss(
                            ...
                            batch_task_losses["balanced_total_loss"] = balanced_loss.item()
                        except Exception as e:
                            self.logger.warning(f"自适应损失平衡失败: {e}，使用原始损失")
            ...
            # 如果启用了性能监控，记录批次时间
            if hasattr(self, 'performance_stats') and 'batch_times' in self.performance_stats:
        ...
        avg_task_losses = {name: np.mean(values) for name, values in task_losses.items()}
    ...
    def _process_batch(self, batch: Dict[str, Any]) -> Dict[str, torch.Tensor]:
        ...
        # 检查多模态处理器是否可用
        if not hasattr(self, 'multimodal_processor') or self.multimodal_processor is None:
            raise RuntimeError("多模态处理器未初始化，请确保正确初始化多模态训练器")
        
        # 检查处理器是否已初始化
        if not hasattr(self.multimodal_processor, 'initialized') or not self.multimodal_processor.initialized:
        ...
        # 验证所有必需的任务输出都存在
        required_tasks = ['text_classification', 'image_classification', 'audio_classification', 
                         'video_classification', 'color_recognition', 'shape_recognition', 
                         'cross_modal_matching', 'contrastive_learning']
    ...
    def evaluate(self) -> Dict[str, float]:
        ...
        with torch.no_grad():
            for batch in self.eval_loader:
                ...
                for task_name, pred in predictions.items():
                    if task_name.endswith("_classification") or task_name.endswith("_recognition"):
        ...
        # 计算平均指标
        avg_loss = total_loss / num_batches if num_batches > 0 else 0.0
        avg_accuracies = {name: np.mean(values) for name, values in task_accuracies.items()}
    ...
    def validate_cross_modal_fusion(self, validation_samples: int = 100) -> Dict[str, Any]:
        """验证跨模态融合质量
        ...
        try:
            ...
            for idx, sample in enumerate(validation_data):
                ...
                if hasattr(self.multimodal_processor, 'extract_attention_weights'):
                    try:
                        sample_attention_weights = self.multimodal_processor.extract_attention_weights(sample)
                        if sample_attention_weights:
                            attention_weights_list.append(sample_attention_weights)
                            self.logger.debug(f"样本{idx}注意力权重提取成功: {len(sample_attention_weights)}个映射")
                    except Exception as e:
                ...
                if self.attention_analyzer and sample_attention_weights:
                    try:
                        ...
                        # 分析注意力权重
                        analysis_result = self.attention_analyzer.analyze_attention_weights(
                        ...
                        if idx < 5:  # 只保存前5个样本的可视化
                            ...
                            self.attention_analyzer.visualize_attention(analysis_result, save_path=vis_path)
            ...
            if modality_similarities:
                ...
                validation_metrics["modality_alignment_scores"]["text_image_similarity"] = avg_similarity
                validation_metrics["modality_alignment_scores"]["similarity_std"] = similarity_std
                validation_metrics["modality_alignment_scores"]["consistency"] = 1.0 - similarity_std / (abs(avg_similarity) + 1e-9)
            
            # 获取投影层缓存信息（如果可用）
            if hasattr(self.multimodal_processor, 'projection_manager'):
                cache_info = self.multimodal_processor.projection_manager.get_cache_info()
                validation_metrics["projection_cache_info"] = cache_info
                
                # 评估投影层缓存效率
                cache_efficiency = cache_info.get("cache_size", 0) / max(cache_info.get("access_counts", 1), 1)
                validation_metrics["fusion_quality_metrics"]["projection_cache_efficiency"] = cache_efficiency
                ...
                try:
                    ...
                    for result in attention_analysis_results:
                        if result.attention_entropy:
                            attention_entropies.extend(result.attention_entropy.values())
                        if result.attention_sparsity:
                            attention_sparsities.extend(result.attention_sparsity.values())
                        fusion_confidences.append(result.fusion_confidence)
                    
                    if attention_entropies:
                        validation_metrics["cross_modal_attention_metrics"]["avg_attention_entropy"] = np.mean(attention_entropies)
                        validation_metrics["cross_modal_attention_metrics"]["attention_entropy_std"] = np.std(attention_entropies)
                    
                    if attention_sparsities:
                        validation_metrics["cross_modal_attention_metrics"]["avg_attention_sparsity"] = np.mean(attention_sparsities)
                    
                    if fusion_confidences:
                        validation_metrics["cross_modal_attention_metrics"]["avg_fusion_confidence"] = np.mean(fusion_confidences)
                    ...
                    # 记录注意力分析样本数
                    validation_metrics["cross_modal_attention_metrics"]["analyzed_samples"] = len(attention_analysis_results)
                    
                except Exception as e:
                    self.logger.warning(f"注意力分析指标计算失败: {e}")
            
            self.logger.info(f"跨模态融合验证完成:")
            if "text_image_similarity" in validation_metrics["modality_alignment_scores"]:
                similarity = validation_metrics["modality_alignment_scores"]["text_image_similarity"]
            ...
            if "projection_cache_info" in validation_metrics:
                ...
                self.logger.info(f"  投影层缓存: {cache_info.get('cache_size', 0)}/{cache_info.get('max_cache_size', 10)}")
            ...
            if "cross_modal_attention_metrics" in validation_metrics:
                ...
                if "avg_attention_entropy" in attn_metrics:
                    self.logger.info(f"  平均注意力熵: {attn_metrics['avg_attention_entropy']:.4f}")
                if "avg_attention_sparsity" in attn_metrics:
                    self.logger.info(f"  平均注意力稀疏度: {attn_metrics['avg_attention_sparsity']:.4f}")
                if "analyzed_samples" in attn_metrics:
                    self.logger.info(f"  分析样本数: {attn_metrics['analyzed_samples']}")
    ...
    def _setup_validation_dataset(self, num_samples: int):
        ...
        try:
            ...
            self.logger.info(f"真实验证数据集已创建，样本数: {len(self.validation_dataset)}")
    ...
    def train(self, num_epochs: Optional[int] = None):
        ...
        for epoch in range(self.current_epoch, num_epochs):
            ...
            # 更新渐进式训练调度器
            phase_config = self.progressive_scheduler.update_progress(epoch)
            ...
            if (epoch + 1) % validation_frequency == 0 or epoch == 0:
                ...
                if "modality_alignment_scores" in fusion_metrics:
                    ...
                    if "text_image_similarity" in alignment_scores:
                        self.metrics_history.setdefault("text_image_similarity", []).append(
                            alignment_scores["text_image_similarity"]
                        )
                
                if "fusion_quality_metrics" in fusion_metrics:
                    quality_metrics = fusion_metrics["fusion_quality_metrics"]
                    for metric_name, metric_value in quality_metrics.items():
                        self.metrics_history.setdefault(metric_name, []).append(metric_value)
    ...
    def _print_training_summary(self, epoch: int, train_metrics: Dict[str, float], eval_metrics: Dict[str, float]):
        """打印训练摘要"""
        summary = self.progressive_scheduler.get_training_summary()
        
        self.logger.info("=" * 80)
        self.logger.info(f"训练摘要 - Epoch {epoch}")
        self.logger.info(f"当前阶段: {summary['phase_name']} (进度: {summary['phase_progress']:.2%})")
    ...
    def load_checkpoint(self, checkpoint_path: str):
        ...
        # 加载多模态处理器状态
        if "processor_state_dict" in checkpoint and hasattr(self.multimodal_processor, 'load_state_dict'):
            self.multimodal_processor.load_state_dict(checkpoint["processor_state_dict"])
        ...
        self.logger.info(f"全局步骤: {self.global_step}, 当前轮次: {self.current_epoch}")
...
def demo_training():
    ...
    print(f"  训练轮次: {summary['progressive_training']['current_epoch']}/{summary['progressive_training']['total_epochs']}")
    print(f"  启用任务: {summary['progressive_training']['phase_config']['enabled_tasks']}")
    ...
    try:
        ...
        print(f"  总体进度: {final_summary['progressive_training']['overall_progress']:.2%}")
