...
class RealMultimodalDataset(Dataset):
    ...
    def __init__(
        ...
        self.annotations_path = Path(
            config.get("annotations_path", "annotations.jsonl")
        )
    ...
    def _load_standard_dataset(self):
        ...
        if self.data_source == DataSourceType.STANDARD_IMAGENET:
            ...
            self._load_librispeech_dataset(
                dataset_dir, audio_datasets, audio_transforms
            )
            ...
            if self.strict_mode:
                raise RuntimeError(
                    f"严格模式已启用：不支持的标准数据集类型: {self.data_source}"
                )
            logger.warning(
                f"不支持的标准数据集类型: {self.data_source}，回退到合成数据"
            )
            self._load_synthetic_data()
    ...
    def _load_coco_dataset(self, dataset_dir, vision_datasets, vision_transforms):
        ...
        try:
            ...
            for img_id in img_ids:
                ...
                # 构建文本描述（基于类别）
                categories = [
                    coco.loadCats(ann["category_id"])[0]["name"] for ann in anns[:3]
                ]
    ...
    def _load_librispeech_dataset(self, dataset_dir, audio_datasets, audio_transforms):
        ...
        # 创建音频转换
        sample_rate = 16000
        transform = audio_transforms.Resample(
        ...
        try:
            if self.mode == "train":
                dataset = audio_datasets.LIBRISPEECH(
                    root=dataset_dir, url="train-clean-100", download=True
                )
            else:
                dataset = audio_datasets.LIBRISPEECH(
                    root=dataset_dir, url="test-clean", download=True
                )
            ...
            for i in range(len(dataset)):
                ...
                item = RealMultimodalItem(
                    ...
                    metadata={
                        "source": "librispeech",
                        "url": (
                            "train-clean-100" if self.mode == "train" else "test-clean"
                        ),
                        "original_index": i,
                    },
                )
    ...
    def _load_kinetics_dataset(self, dataset_dir, vision_datasets, vision_transforms):
        ...
        try:
            ...
            try:
                ...
                logger.info(
                    f"加载Kinetics数据集完成: {len(self.data_items)} 个视频片段"
                )
                return
        ...
        for ext in video_extensions:
            video_files.extend(
                glob.glob(os.path.join(video_dir, f"**/*{ext}"), recursive=True)
            )

        if not video_files:
            logger.warning(f"在目录 {video_dir} 中未找到视频文件")
            if self.strict_mode:
                raise RuntimeError("未找到Kinetics视频文件")
            logger.warning("回退到合成数据")
            self._load_synthetic_data()
            return
    ...
    def _load_synthetic_video_data(self, video_files):
        ...
        if video_files:
            logger.info(
                f"检测到 {len(video_files)} 个视频文件，但真实视频加载功能未实现"
            )
        ...
        try:
            ...
            for i, (image, label) in enumerate(dataset):
                ...
                item = RealMultimodalItem(
                    ...
                    text_tensor=text_tensor,
                    image_tensor=image,
                    labels={"image_classification": [label], "class_index": label},
                    metadata={
                        "source": "cifar100",
                        "index": i,
                        "split": self.mode,
    ...
    def _create_simple_shape_image(
        self, shape_type: str, color_idx: int, image_size: int = 224
    ) -> torch.Tensor:
        ...
        raise RuntimeError(
            ...
            "解决方案：提供真实图像数据，禁止使用任何合成数据生成。\n"
            "降级或回退机制已被禁用，以防止更严重的后果。"
        )
        ...
        with open(self.annotations_path, "r", encoding="utf-8") as f:
            for line_num, line in enumerate(f):
                try:
                    ...
                    # 构建完整路径
                    full_image_path = (
                        self.data_root / image_path if image_path else None
                    )
                    ...
                    item = RealMultimodalItem(
                        ...
                        file_paths={
                            "image": str(full_image_path) if full_image_path else ""
                        },
                        raw_text=text,
    ...
    def _load_multimodal_data(self):
        ...
        for format_name, loader_func in standard_formats:
            ...
            if format_path.exists():
                try:
                    ...
                    if len(self.data_items) == 0:
                        logger.warning(
                            f"{format_name} 未包含有效数据，继续尝试其他格式"
                        )
                        continue
                    return
                except Exception as e:
        ...
        if len(self.data_items) == 0:
            error_msg = (
                "所有多模态数据加载尝试均失败。请确保数据目录包含有效的多模态数据"
            )
            if self.strict_mode:
            ...
            for parsed_record in parsed_dataset.take(10000):  # 限制读取数量
                ...
                if image_data:
                    ...
                    self.image_paths.append(
                        f"tfrecord_image_{len(self.image_paths)}.bin"
                    )
                    # 在实际实现中，可能需要将图像数据保存到临时文件

                text_data = (
                    parsed_record["text"].numpy().decode("utf-8", errors="ignore")
                )
    ...
    def _load_item(self, index: int) -> RealMultimodalItem:
        ...
        # 根据数据源类型加载实际数据
        if item.data_source in [
            DataSourceType.REAL_IMAGE_TEXT,
            DataSourceType.REAL_MULTIMODAL,
        ]:
    ...
    def __getitem__(self, index: int) -> Dict[str, Any]:
        ...
        # 处理文本
        text_tensor = (
            self._process_text(item.raw_text)
            if item.raw_text
            else torch.zeros(self.max_sequence_length, dtype=torch.long)
        )
...
def create_real_multimodal_dataloader(
    ...
    **dataloader_kwargs,
    ...
    logger.info(
        f"创建真实多模态数据加载器: 模式={mode}, 批次大小={default_kwargs['batch_size']}"
    )
...
def test_real_multimodal_dataset():
    ...
    try:
        ...
        synthetic_dataset = RealMultimodalDataset(
            test_config, mode="train", data_source=DataSourceType.SYNTHETIC
        )
