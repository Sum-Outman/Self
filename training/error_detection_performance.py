...
class StatisticalErrorDetector:
    ...
    def _detect_moving_average_anomaly(self, value: float, data_points: List[float]) -> Dict[str, Any]:
        """检测移动平均异常"""
        ...
        # 计算标准差
        std = statistics.stdev(data_points[-long_window:]) if len(data_points[-long_window:]) > 1 else 0.0
    ...
    def _detect_iqr_anomaly(self, value: float, data_points: List[float]) -> Dict[str, Any]:
        """检测IQR（四分位距）异常"""
    ...
    def _detect_seasonal_anomaly(self, value: float, data_points: List[float], timestamp: float) -> Dict[str, Any]:
        """检测季节性异常（完整版）"""
        ...
        if hour in hour_data and len(hour_data[hour]) >= 5:
            ...
            hour_std = statistics.stdev(hour_data[hour]) if len(hour_data[hour]) > 1 else 0.0
...
class MachineLearningErrorDetector:
    ...
    def train_isolation_forest(self, 
        ...
        try:
            ...
            self.logger.info(f"隔离森林模型训练完成: {metric_id}, 数据点={len(data_points)}")
            return True
    ...
    def detect_with_isolation_forest(self, 
                                    metric_id: str, 
    ...
    def cluster_analysis(self, 
                        ...
                        metric_id: str = "default") -> Dict[str, Any]:
        """聚类分析检测异常"""
        if not SKLEARN_AVAILABLE:
        ...
        try:
            ...
            n_clusters = len(unique_labels) - (1 if -1 in unique_labels else 0)  # -1表示噪声
...
class RuleBasedErrorDetector:
    ...
    def check_rules(self, metrics: Dict[str, float], context: Dict[str, Any] = None) -> List[Dict[str, Any]]:
        """检查所有规则"""
        triggered_rules = []
...
class PerformanceDiagnoser:
    ...
    def _detect_performance_degradation(self, metric_id: str):
        ...
        # 检查均值是否显著变化
        mean_change_pct = abs((new_mean - old_mean) / old_mean) * 100 if old_mean != 0 else 0.0
        std_change_pct = abs((new_std - old_std) / old_std) * 100 if old_std != 0 else 0.0
    ...
    def _guess_metric_type(self, metric_id: str) -> str:
        ...
        if any(word in metric_id_lower for word in ["latency", "delay", "response_time", "rt"]):
            return "latency"
        elif any(word in metric_id_lower for word in ["throughput", "tps", "qps", "requests_per_second"]):
            return "throughput"
        elif any(word in metric_id_lower for word in ["accuracy", "precision", "recall", "f1"]):
            return "accuracy"
    ...
    def get_performance_summary(self, metric_id: str) -> Dict[str, Any]:
        ...
        stats = {
            ...
            "cv": (statistics.stdev(values) / statistics.mean(values)) * 100 if statistics.mean(values) != 0 else 0.0
        }
    ...
    def get_bottleneck_analysis(self, metrics: Dict[str, float]) -> List[Dict[str, Any]]:
        """瓶颈分析"""
        bottlenecks = []
        ...
        if len(bottlenecks) >= 2:
            ...
            memory_bottlenecks = [b for b in bottlenecks if b["type"] == "memory_bottleneck"]
            
            if cpu_bottlenecks and memory_bottlenecks:
                bottlenecks.append({
                    "type": "resource_contention",
                    "metric": "multiple",
                    "value": max(cpu_bottlenecks[0]["value"], memory_bottlenecks[0]["value"]),
                    "severity": "critical",
...
class RootCauseAnalyzer:
    ...
    def analyze_error_pattern(self, 
                             error_records: List[ErrorRecord]) -> Dict[str, Any]:
        """分析错误模式"""
        ...
        if timestamps:
            ...
            errors_per_hour = len(error_records) / (time_range / 3600) if time_range > 0 else 0
        else:
            time_range = 0
            errors_per_hour = 0
        ...
        if len(error_records) >= 10:
            ...
            if intervals:
                ...
                # 检查是否存在聚集模式
                clustered_errors = sum(1 for interval in intervals if interval < avg_interval / 2)
                clustering_ratio = clustered_errors / len(intervals) if intervals else 0.0
        ...
        result = {
            ...
            "frequent_errors": sorted(frequent_errors, key=lambda x: x["count"], reverse=True)[:10],
        ...
        if len(error_records) >= 2:
            ...
            for i in range(len(sorted_errors) - 1):
                ...
                if time_diff < 300 and category_same:  # 5分钟内
                    node1_id = f"error_{hashlib.md5(error1.error_id.encode()).hexdigest()[:8]}"
                    node2_id = f"error_{hashlib.md5(error2.error_id.encode()).hexdigest()[:8]}"
    ...
    def _generate_recommendations(self,
                                 ...
                                 frequent_errors: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """生成推荐行动"""
        recommendations = []
        ...
        unique_recs.sort(key=lambda x: (x["priority"], urgency_order.get(x.get("urgency", "low"), 3)))
        
        return unique_recs
    
    def predict_future_errors(self, 
                             historical_data: List[ErrorRecord],
...
class ErrorDetectionAndPerformanceDiagnosisManager:
    ...
    def diagnose_performance(self, performance_metrics: Dict[str, Any]) -> Dict[str, Any]:
        """诊断性能
    ...
    def record_error(self,
        ...
        error_id = hashlib.md5(
            f"{message}{source}{time.time()}".encode()
        ).hexdigest()[:16]
        ...
        if detection_results:
            error_record.correction_suggested = self._generate_correction_suggestion(detection_results)
        
        self.logger.info(f"记录错误: {category.value}/{severity.value}: {message[:50]}...")
    ...
    def record_performance_metric(self,
        ...
        if self.enable_ml_detection:
            ...
            if metric_id not in self.ml_detector.is_trained:
                # 收集历史数据
                historical_values = [
                    r.value for r in self.performance_records 
    ...
    def _detect_error_patterns(self,
        ...
        if self.enable_statistical_detection:
            # 检查错误率
            recent_errors = [
                e for e in self.error_records[-100:]  # 最近100个错误
    ...
    def _generate_correction_suggestion(self, detection_results: List[Dict[str, Any]]) -> str:
        """生成改正建议"""
    ...
    def get_system_health_report(self) -> Dict[str, Any]:
        ...
        error_stats = {
            ...
            "errors_last_hour": len([e for e in self.error_records 
                                    if e.timestamp > time.time() - 3600]),
            "errors_last_day": len([e for e in self.error_records 
        ...
        root_cause_analysis = {}
        if recent_errors:
            root_cause_analysis = self.root_cause_analyzer.analyze_error_pattern(recent_errors)
    ...
    def _calculate_health_score(self, 
                               ...
                               performance_stats: Dict[str, Any]) -> float:
        """计算健康评分（0-100）"""
        score = 100.0
        ...
        # 严重错误惩罚
        critical_errors = error_stats.get("by_severity", {}).get(ErrorSeverity.CRITICAL.value, 0)
    ...
    def get_detector_statistics(self) -> Dict[str, Any]:
        ...
        stats = {
            "statistical_detector": {
                ...
                "statistics_cache_count": len(self.statistical_detector.statistics_cache)
            },
            "ml_detector": {
                "trained_models_count": len([k for k, v in self.ml_detector.is_trained.items() if v]),
            ...
            "performance_diagnoser": {
                ...
                "degradation_alerts_count": sum(len(alerts) for alerts in self.performance_diagnoser.degradation_alerts.values())
            },
            "root_cause_analyzer": {
                "historical_cases_count": len(self.root_cause_analyzer.historical_cases),
...
class AutoRepairAndOptimizationManager:
    ...
    def continuous_optimization(self,
        ...
        if not optimization_func:
            self.logger.warning(f"未知优化目标: {optimization_target}，使用通用优化策略")
    ...
    def _repair_high_error_rate(self,
                               error_context: Dict[str, Any],
                               performance_metrics: Dict[str, float]) -> Dict[str, Any]:
        """修复高错误率问题"""
        ...
        try:
            ...
            return {
                ...
                "estimated_improvement": min(0.8, error_rate * 0.5),  # 估计减少50%错误率
    ...
    def _repair_performance_degradation(self,
                                      error_context: Dict[str, Any],
                                      performance_metrics: Dict[str, float]) -> Dict[str, Any]:
        """修复性能退化问题"""
        start_time = time.time()
        ...
        try:
            ...
            if degradation_factor > 1.5:  # 性能下降超过50%
                repair_actions.append({
                    ...
                    "parameters": {"algorithm": "optimized", "complexity": "O(n log n)"}
                })
        ...
        try:
            ...
            if memory_growth_rate > 0.01:  # 每秒增长1%
                repair_actions.append({
                    ...
                    "parameters": {"gc_frequency": "high", "collect_generations": "all"}
                })
            ...
            return {
                ...
                "repair_actions": repair_actions,
                "execution_time_ms": execution_time_ms,
                "estimated_improvement": 1.0 - min(0.9, memory_growth_rate * 10),  # 减少内存泄漏
                "verification_required": True
            }
    ...
    def _optimize_performance(self,
        ...
        try:
            ...
            target_perf = target_metrics.get("performance_score", current_perf * 1.2)  # 默认提高20%
            ...
            optimization_actions.append({
                ...
                "parameters": {"profiling_enabled": True, "optimization_level": "high"}
            })
            ...
            optimization_actions.append({
                ...
                "parameters": {"optimization_flags": "-O3", "link_time_optimization": True}
            })
            ...
            if current_perf < target_perf * 0.7:  # 性能差距较大
                optimization_actions.append({
                    ...
                    "parameters": {"algorithm_review": True, "complexity_target": "O(n log n)"}
                })
            
            execution_time_ms = (time.time() - start_time) * 1000
            improvement_percentage = ((target_perf - current_perf) / max(0.001, current_perf)) * 100
            ...
            return {
                ...
                "estimated_performance": min(target_perf, current_perf * 1.3),  # 最多提高30%
                "verification_required": True
            }
    ...
    def _optimize_memory(self,
        ...
        try:
            ...
            target_memory = target_metrics.get("memory_usage_mb", current_memory * 0.8)  # 默认减少20%
            ...
            optimization_actions.append({
                ...
                "parameters": {"data_structure_review": True, "use_compact_structures": True}
            })
            ...
            if current_memory > 100:  # 内存使用超过100MB
                optimization_actions.append({
                    ...
                    "parameters": {"memory_pool_size": 1024, "pool_management": True}
                })
            ...
            execution_time_ms = (time.time() - start_time) * 1000
            improvement_percentage = ((current_memory - target_memory) / max(0.001, current_memory)) * 100
            ...
            return {
                ...
                "execution_time_ms": execution_time_ms,
                "improvement_percentage": improvement_percentage,
                "estimated_memory": max(target_memory, current_memory * 0.7),  # 最多减少30%
                "verification_required": True
            }
    ...
    def get_repair_report(self) -> Dict[str, Any]:
        ...
        successful_repairs = len([r for r in self.repair_history if r.get("success", False)])
...
def get_global_error_detection_manager() -> ErrorDetectionAndPerformanceDiagnosisManager:
    """获取全局错误检测和性能诊断管理器"""
...
if __name__ == "__main__":
    ...
    print(f"  统计检测器: {detector_stats['statistical_detector']['data_windows_count']} 个数据窗口")
    print(f"  ML检测器: {detector_stats['ml_detector']['trained_models_count']} 个训练模型")
    ...
    if manager.error_records:
        ...
        root_cause_result = manager.root_cause_analyzer.analyze_error_pattern(recent_errors)
        
        print(f"  分析 {len(recent_errors)} 个错误")
        print(f"  发现 {len(root_cause_result.get('frequent_errors', []))} 个频繁错误模式")
        
        if root_cause_result.get('recommended_actions'):
            print(f"  推荐行动: {len(root_cause_result['recommended_actions'])} 条")
    
    print("\n6. 测试性能诊断...")
    performance_summary = manager.performance_diagnoser.get_performance_summary("latency_metric_0")
    if "error" not in performance_summary:
        print(f"  延迟指标统计: 均值={performance_summary.get('mean', 0):.1f}ms, "
