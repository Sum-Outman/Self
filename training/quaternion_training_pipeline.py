...
from models.quaternion_optimizer import (
    ...
    QuaternionMixedLoss, QuaternionGradientClipper
)
from models.quaternion_dataset import QuaternionDataset, QuaternionDatasetConfig
from models.quaternion_data_pipeline import QuaternionDataPipeline
...
class QuaternionTrainingConfig:
    ...
    optimizer_type: str = "quaternion_adam"  # "quaternion_adam", "quaternion_sgd", "adam", "sgd"
    ...
    loss_weights: Dict[str, float] = field(default_factory=lambda: {
        "angle": 0.5, "dot": 0.3, "double_cover": 0.2
    })
...
class QuaternionTrainingPipeline:
        ...
        if self.config.lr_scheduler_type == "cosine":
            ...
            def linear_lambda(step):
                if step < self.config.warmup_steps:
                    ...
                    progress = float(step - self.config.warmup_steps) / float(max(1, total_steps - self.config.warmup_steps))
    ...
    def evaluate(self):
        ...
        logger.info(f"评估步骤 {self.step_count}: 损失 = {avg_loss:.6f}, 最佳损失 = {self.best_loss:.6f}")
    ...
    def save_checkpoint(self, is_best: bool = False):
        ...
        checkpoint_path = os.path.join(
            ...
            f"checkpoint_step_{self.step_count}.pt"
        )
...
def test_quaternion_training_pipeline():
    ...
    # 创建数据配置
    from models.quaternion_dataset import QuaternionDatasetMode, QuaternionAugmentationType
    data_config = QuaternionDatasetConfig(
        data_sources=[
        ...
        batch_size=8,
        augmentations=[QuaternionAugmentationType.NOISE, QuaternionAugmentationType.RANDOM_ROTATION],
