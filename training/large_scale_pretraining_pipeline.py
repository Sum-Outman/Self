...
class LargeScaleDataPipelineConfig:
    ...
    def __post_init__(self):
        ...
        if self.cache_config.disk_cache_dir is None:
            self.cache_config.disk_cache_dir = str(
                Path.home() / ".cache" / "self_agi" / "data_cache"
            )

        # 创建缓存目录
        Path(self.cache_config.disk_cache_dir).mkdir(parents=True, exist_ok=True)

        logger.info("大规模数据管道配置初始化完成")
        logger.info(f"数据格式: {self.data_format.value}")
        logger.info(
            f"分布式配置: rank={                 self.distributed_rank}, world_size={                 self.distributed_world_size}"
        )
        ...
        with self.lock:
            ...
            if disk_path.exists():
                try:
                    ...
                    # 检查是否过期
                    if (
                        time.time() - os.path.getmtime(disk_path)
        ...
        with self.lock:
            ...
            try:
                for disk_path in self.disk_cache_dir.rglob("*.pkl"):
                    if (
                        current_time - os.path.getmtime(disk_path)
                        > self.config.cache_ttl_seconds
                    ):
                        disk_path.unlink(missing_ok=True)
            except OSError as e:
...
class DataQualityFilter:
    ...
    def filter_image(
        self, image: Image.Image, image_path: Optional[str] = None
    ) -> Tuple[bool, str]:
        """过滤图像数据"""
        try:
            # 检查图像模式
...
class LargeScalePretrainingPipeline(IterableDataset):
    ...
    def _initialize_data_sources(
        self, data_source_configs: List[Dict[str, Any]]
    ) -> List[Any]:
        """初始化数据源"""
        data_sources = []
    ...
    def _apply_augmentations(self, item: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for key in list(augmented_item.keys()):
            if key.endswith("_image") and isinstance(augmented_item[key], Image.Image):
                ...
                # 随机水平翻转
                if random.random() < self.config.augmentation_config.get(
                ...
                if "rotation_range" in self.config.augmentation_config:
                    angle = random.uniform(
                        *self.config.augmentation_config["rotation_range"]
                    )
                ...
                if "color_jitter" in self.config.augmentation_config:
                    ...
                    jitter = ColorJitter(
                        **self.config.augmentation_config["color_jitter"]
                    )
                    image = jitter(image)
    ...
    def get_dataloader(self) -> DataLoader:
        ...
        if (
            ...
            and self.config.distributed_world_size > 1
        ):
...
def create_large_scale_pipeline_example():
    ...
    config = LargeScaleDataPipelineConfig(
        data_sources=[
            {"type": "local", "path": "data/pretraining", "pattern": "**/*.jsonl"}
        ],
        data_format=DataFormat.JSONL,
        processing_config=ProcessingConfig(
            num_workers=8, batch_size=256, prefetch_factor=4, shuffle_buffer_size=50000
        ),
