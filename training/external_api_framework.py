...
class APIProvider(ABC):
    ...
    def prepare_headers(self) -> Dict[str, str]:
        """准备请求头（包含认证信息）"""
        headers = {"User-Agent": "Self-AGI/1.0", "Content-Type": "application/json"}
    ...
    def check_rate_limit(self) -> bool:
        ...
        if len(self.request_timestamps) >= self.config.rate_limit_per_minute:
            self.logger.warning(
                f"速率限制: {len(self.request_timestamps)}/{self.config.rate_limit_per_minute}"
            )
            return False
    ...
    def make_request(self, request: APIRequest) -> APIResponse:
        ...
        try:
            ...
            # 记录请求开始
            self.logger.debug(
                f"发送API请求到: {url}, 方法: {request.method}, 超时: {request.timeout}s"
            )
        ...
        try:
            ...
            if request.method.upper() == "GET":
                response = requests.get(
                    url, params=request.params, headers=headers, timeout=timeout
                )
            elif request.method.upper() == "POST":
            ...
            return APIResponse(
                success=False, error=error_msg, request_time_ms=request_time_ms
            )
...
class LLMAPIProvider(RESTAPIProvider):
    ...
    def complete(
        ...
        request = APIRequest(endpoint=endpoint, method="POST", data=data)
    ...
    def embed(self, texts: List[str], **kwargs) -> APIResponse:
        ...
        if provider == "openai":
            ...
            data["model"] = self.config.metadata.get(
    ...
    def analyze_image(
        ...
        **kwargs,
        ...
        if not image_url and not image_base64:
            return APIResponse(success=False, error="必须提供image_url或image_base64")

        features = features or ["objects", "faces", "labels"]

        # 准备请求数据
        data = {"features": features, **kwargs}
...
class APIManager:
    ...
    def _load_default_configs(self):
        ...
        for config in api_configs:
            try:
                ...
                if success:
                    registered_count += 1
                    self.logger.info(
                        f"已注册API提供者: {config.provider} ({config.api_type.value})"
                    )
                else:
        ...
        if registered_count > 0:
            ...
            self.logger.info(
                "  - AZURE_OPENAI_API_KEY 和 AZURE_OPENAI_ENDPOINT: Azure OpenAI配置"
            )
    ...
    def get_provider(
        self, api_type: APIType, provider: Optional[str] = None
    ...
    def make_request(
        ...
        # 执行请求
        request = APIRequest(
            endpoint=endpoint, method=method, data=data or {}, params=params or {}
        )
    ...
    def vision_analyze(
        ...
        **kwargs,
        ...
        return api_provider.analyze_image(
            image_url=image_url, image_base64=image_base64, **kwargs
        )
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        overall_stats = {
            ...
            "enabled_providers": sum(
                1 for p in self.providers.values() if p.config.enabled
            ),
        ...
        return {"overall": overall_stats, "providers": stats}
...
class ExternalAPILearningManager:
    ...
    def create_learning_session(
        ...
        try:
            ...
            for api_type in api_types:
                ...
                if provider:
                    available_providers.append(
                        (api_type.value, provider.config.provider)
                    )
                else:
            ...
            self.logger.info(
                f"运行模式: {operational_mode}, 可用API提供者: {available_providers}"
            )
    ...
    def collect_learning_data(
        ...
        try:
            ...
            if operational_mode == "limited":
                # 受限模式：没有可用的API提供者
                self.logger.warning(
                    f"会话 {session_id} 在受限模式下运行，没有可用的API提供者"
                )
            ...
            session["progress"] = min(
                1.0, session["data_points_collected"] / self.max_data_points_per_session
            )
            ...
            self.logger.info(
                f"为会话 {session_id} 收集了 {len(collected_data)} 个{data_type}数据点"
            )
    ...
    def _collect_text_data(
        self, session_id: str, query: Optional[str], count: int
    ) -> List[Dict[str, Any]]:
        """收集文本数据"""
        session = self.learning_sessions[session_id]
        text_data = []
        ...
        try:
            # 调用LLM API获取文本数据
            prompt = f"""作为学习数据收集的一部分，请提供以下信息：  学习目标: {session['learning_objective']} 具体要求: {query}  请以结构化的格式提供{count}个相关的学习数据点，每个数据点包含： 1. 主题/标题 2. 详细描述 3. 关键概念 4. 相关示例 5. 学习要点 """
            response = llm_provider.complete(
                prompt=prompt, max_tokens=4000, temperature=0.7
            )
            ...
            if response.success and response.data:
                ...
                self.logger.info(
                    f"从{                         llm_provider.config.provider}收集到文本数据，长度: {                         len(                             str(text_content))}"
                )
    ...
    def _collect_image_data(
        ...
        try:
            ...
            image_search_enabled = (
                self.config.get("enable_image_search", False)
                if hasattr(self, "config")
                else False
            )
            image_search_api_key = (
                self.config.get("image_search_api_key", "")
                if hasattr(self, "config")
                else ""
            )
            ...
            if image_search_enabled and image_search_api_key:
                ...
                #                 })
                self.logger.info(
                    "图像搜索API已配置但未实现，需要集成实际的图像搜索服务"
                )
            else:
                # 没有图像搜索配置，记录信息性日志
                self.logger.info("图像数据收集需要配置图像搜索API或提供图像源")
                self.logger.info(
                    "建议：配置Unsplash、Pexels或Google Images API密钥以启用图像数据收集"
                )
    ...
    def _collect_knowledge_data(
        ...
        try:
            # 调用LLM API获取知识数据
            prompt = f"""作为知识学习数据收集的一部分，请提供以下信息：  学习目标: {session['learning_objective']} 具体要求: {query}  请以结构化的格式提供{count}个相关的知识数据点，每个数据点包含： 1. 概念/术语 2. 详细定义 3. 关键特性 4. 相关示例 5. 应用场景 """
            response = llm_provider.complete(
                prompt=prompt, max_tokens=4000, temperature=0.7
            )
            ...
            if response.success and response.data:
                ...
                if isinstance(knowledge_content, dict):
                    # 尝试提取文本内容
                    if (
                        "choices" in knowledge_content
                        and len(knowledge_content["choices"]) > 0
                    ):
                        knowledge_content = (
                            knowledge_content["choices"][0]
                            .get("message", {})
                            .get("content", "")
                        )
                ...
                self.logger.info(
                    f"从{                         llm_provider.config.provider}收集到知识数据，长度: {                         len(                             str(knowledge_content))}"
                )
    ...
    def _collect_code_data(
        ...
        try:
            # 调用LLM API获取代码数据
            prompt = f"""作为编程学习数据收集的一部分，请提供以下信息：  学习目标: {session['learning_objective']} 具体要求: {query}  请提供{count}个相关的代码示例，每个示例包含： 1. 编程语言 2. 代码片段 3. 功能说明 4. 使用示例 5. 注意事项 """
            response = llm_provider.complete(
                prompt=prompt, max_tokens=4000, temperature=0.7
            )
            ...
            if response.success and response.data:
                ...
                self.logger.info(
                    f"从{                         llm_provider.config.provider}收集到代码数据，长度: {                         len(                             str(code_content))}"
                )
    ...
    def get_learning_data(
        self, session_id: str, data_type: Optional[str] = None, limit: int = 100
    ) -> List[Dict[str, Any]]:
        """获取学习数据
    ...
    def prepare_training_data(
        self, session_id: str, data_type: str, format: str = "text"
    ) -> Dict[str, Any]:
        """准备训练数据
...
if __name__ == "__main__":
    ...
    llm_response = api_manager.llm_complete(
        prompt="你好，世界！", provider="openai", max_tokens=50
    )
    ...
    if created:
        ...
        collected_data = learning_manager.collect_learning_data(
            session_id=session_id, data_type="text", query="Python编程基础", count=3
        )
        ...
        if collected_data:
            ...
            learning_data = learning_manager.get_learning_data(
                session_id=session_id, data_type="text", limit=5
            )
            print(f"学习数据数量: {len(learning_data)}")

            # 准备训练数据
            training_data = learning_manager.prepare_training_data(
                session_id=session_id, data_type="text", format="json"
            )

            if "error" not in training_data:
                print(
                    f"训练数据准备成功，包含 {training_data.get('item_count', 0)} 个项目"
                )
            else:
        ...
        for session_info in all_sessions:
            print(
                f"  - {                     session_info['session_id']}: {                     session_info['learning_objective']} ({                     session_info['status']})"
            )

        # 清理测试会话
        cleaned = learning_manager.clear_old_sessions(
            max_age_hours=0
        )  # 清理所有已完成会话
