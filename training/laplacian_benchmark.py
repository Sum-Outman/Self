...
class LaplacianBenchmark:
    ...
    def _benchmark_basic_operations(self):
        ...
        try:
            ...
            logger.info(
                f"  ✓ {test_name}: {                     avg_time:.2f} ± {                     std_time:.2f} ms, 内存: {                     avg_memory:.2f} MB"
            )
    ...
    def _benchmark_laplacian_matrix(self):
        ...
        try:
            ...
            laplacian_calculator = GraphLaplacian(
                device=self.device, normalization="sym", dtype=torch.float32
            )
            ...
            for i in range(min(self.num_iterations, 5)):  # 减少迭代次数
                ...
                result = laplacian_calculator.compute_knn_laplacian(
                    features=features, k=k_neighbors, return_components=True
                )
        ...
        try:
            ...
            for i in range(self.num_iterations):
                ...
                features = torch.randn(
                    batch_size, feature_dim, device=self.device_torch
                )
    ...
    def _benchmark_laplacian_pinn(self):
        ...
        try:
            ...
            pinn_config = SimplePINNConfig(
                input_dim=coord_dim, output_dim=output_dim, hidden_dims=[64, 64, 64]
            )
            ...
            class SimplePINN(nn.Module):
                def __init__(self, config):
                    ...
                    for hidden_dim in config.hidden_dims:
                        ...
                        layers.append(
                            nn.Tanh() if config.activation == "tanh" else nn.ReLU()
                        )
            ...
            for i in range(self.num_iterations):
                ...
                if laplacian_config.laplacian_reg_enabled:
                    ...
                    loss + laplacian_config.laplacian_reg_lambda * reg_loss
                else:
                    pass
    ...
    def _benchmark_laplacian_cnn(self):
        ...
        try:
            ...
            for i in range(min(self.num_iterations, 5)):  # 减少迭代次数
                ...
                # 创建输入数据
                images = torch.randn(
                    batch_size, channels, height, width, device=self.device_torch
                )
            ...
            class SimpleFusionModel(nn.Module):
                def __init__(self):
                    ...
                    self.fusion = nn.Sequential(
                        nn.Linear(64 + 64, 128), nn.ReLU(), nn.Linear(128, 10)
                    )
        ...
        try:
            ...
            for i in range(self.num_iterations):
                ...
                adjacency = torch.zeros(
                    (batch_size, batch_size), device=self.device_torch
                )
                ...
                # 4. 正则化计算
                torch.trace(features.T @ L @ features)
    ...
    def generate_report(self, results: List[BenchmarkResult] = None) -> str:
        ...
        if successful_tests:
            ...
            report_lines.append(
                f"  最耗时模块: {                     slowest_test.module_name} - {                     slowest_test.test_name} ({                     slowest_test.avg_time_ms:.2f} ms)"
            )
            report_lines.append(
                f"  最高内存模块: {                     highest_memory_test.module_name} - {                     highest_memory_test.test_name} ({                     highest_memory_test.memory_usage_mb:.2f} MB)"
            )
        ...
        if failed_tests:
            ...
            for result in failed_tests:
                report_lines.append(
                    f"  {result.module_name} - {result.test_name}: {result.error_message}"
                )
...
def main():
    ...
    benchmark = LaplacianBenchmark(
        device=device, num_iterations=5  # 减少迭代次数以加快测试速度
    )
