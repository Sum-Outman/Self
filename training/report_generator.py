...
class ChartGenerator:
    ...
    def __init__(
        self, style: str = "seaborn", palette: str = "viridis", dpi: int = 300
    ):
        """初始化图表生成器"""
        self.style = style
    ...
    def generate_loss_curve(
        ...
        if validation_losses:
            if len(validation_losses) <= len(training_losses):
                ...
                ax.plot(
                    val_epochs, validation_losses, "r--", label="验证损失", linewidth=2
                )
            else:
                ax.plot(
                    epochs,
                    validation_losses[: len(training_losses)],
                    "r--",
                    label="验证损失",
                    linewidth=2,
                )
        ...
        if training_losses:
            ...
            ax.annotate(
                ...
                xytext=(
                    min_epoch + len(training_losses) * 0.05,
                    min_loss + max(training_losses) * 0.1,
                ),
    ...
    def generate_accuracy_curve(
        ...
        if validation_accuracies:
            if len(validation_accuracies) <= len(training_accuracies):
                ...
                ax.plot(
                    epochs,
                    validation_accuracies[: len(training_accuracies)],
                    "m--",
                    label="验证准确率",
                    linewidth=2,
                )
        ...
        if training_accuracies:
            ...
            ax.annotate(
                ...
                xytext=(
                    max_epoch + len(training_accuracies) * 0.05,
                    max_acc - max(training_accuracies) * 0.1,
                ),
                arrowprops=dict(arrowstyle="->", color="red", lw=1.5),
                fontsize=10,
                color="red",
    ...
    def generate_learning_rate_curve(
        self,
        learning_rates: List[float],
        title: str = "学习率变化曲线",
        xlabel: str = "步数",
    ...
    def generate_resource_usage_chart(
        ...
        ax2 = axes[0, 1]
        ax2.plot(
            timestamps, resource_metrics.memory_usage_percent, "green", linewidth=2
        )
        ...
        if resource_metrics.gpu_usage_percent:
            ax3.plot(
                timestamps[: len(resource_metrics.gpu_usage_percent)],
                resource_metrics.gpu_usage_percent,
                "red",
                linewidth=2,
            )
        ...
        if resource_metrics.gpu_memory_usage_percent:
            ax4.plot(
                timestamps[: len(resource_metrics.gpu_memory_usage_percent)],
                resource_metrics.gpu_memory_usage_percent,
                "orange",
                linewidth=2,
            )
    ...
    def generate_gradient_distribution(
        self,
        gradient_norms: List[float],
        title: str = "梯度分布",
        xlabel: str = "梯度范数",
...
class ReportGenerator:
    ...
    def __init__(self, config: Optional[ReportConfig] = None):
        ...
        logger.info(
            f"报告生成器初始化: 类型={                 self.config.report_type.value}, 格式={                 self.config.report_format.value}"
        )
        ...
        if self.config.include_charts:
            try:
                ...
                loss_fig = self.chart_generator.generate_loss_curve(
                    ...
                    validation_losses=(
                        training_metrics.validation_losses
                        if hasattr(training_metrics, "validation_losses")
                        else None
                    ),
                    title="训练损失曲线",
                )
                ...
                accuracy_fig = self.chart_generator.generate_accuracy_curve(
                    ...
                    validation_accuracies=(
                        training_metrics.validation_accuracies
                        if hasattr(training_metrics, "validation_accuracies")
                        else None
                    ),
                    title="训练准确率曲线",
                )
                charts["accuracy_curve"] = self.chart_generator.figure_to_base64(
                    accuracy_fig
                )
                ...
                if (
                    ...
                    and len(training_metrics.learning_rates) > 0
                ):
                    lr_fig = self.chart_generator.generate_learning_rate_curve(
                        learning_rates=training_metrics.learning_rates,
                        title="学习率变化曲线",
                    )
                    charts["learning_rate_curve"] = (
                        self.chart_generator.figure_to_base64(lr_fig)
                    )

                # 梯度分布（如果有）
                if (
                    training_metrics.gradient_norms
                    and len(training_metrics.gradient_norms) > 0
                ):
                    gradient_fig = self.chart_generator.generate_gradient_distribution(
                        gradient_norms=training_metrics.gradient_norms, title="梯度分布"
                    )
                    charts["gradient_distribution"] = (
                        self.chart_generator.figure_to_base64(gradient_fig)
                    )
                ...
                if resource_metrics and len(resource_metrics.cpu_usage_percent) > 0:
                    resource_fig = self.chart_generator.generate_resource_usage_chart(
                        resource_metrics=resource_metrics, title="资源使用情况"
                    )
                    charts["resource_usage"] = self.chart_generator.figure_to_base64(
                        resource_fig
                    )
    ...
    def _calculate_statistics(self, report_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        stats = {
            ...
            "samples_per_second": (
                metrics["total_steps"] / metrics["total_duration"]
                if metrics["total_duration"] > 0
                else 0
            ),
            "epochs_per_hour": (
                metrics["total_epochs"] / (metrics["total_duration"] / 3600)
                if metrics["total_duration"] > 0
                else 0
            ),
            "loss_improvement": (
                (
                    (metrics["losses"][0] - metrics["final_loss"])
                    / metrics["losses"][0]
                    * 100
                )
                if metrics["losses"] and metrics["losses"][0] > 0
                else 0
            ),
            "accuracy_improvement": (
                (metrics["final_accuracy"] - metrics["accuracies"][0]) * 100
                if metrics["accuracies"]
                else 0
            ),
            "training_efficiency": (
                metrics["final_accuracy"] / (metrics["total_duration"] / 3600)
                if metrics["total_duration"] > 0
                else 0
            ),
        }
    ...
    def _generate_report_file(self, report_data: Dict[str, Any]) -> str:
        ...
        try:
            ...
            backup_file = os.path.join(
                self.config.output_directory, f"{report_id}_error.json"
            )
