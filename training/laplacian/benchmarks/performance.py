...
class LaplacianBenchmark:
    ...
    def _run_single_benchmark(self, 
        ...
        try:
            ...
            logger.info(f"测试完成: {test_name}, 平均时间: {avg_time:.2f}ms, 内存: {memory_used:.1f}MB")
    ...
    def _test_graph_laplacian(self, n_nodes: int) -> Optional[Any]:
        ...
        # 创建拉普拉斯计算器
        laplacian_calculator = GraphLaplacian(normalization="sym", device=self.device_torch)
    ...
    def _test_laplacian_regularization(self, n_samples: int, n_features: int) -> Optional[torch.Tensor]:
        """测试拉普拉斯正则化"""
        if not MODULES_AVAILABLE:
            return None  # 返回None
    ...
    def generate_report(self) -> Dict[str, Any]:
        ...
        if self.results:
            ...
            report["summary"]["avg_time_all_tests_ms"] = np.mean(avg_times) if avg_times else 0
            report["summary"]["total_memory_used_mb"] = sum(r.memory_usage_mb for r in self.results)
