...
class LaplacianRegularization(LaplacianBase):
    ...
    def __init__(
            ...
            try:
                self.laplacian_calculator = GraphLaplacian(
                    normalization=(
                        ...
                        if isinstance(config.normalization, NormalizationType)
                        else config.normalization
                    ),
        ...
        logger.info(
            f"拉普拉斯正则化初始化: type={config.regularization_type}, lambda={config.lambda_reg}"
        )
    ...
    def _graph_laplacian_regularization(
        ...
        if graph_structure is not None:
            ...
            if GRAPH_MODULE_AVAILABLE:
                graph = create_graph_from_adjacency(
                    adjacency_matrix, device=self.device
                )
            else:
        ...
        self._regularization_stats["graph_construction_time"] += (
        ...
        L = result["laplacian"]
        self._regularization_stats["laplacian_computation_time"] += (
            time.time() - laplacian_start_time
        )
    ...
    def _multi_scale_regularization(
        ...
        for scale_idx in range(num_scales):
            ...
            if graph_structure is not None:
                ...
                scaled_graph = create_graph_from_adjacency(
                    scaled_adj, device=self.device
                )
            else:
                # 从特征构建当前尺度的图
    ...
    def _adaptive_regularization(
        ...
        adaptive_lambda = torch.clamp(
            adaptive_lambda, self.config.min_lambda, self.config.max_lambda
        )
    ...
    def _apply_adaptive_adjustment(
        self, reg_loss: torch.Tensor, features: torch.Tensor
    ) -> torch.Tensor:
        """应用自适应调整
        ...
        self.current_lambda = max(
            self.config.min_lambda, min(self.config.max_lambda, self.current_lambda)
        )
    ...
    def _extract_subgraph(
        self, graph: GraphStructure, mask: torch.Tensor
    ) -> GraphStructure:
        """提取子图
    ...
    def _scale_graph(
        self, graph: GraphStructure, scale_factor: float
    ) -> GraphStructure:
        """图尺度变换
    ...
    def _scale_adjacency(
        self, adjacency: torch.Tensor, scale_factor: float
    ) -> torch.Tensor:
        """邻接矩阵尺度变换"""
        if scale_factor >= 1.0:
    ...
    def _scale_features(
        self, features: torch.Tensor, scale_factor: float
    ) -> torch.Tensor:
        """特征尺度变换"""
        # 应用高斯平滑
        if scale_factor < 1.0:
