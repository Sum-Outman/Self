...
class LaplacianEnhancedCNN(nn.Module):
    ...
    def __init__(
        self, cnn_config: CNNConfig, laplacian_config: LaplacianEnhancedTrainingConfig
    ):
        super().__init__()
        ...
        logger.info(
            f"拉普拉斯增强CNN初始化完成: 架构={                 cnn_config.architecture if hasattr(                     cnn_config, 'architecture') else 'unknown'}, "
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # 拉普拉斯金字塔多尺度特征
        if (
            self.laplacian_pyramid is not None
            and self.laplacian_config.multi_scale_enabled
        ):
    ...
    def _build_laplacian_pyramid(self) -> nn.ModuleList:
        ...
        for i in range(self.laplacian_config.num_scales):
            self.laplacian_config.scale_factors[i]

            # 高斯模糊层
            blur_layer = self._create_gaussian_blur_layer()

            # 下采样层
            downsample_layer = nn.AvgPool2d(kernel_size=2, stride=2, padding=0)
    ...
    def _create_gaussian_blur_layer(
        self, kernel_size: int = 5, sigma: float = 1.0
    ) -> nn.Conv2d:
        """创建高斯模糊卷积层"""
    ...
    def _fuse_features(
        self, base_features: torch.Tensor, laplacian_features: List[torch.Tensor]
    ) -> torch.Tensor:
        """融合基础特征和拉普拉斯特征"""
