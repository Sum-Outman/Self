...
try:
    ...
    try:
        ...
        PINN_MODULE_AVAILABLE = False
        logger.error(
            "PINN模块不可用。根据项目要求'禁止使用虚拟数据'，不能提供虚拟实现。"
        )
...
class LaplacianEnhancedPINN(nn.Module):
    ...
    def __init__(
        self, pinn_config: PINNConfig, laplacian_config: LaplacianEnhancedTrainingConfig
    ):
        super().__init__()
        ...
        logger.info(
            f"拉普拉斯增强PINN初始化完成: 模式={laplacian_config.training_mode}, "
    ...
    def compute_total_loss(
        ...
        if targets is not None:
            pinn_loss, loss_dict = self.pinn_model.compute_total_loss(
                inputs, targets, iteration
            )
        else:
            # 仅物理约束
        ...
        if (
            ...
            laplacian_loss = self.laplacian_regularizer(
                features=outputs, graph_structure=graph
            )
            ...
            if self.laplacian_config.adaptive_lambda:
                ...
                laplacian_loss = (
                    self.laplacian_config.laplacian_reg_lambda * laplacian_loss
                )

            total_loss = total_loss + laplacian_loss

            loss_dict["laplacian_loss"] = laplacian_loss.item()
            loss_dict["laplacian_lambda"] = (
                self.current_lambda
                if self.laplacian_config.adaptive_lambda
                else self.laplacian_config.laplacian_reg_lambda
            )
        ...
        if "laplacian_loss" in loss_dict:
            self.training_stats["laplacian_loss_history"].append(
                loss_dict["laplacian_loss"]
            )
        self.training_stats["pde_loss_history"].append(
            loss_dict.get("pde_residual", 0.0)
        )
    ...
    def _construct_knn_graph(self, inputs: torch.Tensor) -> GraphStructure:
        ...
        return GraphStructure(
            adjacency_matrix=adjacency, graph_type=GraphType.UNDIRECTED
        )
    ...
    def _update_adaptive_lambda(
        self, laplacian_loss: torch.Tensor, pinn_loss: torch.Tensor, iteration: int
    ):
        """更新自适应正则化强度"""
        ...
        if iteration % self.laplacian_config.logging_frequency == 0:
            logger.info(
                f"自适应lambda更新: iteration={iteration}, "
    ...
    def get_training_stats(self) -> Dict[str, Any]:
        ...
        if stats["laplacian_loss_history"]:
            stats["avg_laplacian_loss"] = np.mean(
                stats["laplacian_loss_history"][-100:]
            )
