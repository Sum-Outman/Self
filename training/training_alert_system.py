...
class EmailAlertChannel(AlertChannel):
    ...
    def send(self, alert: Alert) -> bool:
        ...
        try:
            ...
            msg['Subject'] = f"[{alert.severity.value.upper()}] {alert.type.value} - AGI训练报警"
            ...
            消息: {alert.message}
            时间: {alert.timestamp}
            训练ID: {alert.training_id or 'N/A'}
            模型ID: {alert.model_id or 'N/A'}
            ...
            <body>
                ...
                <table border="1" cellpadding="5" cellspacing="0">
                    ...
                    <tr><td>类型</td><td>{alert.type.value}</td></tr>
                    <tr><td>严重程度</td><td><strong>{alert.severity.value}</strong></td></tr>
                    <tr><td>消息</td><td>{alert.message}</td></tr>
                    <tr><td>时间</td><td>{alert.timestamp}</td></tr>
                    <tr><td>训练ID</td><td>{alert.training_id or 'N/A'}</td></tr>
                    <tr><td>模型ID</td><td>{alert.model_id or 'N/A'}</td></tr>
                </table>
                <h3>详情:</h3>
    ...
    def send(self, alert: Alert) -> bool:
        ...
        try:
            ...
            if response.status_code in [200, 201, 202]:
                ...
                self.logger.error(f"Webhook报警发送失败: {response.status_code} - {response.text}")
                return False
...
class SMSAlertChannel(AlertChannel):
    ...
    def send(self, alert: Alert) -> bool:
        ...
        try:
            # 这里实现实际的短信发送逻辑
            # 暂时模拟成功
            self.logger.info(f"短信报警发送模拟: {alert.id} 到 {len(self.phone_numbers)} 个号码")
    ...
    def check_anomaly(self, value: float) -> Optional[Dict[str, Any]]:
        ...
        if z_score > self.threshold:
            return {
                ...
                "anomaly_score": min(1.0, z_score / (self.threshold * 2)),  # 0-1之间的异常分数
            }
...
class TrendAnomalyDetector(AnomalyDetector):
    ...
    def check_anomaly(self, value: float) -> Optional[Dict[str, Any]]:
        ...
        # 计算时间差（秒）
        time_diffs = [(recent_timestamps[i] - recent_timestamps[0]).total_seconds() 
        ...
        try:
            ...
            if residual / residual_std > self.threshold:
                ...
                anomaly_info["anomaly_score"] = min(1.0, (residual / residual_std) / (self.threshold * 2))
                return anomaly_info
            
            # 检查趋势斜率异常
            if slope_magnitude > self.slope_threshold:
                anomaly_info["anomaly_type"] = "slope_anomaly"
                anomaly_info["anomaly_score"] = min(1.0, slope_magnitude / (self.slope_threshold * 2))
                return anomaly_info
...
class TrainingAlertSystem:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.config = config or {
            ...
            "anomaly_detectors": {
                ...
                "accuracy": {"type": "moving_average", "threshold": 2.5, "window_size": 50},
                "resource_usage": {"type": "trend", "threshold": 2.0, "window_size": 30},
            },
            "alert_rules": {
    ...
    def _initialize_anomaly_detectors(self) -> Dict[str, AnomalyDetector]:
        ...
        for detector_name, detector_config in detectors_config.items():
            ...
            if detector_type in detector_classes:
                try:
                    detectors[detector_name] = detector_classes[detector_type](detector_config)
                    self.logger.info(f"异常检测器已初始化: {detector_name} ({detector_type})")
                except Exception as e:
    ...
    def set_training_context(self, training_id: str, model_id: Optional[str] = None):
        ...
        self.logger.info(f"训练上下文设置: training_id={training_id}, model_id={model_id}")
    ...
    def report_metrics(self, metrics: Dict[str, float]):
        ...
        if len(self.training_state["metrics_history"]) > max_history:
            self.training_state["metrics_history"] = self.training_state["metrics_history"][-max_history:]
    ...
    def report_resource_usage(self, resources: Dict[str, float]):
        ...
        if len(self.training_state["resource_history"]) > max_history:
            self.training_state["resource_history"] = self.training_state["resource_history"][-max_history:]
    ...
    def _check_metrics_anomalies(self, metrics: Dict[str, float], timestamp: datetime):
        ...
        if "accuracy" in metrics:
            ...
            if "accuracy" in self.anomaly_detectors:
                ...
                if anomaly:
                    self.anomaly_detectors["accuracy"].add_measurement(accuracy, timestamp)
                    self.trigger_alert(
        ...
        if "loss" in metrics:
            ...
            if metrics["loss"] < min_loss:
                if self.training_state["stall_start_time"] is None:
                    ...
                    self.logger.info(f"训练可能停滞，损失低于阈值: {metrics['loss']:.6f} < {min_loss}")
                else:
                    # 计算停滞时间
                    stall_duration = (timestamp - self.training_state["stall_start_time"]).total_seconds() / 60.0
    ...
    def _check_resource_anomalies(self, resources: Dict[str, float], timestamp: datetime):
        """检查资源异常"""
        alert_rules = self.config.get("alert_rules", {})
        ...
        if "memory_percent" in resources:
            ...
            if resources["memory_percent"] > memory_threshold:
                self.trigger_alert(
                    ...
                    {"memory_percent": resources["memory_percent"], "threshold": memory_threshold}
                )
        ...
        if "gpu_percent" in resources:
            ...
            if resources["gpu_percent"] > gpu_threshold:
                self.trigger_alert(
                    ...
                    {"gpu_percent": resources["gpu_percent"], "threshold": gpu_threshold}
                )
        
        # 资源趋势异常检测
        if "resource_usage" in self.anomaly_detectors and "memory_percent" in resources:
            anomaly = self.anomaly_detectors["resource_usage"].check_anomaly(resources["memory_percent"])
            if anomaly:
                self.anomaly_detectors["resource_usage"].add_measurement(resources["memory_percent"], timestamp)
                self.trigger_alert(
    ...
    def trigger_alert(self, 
        ...
        if existing_alert:
            ...
            self.logger.info(f"重复报警: {alert_id} (重复次数: {existing_alert.recurrence_count})")
            
            # 如果重复次数超过阈值，提升严重程度
            if existing_alert.recurrence_count >= 5 and existing_alert.severity != AlertSeverity.CRITICAL:
                existing_alert.severity = AlertSeverity.CRITICAL
                existing_alert.message = f"[重复报警×{existing_alert.recurrence_count}] {message}"
        ...
        if sent:
            ...
            self.logger.info(f"报警已发送: {alert_id} - {alert_type.value} ({severity.value})")
        else:
    ...
    def _should_silence_alert(self, alert_type: AlertType, severity: AlertSeverity) -> bool:
        """检查是否应该静默报警"""
            ...
            for existing_alert in reversed(self.alert_history):
                try:
                    ...
                    if (existing_alert.type == alert.type and 
                        existing_alert.training_id == alert.training_id and
    ...
    def get_alert_history(self, limit: int = 100, offset: int = 0) -> List[Dict[str, Any]]:
        """获取报警历史"""
        with self.alert_history_lock:
            sorted_history = sorted(self.alert_history, 
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        with self.alert_history_lock:
            ...
            stats["avg_response_time_seconds"] = total_response_time / count if count > 0 else 0.0
...
def get_training_alert_system(config: Optional[Dict[str, Any]] = None) -> TrainingAlertSystem:
    """获取训练报警系统单例
