...
class CheckpointManager:
            ...
            if self.checkpoint_format == CheckpointFormat.PYTORCH:
                ...
                try:
                    ...
                    safetensors.torch.save_file(
                        checkpoint_data.get("model_state_dict", {}), checkpoint_path
                    )
                except ImportError:
    ...
    def _save_best_checkpoint(
        self, checkpoint_data: Dict[str, Any], checkpoint_path: Path, checkpoint_id: str
    ):
        """保存最佳检查点"""
        best_checkpoint_path = self.best_checkpoint_path.with_suffix(
    ...
    def _create_metadata(
        ...
        metadata = CheckpointMetadata(
            ...
            training_config=(
                additional_info.get("training_config", {}) if additional_info else {}
            ),
            ...
            if len(self.checkpoint_history) > self.max_checkpoints:
                ...
                for metadata in checkpoints_to_delete:
                    try:
                        ...
                        # 删除元数据文件
                        metadata_path = (
                            self.metadata_dir / f"{metadata.checkpoint_id}.json"
                        )
                        ...
                        self.logger.warning(
                            f"删除检查点失败 {metadata.checkpoint_id}: {e}"
                        )
    ...
    def _decompress_checkpoint(self, compressed_path: Path) -> Path:
        ...
        try:
            with zipfile.ZipFile(compressed_path, "r") as zipf:
                ...
                # 查找解压后的检查点文件
                extracted_files = list(temp_dir.glob("*.pt")) + list(
                    temp_dir.glob("*.pth")
                )
            ...
            if checkpoint_path.suffix in [".pt", ".pth"]:
                ...
                for key in required_keys:
                    if key not in checkpoint:
                        self.logger.warning(
                            f"检查点缺少必需键 {key}: {checkpoint_path}"
                        )
                        return False
    ...
    def delete_checkpoint(self, checkpoint_id: str) -> bool:
        ...
        try:
            ...
            self.checkpoint_history = [
                m for m in self.checkpoint_history if m.checkpoint_id != checkpoint_id
            ]
...
def get_checkpoint_manager(
    checkpoint_dir: Optional[Union[str, Path]] = None, **kwargs
) -> CheckpointManager:
    """获取检查点管理器单例"""
