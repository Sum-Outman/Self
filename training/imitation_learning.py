...
class BehaviorCloningPolicy(nn.Module):
    ...
    def __init__(
        self, observation_dim: int, action_dim: int, hidden_dims: List[int] = [256, 256]
    ):
        super().__init__()
...
class BehaviorCloningTrainer:
    ...
    def train(self) -> Dict[str, Any]:
        ...
        self.logger.info(
            f"开始行为克隆训练: {len(self.dataset)} 个示范点, {self.epochs} 轮"
        )
...
class DAggerTrainer:
    ...
    def train(self) -> Dict[str, Any]:
        ...
        self.logger.info(
            f"DAgger训练完成: 最终数据集大小={final_result['final_dataset_size']}"
        )
        ...
        for trajectory in trajectories:
            ...
            for obs, _, reward, next_obs, done, info in trajectory:
                ...
                expert_trajectory.append(
                    (obs, expert_action, reward, next_obs, done, info)
                )
    ...
    def _evaluate_policy(self) -> Dict[str, Any]:
        ...
        return {"mean_reward": mean_reward, "num_episodes": num_episodes}
...
class ImitationLearningManager:
    ...
    def _initialize_policy(self):
        ...
        self.logger.info(
            f"策略网络初始化: 观察维度={observation_dim}, 动作维度={action_dim}"
        )

    def collect_demonstrations(
        self, expert_policy: Any, num_episodes: int = 10, max_steps: int = 1000
    ) -> Dict[str, Any]:
        """收集专家示范"""
        ...
        self.logger.info(
            f"示范收集完成: 总示范点={demonstrations_collected}, 数据集大小={len(self.dataset)}"
        )
    ...
    def evaluate(self, num_episodes: int = 10) -> Dict[str, Any]:
        ...
        self.logger.info(
            f"策略评估完成: 平均奖励={mean_reward:.3f}, 成功率={success_rate:.3f}"
        )
...
def create_imitation_learning_manager(
    env: gym.Env, **kwargs
) -> ImitationLearningManager:
    """创建模仿学习管理器（工厂函数）"""
...
if __name__ == "__main__":
    ...
    manager = create_imitation_learning_manager(
        test_env, algorithm=ImitationAlgorithm.BEHAVIOR_CLONING, config={"epochs": 20}
    )
    ...
    print("收集示范数据...")
    collection_result = manager.collect_demonstrations(
        expert, num_episodes=3, max_steps=5
    )
    print(
        f"示范收集结果: {json.dumps(collection_result, indent=2, ensure_ascii=False)}"
    )
