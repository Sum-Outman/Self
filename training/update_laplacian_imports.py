        ...
        for file in files:
            ...
            try:
                with open(file_path, 'r', encoding='utf-8') as f:
                    ...
                    # 检查是否包含任何旧模块名称
                    for old_module in ['laplacian_regularization', 'laplacian_enhanced_training',
                                      'laplacian_integration', 'laplacian_benchmark']:
...
def update_import_line(line: str, file_path: Path) -> Tuple[str, bool]:
    ...
    for old_module in old_modules:
        if module_path == old_module or module_path.startswith(old_module + '.'):
            # 找到旧模块导入
            logger.debug(f"在 {file_path.name} 中找到旧模块导入: {line.strip()}")
            ...
            if old_module == 'training.laplacian_regularization':
                ...
                if import_type == 'from_import':
                    # from training.laplacian_regularization import LaplacianRegularization
                ...
                if import_type == 'from_import':
                    ...
                    for imported in imports:
                        if imported in ['LaplacianEnhancedTrainingConfig', 'UnifiedLaplacianConfig']:
                            ...
                            # 优化器在 optimizers 中
                            new_module = 'training.laplacian.optimizers.laplacian_optimizer'
                            line = line.replace(old_module, new_module)
                            updated = True
                            break
                    ...
                    if not updated:
                        ...
                        comment += f"# 请根据具体需要更新导入路径:\n"
                        comment += f"# - 配置类: training.laplacian.utils.config\n"
                        comment += f"# - PINN模型: training.laplacian.models.pinn\n"
                        comment += f"# - CNN模型: training.laplacian.models.cnn\n"
                        comment += f"# - 优化器: training.laplacian.optimizers.laplacian_optimizer\n"
                        line = comment + line
                        updated = True
...
def generate_migration_report(updated_files: List[Path]) -> None:
    ...
    with open(report_path, 'w', encoding='utf-8') as f:
        ...
        f.write("|------------|------------|------|\n")
        f.write("| `training.laplacian_regularization` | `training.laplacian.core.regularization` | 拉普拉斯正则化组件 |\n")
        f.write("| `training.laplacian_enhanced_training` | `training.laplacian.utils.config`<br>`training.laplacian.models.pinn`<br>`training.laplacian.models.cnn`<br>`training.laplacian.optimizers.laplacian_optimizer` | 拆分为多个子模块 |\n")
        f.write("| `training.laplacian_integration` | `training.laplacian.integration.framework` | 集成框架 |\n")
        f.write("| `training.laplacian_benchmark` | `training.laplacian.benchmarks.performance` | 性能基准测试 |\n\n")
