...
class Curriculum:
    ...
    def __init__(self,
        ...
        self.logger.info(f"课程初始化: {len(tasks)} 个任务, 当前难度={self.current_difficulty.value}")
    ...
    def add_task(self, task_name: str, task_type: str, difficulty_level: int) -> str:
        ...
        self.logger.info(f"任务添加成功: {task_name} ({task_type}), 难度={difficulty.value}, ID={task_id}")
        return task_id
    ...
    def get_current_task(self) -> Optional[TaskDescription]:
        ...
        if not current_tasks:
            self.logger.warning(f"当前难度 {self.current_difficulty.value} 没有可用任务")
            return None  # 返回None
        ...
        worst_task_id = min(
            available_tasks,
            key=lambda tid: self.tasks[tid].get_average_performance()
        )
    ...
    def _check_advancement(self):
        ...
        # 检查是否满足升级条件
        if (mastery_rate >= self.advancement_threshold and 
            mastered_count >= self.min_tasks_per_level):
    ...
    def get_learning_progress(self) -> Dict[str, Any]:
        ...
        for diff in TaskDifficulty:
            ...
            if tasks_in_diff:
                mastered_in_diff = sum(1 for tid in tasks_in_diff if tid in self.mastered_tasks)
                difficulty_stats[diff.value] = {
                    "total": len(tasks_in_diff),
                    "mastered": mastered_in_diff,
                    "mastery_rate": mastered_in_diff / len(tasks_in_diff) if tasks_in_diff else 0.0
                }
...
class IntrinsicMotivationModule:
    ...
    def compute_motivation(self,
                          ...
                          state: np.ndarray,
                          success_rate: float,
                          prev_success_rate: Optional[float] = None) -> float:
        """
        计算内在动机
...
class ExplorationManager:
    ...
    def __init__(self,
        ...
        if strategy == ExplorationStrategy.CURIOSITY_DRIVEN:
            ...
            self.intrinsic_motivation_module = IntrinsicMotivationModule(
                learning_progress_weight=self.config.get("learning_progress_weight", 1.0),
    ...
    def explore(self, exploration_type: str = "curiosity_driven", context: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        """执行探索
        ...
        if exploration_type == "curiosity_driven":
            ...
            result = {
                ...
                "actions": ["explore_new_area", "try_different_approach", "combine_knowledge"],
        ...
        self.logger.info(f"探索执行: 类型={exploration_type}, 选择动作={result['selected_action']}")
        return result
    ...
    def _epsilon_greedy(self, 
                       available_actions: List[int],
                       q_values: Optional[np.ndarray]) -> int:
        """ε-贪婪策略"""
    ...
    def _ucb(self,
        ...
        total_counts = sum(
            self.action_stats.get(a, {}).get("count", 0)
    ...
    def _curiosity_driven(self,
                         available_actions: List[int],
                         state: Optional[np.ndarray]) -> int:
        """好奇心驱动策略"""
    ...
    def _intrinsic_motivation(self,
                             available_actions: List[int],
                             state: Optional[np.ndarray],
                             task_id: Optional[str]) -> int:
        """内在动机策略"""
    ...
    def _diversity_search(self, available_actions: List[int]) -> int:
        ...
        least_tried_actions = [
            action for action, count in zip(available_actions, action_counts)
            if count == min_count
        ]
    ...
    def update_action_stats(self,
        ...
        stats["success_rate"] = stats["successes"] / total_trials if total_trials > 0 else 0.0
...
class AutonomousLearningManager:
    ...
    def start_learning_session(self) -> Optional[TaskDescription]:
        ...
        if self.current_task:
            ...
            self.logger.info(f"开始学习任务: {task_id}, 难度={self.current_task.difficulty.value}")
    ...
    def get_learning_statistics(self) -> Dict[str, Any]:
        ...
        for task_id, history in self.task_performance_history.items():
            ...
            success_rate = history["successes"] / total_trials if total_trials > 0 else 0.0
...
def create_sample_curriculum() -> Curriculum:
    ...
    # 创建不同难度的任务
    task_families = ["text_understanding", "multimodal_processing", "planning", "reasoning"]
...
if __name__ == "__main__":
    ...
    for session in range(5):
        ...
        print(f"学习会话 {session+1}: 任务={task.task_id}, 难度={task.difficulty.value}")
        ...
        print(f"  进度: 总步数={stats['total_training_steps']}, "
