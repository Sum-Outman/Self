...
class LaplaceTransform(nn.Module):
    ...
    def _build_laplace_grid(self) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        sigma_grid, omega_grid = torch.meshgrid(
            sigma_points, omega_points, indexing="ij"
        )
    ...
    def forward(
        self, signal: torch.Tensor, axis: int = -1, return_complex: bool = True
    ...
    def inverse(
        ...
        # 重排维度
        laplace_complex = self._move_transform_axis_to_end(
            laplace_complex, axis, is_laplace=True
        )
    ...
    def _fast_inverse_laplace(self, laplace_flat: torch.Tensor) -> torch.Tensor:
        ...
        # 分离实部和虚部
        signal_real = (
            laplace_real @ self.inverse_matrix_real.T
            + laplace_imag @ self.inverse_matrix_imag.T
        )
        return signal_real
    ...
    def _resize_signal(
        self, signal: torch.Tensor, target_length: int, axis: int
    ) -> torch.Tensor:
        """调整信号长度"""
    ...
    def _move_transform_axis_to_end(
        self, tensor: torch.Tensor, axis: int, is_laplace: bool = False
    ) -> torch.Tensor:
        """将变换轴移动到末尾"""
        if axis < 0:
    ...
    def process_realtime(self, signal_chunk: torch.Tensor) -> torch.Tensor:
        ...
        if self.realtime_buffer is None:
            ...
            # 拼接新数据
            self.realtime_buffer = torch.cat(
                [self.realtime_buffer, signal_chunk], dim=-1
            )
...
class LaplacePyramid(nn.Module):
    ...
    def _apply_gaussian(self, image: torch.Tensor) -> torch.Tensor:
        ...
        smoothed = F.conv2d(
            smoothed, kernel.transpose(2, 3), padding=kernel.shape[-1] // 2
        )
...
class EdgeDetector(nn.Module):
    ...
    def _canny_edge(self, image: torch.Tensor) -> torch.Tensor:
        ...
        # 高斯平滑
        gaussian_kernel = (
            torch.tensor([[1, 2, 1], [2, 4, 2], [1, 2, 1]], dtype=torch.float32).view(
                1, 1, 3, 3
            )
            / 16.0
        )
...
def test_signal_processing():
    ...
    config = SignalProcessingConfig(
        sampling_rate=1000.0, num_points=256, num_sigma_points=20, num_omega_points=40
    )
