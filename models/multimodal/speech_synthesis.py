...
class Attention(nn.Module):
    ...
    def forward(self, query: torch.Tensor, key: torch.Tensor, 
        ...
        # 计算注意力分数
        scores = torch.matmul(Q, K.transpose(1, 2)) / math.sqrt(self.config.decoder_dim)  # [batch, 1, seq_len]
...
class Decoder(nn.Module):
    ...
    def forward(self, encoder_outputs: torch.Tensor, mel_spec: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        """
        解码梅尔频谱图
        ...
        # 初始化第一个输入（零向量）
        prev_mel = torch.zeros(batch_size, self.config.num_mels, device=encoder_outputs.device)
        ...
        # 堆叠输出
        mel_outputs = torch.stack(mel_outputs, dim=1)  # [batch, max_mel_frames, num_mels]
        stop_tokens = torch.stack(stop_tokens, dim=1)  # [batch, max_mel_frames, 1]
        attention_weights = torch.stack(attention_weights, dim=1)  # [batch, max_mel_frames, seq_len]
...
class SpeechSynthesizer(nn.Module):
    ...
    def __init__(self, config: SpeechSynthesisConfig):
        ...
        logger.info(f"语音合成器初始化完成: "
                   f"vocab_size={config.vocab_size}, "
                   f"num_mels={config.num_mels}")
    
    def forward(self, text_tokens: torch.Tensor, mel_spec: torch.Tensor) -> Dict[str, Any]:
        """
        语音合成前向传播
        ...
        """
        # 文本编码
        encoder_outputs = self.text_encoder(text_tokens)  # [batch, seq_len, encoder_dim]
        
        # 解码梅尔频谱图
        mel_outputs, stop_tokens, attention_weights = self.decoder(encoder_outputs, mel_spec)
        ...
        if self.training:
            ...
            stop_loss = F.binary_cross_entropy_with_logits(
                ...
                torch.ones_like(stop_tokens.squeeze(-1))
            )
    ...
    def synthesize(self, text_tokens: torch.Tensor, max_mel_frames: int = 1000) -> Dict[str, Any]:
        """
        语音合成接口
        ...
        with torch.no_grad():
            ...
            for i in range(max_mel_frames):
                ...
                # 注意力
                context, attn_weights = self.decoder.attention(prenet_out, encoder_outputs, encoder_outputs)
            ...
            # 堆叠输出
            if mel_outputs:
                mel_outputs = torch.stack(mel_outputs, dim=1)  # [batch, num_frames, num_mels]
                attention_weights = torch.stack(attention_weights, dim=1)  # [batch, num_frames, seq_len]
...
class Vocabulary:
    ...
    def decode(self, indices: List[int]) -> str:
        ...
        chars = []
        for idx in indices:
            if idx in self.idx_to_char and self.idx_to_char[idx] not in ["<pad>", "<unk>", "<sos>", "<eos>"]:
