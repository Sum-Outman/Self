...
class TextToImageGenerator(nn.Module):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.fc_z = nn.Linear(
            self.latent_dim + text_embedding_dim, self.latent_dim + text_embedding_dim
        )

        logger.info(
            f"初始化TextToImageGenerator: 潜在维度={self.latent_dim}, 图像大小={self.image_size}"
        )
    ...
    def _build_decoder(self, condition_dim: int) -> nn.Module:
        ...
        return nn.Sequential(
            # 起始投影: [batch, latent_dim + condition_dim] -> [batch, 512 * start_size *
            # start_size]
            ...
            nn.ReLU(),
            nn.ConvTranspose2d(
                64, self.image_channels, kernel_size=4, stride=2, padding=1
            ),
    ...
    def encode(
        self, images: torch.Tensor, text_condition: torch.Tensor
    ...
    def compute_loss(
        ...
        # 重构损失（均方误差）
        recon_loss = (
            F.mse_loss(recon_images, original_images, reduction="sum") / batch_size
        )
...
class ImageToTextGenerator(nn.Module):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.position_embedding = nn.Embedding(
            self.max_seq_len, self.text_embedding_dim
        )
        ...
        logger.info(
            f"初始化ImageToTextGenerator: 词汇表大小={                 self.vocab_size}, 最大序列长度={                 self.max_seq_len}"
        )
    ...
    def generate_text(
        ...
        for i in range(max_length - 1):
            ...
            # 准备图像特征作为编码器输出
            memory = image_features.unsqueeze(1).expand(
                -1, seq_len, -1
            )  # [batch, seq_len, text_embedding_dim]

            # Transformer解码
            decoder_output = self.transformer_decoder(
                tgt=text_embeds, memory=memory, tgt_mask=tgt_mask, memory_mask=None
            )
    ...
    def compute_loss(
        self, logits: torch.Tensor, target_text: torch.Tensor
    ) -> Dict[str, torch.Tensor]:
        """计算文本生成损失"""
        ...
        loss = F.cross_entropy(
            logits_flat, targets_flat, ignore_index=0
        )  # 忽略填充token（假设0是填充）
...
class CrossModalGenerationManager(nn.Module):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        logger.info(
            "初始化CrossModalGenerationManager: 包含文本到图像和图像到文本生成器"
        )
