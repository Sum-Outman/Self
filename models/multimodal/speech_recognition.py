...
class SpeechFeatureExtractor(nn.Module):
    ...
    def __init__(self, config: SpeechRecognitionConfig):
        ...
        self.preemphasis.weight.data = torch.tensor(
            [[[-0.97, 1.0]]], dtype=torch.float32
        )
        ...
        self.hamming_window = nn.Parameter(
            torch.hamming_window(window_size).float(), requires_grad=False
        )
    ...
    def _create_mel_filterbank(self) -> torch.Tensor:
        ...
        mel_points = torch.linspace(
            low_mel.item(), high_mel.item(), self.config.num_mels + 2
        )
        ...
        for i in range(self.config.num_mels):
            ...
            if left < center:
                ...
                if center_idx > left_idx:
                    filters[i, left_idx:center_idx] = torch.linspace(
                        0, 1, center_idx - left_idx
                    )
            ...
            if center < right:
                ...
                if right_idx > center_idx:
                    filters[i, center_idx:right_idx] = torch.linspace(
                        1, 0, right_idx - center_idx
                    )
...
class SpeechRecognizer(nn.Module):
    ...
    def forward(
        self, audio: torch.Tensor, targets: Optional[torch.Tensor] = None
    ) -> Dict[str, Any]:
        """
        语音识别前向传播
        ...
        if mel_spec.size(1) == 0:
            # 空音频，返回空结果
            return {"success": False, "error": "音频太短，无法提取特征"}
        ...
        if self.training and targets is not None:
            ...
            ctc_loss = self.ctc_decoder(
                encoder_logits, targets, input_lengths, target_lengths
            )
            ...
            # 总损失
            total_loss = (
                self.config.ctc_weight * ctc_loss.mean()
                + self.config.lm_weight * lm_loss
            )

            return {"loss": total_loss, "ctc_loss": ctc_loss.mean(), "lm_loss": lm_loss}
        else:
            # 推理模式：解码
            input_lengths = torch.full(
                (audio.size(0),),
...
class SpeechRecognitionService:
    ...
    def recognize_audio(self, audio_data: np.ndarray, sample_rate: int = 16000) -> str:
        ...
        try:
            # 转换为Tensor
            audio_tensor = (
                torch.from_numpy(audio_data).float().unsqueeze(0)
            )  # [1, audio_length]
            ...
            if sample_rate != self.model_config.sample_rate:
                # 重采样（完整：截断或填充）
                target_length = int(
                    len(audio_data) * self.model_config.sample_rate / sample_rate
                )
        ...
        try:
            ...
            with wave.open(audio_file_path, "rb") as wav_file:
                ...
                if wav_file.getsampwidth() == 2:
                    fmt = f"<{n_frames}h"
                    audio_array = (
                        np.array(struct.unpack(fmt, audio_data), dtype=np.float32)
                        / 32768.0
                    )
                else:
                    # 完整处理
    ...
    def train(self, dataset, num_epochs: int = 10, batch_size: int = 32):
        ...
        for epoch in range(num_epochs):
            ...
            for batch_idx, batch in enumerate(dataset):
                ...
                # 转换为Tensor
                audio_tensor = torch.tensor(audio_batch, dtype=torch.float32).to(
                    self.device
                )
                ...
                for indices in text_indices:
                    padded = indices + [self.model_config.blank_id] * (
                        max_len - len(indices)
                    )
                    padded_indices.append(padded)

                text_tensor = torch.tensor(padded_indices, dtype=torch.long).to(
                    self.device
                )
            ...
            model_path = self.config.get(
                "model_path", "./data/speech_recognition_model.pt"
            )
