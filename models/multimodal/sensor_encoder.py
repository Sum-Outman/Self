...
class IndustrialSensorEncoder(nn.Module):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        logger.info(
            f"初始化IndustrialSensorEncoder: 嵌入维度={embedding_dim}, 层数={num_layers}"
        )
        logger.info(
            f"输入: {num_channels}通道, {sequence_length}时间步, {self.num_patches}个时间块"
        )
    ...
    def forward(self, sensor_data: torch.Tensor) -> torch.Tensor:
        ...
        # 转置为序列 [batch, embedding_dim, num_patches] -> [batch, num_patches,
        # embedding_dim]
        x = x.transpose(1, 2)
