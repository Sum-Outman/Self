...
class AdaptiveCrossModalAttention(nn.Module):
    ...
    def forward(self, query, key, value, key_padding_mask=None):
        ...
        # 解析预测参数
        num_heads_pred = (
            torch.sigmoid(predicted_params[:, 0]) * (self.max_heads - 1) + 1
        )
        ...
        # 3. 应用自适应缩放和门控
        scale_factor_expanded = scale_factor.view(batch_size, 1, 1).expand(
            -1, query_seq_len, -1
        )
        gate_value_expanded = gate_value.view(batch_size, 1, 1).expand(
            -1, query_seq_len, -1
        )

        scaled_attn_output = attn_output * scale_factor_expanded
        gated_output = (
            query * (1 - gate_value_expanded) + scaled_attn_output * gate_value_expanded
        )
...
class AttentionAnalyzer:
    ...
    def _compute_attention_sparsity(
        self, weights: np.ndarray, threshold: float = 0.01
    ) -> float:
        """计算注意力稀疏度"""
        # 计算低于阈值的权重比例
    ...
    def _classify_attention_pattern(self, weights: np.ndarray) -> str:
        ...
        if query_len == key_len:
            # 检查是否是对角线主导（局部注意力）
            diag_strength = np.mean(
                [weights[i].diagonal().mean() for i in range(batch_size)]
            )
...
class ProjectionLayerManager(nn.Module):
    ...
    def __init__(self, max_cache_size=10, fused_embedding_dim=768, device="cuda"):
        ...
        logger.info(
            f"初始化ProjectionLayerManager: 最大缓存大小={max_cache_size}, 融合维度={fused_embedding_dim}"
        )
    ...
    def _clean_lru_cache(self):
        ...
        # 找到最久未访问的缓存项
        sorted_items = sorted(self.access_timestamps.items(), key=lambda x: x[1])
    ...
    def get_projection(self, input_dim: int) -> nn.Linear:
        ...
        if input_dim == self.fused_embedding_dim:
            # 完整处理）
            logger.debug(
                f"输入维度{input_dim}等于融合维度{self.fused_embedding_dim}，跳过投影"
            )
            return None  # 返回None
...
class HierarchicalFusionNetwork(nn.Module):
    ...
    def __init__(
        ...
        self.modality_gates = nn.ModuleList(
            [
                nn.Sequential(nn.Linear(embedding_dim, embedding_dim), nn.Sigmoid())
    ...
    def forward(
        self, modality_features, modality_masks=None, return_attention_weights=False
    ):
        """
        前向传播
        ...
        """
        modality_features[0].size(0)

        # 1. 模态门控加权
        gated_features = []
        for i, (features, gate) in enumerate(
            zip(modality_features, self.modality_gates)
        ):
            # 平均池化得到模态级特征
        ...
        for i in range(len(gated_features)):
            ...
            if other_indices:
                ...
                if modality_masks and modality_masks[i] is not None:
                    ...
                    key_padding_mask = (
                        torch.cat(other_masks, dim=1)
                        if any(m is not None for m in other_masks)
                        else None
                    )
                ...
                if return_attention_weights and attn_weights is not None:
                    attention_weights_dict[f"modality_{i}_to_others"] = (
...
def test_fusion_networks():
    ...
    for attn_type in attention_types:
        ...
        try:
            ...
            print(
                f"      融合特征数量={len(fused_features)}, 每个形状={fused_features[0].shape}"
            )
    ...
    try:
        projection_manager = ProjectionLayerManager(
            max_cache_size=5, fused_embedding_dim=embedding_dim, device=str(device)
        ).to(device)
        ...
        print(
            f"      缓存大小: {cache_info['cache_size']}/{cache_info['max_cache_size']}"
        )
    ...
    try:
        ...
        analysis = analyzer.analyze_attention_weights(
            test_weights, query_modality="text", key_modality="image"
        )
    ...
    try:
        print(
            f"      可用注意力类型: {CrossModalAttentionFactory.get_available_types()}"
        )
        ...
        for attn_type in CrossModalAttentionFactory.get_available_types():
            ...
            attention_layer = CrossModalAttentionFactory.create_attention(
                attention_type=attn_type, embedding_dim=embedding_dim
            ).to(device)
