...
class ConvBlock(nn.Module):
    ...
    def __init__(self, in_channels: int, out_channels: int, kernel_size: int = 3,
                 ...
                 groups: int = 1, bias: bool = False, config: CNNConfig = None):
        super().__init__()
...
class ResidualBlock(nn.Module):
    ...
    def __init__(self, in_channels: int, out_channels: int, stride: int = 1,
                 downsample: Optional[nn.Module] = None, config: CNNConfig = None):
        super().__init__()
        self.config = config if config is not None else CNNConfig()
        self.stride = stride
        ...
        if downsample is None and (stride != 1 or in_channels != out_channels):
            downsample = nn.Sequential(
                ...
                nn.BatchNorm2d(out_channels, momentum=self.config.norm_momentum,
                               eps=self.config.norm_eps) if self.config.norm_type == "batch_norm" else nn.Identity()
            )
        self.downsample = downsample
        
        # 第一个卷积块
        self.conv1 = ConvBlock(in_channels, out_channels, kernel_size=3,
                              stride=stride, padding=1, config=config)
        ...
        # 注意力模块（可选）
        self.attention = self._create_attention(out_channels) if self.config.use_attention else None
        
        # 随机深度
        self.stochastic_depth = StochasticDepth(self.config.stochastic_depth) \
            if self.config.stochastic_depth > 0 else nn.Identity()
...
class BottleneckBlock(nn.Module):
    ...
    def __init__(self, in_channels: int, out_channels: int, stride: int = 1,
        ...
        # 第一个1x1卷积，减少通道数
        self.conv1 = ConvBlock(in_channels, out_channels // expansion,
                              kernel_size=1, stride=1, padding=0, config=config)
        
        # 第二个3x3卷积
        self.conv2 = ConvBlock(out_channels // expansion, out_channels // expansion,
                              kernel_size=3, stride=stride, padding=1, config=config)
        
        # 第三个1x1卷积，恢复通道数
        self.conv3 = nn.Conv2d(out_channels // expansion, out_channels,
...
class FeaturePyramidNetwork(nn.Module):
    ...
    def __init__(self, in_channels_list: List[int], out_channels: int = 256,
        ...
        for in_channels in in_channels_list:
            lateral_conv = ConvBlock(in_channels, out_channels, kernel_size=1,
                                    stride=1, padding=0, config=config)
            output_conv = ConvBlock(out_channels, out_channels, kernel_size=3,
                                   stride=1, padding=1, config=config)
...
class ResNetEncoder(nn.Module):
    ...
    def __init__(self, config: CNNConfig):
        ...
        # 输入层
        self.conv1 = ConvBlock(config.input_channels, config.base_channels,
                              kernel_size=7, stride=2, padding=3, config=config)
        ...
        for i, num_blocks in enumerate(config.num_layers):
            ...
            stage = self._make_stage(in_channels, out_channels, num_blocks,
        ...
        if config.use_fpn:
            # 收集每个阶段的输出通道
            stage_channels = [config.base_channels * (2 ** i) for i in range(len(config.num_layers))]
    ...
    def _make_stage(self, in_channels: int, out_channels: int, num_blocks: int,
        ...
        if stride != 1 or in_channels != out_channels:
            downsample = nn.Sequential(
                ...
                nn.BatchNorm2d(out_channels) if self.config.norm_type == "batch_norm" else nn.Identity()
            )
        
        blocks = []
        blocks.append(ResidualBlock(in_channels, out_channels, stride,
                                   downsample, self.config))
...
class CNNArchitectureSearch(nn.Module):
    ...
    def __init__(self, config: CNNConfig):
        ...
        self.architecture_params = nn.ParameterDict({
            'kernel_size': nn.Parameter(torch.tensor([0.33, 0.33, 0.34])),  # 初始分布
    ...
    def _build_model(self) -> nn.Module:
        ...
        config = CNNConfig(
            ...
            base_channels=int(self.config.base_channels * self.current_architecture.get('channel_mult', 1.0)),
            num_layers=[self.current_architecture.get('num_block', 3)] * 4,
            use_attention=self.current_architecture.get('attention_type', 'none') != 'none',
    ...
    def search_step(self, x: torch.Tensor, y: torch.Tensor, 
...
class EnhancedVisionEncoder(nn.Module):
    ...
    def forward(self, images: torch.Tensor) -> torch.Tensor:
        ...
        if vit_input.shape[2] != self.vit_encoder.layers[0].self_attn.embed_dim:
            vit_input = nn.Linear(vit_input.shape[2], 
                                 self.vit_encoder.layers[0].self_attn.embed_dim)(vit_input)
    ...
    def get_features(self, images: torch.Tensor) -> Dict[str, torch.Tensor]:
        ...
        # Transformer特征
        vit_input = cnn_features[-1].flatten(2).transpose(1, 2) if isinstance(cnn_features, list) else \
                   cnn_features.flatten(2).transpose(1, 2)
...
class CNNModel(nn.Module):
    ...
    def __init__(self, config: CNNConfig):
        ...
        logger.info(f"CNN模型初始化: 架构={config.architecture}, "
...
class CNNTrainConfig:
    ...
    augmentation_type: str = "standard"  # "standard", "auto_augment", "rand_augment", "trivial_augment"
...
class CNNCheckpointManager:
    ...
    def load_checkpoint(
        ...
        logger.info(f"检查点已加载: {checkpoint_path}, epoch: {checkpoint.get('epoch', 0)}")
...
class CNNTrainer:
    ...
    def __init__(
        ...
        # 混合精度训练
        self.scaler = torch.cuda.amp.GradScaler() if train_config.use_amp and self.device.type == "cuda" else None
        ...
        logger.info(f"CNN训练器初始化: 设备={self.device}, 优化器={train_config.optimizer_type}")
    ...
    def _create_criterion(self) -> nn.Module:
        ...
        if self.config.loss_type == "cross_entropy":
            ...
            def focal_loss(inputs, targets):
                ...
                focal_loss = (self.config.focal_alpha * (1 - pt) ** self.config.focal_gamma * ce_loss).mean()
                return focal_loss
            return focal_loss
        else:
    ...
    def train_epoch(
        ...
        progress_bar = tqdm(train_loader, desc=f"Epoch {epoch+1}/{self.config.num_epochs} [Train]")
    ...
    def validate(
        ...
        with torch.no_grad():
            ...
            for inputs, targets in progress_bar:
                ...
                accuracy = 100.0 * correct / total
                progress_bar.set_postfix({
                    'loss': total_loss / (total // targets.size(0)),
                    'acc': accuracy
                })
        ...
        for epoch in range(self.current_epoch, self.config.num_epochs):
            ...
            if self.scheduler is not None:
                if isinstance(self.scheduler, torch.optim.lr_scheduler.ReduceLROnPlateau):
                    metric = val_accuracy if self.config.best_metric == 'accuracy' else -val_loss
    ...
    def evaluate(
        ...
        logger.info(f"评估结果: 损失={avg_loss:.4f}, "
...
def test_cnn_training_functions():
    ...
    try:
        ...
        initialized_model = weight_initializer.initialize_model(test_model)
        print("   权重初始化测试通过")
