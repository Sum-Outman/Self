...
class AttentionAnalyzer:
    ...
    def __init__(self, output_dir: Optional[str] = None, max_samples: int = 1000):
        ...
        logger.info(f"初始化AttentionAnalyzer: 输出目录={output_dir}, 最大样本数={max_samples}")
    ...
    def analyze_attention_weights(
        ...
        for key, weights in attention_weights.items():
            ...
            # 转换为numpy并展平
            weights_np = weights.cpu().numpy() if torch.is_tensor(weights) else np.array(weights)
            weights_flat = weights_np.flatten()
            
            try:
                # 使用softmax将注意力权重转换为概率分布（确保所有值为正）
                weights_exp = np.exp(weights_flat - np.max(weights_flat))  # 减去最大值防止数值溢出
                weights_normalized = weights_exp / np.sum(weights_exp)
                
                # 计算注意力熵（衡量注意力分布）
                entropy = -np.sum(weights_normalized * np.log(weights_normalized + 1e-9))
            ...
            if "to" in key:
                ...
                if len(parts) == 2:
                    ...
                    if weights is not None:
                        avg_strength = float(weights.mean().item() if torch.is_tensor(weights) else np.mean(weights))
        ...
        for mod_type in modality_types:
            self.stats["modality_counts"][mod_type] = self.stats["modality_counts"].get(mod_type, 0) + 1
    ...
    def visualize_attention(
        ...
        try:
            ...
            # 绘制每个注意力映射
            for idx, (key, weights) in enumerate(list(attention_weights.items())[:num_plots]):
                if weights is None:
                    continue
    ...
    def record_analysis(self, analysis_result: AttentionAnalysisResult) -> None:
        ...
        if len(self.analysis_results) < self.max_samples:
            self.analysis_results.append(analysis_result)
            logger.debug(f"记录分析结果: 样本={analysis_result.sample_id}, 模态={analysis_result.modality_types}")
        else:
    ...
    def generate_summary_report(self) -> Dict[str, Any]:
        ...
        for result in self.analysis_results:
            for mod_type in result.modality_types:
                modality_distribution[mod_type] = modality_distribution.get(mod_type, 0) + 1
        ...
        report = {
            ...
            "attention_quality": {
                "high_entropy_samples": sum(1 for r in self.analysis_results 
                                           if any(e > 2.0 for e in r.attention_entropy.values())),
                "low_entropy_samples": sum(1 for r in self.analysis_results 
        ...
        # 保存报告到文件
        if self.output_dir:
            report_path = os.path.join(self.output_dir, f"attention_summary_{int(time.time())}.json")
...
def plot_attention_heatmap(attention_matrix: np.ndarray, title: str = "Attention Heatmap"):
    """绘制注意力热图（简单工具函数）"""
