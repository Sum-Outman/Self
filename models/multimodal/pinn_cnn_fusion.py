...
class PINNCNNFusionConfig:
    ...
    cnn_feature_levels: List[int] = field(
...
class PINNCNNFusionModel(nn.Module):
    ...
    def __init__(self, config: PINNCNNFusionConfig):
        ...
        self.training_stats = {
            ...
            "fusion_loss_history": [],
            "total_loss_history": [],
            "weight_history": {"visual": [], "physics": [], "fusion": []},
        }

        logger.info(
            f"PINN-CNN融合模型初始化: 模式={config.fusion_mode}, "
    ...
    def forward(
        self, images: torch.Tensor, coordinates: torch.Tensor
        ...
        # 确保输入数据类型与PINN模型匹配
        if hasattr(self.pinn_model, "config") and hasattr(
            self.pinn_model.config, "dtype"
        ):
        ...
        physical_features = physical_features.view(
    ...
    def _extract_spatial_features(
        self, visual_features: torch.Tensor, coordinates: torch.Tensor
    ) -> torch.Tensor:
        """从视觉特征中提取空间对应特征
    ...
    def compute_loss(
        ...
        # 物理损失（PINN约束）
        # 使用坐标和预测值计算物理约束
        physics_loss = self._compute_physics_loss(coordinates, predictions, iteration)
        ...
        if self.weight_manager is not None:
            visual_weight, physics_weight, fusion_weight = (
    ...
    def _compute_physics_loss(
        self, coordinates: torch.Tensor, predictions: torch.Tensor, iteration: int
    ) -> torch.Tensor:
        """计算物理约束损失"""
        ...
        try:
            ...
            physics_loss = self.pinn_model.compute_pde_residual(
                coords_for_pde, pde_function, iteration
            )
    ...
    def _compute_fusion_loss(self, outputs: Dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        # 2. 互信息最大化（完整）
        mi_loss = -self._estimate_mutual_information(visual_features, physical_features)

        # 总融合损失
        fusion_loss = visual_recon_loss + physical_recon_loss + 0.1 * mi_loss
    ...
    def _estimate_mutual_information(
        self, visual_features: torch.Tensor, physical_features: torch.Tensor
    ) -> torch.Tensor:
        """估计互信息（完整实现）"""
    ...
    def _update_training_stats(self, loss_dict: Dict[str, Any], iteration: int):
        ...
        # 权重历史
        self.training_stats["weight_history"]["visual"].append(
            loss_dict["visual_weight"]
        )
        self.training_stats["weight_history"]["physics"].append(
            loss_dict["physics_weight"]
        )
        self.training_stats["weight_history"]["fusion"].append(
            loss_dict["fusion_weight"]
        )

        # 定期日志
        if iteration % 100 == 0:
            logger.info(
                f"训练统计: iteration={iteration}, "
    ...
    def visual_guided_physics_solving(
        self,
        images: torch.Tensor,
        ...
        # 全局平均池化获取全局视觉特征
        visual_global = (
            F.adaptive_avg_pool2d(visual_features, (1, 1)).squeeze(-1).squeeze(-1)
        )  # [B, C_feat]
        ...
        visual_guide = self.visual_guide_projection(
            visual_global
        )  # [B, pinn_output_dim]
    ...
    def _solve_physics_with_visual_guide(
        self,
        coordinates: torch.Tensor,
        ...
        # 复制视觉指导以匹配坐标数量
        visual_guide_expanded = (
            visual_guide.unsqueeze(1)
            .repeat(1, num_points, 1)
            .view(-1, visual_guide.shape[-1])
        )
        ...
        if not hasattr(self, "guided_pinn_input_layer"):
            self.guided_pinn_input_layer = nn.Linear(
                coord_dim + visual_guide.shape[-1], self.config.pinn_input_dim
    ...
    def physics_constrained_image_generation(
        self,
        ...
        if style_reference is not None:
            ...
            style_global = (
                F.adaptive_avg_pool2d(style_features, (1, 1)).squeeze(-1).squeeze(-1)
            )  # [B, 512]
            style_global = style_global.unsqueeze(1).repeat(
                1, num_points, 1
            )  # [B, N, 512]
    ...
    def enhanced_fusion(
        ...
        if fusion_mode == "bidirectional":
            # 双向引导融合
            # 视觉引导物理特征
            visual_guided_physics = self._visual_guide_physics(
                visual_features, physical_features
            )

            # 物理约束视觉特征
            physics_constrained_visual = self._physics_constrain_visual(
                visual_features, physical_features
            )

            # 融合两者
            fused_features = torch.cat(
                [visual_guided_physics, physics_constrained_visual], dim=-1
            )

        elif fusion_mode == "visual_guided":
            # 视觉引导物理特征
            fused_features = self._visual_guide_physics(
                visual_features, physical_features
            )

        elif fusion_mode == "physics_guided":
            # 物理约束视觉特征
            fused_features = self._physics_constrain_visual(
                visual_features, physical_features
            )
    ...
    def _visual_guide_physics(
        ...
        if visual_dim != physical_dim:
            ...
            physical_features_proj = self.physics_projection_to_visual(
                physical_features
            )
        else:
...
class AttentionFusion(nn.Module):
    ...
    def forward(self, visual_features: torch.Tensor, physical_features: torch.Tensor):
        ...
        fused, _ = self.attention(
            query=visual_proj, key=physical_proj, value=physical_proj
        )
    ...
    def _update_weights(self):
        ...
        for i, key in enumerate(keys):
            if key in loss_ratios:
                ...
                # 限制权重范围
                self.weights[i] = max(
                    self.min_weight, min(self.max_weight, self.weights[i])
                )
...
def test_pinn_cnn_fusion():
    ...
    try:
        ...
        print(
            f"   测试数据: 图像形状={test_images.shape}, 坐标形状={test_coords.shape}"
        )
        ...
        print(
            f"   损失字典: visual={loss_dict['visual_loss']:.6f}, "
            f"physics={loss_dict['physics_loss']:.6f}"
        )
    ...
    try:
        ...
        fusion = AttentionFusion(
            cnn_feature_dim=512, pinn_feature_dim=1, hidden_dim=256, num_heads=8
        )
        ...
        print(
            f"   注意力融合: 输入形状={test_visual.shape}+{test_physical.shape}, "
            f"输出形状={fused.shape}"
        )

        # 测试自适应融合
        fusion_adaptive = AdaptiveFusion(
            cnn_feature_dim=512, pinn_feature_dim=1, hidden_dim=256, num_layers=2
        )
    ...
    try:
        ...
        if "model" not in locals() or model is None:
            ...
            print(
                f"      视觉引导物理求解成功: 物理解形状={guided_result['physics_solution'].shape}"
            )
            ...
            generation_result = model.physics_constrained_image_generation(
                ...
                generation_constraints={
                    "conserve_brightness": True,
                    "smoothness": True,
                },
            )
            print(
                f"      物理约束视觉生成成功: 生成点形状={                     generation_result['generated_image_points'].shape}"
            )
