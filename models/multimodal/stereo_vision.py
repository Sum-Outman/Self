...
class StereoVisionModule(nn.Module):
    ...
    def forward(
        ...
        disparity_values = torch.arange(
            self.max_disparity,
            dtype=torch.float32,
            device=disparity_logits.device
        )
        disparity_map = torch.sum(
            disparity_prob * disparity_values.view(1, -1, 1, 1),
            dim=1,
            keepdim=True
        )
        ...
        spatial_features = self.spatial_feature_extractor(
            torch.cat([
                left_features.mean(dim=[2, 3]),
                point_cloud.mean(dim=1)
            ], dim=1)
        )
    ...
    def detect_objects_3d(
        ...
        for region in object_regions:
            ...
            if indices:
                indices = torch.tensor(indices, dtype=torch.long, device=point_cloud.device)
    ...
    def calibrate(
        ...
        try:
            ...
            ret, _, _, _, _, R, T, E, F = cv2.stereoCalibrate(
                ...
                gray_left.shape[::-1], None, None, None, None, flags=flags
            )
...
class StereoVisionService:
    ...
    def get_service_info(self) -> Dict[str, Any]:
        ...
        return {
            "status": "loaded",
            "module_info": self._module.get_module_info()
        }
