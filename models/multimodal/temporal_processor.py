...
class TemporalMultimodalProcessor(nn.Module):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        logger.info(
            f"初始化TemporalMultimodalProcessor: 时序隐藏维度={                 self.temporal_hidden_dim}, 层数={                 self.num_temporal_layers}"
        )
    ...
    def forward(self, batch: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if image_sequence is not None:
            ...
            image_features = torch.stack(
                image_features_list, dim=1
            )  # [batch, seq_len, image_dim]
    ...
    def detect_cross_modal_events(
        self, temporal_features: torch.Tensor, threshold: float = 0.7
    ) -> List[Dict[str, Any]]:
        """检测跨模态事件"""
    ...
    def compute_temporal_alignment(
        self, modality_a: torch.Tensor, modality_b: torch.Tensor
    ) -> Dict[str, float]:
        """计算时序对齐指标"""
        ...
        similarity_matrix = torch.zeros(
            batch_size, seq_len, seq_len, device=modality_a.device
        )
        ...
        for b in range(batch_size):
            ...
            offset = (
                (col_indices - torch.arange(seq_len, device=modality_a.device))
                .float()
                .abs()
                .mean()
                .item()
