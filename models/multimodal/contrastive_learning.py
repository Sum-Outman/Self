...
class ContrastiveAlignmentModel(nn.Module):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        sensor_sequence_length = config.get("sensor_sequence_length", 100)
        sensor_patch_size = config.get("sensor_patch_size", 10)
        sensor_num_channels = config.get("sensor_num_channels", 9)
        ...
        # 温度参数 (对比学习)
        self.temperature = nn.Parameter(torch.ones([]) * config.get("initial_temperature", 0.07))
        
        # 负样本队列 (用于动量对比)
        self.queue_size = config.get("queue_size", 65536)
        self.momentum = config.get("momentum", 0.999)
        
        logger.info(f"初始化ContrastiveAlignmentModel: 共享维度={shared_dim}, 温度={self.temperature.item()}")
    ...
    def forward(self, batch: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if image_input is not None:
            if image_input.dim() == 2 and image_input.size(1) == self.image_embedding_dim:
                # 输入已经是特征张量，跳过编码器
        ...
        if audio_input is not None:
            if audio_input.dim() == 2 and audio_input.size(1) == self.audio_embedding_dim:
                # 输入已经是特征张量，跳过编码器
        ...
        if sensor_input is not None:
            if sensor_input.dim() == 2 and sensor_input.size(1) == self.sensor_embedding_dim:
                # 输入已经是特征张量，跳过编码器
        ...
        # 投影到共享空间
        text_projected = self.text_projection(text_features) if text_features is not None else None
        image_projected = self.image_projection(image_features) if image_features is not None else None
        audio_projected = self.audio_projection(audio_features) if audio_features is not None else None
        sensor_projected = self.sensor_projection(sensor_features) if sensor_features is not None else None
    ...
    def _compute_contrastive_loss(self, features_a: torch.Tensor, features_b: torch.Tensor, modality_pair: str) -> torch.Tensor:
        """计算对比损失 (InfoNCE)"""
        ...
        if self.training:
            with torch.no_grad():
                ...
                logger.debug(f"对比损失 [{modality_pair}]: loss={loss.item():.4f}, 准确率={accuracy.item():.4f}")
        
        return loss
    
    def compute_alignment_accuracy(self, features_a: torch.Tensor, features_b: torch.Tensor) -> Dict[str, float]:
        """计算对齐准确率"""
        batch_size = features_a.size(0)
        ...
        recall_at_5 = (similarity.topk(5, dim=1).indices == labels.unsqueeze(1)).any(dim=1).float().mean().item()
