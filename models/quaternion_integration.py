...
from models.quaternion_core import (
    ...
    quaternion_angle_loss, quaternion_dot_loss,
    quaternion_exp, quaternion_log,
    quaternion_from_angular_velocity, quaternion_weighted_average
)
from models.quaternion_nn import (
    QuaternionLinear, QuaternionAttention, QuaternionTransformerBlock,
    QuaternionConv1D
)
...
class VisualInertialOdometry:
    ...
    def update_imu(self, accelerometer: np.ndarray, gyroscope: np.ndarray, timestamp: float):
        """
        更新IMU数据
        ...
        if self.use_quaternion:
            ...
            self.position = self.position + self.velocity * dt + 0.5 * accel_world * dt * dt
        else:
            # 使用欧拉角更新姿态
    ...
    def _update_orientation_euler(self, gyroscope: np.ndarray, dt: float):
        ...
        # 保持角度在合理范围内
        self.orientation_euler = np.mod(self.orientation_euler + np.pi, 2 * np.pi) - np.pi
    ...
    def update_visual(self, features: List[np.ndarray], timestamp: float):
        ...
        if self.last_camera_time is None:
            dt = 1.0 / self.config.camera_rate
        else:
...
class QuaternionInverseKinematics:
    ...
    def solve(self, target_position: np.ndarray, target_orientation: Union[np.ndarray, Quaternion],
              initial_joints: Optional[np.ndarray] = None) -> np.ndarray:
        """
        求解逆运动学
        ...
        if isinstance(target_orientation, Quaternion):
            ...
            if target_orientation.shape == (4,):
                # 已经是四元数
                target_q = Quaternion(target_orientation[0], target_orientation[1],
    ...
    def jacobian(self, joint_angles: np.ndarray) -> np.ndarray:
        ...
        for i in range(n_joints):
            ...
            if self.use_quaternion:
                ...
                # 四元数差分（在切空间）
                delta_q = Quaternion(q_plus[0], q_plus[1], q_plus[2], q_plus[3]) * \
                         Quaternion(q_minus[0], q_minus[1], q_minus[2], q_minus[3]).inverse()
...
class QuaternionMotionPlanner:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        # 四元数插值方法
        self.interpolation_method = config.get("interpolation_method", "slerp")  # "slerp", "squad"
        
        logger.info(f"四元数运动规划器初始化完成，使用四元数: {self.use_quaternion}")
    
    def plan_trajectory(self, start_pose: Dict[str, Any], end_pose: Dict[str, Any],
                       duration: float, dt: float = 0.01) -> List[Dict[str, Any]]:
        """
        规划轨迹
        ...
        if self.use_quaternion:
            # 四元数姿态
            if "orientation" in start_pose and isinstance(start_pose["orientation"], (list, np.ndarray)):
                start_q = Quaternion(*start_pose["orientation"])
            else:
                start_q = Quaternion.from_euler(*start_pose["orientation_euler"])
            
            if "orientation" in end_pose and isinstance(end_pose["orientation"], (list, np.ndarray)):
    ...
    def _squad_interpolation(self, q0: Quaternion, q1: Quaternion, t: float) -> Quaternion:
        """简化的SQUAD四元数插值"""
    ...
    def plan_avoidance_trajectory(self, start_pose: Dict[str, Any], end_pose: Dict[str, Any],
                                obstacles: List[Dict[str, Any]], duration: float) -> List[Dict[str, Any]]:
        """避障轨迹规划"""
        # 完整避障规划
            ...
            for obstacle in obstacles:
                ...
                if distance < obs_radius:
                    ...
                    if self.use_quaternion:
                        ...
                        trajectory[i]["orientation_euler"] = adjusted_q.to_euler().tolist()
        
        return trajectory
    
    def _avoid_obstacle(self, position: np.ndarray, obstacle_pos: np.ndarray, radius: float) -> np.ndarray:
        """避障调整"""
...
class QuaternionMultimodalFusion:
    ...
    def fuse_modalities(self, visual_data: Dict[str, Any], inertial_data: Dict[str, Any],
                       proprioceptive_data: Dict[str, Any]) -> Dict[str, Any]:
        """
        融合多模态数据
    ...
    def _extract_pose(self, data: Dict[str, Any], modality: str) -> Optional[Dict[str, Any]]:
        """从数据中提取位姿"""
        if data is None:
            return None  # 返回None
    ...
    def _pose_to_quaternion(self, pose: Dict[str, Any]) -> Quaternion:
        ...
        if "orientation" in pose:
            ...
            if isinstance(orientation, (list, np.ndarray)) and len(orientation) == 4:
                # 已经是四元数
                return Quaternion(orientation[0], orientation[1], orientation[2], orientation[3])
    ...
    def _joints_to_pose(self, joint_data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        if len(joint_angles) >= 3:
            # 完整的正运动学计算
            x = 0.5 * np.cos(joint_angles[0]) + 0.3 * np.cos(joint_angles[0] + joint_angles[1])
            y = 0.5 * np.sin(joint_angles[0]) + 0.3 * np.sin(joint_angles[0] + joint_angles[1])
            z = 0.2
            
            # 末端姿态（完整）
            orientation_euler = [0, 0, joint_angles[0] + joint_angles[1] + joint_angles[2]]
    ...
    def _weighted_average_fusion(self, quaternions: List[Quaternion], weights: np.ndarray) -> Quaternion:
        """加权平均四元数融合"""
        # 转换为向量
        ...
        return Quaternion(fused_vector[0], fused_vector[1], fused_vector[2], fused_vector[3])
    
    def _kalman_fusion(self, quaternions: List[Quaternion], weights: np.ndarray) -> Quaternion:
        """卡尔曼滤波四元数融合（完整版）"""
        # 完整实现：加权平均
...
def test_quaternion_integration():
    ...
    end_pose = {
        "position": [1, 1, 1],
        "orientation": Quaternion.from_euler(np.pi/4, np.pi/6, np.pi/3).as_vector().tolist(),
