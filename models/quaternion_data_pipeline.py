...
class QuaternionDataItem:
    ...
    def _convert_to_quaternion(self):
        ...
        try:
            ...
            logger.error(
                f"四元数转换失败 (ID: {                     self.item_id}): {                     str(e)}, 数据: {                     self.raw_data}, 格式: {                     self.rotation_format}"
            )
            self.is_valid = False
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "raw_data": (
                self.raw_data.tolist()
                if hasattr(self.raw_data, "tolist")
                else self.raw_data
            ),
            "quaternion": (
                self.quaternion.tolist() if self.quaternion is not None else None
            ),
...
class QuaternionDataPipeline:
    ...
    def add_item(
        ...
        self.stats["avg_processing_time"] = (
            self.stats["avg_processing_time"] * (self.stats["total_items"] - 1)
            + processing_time
    ...
    def convert_imu_data(
        ...
        try:
            ...
            metadata = {"algorithm": algorithm, "dt": dt, "gyro_norm": float(gyro_norm)}
    ...
    def get_tensor_batch(
        ...
        返回:
            ...
            metadata_list: 元数据列表
        """
        quaternions_np, metadata_list = self.get_quaternion_batch(
            batch_size, source_filter
        )
    ...
    def interpolate_quaternions(
        ...
        # 确保四元数归一化
        quaternions_norm = quaternions / np.linalg.norm(
...
def test_quaternion_data_pipeline():
    ...
    pipeline = QuaternionDataPipeline(
        cache_size=100, batch_size=10, enable_validation=True, enable_logging=False
    )
    ...
    assert np.allclose(
        np.linalg.norm(item.quaternion), 1.0, rtol=1e-5
    ), "四元数未归一化"
    ...
    quaternions = np.array(
        [[1.0, 0.0, 0.0, 0.0], [np.cos(np.pi / 8), np.sin(np.pi / 8), 0.0, 0.0]]
    )
