...
class QuaternionIntegrationDemo(nn.Module):
    ...
    def _init_quaternion_layers(self):
        ...
        # 实数到四元数投影
        self.real_to_quaternion = nn.Linear(
        ...
        # 四元数到实数投影
        self.quaternion_to_real = nn.Linear(
            self.config.hidden_size, self.config.hidden_size)
...
class QuaternionEnhancedTransformer(nn.Module):
    ...
    def __init__(self, config: AGIModelConfig):
        ...
        if config.quaternion_neural_network_enabled:
            ...
            self.quaternion_ffn = QuaternionLinear(
                quaternion_dim, config.intermediate_size // 4)
            self.quaternion_ffn_out = QuaternionLinear(
                config.intermediate_size // 4, quaternion_dim)
        else:
