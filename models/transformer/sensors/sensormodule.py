...
class SensorModule(nn.Module):
    ...
    def _adaptive_sensor_fusion(self, encoded_features: List[torch.Tensor], sensor_types: List[str]) -> torch.Tensor:
        """自适应传感器融合算法
    ...
    def _calculate_fusion_confidence(self, encoded_features: List[torch.Tensor], sensor_types: List[str]) -> float:
        """计算融合置信度
        ...
        # 综合置信度
        confidence = base_confidence + diversity_bonus + consistency_bonus + quality_bonus
        ...
        try:
            ...
            for i in range(len(features)):
                for j in range(i + 1, len(features)):
                    ...
                    # 计算余弦相似度
                    cos_sim = F.cosine_similarity(feat_i.unsqueeze(0), feat_j.unsqueeze(0))
