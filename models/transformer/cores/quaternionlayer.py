...
class QuaternionTensor:
    ...
    def __mul__(self, other: Union['QuaternionTensor', torch.Tensor, float]) -> 'QuaternionTensor':
        """四元数乘法或标量乘法"""
...
class QuaternionConv2d(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # 重塑输入: [batch, in_channels, height, width, 4] -> [batch, in_channels*4, height, width]
        ...
        # 重塑输出: [batch, out_channels*4, height', width'] -> [batch, out_channels, height', width', 4]
...
class QuaternionAttention(nn.Module):
    ...
    def forward(
        ...
        # 缩放点积注意力
        attn_scores = torch.matmul(q_real, k_real.transpose(-2, -1)) / math.sqrt(self.head_dim)
        
        # 应用掩码
        if key_padding_mask is not None:
            attn_scores = attn_scores.masked_fill(
                key_padding_mask.unsqueeze(0).unsqueeze(1).unsqueeze(2),
                float('-inf')
            )
...
class QuaternionRotationLayer(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # 应用可学习的旋转参数
        # 将旋转参数归一化为单位四元数
        rotation_norm = torch.sqrt(torch.sum(self.rotation_params**2, dim=-1, keepdim=True) + 1e-8)
        ...
        # 归一化输出四元数
        norm = torch.sqrt(torch.sum(rotated_quaternions**2, dim=-1, keepdim=True) + 1e-8)
