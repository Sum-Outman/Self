...
class SensorDataProcessor:
    ...
    def process_imu_data(self, imu_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            euler_for_readings = orientation[:3] if len(orientation) >= 3 else [0.0, 0.0, 0.0]
            ...
            sensor_data = {
                ...
                "metadata": {
                    "sensor_type": "imu",
                    "units": ["m/s²", "m/s²", "m/s²", "rad/s", "rad/s", "rad/s", "rad", "rad", "rad", "°C"],
                    "timestamp": time.time(),
                    "data_format": "robot_imu_v1",
                    "orientation_type": orientation_type,
                    "orientation_values": orientation_values,
                    "orientation_dim": len(orientation_values),
                    "has_quaternion": orientation_type in ["quaternion", "quaternion_with_euler", "euler_with_quaternion"]
                }
            }
    ...
    def process_joint_data(self, joint_states: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
        """处理关节数据
    ...
    def process_all_sensors(self, sensor_data: Dict[str, Dict[str, Any]]) -> List[Dict[str, Any]]:
        """处理所有传感器数据
...
class RobotMultimodalController:
    ...
    def process_robot_command(self, 
                             text_command: Optional[str] = None,
                             sensor_data: Optional[Dict[str, Any]] = None,
                             image_data: Optional[Dict[str, Any]] = None,
                             audio_data: Optional[Dict[str, Any]] = None) -> List[RobotMultimodalCommand]:
        """处理机器人命令（多模态输入）
        ...
        try:
            ...
            if sensor_data:
                # 转换传感器数据为多模态格式
                processed_sensors = self.sensor_processor.process_all_sensors(sensor_data)
                if processed_sensors:
                    multimodal_inputs["sensor_data"] = processed_sensors[0]  # 取第一个传感器
            ...
            # 使用多模态处理器处理输入
            multimodal_result = self.multimodal_processor.process_multimodal(**multimodal_inputs)
            
            # 根据处理结果生成机器人命令
            if multimodal_result.get("success", False):
                commands = self._generate_commands_from_multimodal(multimodal_result, text_command)
    ...
    def _process_simple_command(self, text_command: Optional[str]) -> List[RobotMultimodalCommand]:
        """处理简单文本命令（多模态处理器不可用时）
        ...
        if any(word in command_lower for word in ["向前", "前进", "forward"]):
            commands.append(RobotMultimodalCommand(
    ...
    def _generate_commands_from_multimodal(self, 
                                          multimodal_result: Dict[str, Any],
                                          original_text: Optional[str] = None) -> List[RobotMultimodalCommand]:
        """从多模态处理结果生成机器人命令
        ...
        try:
            ...
            # 如果有文本模态，优先使用文本命令
            text_modality = next((m for m in modalities if isinstance(m, dict) and m.get("type") == "text"), None)
            ...
            # 如果有传感器数据，生成自适应命令
            sensor_modality = next((m for m in modalities if isinstance(m, dict) and m.get("type") == "sensor"), None)
            if sensor_modality:
                commands.append(RobotMultimodalCommand(
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "multimodal_enabled": self.config["enable_multimodal"] and self.multimodal_processor is not None,
    ...
    def execute_commands(self, commands: List[RobotMultimodalCommand], 
                        robot_interface: Optional[Any] = None) -> Dict[str, Any]:
        """执行机器人命令
        ...
        for i, command in enumerate(commands):
            try:
                # 记录命令执行
                self.logger.info(f"执行命令 {i+1}/{len(commands)}: {command.command_type} ({command.confidence:.2f})")
...
class RobotLearningController:
    ...
    def start_reinforcement_learning(self, task_description: str, reward_function: Callable):
        """开始强化学习
...
def create_robot_multimodal_integration(config: Optional[Dict[str, Any]] = None) -> RobotMultimodalController:
    """创建机器人多模态集成控制器（工厂函数）
        ...
        for cmd in test_commands:
            ...
            if commands:
                ...
                for i, robot_cmd in enumerate(commands):
                    print(f"    {i+1}. {robot_cmd.command_type}: {robot_cmd.target} (置信度: {robot_cmd.confidence:.2f})")
            else:
                print("  未生成命令")
