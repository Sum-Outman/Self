...
class FinancialReasoningEngine:
    ...
    def _calculate_investment_return(
        self, principal: float, rate: float, years: int, compounding_frequency: int = 1
    ) -> Dict[str, float]:
        """计算投资回报"""
        # 复利计算
        ...
        return {
            ...
            "annualized_return": (
                (amount / principal) ** (1 / years) - 1 if years > 0 else 0
            ),
        }

    def _assess_risk(
        self, investment_amount: float, volatility: float, time_horizon: int
    ) -> Dict[str, Any]:
        """风险评估"""
        # 完整风险评估模型
    ...
    def _analyze_cash_flow(
        self, cash_flows: List[float], discount_rate: float = 0.05
    ) -> Dict[str, Any]:
        """现金流分析"""
        npv = 0.0  # 净现值
        ...
        if len(cash_flows) > 1:
            ...
            if cash_flows[0] != 0:
                irr_estimate = (total_cf / abs(cash_flows[0])) ** (
                    1 / len(cash_flows)
                ) - 1
    ...
    def _calculate_financial_ratio(self, metrics: Dict[str, float]) -> Dict[str, float]:
        ...
        if "current_assets" in metrics and "current_liabilities" in metrics:
            ratios["current_ratio"] = metrics["current_assets"] / max(
                metrics["current_liabilities"], 1
            )

        # 杠杆比率
        if "total_debt" in metrics and "total_assets" in metrics:
            ratios["debt_to_assets"] = metrics["total_debt"] / max(
                metrics["total_assets"], 1
            )

        # 盈利能力比率
        if "net_income" in metrics and "revenue" in metrics:
            ratios["net_profit_margin"] = metrics["net_income"] / max(
                metrics["revenue"], 1
            )

        # 效率比率
        if "revenue" in metrics and "total_assets" in metrics:
            ratios["asset_turnover"] = metrics["revenue"] / max(
                metrics["total_assets"], 1
            )
    ...
    def infer(self, query: str, context: Optional[Dict] = None) -> Dict[str, Any]:
        ...
        if any(
            word in query_lower
            for word in ["投资回报", "收益率", "investment", "return"]
        ):
            if len(numbers) >= 3:
                principal = numbers[0]
        ...
        # 检测现金流问题
        if any(
            word in query_lower for word in ["现金流", "净现值", "cash flow", "npv"]
        ):
            if len(numbers) >= 1:
                cash_flows = numbers
    ...
    def assess_investment_risk(
        self, amount: float, volatility: float, years: int
    ) -> str:
        """评估投资风险（公开方法）"""
        result = self._assess_risk(amount, volatility, years)
