...
class KnowledgeVersionControl:
    ...
    def _load_versions(self) -> None:
        ...
        try:
            ...
            self.logger.info(
                f"加载版本数据: {sum(len(v) for v in self._versions.values())} 个版本"
            )
    ...
    def get_version_history(
        self, knowledge_id: str, limit: int = 20, offset: int = 0
    ...
    def compare_versions(
        self, knowledge_id: str, version_a: int, version_b: int
    ) -> Dict[str, Any]:
        """比较两个版本的差异
        ...
        diff_result = {
            ...
            "version_a": version_a,
            "version_b": version_b,
            "content_hash_same": version_a_obj.content_hash
            == version_b_obj.content_hash,
            "fields_changed": [],
    ...
    def cleanup_old_versions(
        self, max_versions_per_knowledge: int = 50, max_total_versions: int = 1000
    ) -> Dict[str, int]:
        """清理旧版本
        ...
        return {"total_deleted": total_deleted, "knowledge_deleted": knowledge_deleted}
...
def get_version_controller(
    storage_path: Optional[str] = None, logger: Optional[logging.Logger] = None
