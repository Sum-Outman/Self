...
class KnowledgeReasoner:
    ...
    def infer_relations(
        ...
        # 使用图遍历推理
        traversal_inferences = self._traversal_inference(
            node_id, max_depth, relation_types
        )
    ...
    def _apply_rules(
        self, node_id: str, max_depth: int, relation_types: Optional[List[str]] = None
    ) -> List[Dict[str, Any]]:
        """应用推理规则"""
        inferred_relations = []
        ...
        for rule_name, rule_info in self.rules.items():
            ...
            for match in rule_matches:
                ...
                if (
                    ...
                    and inferred_relation not in relation_types
                ):
                    continue

                # 检查是否已存在该关系
                if not self._relation_exists(
                    inferred_source, inferred_target, inferred_relation
                ):
    ...
    def _match_rule_pattern(
        self, start_node: str, pattern: List[Tuple[str, str, str]]
    ) -> List[Dict[str, str]]:
        """匹配规则模式
                ...
                for edge_key, edge_data in edge_data_dict.items():
                    ...
                    if neighbor_id not in visited or depth > 0:
                        ...
                        if depth > 0 and neighbor_id != node_id:
                            ...
                            if not self._relation_exists(
                                node_id, neighbor_id, composite_relation
                            ):
                                inferred_relations.append(
                                    {
                                        "source": node_id,
                                        ...
                                        "weight": new_path_info["total_weight"]
                                        * 0.8,  # 衰减
                                        "depth": depth + 1,
    ...
    def find_similar_nodes(
        self, node_id: str, similarity_threshold: float = 0.6, max_results: int = 10
    ) -> List[Dict[str, Any]]:
        """查找相似节点
    ...
    def _calculate_node_similarity(
        ...
        # 2. 结构相似度（邻居重叠）
        neighbors1 = (
            set(self.graph.neighbors(node1_id)) if node1_id in self.graph else set()
        )
        neighbors2 = (
            set(self.graph.neighbors(node2_id)) if node2_id in self.graph else set()
        )
    ...
    def _explain_similarity(
        ...
        # 检查直接关系
        if self.graph.has_edge(node1_id, node2_id) or self.graph.has_edge(
            node2_id, node1_id
        ):
    ...
    def discover_patterns(
        self, min_support: int = 2, max_pattern_size: int = 3
    ) -> List[Dict[str, Any]]:
        """发现知识图谱中的模式
    ...
    def _extract_local_patterns(
        self, center_node: str, max_size: int
        ...
        for i in range(min(len(neighbors), max_size)):
            ...
            for neighbor_subset in itertools.combinations(neighbors, i + 1):
                ...
                for neighbor in neighbor_subset:
                    # 获取关系
                    for edge_key, edge_data in self.graph[center_node][
                        neighbor
                    ].items():
    ...
    def _pattern_to_key(self, pattern: List[Tuple[str, str, str]]) -> str:
        ...
        for source, relation, target in sorted_pattern:
            source_type = (
                self.graph.nodes[source].get("type", "unknown")
                if source in self.graph
                else "unknown"
            )
            target_type = (
                self.graph.nodes[target].get("type", "unknown")
    ...
    def complete_knowledge(
        ...
        similar_nodes = self.find_similar_nodes(
            node_id, similarity_threshold=0.5, max_results=5
        )
    ...
    def query_with_reasoning(
        self, query: Dict[str, Any], reasoning_depth: int = 2
    ) -> Dict[str, Any]:
        """带推理的查询
        ...
        if query_type == "find_related":
            ...
            if node_id:
                ...
                inferred = self.infer_relations(
                    node_id, max_depth, [relation] if relation else None
                )
            ...
            if source_id and target_id:
                paths = self.knowledge_graph.find_paths(
                    source_id, target_id, max_length
                )
                result["direct_results"] = paths

                # 推理可能的新路径（通过中间节点）
                result["reasoning_chains"] = self._infer_indirect_paths(
                    source_id, target_id, max_length
                )
    ...
    def _infer_indirect_paths(
        self, source_id: str, target_id: str, max_length: int
...
def get_global_knowledge_reasoner(
...
if __name__ == "__main__":
    ...
    kg.add_node(
        "mammal", "concept", {"name": "哺乳动物", "description": "哺乳动物类别"}
    )
    ...
    for inf in inferences[:5]:  # 显示前5个
        print(
            f"{inf['source']} --[{                 inf['relation']}]--> {                 inf['target']} (深度: {                 inf['depth']})"
        )
    ...
    for sug in suggestions[:3]:
        print(
            f"  建议: {                 sug['source']} --[{                 sug['relation']}]--> {                 sug['target']} (置信度: {                 sug['confidence']:.2f})"
        )
