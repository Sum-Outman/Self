...
class KnowledgeManager:
    ...
    def _init_embedding_model(self):
        ...
        try:
            ...
            self.logger.info(
                f"加载从零开始的文本嵌入器: embedding_dim={self.config.get('embedding_dim', 384)}"
            )
        except Exception as e:
            self.logger.warning(f"无法加载从零开始的文本嵌入器: {e}")
            self.embedding_model = None
    ...
    def search(
        ...
        try:
            ...
            # 执行向量搜索
            vector_results = self.store.search_by_vector(
            ...
            if format == "json":
                ...
                for item in all_knowledge:
                    ...
                    if "metadata" in item and isinstance(item["metadata"], dict):
                        fieldnames.update(
                            f"metadata_{k}" for k in item["metadata"].keys()
                        )
                    ...
                    for key, value in item.items():
                        if key == "metadata" and isinstance(value, dict):
                            ...
                            for meta_key, meta_value in value.items():
                                ...
                                meta_elem.text = (
                                    str(meta_value) if meta_value is not None else ""
                                )
                        else:
                    ...
                    for child in item_elem:
                        if child.tag == "metadata":
                            ...
                            for meta_elem in child:
                                metadata[meta_elem.tag] = (
                                    meta_elem.text if meta_elem.text is not None else ""
                                )
                            item["metadata"] = metadata
                        else:
                            item[child.tag] = (
    ...
    def build_knowledge_graph_from_text(
        ...
        try:
            ...
            self.logger.info(
                f"开始从文本构建知识图谱（领域: {domain}, 文本长度: {len(text)}）"
            )
            ...
            # 构建知识图谱
            graph_results = self._build_graph_from_entities_and_relations(
                entities, relations
            )
        ...
        if entities:
            ...
            for entity in entities:
                ...
                entity["confidence"] = min(
                    entity["confidence"] + (freq_score * 0.2), 1.0
                )

        return entities

    def _extract_relations_from_text(
        self, text: str, entities: List[Dict[str, Any]], domain: str = "general"
    ) -> List[Dict[str, Any]]:
        """从文本中提取实体间的关系
        ...
        # 根据领域调整关系提取规则
        self._get_domain_relation_patterns(domain)
                    ...
                    for left_entity in left_entities:
                        for right_entity in right_entities:
                            ...
                            relation["relation_category"] = (
                                self._determine_relation_category(verb, domain)
                            )
    ...
    def _build_graph_from_entities_and_relations(
        ...
        for relation in relations:
            ...
            if source_id in entity_nodes and target_id in entity_nodes:
                ...
                # 映射到标准关系类型
                mapped_relation_type = self._map_relation_to_standard_type(
                    relation_type
                )
    ...
    def _determine_entity_type(
        self, entity_text: str, context: str, domain_rules: Dict[str, Any]
    ) -> str:
        """确定实体类型
        ...
        if any(char.isdigit() for char in entity_text):
            if any(unit in entity_text for unit in ["元", "美元", "USD", "RMB"]):
                return (
                    "currency"
                    if domain_rules.get("entity_types")
                    and "currency" in domain_rules["entity_types"]
                    else "numeric"
                )
            return "numeric"
    ...
    def _is_duplicate_entity(
        ...
        for entity in existing_entities:
            ...
            # 检查是否在同一句子中的相似位置
            if (
                entity["sentence"] == new_entity["sentence"]
                and abs(entity["position"] - new_entity["position"]) < 5
            ):
                return True
