...
class QuaternionTensor:
    ...
    def to_euler(q: torch.Tensor, order: str = "zyx"):
        ...
        pitch = torch.where(
            torch.abs(sinp) >= 0.9999, torch.sign(sinp) * torch.pi / 2, torch.asin(sinp)
        )
    ...
    def slerp(q1: torch.Tensor, q2: torch.Tensor, t: Union[float, torch.Tensor]):
        ...
        if non_linear_mask.any():
            ...
            # SLERP插值
            interpolated = (
                w1.unsqueeze(1) * q1_nonlinear + w2.unsqueeze(1) * q2_nonlinear
            )
...
def quaternion_weighted_average(
    quaternions: np.ndarray, weights: np.ndarray = None
) -> np.ndarray:
    """
    加权平均四元数（用于滤波和融合）
    ...
    for _ in range(max_iter):
        ...
        delta_q = Quaternion(
            delta_q_vec[0], delta_q_vec[1], delta_q_vec[2], delta_q_vec[3]
        )
        avg_quat = Quaternion(avg_q[0], avg_q[1], avg_q[2], avg_q[3])
