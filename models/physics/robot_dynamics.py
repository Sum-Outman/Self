...
class LagrangianDynamics(nn.Module):
    ...
    def __init__(self, config: RobotConfig):
        ...
        # 动力学参数 (可学习的)
        self.link_masses = nn.Parameter(torch.tensor(config.link_masses, dtype=config.dtype))
        self.com_positions = nn.Parameter(torch.tensor(config.com_positions, dtype=config.dtype))
        ...
        # 摩擦和阻尼参数
        self.friction_coeffs = nn.Parameter(torch.tensor(config.friction_coeffs, dtype=config.dtype))
        self.damping_coeffs = nn.Parameter(torch.tensor(config.damping_coeffs, dtype=config.dtype))
        
        # 重力向量
        self.gravity = torch.tensor(config.gravity, dtype=config.dtype, device=self.device)
    ...
    def jacobian(self, q: torch.Tensor) -> torch.Tensor:
        ...
        """
        batch_size = q.shape[0]
        J = torch.zeros(batch_size, 6, self.num_joints, device=self.device, dtype=q.dtype)
    ...
    def mass_matrix(self, q: torch.Tensor) -> torch.Tensor:
        ...
        """
        batch_size = q.shape[0]
        M = torch.zeros(batch_size, self.num_joints, self.num_joints, 
        ...
        for i in range(self.num_joints):
            ...
            for j in range(i, self.num_joints):
                ...
                for k in range(max(i, j), self.num_joints):
                    ...
                    inertia_contrib = torch.sum(J_w_i * (inertia_k @ J_w_j.unsqueeze(-1)).squeeze(-1), dim=1)
    ...
    def coriolis_matrix(self, q: torch.Tensor, qd: torch.Tensor) -> torch.Tensor:
        ...
        """
        batch_size = q.shape[0]
        C = torch.zeros(batch_size, self.num_joints, self.num_joints,
                       device=self.device, dtype=q.dtype)
    ...
    def inverse_dynamics(self, q: torch.Tensor, qd: torch.Tensor,
        ...
        # 计算扭矩: τ = Mq̈ + Cq̇ + g + f
        tau = torch.bmm(M, qdd.unsqueeze(-1)).squeeze(-1) + \
              torch.bmm(C, qd.unsqueeze(-1)).squeeze(-1) + g + friction
    ...
    def jacobian_com(self, q: torch.Tensor, joint_idx: int) -> torch.Tensor:
        """计算到指定连杆质心的雅可比矩阵"""
        batch_size = q.shape[0]
        J_com = torch.zeros(batch_size, 6, self.num_joints, 
                           device=self.device, dtype=q.dtype)
        ...
        for i in range(joint_idx + 1):
            ...
            # 线性速度雅可比
            for j in range(i, self.num_joints):
                J_com[:, 0, j] += -(self.config.link_lengths[i] + com_offset[0]) * torch.sin(angle_sum)
                J_com[:, 1, j] += (self.config.link_lengths[i] + com_offset[0]) * torch.cos(angle_sum)
...
class RobotPINN(nn.Module):
    ...
    def physics_loss(self, q: torch.Tensor, qd: torch.Tensor, 
                    qdd_pred: torch.Tensor, tau: torch.Tensor) -> torch.Tensor:
        """物理损失: 动力学方程残差"""
    ...
    def energy_loss(self, q: torch.Tensor, qd: torch.Tensor,
                   q_next: torch.Tensor, qd_next: torch.Tensor,
        ...
        kinetic_next = 0.5 * torch.sum(qd_next.unsqueeze(1) @ M_next @ qd_next.unsqueeze(-1), dim=[1, 2])
