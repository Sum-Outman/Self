...
class IMUModel(nn.Module):
    ...
    def __init__(self, config: SensorConfig):
        ...
        # 可学习的偏置参数
        self.accel_bias = nn.Parameter(torch.tensor(config.imu_accel_bias, dtype=config.dtype))
        self.gyro_bias = nn.Parameter(torch.tensor(config.imu_gyro_bias, dtype=config.dtype))
        
        # 尺度因子参数
        self.accel_scale = nn.Parameter(torch.eye(3, dtype=config.dtype) * config.imu_accel_scale)
        self.gyro_scale = nn.Parameter(torch.eye(3, dtype=config.dtype) * config.imu_gyro_scale)
        
        # 噪声协方差
        self.accel_noise_cov = nn.Parameter(torch.eye(3, dtype=config.dtype) * config.noise_std**2)
        self.gyro_noise_cov = nn.Parameter(torch.eye(3, dtype=config.dtype) * config.noise_std**2)
        
        # 移动到设备
        self.to(self.device)
        self.to(config.dtype)
    
    def forward(self, true_accel: torch.Tensor, true_gyro: torch.Tensor,
        ...
        if self.config.bias_drift > 0:
            bias_drift = torch.randn(batch_size, 3, device=self.device) * self.config.bias_drift * dt
            measured_accel += bias_drift
            measured_gyro += bias_drift
        
        return measured_accel, measured_gyro
    
    def _generate_noise(self, batch_size: int, covariance: torch.Tensor) -> torch.Tensor:
        """生成噪声"""
        if self.config.noise_type == "gaussian":
            ...
            # 拉普拉斯噪声
            noise = torch.distributions.Laplace(0, self.config.noise_std).sample(
                (batch_size, 3)
            ).to(self.device)
        elif self.config.noise_type == "student_t":
            # 学生t分布噪声
            noise = torch.distributions.StudentT(3).sample(
                (batch_size, 3)
            ).to(self.device) * self.config.noise_std
        else:
            raise ValueError(f"未知的噪声类型: {self.config.noise_type}")
        
        return noise
    
    def physics_constraints(self, accel_measurements: torch.Tensor,
        ...
        gravity = torch.tensor([0, 0, -9.81], device=self.device, dtype=self.config.dtype)
        ...
        # 完整处理
        correlation_constraint = torch.mean(torch.sum(accel_measurements * gyro_measurements, dim=1)**2)
    ...
    def axis_angle_to_quaternion(self, axis: torch.Tensor, angle: torch.Tensor) -> torch.Tensor:
        """轴角转四元数，使用统一的四元数核心库"""
...
class ForceTorqueSensor(nn.Module):
    ...
    def __init__(self, config: SensorConfig):
        ...
        # 尺度因子和偏置
        self.force_scale = nn.Parameter(torch.eye(3, dtype=config.dtype) * config.ft_force_scale)
        self.torque_scale = nn.Parameter(torch.eye(3, dtype=config.dtype) * config.ft_torque_scale)
        ...
        # 噪声
        self.force_noise_cov = nn.Parameter(torch.eye(3, dtype=config.dtype) * config.noise_std**2)
        self.torque_noise_cov = nn.Parameter(torch.eye(3, dtype=config.dtype) * config.noise_std**2)
        
        # 移动到设备
        self.to(self.device)
        self.to(config.dtype)
    
    def forward(self, true_force: torch.Tensor, true_torque: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        """生成力扭矩测量值"""
        batch_size = true_force.shape[0]
        
        # 应用尺度因子和偏置
        force_scaled = true_force @ self.force_scale.T + self.force_bias.unsqueeze(0)
        torque_scaled = true_torque @ self.torque_scale.T + self.torque_bias.unsqueeze(0)
    ...
    def physics_constraints(self, force_measurements: torch.Tensor,
                           torque_measurements: torch.Tensor,
                           contact_positions: torch.Tensor,
                           external_forces: torch.Tensor) -> Dict[str, torch.Tensor]:
        """物理约束检查"""
        constraints = {}
...
class CameraModel(nn.Module):
    ...
    def __init__(self, config: SensorConfig):
        ...
        # 噪声
        self.pixel_noise_std = nn.Parameter(torch.tensor(config.noise_std, dtype=config.dtype))
    ...
    def project(self, points_3d: torch.Tensor, camera_pose: torch.Tensor) -> torch.Tensor:
        """3D点投影到图像平面
        ...
        # 转换到相机坐标系
        points_homogeneous = torch.cat([points_3d, torch.ones(batch_size, num_points, 1, 
                                                              device=self.device, dtype=self.config.dtype)], dim=2)
        points_camera = torch.bmm(points_homogeneous, camera_pose.transpose(1, 2))[:, :, :3]
    ...
    def physics_constraints(self, points_2d: torch.Tensor, points_3d: torch.Tensor,
                           camera_pose: torch.Tensor) -> Dict[str, torch.Tensor]:
        """物理约束检查"""
        constraints = {}
...
class SensorFusionPINN(nn.Module):
    ...
    def forward(self, sensor_measurements: Dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        if features:
            ...
            if x.shape[1] < 20:
                # 填充
                padding = torch.zeros(x.shape[0], 20 - x.shape[1], 
            ...
            state = torch.zeros(batch_size, 13, device=self.device, dtype=self.config.dtype)
        
        return state
    
    def physics_loss(self, sensor_measurements: Dict[str, torch.Tensor],
                    estimated_state: torch.Tensor, dt: float) -> torch.Tensor:
        """物理损失：传感器数据一致性"""
        loss = 0.0
...
def test_sensor_physics():
    ...
    batch_size = 4
    true_accel = torch.ones(batch_size, 3, dtype=config.dtype) * torch.tensor([0, 0, -9.81])
    ...
    # 测试力传感器模型
    print("\n2. 测试力传感器模型:")
    ft_config = SensorConfig(sensor_type="force_torque", use_gpu=False, dtype=torch.float64)
    ft_sensor = ForceTorqueSensor(ft_config)
    
    true_force = torch.ones(batch_size, 3, dtype=config.dtype) * torch.tensor([0, 0, -10])
    true_torque = torch.ones(batch_size, 3, dtype=config.dtype) * torch.tensor([0, 0, 0.1])
    ...
    # 测试相机模型
    print("\n3. 测试相机模型:")
    camera_config = SensorConfig(sensor_type="camera", use_gpu=False, dtype=torch.float64)
    camera = CameraModel(camera_config)
    
    points_3d = torch.tensor([[[1, 0, 2], [0, 1, 2], [-1, 0, 2]]], dtype=config.dtype).repeat(batch_size, 1, 1)
    ...
    # 测试物理损失
    physics_loss = fusion_pinn.physics_loss(sensor_measurements, estimated_state, dt=0.01)
