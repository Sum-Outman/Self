...
class MultiPhysicsConfig:
    ...
    # 物理场组合
    physics_fields: List[str] = field(default_factory=lambda: ["structural", "thermal", "fluid"])
...
class PhysicsField(nn.Module):
    ...
    def compute_residual(self, state: torch.Tensor, 
                        coupling_states: Dict[str, torch.Tensor]) -> torch.Tensor:
        """计算残差
    ...
    def compute_jacobian(self, state: torch.Tensor,
                        coupling_states: Dict[str, torch.Tensor]) -> torch.Tensor:
        """计算雅可比矩阵
...
class StructuralField(PhysicsField):
    ...
    def _initialize_matrices(self):
        ...
        # 刚度矩阵（完整）
        self.stiffness_matrix = torch.eye(n, dtype=self.config.dtype, device=self.device) * self.youngs_modulus
        
        # 质量矩阵（对角）
        self.mass_matrix = torch.eye(n, dtype=self.config.dtype, device=self.device) * self.density
...
class ThermalField(PhysicsField):
    ...
    def _initialize_matrices(self):
        ...
        # 热传导矩阵（完整）
        self.conductivity_matrix = torch.eye(n, dtype=self.config.dtype, device=self.device) * self.thermal_conductivity
        
        # 热容矩阵
        self.capacity_matrix = torch.eye(n, dtype=self.config.dtype, device=self.device) * self.specific_heat * self.density
...
class FluidField(PhysicsField):
    ...
    def _initialize_matrices(self):
        ...
        # 对流矩阵（完整）
        self.convection_matrix = torch.eye(n, dtype=self.config.dtype, device=self.device) * 0.1
        
        # 扩散矩阵（粘度）
        self.diffusion_matrix = torch.eye(n, dtype=self.config.dtype, device=self.device) * self.viscosity
        
        # 散度矩阵（压力-速度耦合）
        self.divergence_matrix = torch.eye(n, dtype=self.config.dtype, device=self.device) * 0.01
    ...
    def compute_residual(self, state: torch.Tensor,
        ...
        # 组合残差
        momentum_residual = inertia + convection - diffusion + pressure_gradient - body_force
    ...
    def compute_jacobian(self, state: torch.Tensor,
        ...
        J22 = torch.zeros(n_pressure, n_pressure, dtype=self.config.dtype, device=self.device)  # ∂R_p/∂p
        ...
        if state.shape[0] >= n_vel + n_pressure + n_vel:
            ...
            if self.config.time_integration == "implicit":
                # 在J11块上添加质量项
                mass_term = torch.eye(n_vel, dtype=self.config.dtype, device=self.device) * self.density / dt
...
class MonolithicCoupling(nn.Module):
    ...
    def forward(self, initial_states: Dict[str, torch.Tensor] = None) -> Dict[str, torch.Tensor]:
        """整体耦合求解"""
        ...
        if initial_states is None:
            ...
            for name, field in self.fields.items():
                ...
                if isinstance(field, StructuralField):
                    n = field.num_nodes * 3
                    initial_states[name] = torch.zeros(3 * n, dtype=self.config.dtype, device=self.device)
                elif isinstance(field, ThermalField):
                    n = field.num_nodes
                    initial_states[name] = torch.zeros(2 * n, dtype=self.config.dtype, device=self.device)
                elif isinstance(field, FluidField):
                    n_vel = field.num_nodes * 3
                    n_pressure = field.num_nodes
                    initial_states[name] = torch.zeros(n_vel + n_pressure + n_vel, 
        ...
        for iteration in range(self.config.max_iterations):
            ...
            if residual_norm < self.config.tolerance:
                logger.info(f"整体耦合收敛于迭代 {iteration}, 残差范数: {residual_norm.item():.6e}")
                break
    ...
    def combined_residual(self, combined_state: torch.Tensor,
                         field_slices: Dict[str, Tuple[int, int]]) -> torch.Tensor:
        """计算组合残差"""
        residuals = []
    ...
    def combined_jacobian(self, combined_state: torch.Tensor,
                         field_slices: Dict[str, Tuple[int, int]]) -> torch.Tensor:
        """计算组合雅可比矩阵"""
        total_size = combined_state.shape[0]
        J = torch.zeros(total_size, total_size, dtype=self.config.dtype, device=self.device)
        ...
        for name, field in self.fields.items():
            ...
            # 确保雅可比矩阵维度匹配
            if field_jacobian.shape[0] == (end - start) and field_jacobian.shape[1] == (end - start):
                J[start:end, start:end] = field_jacobian
            else:
                # 如果维度不匹配，使用单位矩阵
                J[start:end, start:end] = torch.eye(end - start, dtype=self.config.dtype, device=self.device)
        ...
        for i, (name_i, field_i) in enumerate(self.fields.items()):
            ...
            for j, (name_j, field_j) in enumerate(self.fields.items()):
                ...
                # 创建耦合矩阵（随机，完整）
                coupling_matrix = torch.randn(size_i, size_j, dtype=self.config.dtype, device=self.device) * coupling_strength
    ...
    def forward(self, initial_states: Dict[str, torch.Tensor] = None) -> Dict[str, torch.Tensor]:
        ...
        if initial_states is None:
            ...
            for name, field in self.fields.items():
                if isinstance(field, StructuralField):
                    n = field.num_nodes * 3
                    initial_states[name] = torch.zeros(n, dtype=self.config.dtype, device=self.device)
                elif isinstance(field, ThermalField):
                    n = field.num_nodes
                    initial_states[name] = torch.zeros(n, dtype=self.config.dtype, device=self.device)
                elif isinstance(field, FluidField):
                    n_vel = field.num_nodes * 3
                    n_pressure = field.num_nodes
                    initial_states[name] = torch.zeros(n_vel + n_pressure, dtype=self.config.dtype, device=self.device)
        ...
        for coupling_iter in range(self.config.coupling_iterations):
            ...
            for name, field in self.fields.items():
                ...
                # 求解单个物理场
                field_state = self.solve_single_field(field, current_states[name], coupling_states)
            ...
            # 计算最大状态变化
            max_delta = max(torch.norm(current_states[name] - prev_states[name]) for name in self.fields)
            logger.debug(f"耦合迭代 {coupling_iter}: 最大状态变化 = {max_delta:.6e}")
        
        return current_states
    
    def solve_single_field(self, field: PhysicsField, initial_state: torch.Tensor,
                          coupling_states: Dict[str, torch.Tensor]) -> torch.Tensor:
        """求解单个物理场"""
...
class MultiPhysicsPINN(nn.Module):
    ...
    def forward(self, coordinates: torch.Tensor) -> Dict[str, torch.Tensor]:
        ...
        for name, field in self.fields.items():
            if isinstance(field, StructuralField):
                ...
                pressure = outputs[:, start_idx + n_vel:start_idx + n_vel + n_pressure]
                field_outputs[name] = torch.cat([velocity, pressure], dim=1)
                start_idx += n_vel + n_pressure
        
        return field_outputs
    
    def physics_loss(self, coordinates: torch.Tensor, 
                    field_outputs: Dict[str, torch.Tensor]) -> torch.Tensor:
        """物理损失：多物理场方程残差"""
        total_loss = 0.0
    ...
    def coupling_condition_loss(self, field_outputs: Dict[str, torch.Tensor]) -> torch.Tensor:
        """耦合条件损失"""
        loss = 0.0
        ...
        if "structural" in field_outputs and "fluid" in field_outputs:
            ...
            fluid_velocity = fluid_output[:, :n*3].reshape(-1, n, 3)[:, :, 0]  # 取x方向速度
            
            velocity_continuity = torch.mean((structural_velocity - fluid_velocity)**2)
            loss += velocity_continuity
        
        return loss
    
    def boundary_condition_loss(self, coordinates: torch.Tensor,
                               field_outputs: Dict[str, torch.Tensor]) -> torch.Tensor:
        """边界条件损失"""
        loss = 0.0
