...
class PhysicsConstraint(ABC):
    ...
    def compute_loss(
        self, model: nn.Module, inputs: torch.Tensor, outputs: torch.Tensor, **kwargs
    ) -> torch.Tensor:
        """计算约束损失 - 修复版
        ...
        if kwargs.get("verbose", False) or len(self.history) % 100 == 0:
            logger.debug(
                f"PhysicsConstraint '{self.name}': PDE损失 = {pde_loss.item():.6f}, "
                f"加权损失 = {weighted_loss.item():.6f}"
            )

        return weighted_loss

    def _compute_burgers_residual(
        self, model: nn.Module, inputs: torch.Tensor, outputs: torch.Tensor, **kwargs
    ) -> torch.Tensor:
        ...
        u_x = grads[:, 0:1]  # 对x的导数
        u_t = (
            grads[:, 1:2] if inputs.shape[-1] > 1 else torch.zeros_like(u_x)
        )  # 对t的导数
    ...
    def update_weight(self, loss_value: float, iteration: int):
        ...
        if len(self.history) > 10:
            ...
            historical_mean = (
                np.mean(self.history[:-10]) if len(self.history) > 10 else recent_mean
            )
...
class PINNModel(nn.Module):
    ...
    def __init__(self, config: PINNConfig):
        ...
        self.adaptive_manager = (
            AdaptiveWeightManager() if config.adaptive_weighting else None
        )
        ...
        if config.use_mixed_precision and config.use_gpu and torch.cuda.is_available():
            self.mixed_precision_manager = MixedPrecisionManager(
                enabled=config.amp_enabled, dtype=config.mixed_precision_dtype
            )
        ...
        if config.enable_incremental_cache:
            ...
            logger.info(
                f"增量式缓存管理器初始化: 最大缓存大小={config.max_cache_size}, "
        ...
        logger.info(
            f"PINN模型初始化完成: device={self.device}, dtype={config.dtype}, "
    ...
    def compute_total_loss(
        ...
        for constraint in self.constraints:
            ...
            if (
                ...
                and iteration % self.config.weight_update_freq == 0
            ):
    ...
    def train_step(
        ...
        if self.config.grad_clip > 0:
            if (
                self.mixed_precision_manager is not None
                and self.mixed_precision_manager.grad_scaler is not None
            ):
                # 对于混合精度，需要先unscale梯度
    ...
    def compute_pde_residual(
        self, inputs: torch.Tensor, pde_function: Callable, iteration: int = 0
    ) -> torch.Tensor:
        """计算PDE残差（支持缓存）"""
    ...
    def enforce_boundary_conditions(
        ...
        if boundary_values.shape[0] != boundary_points.shape[0]:
            raise ValueError(
                f"边界点数量({                     boundary_points.shape[0]})与边界值数量({                     boundary_values.shape[0]})不匹配"
            )
    ...
    def _enforce_dirichlet_hard(
        self, boundary_points: torch.Tensor, boundary_values: torch.Tensor
    ):
        """强制Dirichlet边界条件（硬约束）
        ...
        for name, loss in loss_dict.items():
            ...
            if len(self.weight_history[name]) > 10:
                ...
                if recent_mean > 0.5:
                    ...
                    # 权重限制
                    self.constraint_weights[name] = min(
                        self.constraint_weights[name], 10.0
                    )
                    ...
                    # 权重限制
                    self.constraint_weights[name] = max(
                        self.constraint_weights[name], 0.1
                    )
...
class MixedPrecisionManager:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "loss_scale_history": (
                self.loss_scale_history[-10:] if self.loss_scale_history else []
            ),
            "overflow_count": self.overflow_counter,
            "total_iterations": self.total_iterations,
            "avg_loss_scale": (
                np.mean(self.loss_scale_history) if self.loss_scale_history else 1.0
            ),
        }
...
class DistributedTrainingManager:
    ...
    def _init_distributed_training(self):
        ...
        try:
            ...
            if not dist.is_initialized():
                ...
                logger.info(
                    f"分布式训练初始化完成: rank={rank}, world_size={world_size}, "
                ...
                self.initialized = True
                logger.info(
                    f"分布式训练已初始化: rank={self.rank}, world_size={self.world_size}"
                )
    ...
    def create_distributed_sampler(self, dataset):
        ...
        try:
            ...
            logger.info(
                f"创建分布式采样器: rank={self.rank}, world_size={self.world_size}"
            )
            return sampler
...
class IncrementalCacheManager:
    ...
    def __init__(
        ...
        self.logger.info(
            f"增量式缓存管理器初始化: 最大缓存大小={max_cache_size}, "
    ...
    def get(
        ...
        if cache_key in self.cache:
            ...
            # 记录命中
            self.logger.debug(
                f"缓存命中: key={cache_key}, 值类型={type(value).__name__}"
            )
            ...
            if compute_func is not None:
                ...
                self.logger.debug(
                    f"计算完成并缓存: key={cache_key}, 计算时间={compute_time:.4f}s"
                )
    ...
    def put(
        ...
        self.logger.debug(
            f"手动缓存: key={cache_key}, 重要性={importance}, 大小={metadata['value_size']}"
        )
    ...
    def invalidate_pattern(self, pattern: str):
        ...
        if keys_to_remove:
            self.logger.debug(
                f"模式失效: pattern={pattern}, 失效数量={len(keys_to_remove)}"
            )
    ...
    def _check_and_evict(self):
        ...
        self.logger.debug(
            f"缓存满，开始淘汰: 当前大小={len(self.cache)}, "
    ...
    def _evict_lfu(self, num_to_evict: int) -> List[str]:
        ...
        # 按访问次数排序
        items = [
            (key, metadata.get("access_count", 0))
            for key, (_, metadata) in self.cache.items()
        ]
    ...
    def _evict_adaptive(self, num_to_evict: int) -> List[str]:
        ...
        for key, (value, metadata) in self.cache.items():
            ...
            # 计算时间衰减因子（最近访问的应该保留）
            time_factor = max(
                0.0, 1.0 - (current_time - last_access) / 3600.0
            )  # 1小时衰减
    ...
    def _estimate_value_size(self, value: Any) -> int:
        ...
        # 如果是字典
            ...
            for k, v in value.items():
                total_size += self._estimate_value_size(k) + self._estimate_value_size(
                    v
                )
            return total_size
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        if self.enable_adaptive_sizing:
            ...
            if self.memory_usage_history:
                stats["avg_memory_usage_mb"] = np.mean(self.memory_usage_history) / (
                    1024 * 1024
                )
    ...
    def adaptive_size_adjustment(self, available_memory_mb: float):
        ...
        if self.cache:
            total_size = sum(
                metadata.get("value_size", 0) for _, metadata in self.cache.values()
            )
        ...
        if avg_item_size_mb > 0:
            ...
            self.memory_usage_history.append(
                target_memory_mb * 1024 * 1024
            )  # 转换为字节

            self.logger.info(
                f"自适应大小调整: 可用内存={available_memory_mb:.2f}MB, "
...
class CacheKeyGenerator:
    ...
    def generate_pde_residual_key(
        self, inputs: torch.Tensor, model: nn.Module, pde_type: str, iteration: int = 0
    ) -> str:
        """生成PDE残差计算缓存键
...
def burgers_equation(
    x: torch.Tensor, u: torch.Tensor, model: nn.Module
) -> torch.Tensor:
    """Burgers方程残差"""
...
def navier_stokes_equation(
    ...
    # 组合残差
    residual = torch.cat(
        [continuity_residual, x_momentum_residual, y_momentum_residual], dim=1
    )
...
def schrodinger_equation(
    x: torch.Tensor, psi: torch.Tensor, model: nn.Module
) -> torch.Tensor:
    """薛定谔方程残差（时间相关）
    ...
    real_residual = (
        -hbar * psi_imag_t + (hbar**2 / (2 * mass)) * laplacian_real - V * psi_real
    )

    # 虚部: ħ ∂ψ_real/∂t = -ħ²/(2m) ∇²ψ_imag + V ψ_imag
    imag_residual = (
        hbar * psi_real_t + (hbar**2 / (2 * mass)) * laplacian_imag - V * psi_imag
    )
...
def maxwell_equations(
    x: torch.Tensor, fields: torch.Tensor, model: nn.Module
) -> torch.Tensor:
    """麦克斯韦方程残差（时域）
...
def reaction_diffusion_equation(
    x: torch.Tensor, u: torch.Tensor, model: nn.Module
) -> torch.Tensor:
...
def get_pde_function(pde_type: str) -> Callable:
    ...
    if pde_type not in pde_functions:
        raise ValueError(
            f"未知的PDE类型: {pde_type}。可用类型: {list(pde_functions.keys())}"
        )

    return pde_functions[pde_type]


def create_multi_physics_system(
    pde_types: List[str], coupling_strength: float = 0.1
) -> Callable:
    """创建多物理场耦合系统
        ...
        for i, residual in enumerate(residuals):
            ...
            for j, other_residual in enumerate(residuals):
                if i != j:
                    ...
                    coupled_residual = (
                        coupled_residual + coupling_strength * other_residual
                    )
...
def create_adaptive_mesh_refiner(
    ...
    def adaptive_refiner(
        x: torch.Tensor, u: torch.Tensor, residual: torch.Tensor, current_level: int = 0
        ...
        if refined_points_list:
            ...
            new_u = torch.zeros(
                new_points.shape[0], u.shape[1], device=u.device, dtype=u.dtype
            )
...
def create_uncertainty_quantifier(
    num_samples: int = 100, mc_dropout_rate: float = 0.1
) -> Callable:
    """创建不确定性量化器
    ...
    def quantify_uncertainty(
        model: nn.Module, x: torch.Tensor, pde_func: Callable
        ...
        residuals = torch.stack(
            residuals, dim=0
        )  # [num_samples, batch_size, residual_dim]
...
def test_pinn_framework():
    ...
    multi_physics_func = create_multi_physics_system(
        ["heat", "reaction_diffusion"], coupling_strength=0.05
    )
    ...
    refiner = create_adaptive_mesh_refiner(
        initial_resolution=50, refinement_threshold=0.05, max_refinement_level=3
    )
    ...
    # 应用网格细化
    refined_points, refined_u = refiner(
        test_points, test_u, test_residual, current_level=0
    )
    ...
    uncertainty_quantifier = create_uncertainty_quantifier(
        num_samples=50, mc_dropout_rate=0.1
    )

    # 创建带dropout的测试模型
    test_config = PINNConfig(
        input_dim=2, output_dim=1, hidden_dim=16, num_layers=2, activation="tanh"
    )
    ...
    uncertainty_results = uncertainty_quantifier(
        test_model, test_inputs, burgers_equation
    )

    print(f"   均值预测形状: {uncertainty_results['mean_prediction'].shape}")
    print(f"   标准差预测形状: {uncertainty_results['std_prediction'].shape}")
    print(
        f"   平均标准差: {torch.mean(uncertainty_results['std_prediction']).item():.6f}"
    )
        ...
        try:
            ...
            if pde_type == "maxwell":
                test_input = torch.rand(
                    10, 4, device=device, dtype=dtype
                )  # (t, x, y, z)
                test_output = torch.rand(
                    10, 6, device=device, dtype=dtype
            ...
            print(
                f"   {pde_type}: 残差形状={                     residual.shape}, 均值={                     torch.mean(residual).item():.6f}"
            )
        except Exception as e:
    ...
    if cache_stats:
        print("b. 缓存统计:")
        print(
            f"   缓存大小: {cache_stats['cache_size']}/{cache_stats['max_cache_size']}"
        )
