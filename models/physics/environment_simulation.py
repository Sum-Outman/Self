...
class EnvironmentConfig:
    ...
    # 环境类型
    environment_type: str = "rigid_body"  # "rigid_body", "soft_body", "fluid", "deformable"
...
class RigidBody:
    ...
    def update_inertia(self):
        ...
        if self.vertices is not None:
            ...
            self.inertia = torch.diag(torch.tensor([Ixx, Iyy, Izz], dtype=self.config.dtype))
...
class CollisionDetector:
    ...
    def detect_collisions(self, bodies: List[RigidBody]) -> List[Dict[str, Any]]:
        ...
        for i in range(n_bodies):
            for j in range(i + 1, n_bodies):
                ...
                elif body_a.collision_shape == "sphere" and body_b.collision_shape == "sphere":
    ...
    def sphere_sphere_collision(self, sphere_a: RigidBody, sphere_b: RigidBody) -> Dict[str, Any]:
        """球-球碰撞检测"""
        # 球心距离
    ...
    def get_transformed_aabb(self, body: RigidBody) -> Tuple[torch.Tensor, torch.Tensor]:
        """获取变换后的轴对齐边界盒"""
        ...
        vertices_homogeneous = torch.cat([vertices_local, torch.ones(8, 1, dtype=self.config.dtype)], dim=1)
...
class ContactSolver:
    ...
    def solve_contacts(self, bodies: List[RigidBody], 
                      collisions: List[Dict[str, Any]]) -> Tuple[List[torch.Tensor], List[torch.Tensor]]:
        """求解接触力"""
        n_bodies = len(bodies)
        contact_forces = [torch.zeros(3, dtype=self.config.dtype) for _ in range(n_bodies)]
        contact_torques = [torch.zeros(3, dtype=self.config.dtype) for _ in range(n_bodies)]
    ...
    def compute_contact_force(self, body_a: RigidBody, body_b: RigidBody,
                             collision: Dict[str, Any]) -> Tuple[torch.Tensor, torch.Tensor,
        ...
        # 相对速度
        v_a = body_a.linear_velocity + torch.cross(body_a.angular_velocity, p - body_a.position)
        v_b = body_b.linear_velocity + torch.cross(body_b.angular_velocity, p - body_b.position)
        v_rel = v_b - v_a
        ...
        # 如果物体正在分离，不需要力
        if vn > 0:
            return (torch.zeros_like(n), torch.zeros_like(n),
                    torch.zeros_like(n), torch.zeros_like(n))
...
class PhysicsSimulator(nn.Module):
    ...
    def forward(self, bodies: List[RigidBody], dt: float = None) -> List[RigidBody]:
        ...
        # 计算接触力
        contact_forces, contact_torques = self.contact_solver.solve_contacts(bodies, collisions)
        
        # 更新每个物体
        for i, body in enumerate(bodies):
            # 总力：重力 + 外力 + 接触力
            total_force = torch.tensor(self.config.gravity, dtype=self.config.dtype) * body.mass
            ...
            # 更新姿态（四元数积分）
            # 角速度四元数
            omega = torch.cat([torch.zeros(1, dtype=self.config.dtype), body.angular_velocity])
...
class FluidSimulator(nn.Module):
    ...
    def forward(self, dt: float) -> torch.Tensor:
        ...
        if self.velocity_field is None:
            self.velocity_field = torch.zeros(3, self.grid_size, self.grid_size, self.grid_size,
                                             dtype=self.config.dtype, device=self.device)
            self.pressure_field = torch.zeros(self.grid_size, self.grid_size, self.grid_size,
                                             dtype=self.config.dtype, device=self.device)
        ...
        for i in range(self.grid_size):
            for j in range(self.grid_size):
                for k in range(self.grid_size):
                    # 当前位置
                    x = torch.tensor([i, j, k], dtype=self.config.dtype, device=self.device)
                ...
                for j in range(1, self.grid_size - 1):
                    for k in range(1, self.grid_size - 1):
                        self.velocity_field[:, i, j, k] = (
                            self.velocity_field[:, i, j, k] + alpha * (
    ...
    def project(self):
        """投影步（保持不可压缩性）"""
        # 计算散度
        divergence = torch.zeros(self.grid_size, self.grid_size, self.grid_size,
                                dtype=self.config.dtype, device=self.device)
        ...
        for i in range(1, self.grid_size - 1):
            for j in range(1, self.grid_size - 1):
                for k in range(1, self.grid_size - 1):
                    divergence[i, j, k] = (
                        (self.velocity_field[0, i+1, j, k] - self.velocity_field[0, i-1, j, k]) +
                        (self.velocity_field[1, i, j+1, k] - self.velocity_field[1, i, j-1, k]) +
                        (self.velocity_field[2, i, j, k+1] - self.velocity_field[2, i, j, k-1])
                    ) / (2 * self.cell_size)
        ...
        for i in range(1, self.grid_size - 1):
            for j in range(1, self.grid_size - 1):
                for k in range(1, self.grid_size - 1):
                    self.velocity_field[0, i, j, k] -= (pressure[i+1, j, k] - pressure[i-1, j, k]) / (2 * self.cell_size)
                    self.velocity_field[1, i, j, k] -= (pressure[i, j+1, k] - pressure[i, j-1, k]) / (2 * self.cell_size)
                    self.velocity_field[2, i, j, k] -= (pressure[i, j, k+1] - pressure[i, j, k-1]) / (2 * self.cell_size)
