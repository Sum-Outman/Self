...
class FromScratchTextEmbedder(nn.Module):
    ...
    def __init__(
        self, embedding_dim: int = 384, vocab_size: int = 10000, max_length: int = 512
    ):
        super().__init__()
        ...
        logger.info(
            f"从零开始的文本嵌入器初始化: embedding_dim={embedding_dim}, vocab_size={vocab_size}"
        )
    ...
    def encode(
        self, text: str, convert_to_tensor: bool = False
...
class IndustrialTextEmbedder(FromScratchTextEmbedder):
    ...
    def __init__(
        self, embedding_dim: int = 384, vocab_size: int = 50000, max_length: int = 512
    ):
        """初始化工业级文本嵌入器
        ...
        logger.info(
            f"工业级文本嵌入器初始化: embedding_dim={embedding_dim}, vocab_size={vocab_size}"
        )
...
def create_from_scratch_embedder(
    config: Dict[str, Any] = None,
) -> FromScratchTextEmbedder:
    """创建从零开始的嵌入器
...
class AGITextEmbedder(nn.Module):
    ...
    def __init__(self, agi_model: nn.Module, use_transformer: bool = False):
        ...
        logger.info(
            f"AGI文本嵌入器初始化: 嵌入维度={self.embedding_dim}, "
    ...
    def forward(self, text: str) -> torch.Tensor:
        ...
        # 获取词嵌入
        word_embeddings = self.agi_model.word_embeddings(
            input_ids
...
def create_agi_text_embedder(
    agi_model: nn.Module, config: Dict[str, Any] = None
) -> AGITextEmbedder:
    """创建AGI文本嵌入器
