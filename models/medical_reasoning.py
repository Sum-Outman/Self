...
class MedicalReasoningEngine:
    ...
    def _analyze_symptoms(
        self, symptoms: List[str], patient_age: int, patient_gender: str
    ) -> Dict[str, Any]:
        """症状分析"""
        # 完整症状分析逻辑
        common_patterns = {
    ...
    def _calculate_dosage(
        self, drug_name: str, patient_weight: float, condition_severity: str
    ) -> Dict[str, Any]:
        """药物剂量计算"""
        # 完整剂量计算
        standard_dosages = {
        ...
        # 根据严重程度调整
        severity_multiplier = {"mild": 0.5, "moderate": 1.0, "severe": 1.5}.get(
    ...
    def _assess_vital_signs(self, vital_signs: Dict[str, float]) -> Dict[str, Any]:
        ...
        return {
            ...
            "recommendations": (
                ["定期监测生理参数"] if overall_status == "正常" else ["建议就医检查"]
            ),
        }
    ...
    def infer(self, query: str, context: Optional[Dict] = None) -> Dict[str, Any]:
        ...
        # 检测症状分析问题
        if any(
            word in query_lower for word in ["症状", "symptom", "诊断", "diagnosis"]
        ):
            # 完整处理
            return {
                "success": True,
        ...
        if any(
            word in query_lower
            for word in ["心率", "血压", "体温", "heart rate", "blood pressure"]
        ):
            # 完整处理
            return {
                "success": True,
    ...
    def analyze_symptoms(
        self, symptoms: List[str], age: int, gender: str
    ) -> Dict[str, Any]:
        """症状分析（公开方法）"""
        return self._analyze_symptoms(symptoms, age, gender)

    def calculate_drug_dosage(
        self, drug_name: str, weight: float, severity: str = "moderate"
    ) -> float:
        """计算药物剂量（公开方法）"""
