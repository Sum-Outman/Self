...
class EvolutionPersistenceSystem:
    ...
    def save_evolution_state(self, 
        ...
        try:
            # 收集进化管理器状态
            evolution_manager_state = self._extract_evolution_manager_state(evolution_manager)
            ...
            if hasattr(evolution_manager, 'get_system_stats'):
                system_stats = evolution_manager.get_system_stats()
            ...
            state = EvolutionState(
                ...
                performance_benchmarks=self._collect_performance_benchmarks(evolution_manager, model),
                model_configurations=model_configurations,
                hyperparameter_space=self._extract_hyperparameter_space(evolution_manager),
                last_successful_evolution_id=self._get_last_successful_id(evolution_history),
            ...
            # 完整的状态文件（快速加载）
            summary_path = os.path.join(self.states_directory, f"{state_id}_summary.json")
    ...
    def save_architecture_snapshot(self,
                                 model: nn.Module,
        ...
        try:
            ...
            # 保存为JSON
            json_path = os.path.join(self.architectures_directory, f"{architecture_id}.json")
            with open(json_path, 'w') as f:
                json.dump(snapshot, f, indent=2, default=self._json_serializer)
            
            # 如果PyTorch可用，保存模型状态
            if TORCH_AVAILABLE and snapshot["model_state"] is not None:
                model_path = os.path.join(self.architectures_directory, f"{architecture_id}_model.pt")
                torch.save(model.state_dict(), model_path)
                snapshot["model_state_path"] = model_path
            
            logger.info(f"架构快照已保存: {architecture_id}, 性能={performance_metrics}")
    ...
    def save_checkpoint(self,
        ...
        try:
            ...
            checkpoint = {
                ...
                "model_state": model.state_dict() if TORCH_AVAILABLE else None,
                "trainer_state": self._extract_trainer_state(trainer) if TORCH_AVAILABLE else None
            }
            
            # 保存为JSON
            json_path = os.path.join(self.checkpoints_directory, f"{checkpoint_id}.json")
            with open(json_path, 'w') as f:
                json.dump(checkpoint, f, indent=2, default=self._json_serializer)
            
            # 如果PyTorch可用，保存完整检查点
            if TORCH_AVAILABLE and checkpoint["model_state"] is not None:
                checkpoint_path = os.path.join(self.checkpoints_directory, f"{checkpoint_id}.pt")
    ...
    def load_evolution_state(self, state_id: str) -> Optional[EvolutionState]:
        ...
        try:
            ...
            if not os.path.exists(json_path):
                # 尝试其他可能的路径
                json_path = os.path.join(self.states_directory, f"{state_id}_summary.json")
            ...
            if os.path.exists(json_path):
                ...
                logger.info(f"进化状态已加载: {state_id}, 时间戳={datetime.fromtimestamp(state.timestamp)}")
                return state
    ...
    def load_architecture_snapshot(self, architecture_id: str) -> Optional[Dict[str, Any]]:
        """加载架构快照
        ...
        try:
            ...
            if os.path.exists(json_path):
                ...
                # 如果存在模型状态文件，加载它
                model_state_path = os.path.join(self.architectures_directory, f"{architecture_id}_model.pt")
    ...
    def restore_evolution_state(self, 
                              state_id: str,
                              evolution_manager: Any,
        ...
        try:
            ...
            # 恢复进化管理器状态
            self._restore_evolution_manager_state(evolution_manager, state.evolution_manager_state)
            ...
            if model is not None and TORCH_AVAILABLE:
                ...
                if state.best_architectures:
                    ...
                    arch_id = latest_arch.get("architecture_id") or latest_arch.get("snapshot_id")
                ...
                if filename.endswith('_summary.json'):
                    ...
                    try:
                        ...
                        states.append({
                            ...
                            "evolution_history_count": summary.get("evolution_history_count", 0),
                            "best_architectures_count": summary.get("best_architectures_count", 0)
                        })
                ...
                if filename.endswith('.json'):
                    ...
                    try:
                        ...
                        checkpoints.append({
                            ...
                            "evolution_plan_id": checkpoint.get("evolution_plan", {}).get("plan_id", "unknown"),
                            "performance_data": checkpoint.get("performance_data", {})
                        })
    ...
    def export_evolution_data(self, 
        ...
        try:
            if output_path is None:
                ...
                output_path = os.path.join(self.base_directory, f"evolution_export_{timestamp}")
    ...
    def cleanup_old_data(self, 
                        max_age_days: int = 30,
                        keep_best_architectures: int = 10) -> Dict[str, Any]:
        """清理旧数据
        ...
        try:
            ...
            if len(architectures) > keep_best_architectures:
                ...
                architectures_to_delete = [a for a in architectures if a not in architectures_to_keep]
                
                for arch in architectures_to_delete:
                    arch_id = arch["architecture_id"]
                    
                    # 删除JSON文件
                    json_path = os.path.join(self.architectures_directory, f"{arch_id}.json")
                    if os.path.exists(json_path):
                        os.remove(json_path)
                    
                    # 删除模型文件
                    model_path = os.path.join(self.architectures_directory, f"{arch_id}_model.pt")
    ...
    def _extract_evolution_manager_state(self, evolution_manager: Any) -> Dict[str, Any]:
        """提取进化管理器状态"""
        state = {}
    ...
    def _extract_trainer_config(self, trainer: Any) -> Dict[str, Any]:
        ...
        common_attrs = [
            ...
            "num_epochs", "warmup_steps", "max_grad_norm"
        ]
    ...
    def _extract_hyperparameter_space(self, evolution_manager: Any) -> Dict[str, Any]:
        ...
        try:
            ...
            # 3. 检查是否有进化算法实例
                ...
                if hasattr(ea, 'config'):
                    ...
                    if isinstance(ea_config, dict):
                        # 提取与超参数相关的配置
                        hp_keys = ['mutation_rate', 'crossover_rate', 'population_size', 
                                 'selection_pressure', 'elitism_count']
                        for key in hp_keys:
                            if key in ea_config:
            ...
            logger.info(f"超参数空间提取完成: 共{len(hyperparameter_space)}个参数, "
    ...
    def _calculate_state_hash(self, state: EvolutionState) -> str:
        ...
        try:
            # 转换为可哈希的字符串
            state_str = json.dumps(asdict(state), default=self._json_serializer, sort_keys=True)
    ...
    def _collect_performance_benchmarks(self, evolution_manager: Any, model: Optional[nn.Module]) -> Dict[str, float]:
        ...
        try:
            ...
            if hasattr(evolution_manager, 'evolution_history'):
                ...
                if isinstance(evolution_history, list) and len(evolution_history) > 0:
                    # 获取最近的成功进化记录
                    recent_successful = [record for record in evolution_history 
                                       if record.get('success', False) and 
                    ...
                    if recent_successful:
                        ...
                        logger.debug(f"从进化历史提取性能数据: {len(recent_records)}条记录")
            ...
            if benchmarks:
                ...
                logger.info(f"性能基准收集完成: 共{len(benchmarks)}个指标, "
    ...
    def _collect_system_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "total_files": len(os.listdir(self.base_directory)) if os.path.exists(self.base_directory) else 0,
            "last_updated": datetime.now().isoformat()
        }
    
    def _get_last_successful_id(self, evolution_history: List[Dict[str, Any]]) -> Optional[str]:
        """获取最近一次成功的进化ID"""
    ...
    def _save_as_pickle(self, state: EvolutionState, model: Optional[nn.Module], filepath: str):
        """保存为Pickle"""
        try:
            import pickle
            ...
            data_package = {
                "state": state,
                "model_state": model.state_dict() if model is not None and TORCH_AVAILABLE else None,
    ...
    def _export_to_html(self, export_data: Dict[str, Any], filepath: str):
        ...
        <body>
            <div class="container">
                <h1>SelfAGI进化数据导出</h1>
                <div class="timestamp">导出时间: {datetime.now().isoformat()}</div>
                
                <div class="stats">
                    <h3>系统统计</h3>
                    <p>进化状态数: {len(export_data.get('available_states', []))}</p>
                    <p>架构快照数: {len(export_data.get('available_architectures', []))}</p>
                    <p>检查点数: {len(export_data.get('available_checkpoints', []))}</p>
                </div>
        ...
        for state in export_data.get("available_states", []):
            html_content += f"""
                        <tr>
                            ...
                            <td>{datetime.fromtimestamp(state.get('timestamp', 0)).strftime('%Y-%m-%d %H:%M:%S')}</td>
                            <td>{state.get('evolution_history_count', 0)}</td>
                            <td>{state.get('best_architectures_count', 0)}</td>
                            <td>{state.get('state_hash', '')[:8]}</td>
                        </tr>
            """
        ...
        for arch in export_data.get("available_architectures", []):
            ...
            html_content += f"""
                        <tr>
                            ...
                            <td>{datetime.fromtimestamp(arch.get('timestamp', 0)).strftime('%Y-%m-%d %H:%M:%S')}</td>
                            <td>{perf.get('accuracy', 0):.4f}</td>
                            <td>{perf.get('inference_time_ms', 0):.1f} ms</td>
                            <td>{arch.get('model_hash', '')[:8]}</td>
                        </tr>
            """
        ...
        for checkpoint in export_data.get("available_checkpoints", []):
            html_content += f"""
                        <tr>
                            <td>{checkpoint.get('checkpoint_id', '')}</td>
                            <td>{datetime.fromtimestamp(checkpoint.get('timestamp', 0)).strftime('%Y-%m-%d %H:%M:%S')}</td>
                            <td>{checkpoint.get('evolution_plan_id', 'unknown')}</td>
                        </tr>
            """
        ...
        """.format(
            ...
            export_timestamp=datetime.fromtimestamp(export_data.get("export_timestamp", time.time())).isoformat()
        )
        
        with open(filepath, 'w', encoding='utf-8') as f:
            f.write(html_content)
    
    def _restore_evolution_manager_state(self, evolution_manager: Any, state: Dict[str, Any]):
        """恢复进化管理器状态"""
        try:
...
def create_evolution_persistence_system(config: Optional[Dict[str, Any]] = None) -> EvolutionPersistenceSystem:
    """创建进化持久化系统"""
    if config is None:
        config = {
            "base_directory": "./data/evolution"
        }
