...
class VisualFeature:
    ...
    def __init__(self, feature_type: FeatureType, position: Tuple[float, float], 
                 descriptor: Optional[np.ndarray] = None, confidence: float = 1.0):
...
class VisualServoingController:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        # 伺服类型
        self.servoing_type = VisualServoingType(self.config.get("servoing_type", "image_based"))
        ...
        self.robot_params = {
            ...
            "joint_limits": self.config.get("joint_limits", [(-180, 180)] * 6), # 关节限制
            "max_velocity": self.config.get("max_velocity", [30.0] * 6), # 最大关节速度 (度/秒)
        }
    ...
    def _compute_ibvs_control(self, error: float) -> List[float]:
        ...
        # 计算特征点误差向量
        error_vector = []
        for target_feature, current_feature in zip(self.target_features, self.current_features):
            if target_feature.feature_type == FeatureType.POINT and current_feature.feature_type == FeatureType.POINT:
                dx = target_feature.position[0] - current_feature.position[0]
                dy = target_feature.position[1] - current_feature.position[1]
        ...
        # 计算控制量（末端执行器速度）
        try:
            v = -self.control_params["gain"] * np.linalg.inv(J_T @ J + lambda_param**2 * I) @ J_T @ error_vector
            v = v.flatten().tolist()
    ...
    def _control_iteration(self) -> None:
        ...
        with self._lock:
            ...
            if self.iteration_count > self.control_params["max_iterations"]:
                self.logger.warning(f"达到最大迭代次数: {self.control_params['max_iterations']}")
                self.is_running = False
                return
            ...
            if self.iteration_count % 10 == 0:
                self.logger.debug(f"迭代 {self.iteration_count}, 误差: {error:.6f}, 控制: {control_signal}")
    
    def add_feature_update_callback(self, callback: Callable[[List[VisualFeature]], None]) -> None:
        """添加特征更新回调"""
    ...
    def add_control_update_callback(self, callback: Callable[[List[float]], None]) -> None:
        """添加控制更新回调"""
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        with self._lock:
            return {
                ...
                "control_signal": self.control_history[-1] if self.control_history else []
            }
...
class FeatureDetector:
    ...
    def track_features(self, prev_features: List[VisualFeature], prev_image: np.ndarray, 
                      current_image: np.ndarray) -> List[VisualFeature]:
        """跟踪特征
...
def get_visual_servoing_controller(config: Optional[Dict[str, Any]] = None) -> VisualServoingController:
    """获取视觉伺服控制器单例实例"""
