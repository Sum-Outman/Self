...
class ImpedanceController:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.impedance_params = {
            ...
            "stiffness": self.config.get(
                "stiffness", np.eye(3) * 100
            ),  # 刚度矩阵 (3x3)
            "desired_force": self.config.get(
                "desired_force", [0.0, 0.0, -5.0]
            ),  # 期望力 (N)
            "desired_position": self.config.get(
    ...
    def set_impedance_params(
        self, mass: np.ndarray, damping: np.ndarray, stiffness: np.ndarray
    ) -> None:
        """设置阻抗参数"""
        ...
        self.logger.info(
            f"阻抗参数更新: M={mass[0,                               0]:.2f}, B={damping[0,                                                   0]:.2f}, K={stiffness[0,                                                                         0]:.2f}"
        )
...
class AdmittanceController:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.admittance_params = {
            ...
            "stiffness": self.config.get(
                "stiffness", np.eye(3) * 200
            ),  # 刚度矩阵 (3x3)
            "desired_force": self.config.get(
                "desired_force", [0.0, 0.0, -5.0]
            ),  # 期望力 (N)
            "desired_velocity": self.config.get(
    ...
    def update(self, measured_force: np.ndarray, dt: float) -> np.ndarray:
        ...
        self.admittance_params["mass"]
        B = self.admittance_params["damping"]
        self.admittance_params["stiffness"]
    ...
    def set_admittance_params(
        ...
        self.logger.info(
            f"导纳参数更新: M={mass[0,                               0]:.2f}, B={damping[0,                                                   0]:.2f}, K={stiffness[0,                                                                         0]:.2f}"
        )
...
class HybridForcePositionController:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        # 选择矩阵：1表示力控制，0表示位置控制
        self.selection_matrix = self.config.get(
            "selection_matrix",
            np.array(
                [
                    [0, 0, 0],  # X方向：位置控制
        ...
        self.desired_position = np.array(
            self.config.get("desired_position", [0.0, 0.0, 0.0])
        )
        self.desired_force = np.array(
            self.config.get("desired_force", [0.0, 0.0, -5.0])
        )
...
class ForceControlSystem:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        # 控制类型
        self.control_type = ForceControlType(
            self.config.get("control_type", "impedance")
        )
        ...
        if self.control_type == ForceControlType.IMPEDANCE:
            self.controller = ImpedanceController(
                self.config.get("controller_config", {})
            )
            ...
            self.controller = HybridForcePositionController(
                self.config.get("controller_config", {})
            )
        else:
            self.controller = ImpedanceController(
    ...
    def set_desired_force(self, desired_force: List[float]) -> None:
        ...
        if hasattr(self.controller, "set_desired_force"):
            ...
            self.logger.warning("当前控制器不支持设置期望力")
    ...
    def set_desired_position(self, desired_position: List[float]) -> None:
        ...
        if hasattr(self.controller, "set_desired_position"):
            ...
            self.logger.warning("当前控制器不支持设置期望位置")
    ...
    def add_status_update_callback(
