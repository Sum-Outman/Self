...
class HardwareInterface:
    ...
    def __init__(self, component: HardwareComponent, mode: HardwareMode = HardwareMode.SIMULATION):
        self.component = component
        self.mode = mode
...
class RealHardwareInterface(HardwareInterface):
    ...
    def send_command(self, command: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if success:
                self.logger.debug(f"真实硬件命令执行成功: {command.get('type', 'unknown')}")
            else:
                self.logger.warning(f"真实硬件命令执行失败: {command.get('type', 'unknown')}")
            return success
...
class HardwareManager:
    ...
    def register_interface(self, component: HardwareComponent, interface: HardwareInterface) -> None:
        """注册硬件接口"""
    ...
    def switch_mode(self, new_mode: HardwareMode, smooth: bool = True) -> bool:
        ...
        self.logger.info(f"开始切换硬件模式: {old_mode.value} -> {new_mode.value}, 平滑切换: {smooth}")
    ...
    def _smooth_switch_mode(self, new_mode: HardwareMode) -> bool:
            ...
            try:
                ...
                if error_recovery:
                    ...
                    try:
                        ...
                        self.logger.error(f"恢复硬件接口失败 {component.value}: {recover_error}")
        ...
        self.logger.info(f"平滑切换完成，成功率: {success_count}/{total_count} ({success_rate:.1%})")
        return success
    ...
    def _immediate_switch_mode(self, new_mode: HardwareMode) -> bool:
        ...
        self.logger.info(f"立即切换完成，成功率: {success_count}/{total_count} ({success_rate:.1%})")
        return success
    ...
    def _notify_mode_change_start(self, old_mode: HardwareMode, new_mode: HardwareMode) -> None:
        """通知模式切换开始"""
    ...
    def remove_mode_listener(self, listener: Callable[[HardwareMode, HardwareMode], None]) -> None:
        """移除模式切换监听器"""
        if listener in self.mode_listeners:
            self.mode_listeners.remove(listener)
    
    def get_component_state(self, component: HardwareComponent) -> Optional[HardwareState]:
        """获取组件状态"""
    ...
    def send_component_command(self, component: HardwareComponent, command: Dict[str, Any]) -> bool:
        """发送组件命令"""
        interface = self.interfaces.get(component)
        if interface:
    ...
    def get_system_health(self) -> Dict[str, Any]:
        ...
        for component, interface in self.interfaces.items():
            ...
            # 简单健康检查
            if interface.last_update_time > 0 and (time.time() - interface.last_update_time) < 10:
