...
class QuaternionDatasetConfig:
    ...
    # 数据增强配置
    augmentations: List[QuaternionAugmentationType] = field(
    ...
    def __post_init__(self):
        ...
        if self.cache_dir is None:
            self.cache_dir = os.path.join(
                os.path.expanduser("~"), ".cache", "self_agi", "quaternion_dataset"
            )
...
class QuaternionDataset(Dataset):
    ...
    def __init__(
        ...
        logger.info(
            f"四元数数据集初始化完成: 模式={self.mode.value}, 样本数={len(self.data_cache)}"
        )
    ...
    def _init_data_sources(self):
        ...
        for source_config in self.config.data_sources:
            ...
            if source_type == "file":
                ...
                if data:
                    self.data_sources.append({"type": "memory", "data": data})

            elif source_type == "pipeline":
                # 直接使用数据管道
                self.data_sources.append(
                    {"type": "pipeline", "pipeline": self.data_pipeline}
                )
    ...
    def _load_data(self):
        ...
        logger.info(
            f"数据加载完成: 总样本数={self.stats['total_samples']}, 有效样本数={total_loaded}"
        )
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, Dict[str, Any]]:
        ...
        if (
            ...
            and cache_key in self.cache_indices
            and self.cache_indices[cache_key] < len(self.data_cache)
        ):
        ...
        # 数据增强（仅训练模式）
        if (
            self.mode == QuaternionDatasetMode.TRAIN
            and random.random() < self.config.augmentation_prob
        ):
    ...
    def get_dataloader(self, **kwargs) -> DataLoader:
        ...
        loader_kwargs = {
            ...
            "shuffle": (
                self.config.shuffle
                if self.mode == QuaternionDatasetMode.TRAIN
                else False
            ),
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        stats_copy["augmentation_rate"] = (
            stats_copy["augmented_samples"] / total_accesses
            if total_accesses > 0
            else 0.0
        )
