...
class CompressionConfig:
    ...
    # 复合压缩配置
    composite_steps: List[CompressionStrategy] = field(
    ...
    # 通用配置
    layer_types_to_compress: List[str] = field(
    ...
    def __post_init__(self):
        ...
        if self.target_sparsity < 0 or self.target_sparsity > 1:
            warnings.warn(
                f"target_sparsity必须在[0,1]范围内，已调整为0.5 (原值: {self.target_sparsity})"
            )
            self.target_sparsity = 0.5

        if self.quantization_bits not in [4, 8, 16, 32]:
            warnings.warn(
                f"quantization_bits必须是4、8、16或32，已调整为8 (原值: {self.quantization_bits})"
            )
            self.quantization_bits = 8

        if self.temperature <= 0:
            warnings.warn(
                f"temperature必须大于0，已调整为3.0 (原值: {self.temperature})"
            )
        ...
        if self.rank_ratio <= 0 or self.rank_ratio >= 1:
            warnings.warn(
                f"rank_ratio必须在(0,1)范围内，已调整为0.25 (原值: {self.rank_ratio})"
            )
            self.rank_ratio = 0.25

        if self.num_shared_groups < 1:
            warnings.warn(
                f"num_shared_groups必须大于0，已调整为4 (原值: {self.num_shared_groups})"
            )
...
class ModelCompressor:
    ...
    def compress(
        ...
        if evaluator is not None:
            ...
            self.compression_stats["accuracy_drop"] = (
                original_accuracy - compressed_accuracy
            )
            self.logger.info(
                f"精度变化: {                     original_accuracy:.4f} -> {                     compressed_accuracy:.4f}, 下降: {                     self.compression_stats['accuracy_drop']:.4f}"
            )
        ...
        self.logger.info(
            f"模型压缩完成: 压缩率={self.compression_stats['compression_ratio']:.2f}x, "
    ...
    def _apply_pruning(self, model: nn.Module) -> nn.Module:
        """应用模型剪枝"""
        self.logger.info(
            f"应用模型剪枝: 方法={                 self.config.pruning_method.value}, 目标稀疏度={                 self.config.target_sparsity}"
        )
        ...
        self.compression_stats["sparsity_achieved"] = (
            1.0 - (non_zero_params / total_params) if total_params > 0 else 0
        )

        self.logger.info(
            f"剪枝完成: 稀疏度={self.compression_stats['sparsity_achieved']:.3f}, "
    ...
    def _pytorch_pruning(self, model: nn.Module) -> nn.Module:
        ...
        for name, module in model.named_modules():
            ...
            if (
                module_type in self.config.layer_types_to_compress
                and name not in self.config.skip_layers
            ):
            ...
            try:
                if self.config.pruning_method == PruningMethod.L1_UNSTRUCTURED:
                    prune.l1_unstructured(
                        module, name=param_name, amount=self.config.target_sparsity
                    )
    ...
    def _custom_pruning(self, model: nn.Module) -> nn.Module:
        ...
        for name, param in model.named_parameters():
            if "weight" in name and not any(
                skip in name for skip in self.config.skip_layers
            ):
                # 获取层类型
                ...
                if module_type in self.config.layer_types_to_compress:
                    ...
                    if self.config.pruning_method == PruningMethod.L1_UNSTRUCTURED:
                        ...
                        self.config.pruning_method == PruningMethod.RANDOM_UNSTRUCTURED
                    ):
    ...
    def _apply_quantization(
        ...
        """应用模型量化"""
        self.logger.info(
            f"应用模型量化: 方法={                 self.config.quantization_method.value}, 位数={                 self.config.quantization_bits}"
        )
    ...
    def _pytorch_quantization(
        ...
        if self.config.quantization_method == QuantizationMethod.POST_TRAINING_STATIC:
            ...
            self.config.quantization_method == QuantizationMethod.POST_TRAINING_DYNAMIC
        ):
            # 动态量化
            ...
            self.logger.warning(
                f"不支持的PyTorch量化方法: {self.config.quantization_method}"
            )
            quantized_model = model
    ...
    def _calculate_quantization_error(
        self, original_model: nn.Module, quantized_model: nn.Module
    ):
        """计算量化误差"""
        total_error = 0.0
        num_params = 0
    ...
    def _apply_knowledge_distillation(
        ...
        for epoch in range(self.config.fine_tune_epochs):
            ...
            if (epoch + 1) % max(1, self.config.fine_tune_epochs // 5) == 0:
                self.logger.info(
                    f"蒸馏epoch {epoch + 1}/{self.config.fine_tune_epochs}, 损失: {total_loss:.4f}"
                )
            ...
            if isinstance(module, nn.Linear):
                ...
                if hasattr(module, "out_features"):
                    ...
                    if new_out_features > 10:  # 保持最小维度
                        ...
                        # 替换模块（简化实现）
                        self.logger.debug(
                            f"简化层: {name}, {module.out_features} -> {new_out_features}"
                        )
    ...
    def _apply_weight_sharing(self, model: nn.Module) -> nn.Module:
        ...
        for (name, param), flat_weights in zip(weight_locations, all_weights):
            ...
            self.logger.debug(
                f"权重共享层: {name}, 组数={len(torch.unique(param_assignments))}"
            )

        self.logger.info(
            f"权重共享完成: 原始权重数={combined_weights.numel()}, "
            f"共享后唯一权重数={len(shared_weights)}"
        )

        return model

    def _kmeans_weight_sharing(
        self, weights: torch.Tensor, num_clusters: int
        ...
        for _ in range(20):  # 最大迭代次数
            ...
            distances = torch.cdist(
                weights.unsqueeze(1), centroids.unsqueeze(1)
            ).squeeze()
    ...
    def _apply_low_rank_decomposition(self, model: nn.Module) -> nn.Module:
        ...
        for name, module in model.named_modules():
            if isinstance(module, nn.Linear) and name not in self.config.skip_layers:
                ...
                # 计算目标秩
                rank = max(
                    1, int(min(in_features, out_features) * self.config.rank_ratio)
                )
                ...
                self.logger.debug(
                    f"低秩分解层: {name}, 秩={rank}/{min(in_features, out_features)}"
                )

        return decomposed_model

    def _apply_composite_compression(
        self, model: nn.Module, dataloader: Optional[Any] = None
    ) -> nn.Module:
        """应用复合压缩"""
    ...
    def _fine_tune_model(self, model: nn.Module, dataloader: Any) -> nn.Module:
        ...
        for epoch in range(self.config.fine_tune_epochs):
            ...
            if (epoch + 1) % max(1, self.config.fine_tune_epochs // 5) == 0:
                self.logger.info(
                    f"微调epoch {epoch + 1}/{self.config.fine_tune_epochs}, 损失: {total_loss:.4f}"
                )
    ...
    def _record_model_info(self, model: nn.Module, stage: str):
        ...
        self.logger.info(
            f"{stage}模型信息: 总参数={total_params:,}, 可训练参数={trainable_params:,}"
        )
    ...
    def _calculate_compression_stats(
        ...
        if original_memory > 0:
            self.compression_stats["compression_ratio"] = (
                original_memory / compressed_memory
            )
            self.compression_stats["memory_reduction"] = (
                1 - compressed_memory / original_memory
            ) * 100

    def _get_module_type_from_param_name(
        self, model: nn.Module, param_name: str
    ) -> str:
        """从参数名获取模块类型"""
        # 简化实现：从参数名中提取模块类型
        ...
        if len(parts) >= 2:
            ...
            try:
                ...
                logging.getLogger(__name__).warning(
                    f"未找到模块 {module_name}，返回Unknown: {e}"
                )
    ...
    def get_compression_report(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "config": {
                ...
                "pruning_method": (
                    self.config.pruning_method.value
                    if hasattr(self.config, "pruning_method")
                    else None
                ),
                "quantization_method": (
                    self.config.quantization_method.value
                    if hasattr(self.config, "quantization_method")
                    else None
                ),
                "quantization_bits": (
...
class ModelQuantizationTool:
    ...
    def quantize(
        self, method: str = "int8_static", dataloader: Optional[Any] = None
    ) -> nn.Module:
        """量化模型
    ...
    def _pytorch_quantize(
        self, config: Dict[str, Any], dataloader: Optional[Any]
    ) -> nn.Module:
        """使用PyTorch量化"""
    ...
    def analyze_quantization_sensitivity(
        self, dataloader: Any, evaluator: Callable
    ) -> Dict[str, Any]:
        """分析量化敏感度
    ...
    def _quantize_single_layer(
        self, layer_name: str, layer_module: nn.Module
    ) -> nn.Module:
        """量化单个层（用于敏感度分析）"""
        # 创建模型副本
...
def test_model_compression():
    ...
    print(
        f"剪枝报告: 压缩率={report['compression_stats']['compression_ratio']:.2f}x, "
    ...
    config = CompressionConfig(
        ...
        composite_steps=[CompressionStrategy.PRUNING, CompressionStrategy.QUANTIZATION],
        target_sparsity=0.2,
        quantization_bits=8,
    )
    compressor = ModelCompressor(config)
    ...
    report = compressor.get_compression_report()
    print(
        f"复合压缩报告: 压缩率={report['compression_stats']['compression_ratio']:.2f}x, "
