...
from models.quaternion_core import QuaternionNormalization
...
class QuaternionLinear(nn.Module):
    ...
    def __init__(
        ...
        if bias:
            self.bias = nn.Parameter(torch.Tensor(out_features * quaternion_dim))
        else:
...
class QuaternionAttention(nn.Module):
    ...
    def _create_rotary_embeddings(self, max_seq_length: int = 8192) -> torch.Tensor:
        ...
        div_term = torch.exp(
            torch.arange(0, angle_dim, 2) * -(math.log(10000.0) / angle_dim)
        )
    ...
    def _apply_rotary_embeddings(
        ...
        pos_emb = self.rotary_embeddings[
            position_ids
        ...
        real = x_reshaped[
            ..., 0
        ]  # 实部 [batch_size, seq_length, num_heads, head_size//4]
        imag = x_reshaped[
            ..., 1:
        ]  # 虚部 [batch_size, seq_length, num_heads, head_size//4, 3]

        # 重塑位置编码以匹配四元数格式
        pos_emb_reshaped = pos_emb.view(
            batch_size, seq_length, num_heads, head_size // 4, 4
        )

        # 提取旋转角度（只使用实部和第一个虚部）
        cos_theta = pos_emb_reshaped[
            ..., 0
        ]  # 余弦部分 [batch_size, seq_length, num_heads, head_size//4]
        sin_theta = pos_emb_reshaped[
            ..., 1
        ]  # 正弦部分 [batch_size, seq_length, num_heads, head_size//4]
        ...
        imag_yz_rot = imag[
            ..., 1:
        ]  # [batch_size, seq_length, num_heads, head_size//4, 2]

        # 合并旋转后的分量
        imag_rot = torch.cat(
            [imag_x_rot.unsqueeze(-1), imag_yz_rot], dim=-1  # 虚部x  # 虚部y,z
        )  # [batch_size, seq_length, num_heads, head_size//4, 3]

        x_rotated = torch.cat(
            [real_rot.unsqueeze(-1), imag_rot], dim=-1  # 实部  # 虚部
        )  # [batch_size, seq_length, num_heads, head_size//4, 4]
