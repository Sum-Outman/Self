...
class PhysicsReasoningEngine:
    ...
    def _apply_newton_laws(self, mass: float, force: float, initial_velocity: float, time: float) -> Dict[str, float]:
        """应用牛顿运动定律"""
        # F = m*a => a = F/m
    ...
    def _apply_energy_conservation(self, mass: float, velocity: float, height: float, gravity: float = 9.81) -> Dict[str, float]:
        """应用能量守恒定律"""
    ...
    def _apply_momentum_conservation(self, mass1: float, velocity1: float, mass2: float, velocity2: float) -> Dict[str, float]:
        """应用动量守恒定律"""
    ...
    def _calculate_projectile_motion(self, initial_velocity: float, launch_angle: float, gravity: float = 9.81) -> Dict[str, float]:
        """计算抛射体运动"""
    ...
    def infer(self, query: str, context: Optional[Dict] = None) -> Dict[str, Any]:
        ...
        # 检测能量守恒问题
        if any(word in query_lower for word in ["能量", "守恒", "energy", "conservation"]):
            if len(numbers) >= 3:
                mass = numbers[0]
                velocity = numbers[1]
                height = numbers[2]
    ...
    def calculate_velocity(self, initial_velocity: float, acceleration: float, time: float) -> float:
        """计算速度（公开方法）"""
    ...
    def calculate_potential_energy(self, mass: float, height: float, gravity: float = 9.81) -> float:
        """计算势能（公开方法）"""
