...
class DecisionEngine:
    ...
    def _initialize_action_map(self) -> Dict[int, Dict[str, Any]]:
        ...
        actions = [
            # 探索类动作
            {
                ...
                "name": "环境扫描",
                "description": "扫描周围环境，收集信息",
            },
            {
                "id": 1,
                "type": DecisionType.EXPLORATION,
                "name": "传感器校准",
                "description": "校准传感器，提高数据精度",
            },
            {
                "id": 2,
                "type": DecisionType.EXPLORATION,
                "name": "未知区域探索",
                "description": "探索未知区域，扩展认知",
            },
            # 利用类动作
            {
                "id": 3,
                "type": DecisionType.EXPLOITATION,
                "name": "执行已知任务",
                "description": "执行已掌握的任务",
            },
            {
                "id": 4,
                "type": DecisionType.EXPLOITATION,
                "name": "优化当前策略",
                "description": "基于现有知识优化策略",
            },
            {
                "id": 5,
                "type": DecisionType.EXPLOITATION,
            ...
            },
            {
                ...
                "type": DecisionType.SAFETY,
                "name": "紧急停止",
                "description": "紧急停止当前操作",
            },
            # 学习类动作
            {
                "id": 9,
                "type": DecisionType.LEARNING,
                "name": "模型训练",
                "description": "训练决策模型",
            },
            {
            ...
            {
                "id": 12,
                "type": DecisionType.COOPERATION,
                "name": "请求协助",
                "description": "向其他系统请求协助",
            },
            {
                "id": 13,
                "type": DecisionType.COOPERATION,
                "name": "资源共享",
                "description": "与其他系统共享资源",
            },
            {
                "id": 14,
                "type": DecisionType.COOPERATION,
            ...
            # 其他动作
            {
                ...
                "type": DecisionType.EXPLOITATION,
                "name": "等待观察",
                "description": "等待并观察环境变化",
            },
            {
                "id": 19,
                "type": DecisionType.EXPLORATION,
                "name": "测试新策略",
                "description": "测试新的决策策略",
            },
        ]
    ...
    def make_decision(self, state: EnvironmentState) -> Optional[Decision]:
        ...
        try:
            ...
            if (
                ...
                self.logger.warning(
                    f"决策被拒绝: 风险={risk:.3f}, 置信度={confidence:.3f}"
                )
                return None  # 返回None
            ...
            self.stats["avg_decision_time"] = (
                self.stats["avg_decision_time"] * (self.stats["total_decisions"] - 1)
                + decision_time
    ...
    def update_decision_result(
        ...
        self.stats["avg_reward"] = (
            self.stats["avg_reward"]
            * (self.stats["successful_decisions"] + self.stats["failed_decisions"] - 1)
            + actual_reward
        ...
        self.logger.info(
            f"决策结果更新: {decision_id}, 成功: {success}, 奖励: {actual_reward}"
        )
        return True
    ...
    def _learn_from_experience(self):
        ...
        with torch.no_grad():
            ...
            # 如果回合结束，下一个状态的Q值为0
            target_q = rewards_tensor + self.config["gamma"] * max_next_q * (
                ~dones_tensor
            )
        ...
        self.logger.debug(
            f"学习步骤: {self.steps_done}, 损失: {loss.item():.4f}, 探索率: {self.epsilon:.4f}"
        )
