...
class QuaternionAdam(optim.Adam):
    ...
    def step(self, closure: Optional[Callable] = None):
        ...
        if (
            ...
            and self.projection_frequency > 0
            and self.step_count % self.projection_frequency == 0
        ):
...
class QuaternionMixedLoss(QuaternionLoss):
    ...
    def __init__(self, weights: Dict[str, float] = None, reduction: str = "mean"):
        """
        初始化混合损失
...
class QuaternionGradientClipper:
    ...
    def _clip_quaternion_gradients(self, model: nn.Module):
        ...
        if quaternion_params:
            ...
            torch.nn.utils.clip_grad_norm_(
                quaternion_grads, self.max_norm, self.norm_type
            )

        # 裁剪其他参数梯度
        if other_params:
            other_grads = [p.grad for p in other_params]
            torch.nn.utils.clip_grad_norm_(other_grads, self.max_norm, self.norm_type)
    ...
    def _separate_parameter_groups(self):
        ...
        if other_params:
            self.param_groups.append(
                {"params": other_params, "lr": self.optimizer.param_groups[0]["lr"]}
            )

        if quaternion_params:
            quaternion_lr = (
                self.optimizer.param_groups[0]["lr"] * self.quaternion_lr_multiplier
            )
            self.param_groups.append({"params": quaternion_params, "lr": quaternion_lr})
...
def test_quaternion_optimizer():
    ...
    optimizer = QuaternionAdam(
        model.parameters(), lr=0.001, enforce_unit_norm=True, projection_frequency=5
    )
