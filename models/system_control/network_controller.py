...
class UDPServer:
    ...
    def start(self) -> bool:
        ...
        try:
            ...
            self.server_socket.bind((self.host, self.port))
            self.is_running = True
            self.server_thread = threading.Thread(target=self._receive_messages, daemon=True)
            self.server_thread.start()
...
class HTTPServer:
    ...
    def _handle_client(self, client_socket: socket.socket, client_address: Tuple[str, int]):
        ...
        try:
            ...
            if path in self.routes:
                ...
                # 默认404响应
                response = self._create_response(404, "Not Found", "text/html", "<h1>404 Not Found</h1>")
            
            client_socket.send(response.encode('utf-8'))
        except Exception as e:
            logger.error(f"处理HTTP客户端时出错: {e}")
            error_response = self._create_response(500, "Internal Server Error", "text/html", "<h1>500 Internal Server Error</h1>")
            try:
    ...
    def _create_response(self, status_code: int, status_text: str, content_type: str, body: str) -> str:
        """创建HTTP响应"""
...
class NetworkConfigurationManager:
            ...
            for line in output.split('\n'):
                ...
                if line and not line.startswith(' ') and ':' in line and not line.startswith('Windows IP'):
                    if current_interface:
                        self.interfaces[current_interface['name']] = NetworkInterface(**current_interface)
                    ...
                    current_interface = {
                        ...
                        'speed_mbps': 0,
                        'is_up': False,
                        'is_wireless': '无线' in interface_name or 'Wi-Fi' in interface_name
                    }
                ...
                # 解析IP地址
                    ...
                    parts = line.split(':')
                    if len(parts) > 1:
                        current_interface['ip_address'] = parts[1].strip().split('(')[0].strip()
            ...
            for line in output.split('\n'):
                ...
                if line and not line.startswith(' ') and ':' in line:
                    ...
                    current_interface = {
                        ...
                        'speed_mbps': 0,
                        'is_up': 'UP' in line,
                        'is_wireless': 'wlan' in interface_name or 'wlp' in interface_name
                    }
    ...
    def set_static_ip(self, interface_name: str, ip_address: str, netmask: str, gateway: str) -> bool:
        """设置静态IP地址"""
        try:
            # Windows系统
            if platform.system() == "Windows":
...
class NetworkMonitor:
    ...
    def _collect_metrics(self):
        ...
        try:
            ...
            if platform.system() == "Windows":
                ...
                raise RuntimeError(f"不支持的操作系统: {platform.system()}。网络指标收集需要Windows或Linux系统。项目要求禁止使用虚拟数据。")
        except Exception as e:
    ...
    def _collect_basic_metrics(self):
        ...
        必须使用真实网络监控器收集网络指标。
        """
        raise RuntimeError("模拟网络指标收集已禁用。请确保网络监控器已正确初始化并可用。项目要求禁止使用虚拟数据，必须使用真实网络监控。")
...
class NetworkController:
    ...
    def create_tcp_server(self, server_id: str, host: str = "0.0.0.0", port: int = 8080) -> bool:
        """创建TCP服务器"""
    ...
    def create_udp_server(self, server_id: str, host: str = "0.0.0.0", port: int = 9090) -> bool:
        """创建UDP服务器"""
    ...
    def create_http_server(self, server_id: str, host: str = "0.0.0.0", port: int = 80) -> bool:
        """创建HTTP服务器"""
    ...
    def set_static_ip(self, interface_name: str, ip_address: str, netmask: str, gateway: str) -> bool:
        """设置静态IP"""
        return self.config_manager.set_static_ip(interface_name, ip_address, netmask, gateway)
