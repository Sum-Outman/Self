...
try:
    from models.system_control.system_monitor import SystemMonitor, SystemMetric, SystemAlert, SystemStatus, AlertLevel
...
class SystemHealthManager:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        if SYSTEM_MONITOR_AVAILABLE:
            try:
                monitor_config = {
                    "monitoring_interval": self.config["monitoring_interval"] / 2,  # 更频繁的监控
    ...
    def _perform_health_check(self):
        ...
        for component, metrics in system_metrics.items():
            health_status, issue_info = self._evaluate_component_health(component, metrics)
            ...
            for metric_name, metric_value in metrics.items():
                ...
                # 限制历史大小
                if len(self.health_history[component]) > self.config["health_history_size"]:
                    self.health_history[component] = self.health_history[component][-self.config["health_history_size"]:]
    ...
    def _collect_system_metrics(self) -> Dict[str, Dict[str, float]]:
        ...
        if not self.system_monitor:
            raise RuntimeError("系统监控器未初始化。项目要求禁止使用虚拟数据，必须使用真实系统监控器。")
        ...
        try:
            ...
            for metric in current_metrics:
                component = metric.metric_id.split("_")[0] if "_" in metric.metric_id else "system"
            ...
            raise RuntimeError("无法收集系统指标。项目要求禁止使用虚拟数据，必须确保系统监控器正常工作。") from e
    ...
    def _collect_simulated_metrics(self) -> Dict[str, Dict[str, float]]:
        ...
        必须使用真实系统监控器获取指标数据。
        """
        raise RuntimeError("模拟指标收集已禁用。请确保系统监控器已正确初始化并可用。项目要求禁止使用虚拟数据，必须使用真实硬件和系统监控。")

    def _evaluate_component_health(self, component: str, metrics: Dict[str, float]) -> Tuple[HealthStatus, Dict[str, Any]]:
        """评估组件健康状态
    ...
    def _update_system_health(self, component_issues: List[Dict[str, Any]]):
        ...
        # 统计问题严重程度
        failed_components = [issue for issue in component_issues if issue["status"] == HealthStatus.FAILED]
        warning_components = [issue for issue in component_issues if issue["status"] == HealthStatus.WARNING]
        ...
        for callback in self.status_callbacks:
            try:
                callback(self.system_health_status, f"检测到{len(component_issues)}个组件问题")
            except Exception as e:
    ...
    def _check_and_repair(self):
        ...
        for component, issue_info in self.active_issues.items():
            ...
            elif status == HealthStatus.WARNING.value and self.config.get("auto_repair_warnings", False):
        ...
        for component, issue_info, issue_type in repair_candidates:
            ...
            if repair_needed:
                self.logger.info(f"检测到需要修复的问题: {component} - {issue_info.get('issue_info', {}).get('issue', '未知问题')}")
                self._perform_repair(component, issue_info)
                # 每次循环只修复一个问题，避免同时修复多个问题
                break

    def _should_repair(self, component: str, issue_info: Dict[str, Any], issue_type: str) -> bool:
        """判断是否应该执行修复
    ...
    def _perform_repair(self, component: str, issue_info: Dict[str, Any]):
        ...
        try:
            ...
            issue_description = issue_info.get("issue_info", {}).get("issue", "未知问题")
            ...
            repair_success = self._execute_repair_action(component, repair_action, issue_info)
            ...
            # 验证修复效果
            validation_score = self._validate_repair(component, metrics_before, metrics_after, issue_info)
            ...
            if len(self.repair_history) > self.config["repair_history_size"]:
                self.repair_history = self.repair_history[-self.config["repair_history_size"]:]
    ...
    def _determine_repair_action(self, component: str, issue_info: Dict[str, Any]) -> RepairAction:
        """确定修复动作
    ...
    def _execute_repair_action(self, component: str, action: RepairAction, issue_info: Dict[str, Any]) -> bool:
        """执行修复动作 - 完整实现
        ...
        try:
            ...
            if action == RepairAction.RESTART_COMPONENT:
                ...
                if component_type == "service":
                    ...
                    import subprocess
                    service_name = component_config.get("service_name", f"self-agi-{component}")
                    try:
                        ...
                        if result.returncode == 0:
                            ...
                            self.logger.error(f"服务 {service_name} 重启失败: {result.stderr}")
                            return False
                    except subprocess.TimeoutExpired:
                        self.logger.error(f"服务 {service_name} 重启超时")
                        return False
                    except FileNotFoundError:
                        self.logger.warning(f"systemctl不可用，尝试使用普通重启")
                        # 其他重启方式...
                    ...
                    if pid:
                        ...
                        try:
                            ...
                            if cmd:
                                ...
                                self.logger.info(f"进程 {component} (PID: {pid}) 重启成功")
                                return True
                ...
                if config_path and os.path.exists(config_path):
                    ...
                    if hasattr(self, 'config_manager'):
                        self.config_manager.update_component_config(component, new_config)
                        self.logger.info(f"组件 {component} 配置已更新")
                        return True
                    else:
                        self.logger.warning("配置管理器不可用")
                else:
                    self.logger.warning(f"组件 {component} 的配置文件不存在: {config_path}")
                ...
                if component_dir and os.path.exists(os.path.join(component_dir, ".git")):
                    import subprocess
                    try:
                        subprocess.run(["git", "-C", component_dir, "reset", "--hard", "HEAD"], 
                                     capture_output=True, text=True, timeout=30)
                        subprocess.run(["git", "-C", component_dir, "clean", "-fd"], 
                                     capture_output=True, text=True, timeout=30)
                ...
                if hasattr(self, 'optimizer'):
                    ...
                    success = self.optimizer.optimize_component(component, optimization_params)
                    if success:
                ...
                for temp_dir in temp_dirs:
                    if os.path.exists(temp_dir):
                        ...
                        try:
                            ...
                            self.logger.error(f"清理临时目录失败: {temp_dir}, 错误: {e}")
                ...
                for cache_dir in cache_dirs:
                    if os.path.exists(cache_dir):
                        try:
                            ...
                            self.logger.error(f"清理缓存目录失败: {cache_dir}, 错误: {e}")
                ...
                if self.self_correction_module_available:
                    try:
                        ...
                        # 调用自我改正模块进行学习
                        correction_result = self.self_correction_module.learn_and_adapt(learning_data)
                        
                        if correction_result.get("success", False):
                            self.logger.info(f"组件 {component} 学习并适应成功: {correction_result.get('message')}")
                            # 应用学习结果
                            adaptations = correction_result.get("adaptations", {})
                            for param, value in adaptations.items():
                                self._apply_parameter_adjustment(component, param, value)
                            return True
                        else:
                            self.logger.warning(f"组件 {component} 学习并适应失败: {correction_result.get('message')}")
                            return False
                    except Exception as e:
    ...
    def _validate_repair(self, component: str, metrics_before: Dict[str, float], 
                        metrics_after: Dict[str, float], issue_info: Dict[str, Any]) -> float:
        """验证修复效果
        ...
        if issue_metric and issue_metric in metrics_after:
            ...
            if "usage" in issue_metric.lower() or "temperature" in issue_metric.lower():
                # 使用率/温度类指标：越低越好
                improvement = max(0, issue_value - after_value) / issue_value if issue_value > 0 else 0
            else:
                # 其他指标：假设越高越好
                improvement = max(0, after_value - issue_value) / (100 - issue_value) if issue_value < 100 else 0
        ...
        for metric_name, before_value in metrics_before.items():
            if metric_name in metrics_after:
                ...
                # 判断是否改善
                if "usage" in metric_name.lower() or "temperature" in metric_name.lower():
                    # 使用率/温度类指标：降低为改善
                    ...
                    if abs(change) > 10:  # 显著变化
                        insights["insights"].append({
                            ...
                            "direction": "improvement" if (
                                ("usage" in metric_name.lower() and change < 0) or
                                ("performance" in metric_name.lower() and change > 0)
                            ) else "deterioration",
                        })
    ...
    def get_system_health_status(self) -> Dict[str, Any]:
        ...
        # 更新统计信息中的运行时间
        system_health["stats"]["system_uptime"] = time.time() - self.stats["system_uptime"]
        
        return system_health

    def get_health_history(self, component: Optional[str] = None, limit: int = 50) -> List[Dict[str, Any]]:
        """获取健康历史
    ...
    def get_repair_history(self, limit: int = 20) -> List[Dict[str, Any]]:
        ...
        返回:
            修复历史列表
        """
        history = self.repair_history[-limit:] if limit > 0 else self.repair_history.copy()
        return [record.to_dict() for record in history]

    def trigger_manual_repair(self, component: str, action: Optional[RepairAction] = None) -> bool:
        """触发手动修复
