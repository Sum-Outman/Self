...
class ServiceManager:
    ...
    def _parse_systemd_service_info(
        self, service_name: str, output: str
    ) -> ServiceInfo:
        """解析systemd服务信息"""
        lines = output.split("\n")
        info = {}
    ...
    def list_services(
        self, filter_by_status: Optional[ServiceStatus] = None
    ) -> List[ServiceInfo]:
        """列出所有服务"""
        services = []
            ...
            for line in output.split("\n"):
                ...
                if line.startswith("SERVICE_NAME:"):
                    ...
                    if current_service and "name" in current_service:
                        service_info = self._create_service_info_from_windows_output(
                            current_service
                        )
                        if service_info:
    ...
    def _create_service_info_from_windows_output(
                    ...
                    if len(parts) >= 5:
                        ...
                        if load_state == "loaded":
                            ...
                            result_enable = subprocess.run(
                                cmd_enable, capture_output=True, text=True
                            )
                            if (
                                result_enable.returncode == 0
                                and "enabled" in result_enable.stdout
                            ):
                                start_type = ServiceStartType.AUTOMATIC
    ...
    def _create_macos_launchd_task(self, task: ScheduledTask) -> bool:
        ...
        try:
            # 创建plist文件
            plist_content = f"""<?xml version="1.0" encoding="UTF-8"?> <!DOCTYPE plist PUBLIC "-//Apple//DTD PLIST 1.0//EN" "http://www.apple.com/DTDs/PropertyList-1.0.dtd"> <plist version="1.0"> <dict>     <key>Label</key>     <string>{task.name}</string>     <key>ProgramArguments</key>     <array>         <string>/bin/sh</string>         <string>-c</string>         <string>{task.command}</string>     </array>     <key>StartCalendarInterval</key>     <dict>         <key>Hour</key>         <integer>0</integer>         <key>Minute</key>         <integer>0</integer>     </dict>     <key>RunAtLoad</key>     <true/> </dict> </plist>"""
        ...
        try:
            ...
            for line in lines:
                ...
                if line.startswith("任务名:"):
                    if current_task:
                        task = self._create_scheduled_task_from_windows_output(
                            current_task
                        )
                        if task:
                            tasks.append(task)
    ...
    def _create_scheduled_task_from_windows_output(
        self, task_data: Dict[str, str]
    ...
    def start_service_monitoring(self, interval: float = 30.0):
        ...
        self.monitor_thread = threading.Thread(
            target=self._service_monitoring_loop, args=(interval,), daemon=True
        )
            ...
            try:
                ...
                for service_name in critical_services:
                    ...
                    if service_info and service_info.status != ServiceStatus.RUNNING:
                        logger.warning(
                            f"关键服务状态异常: {service_name} - {service_info.status}"
                        )
...
if __name__ == "__main__":
    ...
    for i, service in enumerate(services[:5]):
        print(
            f"{i +                 1}. {                 service.name}: {                 service.status.value} ({                 service.start_type.value})"
        )
