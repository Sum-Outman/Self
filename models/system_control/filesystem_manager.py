...
class FileSystemManager:
    ...
    def get_file_info(self, file_path: str) -> Optional[FileInfo]:
        ...
        try:
            ...
            checksum = ""
            if (
                file_type == FileType.FILE and stat_info.st_size < 100 * 1024 * 1024
            ):  # 小于100MB
                try:
    ...
    def read_file(
        self, file_path: str, binary: bool = False
    ...
    def write_file(
        self, file_path: str, content: Union[str, bytes], append: bool = False
    ) -> bool:
        """写入文件内容"""
        try:
            # 确保目录存在
            ...
            mode = (
                "ab"
                if append
                else "wb" if isinstance(content, bytes) else "a" if append else "w"
            )
    ...
    def search_files(
        ...
        try:
            ...
            if recursive:
                ...
                # 非递归遍历
                def walk_func(dir): return [(dir, [], os.listdir(dir))]
            ...
            for dirpath, dirnames, filenames in walk_func(root_dir):
                ...
                for filename in filenames:
                    if fnmatch.fnmatch(filename, pattern):
                        ...
                        if file_info and (
                            file_type is None or file_info.file_type == file_type
                        ):
    ...
    def _get_windows_disk_info(self, path: str) -> Optional[DiskInfo]:
        ...
        try:
            ...
            if "SSD" in path.upper() or "NVME" in path.upper():
                disk_type = (
                    DiskType.SSD if "NVME" not in path.upper() else DiskType.NVME
                )
            ...
            disk_info = DiskInfo(
                ...
                block_size=4096,  # 默认值
                read_only=(file_system_flags.value & 0x80000)
                != 0,  # FILE_READ_ONLY_VOLUME
        ...
        try:
            ...
            try:
                ...
                result = subprocess.run(
                    ["df", "-T", path], capture_output=True, text=True
                )
    ...
    def start_file_monitoring(
        self, path: str, handler: Callable[[FileSystemEvent], None]
    ) -> bool:
        """开始文件监控"""
        try:
            ...
            monitor_thread = threading.Thread(
                target=self._monitor_filesystem, args=(path,), daemon=True
            )
        ...
        try:
            ...
            while path in self.file_event_handlers:
                ...
                for file_path in current_state:
                    if (
                        file_path not in initial_state
                        and current_state[file_path]["exists"]
                    ):
                ...
                for file_path in initial_state:
                    if (
                        file_path not in current_state
                        or not current_state[file_path]["exists"]
                    ):
                        event = FileSystemEvent(
                ...
                for file_path in current_state:
                    if (
                        file_path in initial_state
                        and current_state[file_path]["exists"]
                        and initial_state[file_path]["exists"]
                    ):

                        if (
                            current_state[file_path]["mtime"]
                            != initial_state[file_path]["mtime"]
                            or current_state[file_path]["size"]
                            != initial_state[file_path]["size"]
                        ):
...
if __name__ == "__main__":
    ...
    if file_info:
        print(
            f"文件信息: 名称={                 file_info.name}, 大小={                 file_info.size} 字节, 类型={                 file_info.file_type.value}"
        )
    ...
    for i, disk in enumerate(disks[:3]):  # 只显示前3个
        usage_percent = (
            (disk.used_size / disk.total_size * 100) if disk.total_size > 0 else 0
        )
