...
class ProcessManager:
    ...
    def start_process(
        ...
        try:
            ...
            monitor_thread = threading.Thread(
                target=self._monitor_process, args=(pid, process), daemon=True
            )
            ...
            while True:
                ...
                if return_code is not None:
                    ...
                    try:
                        ...
                        if pid in self.processes:
                            self.processes[pid].metadata["stdout"] = (
                                stdout.decode("utf-8", errors="ignore")
                                if stdout
                                else ""
                            )
                            self.processes[pid].metadata["stderr"] = (
                ...
                try:
                    ...
                    with psutil_process.oneshot():
                        ...
                        if pid in self.processes:
                            ...
                            self.processes[pid].num_threads = (
                                psutil_process.num_threads()
                            )
            ...
            if limits.file_size_limit is not None:
                ...
                if platform.system() == "Linux":
                    ...
                    resource.prlimit(
                        ...
                        (limits.file_size_limit, limits.file_size_limit),
                    )
            ...
            if limits.open_files_limit is not None:
                if platform.system() == "Linux":
                    ...
                    resource.prlimit(
                        ...
                        (limits.open_files_limit, limits.open_files_limit),
                    )
        ...
        try:
            ...
            if limits.cpu_time_limit is not None:
                ...
                if total_cpu_time > limits.cpu_time_limit:
                    logger.warning(
                        f"进程 {pid} 超出CPU时间限制: {total_cpu_time} > {limits.cpu_time_limit}"
                    )
            ...
            if limits.memory_limit is not None:
                ...
                if memory_info.rss > limits.memory_limit:
                    logger.warning(
                        f"进程 {pid} 超出内存限制: {memory_info.rss} > {limits.memory_limit}"
                    )
    ...
    def _terminate_process(
        self, pid: int, reason: str = "", force: bool = False
    ) -> bool:
        """终止单个进程"""
        try:
            psutil_process = psutil.Process(pid)
    ...
    def get_process_info(self, pid: int) -> Optional[ProcessInfo]:
        ...
        try:
            if pid in self.processes:
                ...
                psutil_process = psutil.Process(pid)
                with psutil_process.oneshot():
                    self.processes[pid].cpu_percent = psutil_process.cpu_percent(
                        interval=0
                    )
                    memory_info = psutil_process.memory_info()
                ...
                with psutil_process.oneshot():
                    process_info = ProcessInfo(
                        ...
                        status=(
                            ...
                            if psutil_process.status() == psutil.STATUS_RUNNING
                            else ProcessStatus.SUSPENDED
                        ),
    ...
    def start_system_monitoring(self, interval: float = 5.0):
        ...
        self.monitor_thread = threading.Thread(
            target=self._system_monitoring_loop, args=(interval,), daemon=True
        )
    ...
    def cleanup(self):
        ...
        # 终止所有管理的进程
        for pid in list(self.processes.keys()):
            if self.processes[pid].status in [
                ProcessStatus.RUNNING,
...
if __name__ == "__main__":
    ...
    if pid:
        ...
        if proc_info:
            print(
                f"进程信息: 名称={                     proc_info.name}, CPU使用率={                     proc_info.cpu_percent}%, 内存使用率={                     proc_info.memory_percent}%"
            )
