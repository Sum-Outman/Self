...
class NAOqiRealController(RealHardwareInterface):
    ...
    def __init__(
        ...
        """
        super().__init__(
            hardware_type=HardwareType.ROBOT, interface_name=f"naoqi_{robot_id}"
        )
        ...
        if connection_type == NAOqiConnectionType.SIMULATED:
            raise ValueError(
                "模拟连接已禁用。项目要求禁止使用虚拟数据，必须使用真实NAOqi连接（DIRECT或PROXY）。"
            )
        ...
        with self._connection_lock:
            try:
                ...
                if self.connection_type == NAOqiConnectionType.SIMULATED:
                    # 模拟连接已禁用
                    raise ValueError(
                        "模拟连接已禁用。项目要求禁止使用虚拟数据，必须使用真实NAOqi连接。"
                    )
                ...
                try:
                    ...
                    # 连接到机器人
                    self._motion_proxy = ALProxy(
                        "ALMotion", self.robot_ip, self.robot_port
                    )
                    self._posture_proxy = ALProxy(
                        "ALRobotPosture", self.robot_ip, self.robot_port
                    )
                    self._memory_proxy = ALProxy(
                        "ALMemory", self.robot_ip, self.robot_port
                    )
                    self._tts_proxy = ALProxy(
                        "ALTextToSpeech", self.robot_ip, self.robot_port
                    )

                    # 尝试连接其他代理（可选）
                    try:
                        self._audio_proxy = ALProxy(
                            "ALAudioDevice", self.robot_ip, self.robot_port
                        )
                    except BaseException:
                        logger.warning("音频代理连接失败")

                    try:
                        self._video_proxy = ALProxy(
                            "ALVideoDevice", self.robot_ip, self.robot_port
                        )
                    except BaseException:
                        logger.warning("视频代理连接失败")

                    try:
                        self._sensor_proxy = ALProxy(
                            "ALSensors", self.robot_ip, self.robot_port
                        )
                    except BaseException:
                        logger.warning("传感器代理连接失败")

                    try:
                        self._autonomous_life_proxy = ALProxy(
                            "ALAutonomousLife", self.robot_ip, self.robot_port
                        )
                    except BaseException:
                    ...
                    raise ImportError(
                        "naoqi模块未安装。项目要求禁止使用模拟模式，必须安装NAOqi SDK。"
                    ) from e
    ...
    def get_joint_position(self, joint_name: Union[str, NAOqiJoint]) -> float:
        ...
        try:
            ...
            if not self._connected:
                # 未连接，无法获取关节位置
                raise RuntimeError(
                    "机器人未连接。项目要求禁止使用虚拟数据，必须先连接真实机器人。"
                )
            ...
            if self._motion_proxy:
                try:
                    ...
                    raise RuntimeError(
                        f"获取关节位置失败: {e}。项目要求禁止使用虚拟数据，必须使用真实硬件。"
                    ) from e

            # 没有可用的运动代理
            raise RuntimeError(
                "运动代理未初始化。项目要求禁止使用虚拟数据，必须确保机器人硬件连接正常。"
            )
    ...
    def set_joint_position(
        ...
        try:
            ...
            # 模拟模式已禁用
            raise RuntimeError(
                "模拟设置关节位置已禁用。项目要求禁止使用虚拟数据，必须使用真实机器人硬件。"
            )
    ...
    def get_sensor_data(
        self, sensor_type: Union[str, NAOqiSensorType]
    ) -> Dict[str, Any]:
        """获取传感器数据
        ...
        try:
            ...
            try:
                ...
                raise RuntimeError(
                    f"获取传感器数据失败: {e}。项目要求禁止使用虚拟数据，必须使用真实传感器硬件。"
                ) from e
        ...
        try:
            ...
            if sensor_type == NAOqiSensorType.BATTERY.value:
                ...
                if self._memory_proxy:
                    battery_level = self._memory_proxy.getData(
                ...
                # 获取角度数据
                if self._memory_proxy:
                    angle_x = self._memory_proxy.getData(
                        "Device/SubDeviceList/InertialSensor/AngleX/Sensor/Value"
                    )
                    angle_y = self._memory_proxy.getData(
                ...
                NAOqiSensorType.JOINT_TEMPERATURE.value,
    ...
    def _get_simulated_sensor_data(self, sensor_type: str) -> Dict[str, Any]:
        """获取模拟传感器数据（已禁用）"""
        raise RuntimeError(
            "模拟传感器数据获取已禁用。项目要求禁止使用虚拟数据，必须使用真实传感器硬件。"
        )
    ...
    def speak_text(self, text: str, language: str = "Chinese") -> bool:
        ...
        try:
            ...
            # 模拟模式已禁用
            raise RuntimeError(
                "模拟说话已禁用。项目要求禁止使用虚拟数据，必须使用真实机器人硬件。"
            )
    ...
    def perform_posture(self, posture_name: str) -> bool:
        ...
        try:
            ...
            # 模拟模式已禁用
            raise RuntimeError(
                "模拟执行姿势已禁用。项目要求禁止使用虚拟数据，必须使用真实机器人硬件。"
            )
    ...
    def enable_autonomous_life(self, enable: bool = True) -> bool:
        ...
        try:
            ...
            # 模拟模式已禁用
            raise RuntimeError(
                "模拟自主生命已禁用。项目要求禁止使用虚拟数据，必须使用真实机器人硬件。"
            )
    ...
    def emergency_stop(self) -> bool:
        ...
        try:
            ...
            # 模拟模式已禁用
            raise RuntimeError(
                "模拟紧急停止已禁用。项目要求禁止使用虚拟数据，必须使用真实机器人硬件。"
            )
