...
class SensorData:
    ...
    def __init__(self, sensor_type: SensorType, value: Any, 
                 unit: str = "", timestamp: float = None, 
                 accuracy: float = 1.0, metadata: Dict[str, Any] = None):
        """
        初始化传感器数据
...
class RealSensorInterface(RealHardwareInterface):
    ...
    def __init__(
        ...
        super().__init__(
            ...
            interface_name=f"sensor_{sensor_id}"
        )
        ...
        self.calibration_offset = interface_config.get("calibration_offset", 0.0)  # 校准偏移
        self.calibration_scale = interface_config.get("calibration_scale", 1.0)    # 校准比例
        ...
        with self._interface_lock:
            try:
                if self.sensor_interface == SensorInterface.I2C:
                    ...
                    raise HardwareError(f"不支持的传感器接口类型: {self.sensor_interface}")
    ...
    def _init_analog_interface(self):
        ...
        try:
            ...
            raise ImportError("adafruit_ads1x15未安装。项目要求禁止使用模拟模式，必须安装硬件库。") from e
        except Exception as e:
            logger.error(f"模拟传感器接口初始化失败: {e}")
            raise RuntimeError(f"模拟传感器接口初始化失败: {e}。项目要求禁止使用模拟模式，必须确保模拟传感器硬件可用。") from e
    ...
    def _init_digital_interface(self):
        ...
        try:
            ...
            try:
                ...
                self._digital_device = gpiozero.Button(
                    pin, pull_up=pull_up
                )
                ...
                raise ImportError("GPIO库（RPi.GPIO或gpiozero）未安装。项目要求禁止使用模拟模式，必须安装硬件库。")
    ...
    def _init_usb_interface(self):
        ...
        try:
            ...
            logger.info(f"USB传感器接口配置: VID=0x{vendor_id:04x}, PID=0x{product_id:04x}")
    ...
    def _connect_bluetooth(self) -> bool:
        ...
        try:
            ...
            self._bluetooth_socket.connect((self._bluetooth_address, self._bluetooth_port))
            return True
    ...
    def is_connected(self) -> bool:
        ...
        try:
            if self.sensor_interface == SensorInterface.SERIAL:
                return hasattr(self, '_serial') and self._serial and self._serial.is_open
                ...
                return hasattr(self, '_bluetooth_socket') and self._bluetooth_socket is not None
    ...
    def calibrate(self, reference_value: float = None) -> bool:
        ...
        try:
            ...
            logger.info(f"传感器校准完成: offset={self.calibration_offset}, scale={self.calibration_scale}")
            return True
    ...
    def _read_analog_value(self) -> float:
        ...
        try:
            if self._analog_channel == "simulated":
                # 模拟模式已禁用
                raise RuntimeError("模拟传感器模式已禁用。项目要求禁止使用虚拟数据，必须使用真实硬件。")
            else:
                # 实际硬件读取
    ...
    def _read_digital_value(self) -> bool:
        ...
        try:
            if self._gpio_library == "RPi.GPIO":
                ...
                # 模拟模式已禁用
                raise RuntimeError("模拟数字传感器模式已禁用。项目要求禁止使用虚拟数据，必须使用真实硬件。")
            else:
    ...
    def _read_simulated_value(self) -> Any:
        """读取模拟传感器值（已禁用）"""
        raise RuntimeError("模拟传感器值读取已禁用。项目要求禁止使用虚拟数据，必须使用真实硬件传感器。")
    ...
    def _sampling_loop(self):
        ...
        while self._sampling_running and self.is_connected():
            try:
                # 计算采样间隔
                sample_interval = 1.0 / self.sampling_rate if self.sampling_rate > 0 else 1.0
