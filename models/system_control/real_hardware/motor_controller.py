...
class RealMotorController(RealHardwareInterface):
    ...
    def __init__(
        ...
        """
        super().__init__(
            hardware_type=HardwareType.MOTOR, interface_name=f"motor_{motor_id}"
        )
        ...
        with self._interface_lock:
            try:
                if self.control_interface == ControlInterface.PWM:
                    ...
                    raise ValueError(
                        f"不支持的接口类型: {self.control_interface}。项目要求禁止使用模拟模式，必须使用真实硬件接口。"
                    )
                ...
                raise RuntimeError(
                    "硬件接口初始化失败。项目要求禁止使用模拟模式，必须确保硬件接口正常工作。"
                ) from e
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    # 如果没有硬件库，抛出异常
                    raise ImportError(
                        "未找到任何PWM硬件库（RPi.GPIO、gpiozero、Adafruit_PCA9685）。项目要求禁止使用模拟模式，必须安装硬件库。"
                    )
        ...
        try:
            if self._pwm_library == "RPi.GPIO":
                ...
                self._pwm = gpiozero.PWMOutputDevice(
                    self.pwm_pin, frequency=self.pwm_frequency
                )
                self._pwm.value = 0
            ...
            raise RuntimeError(
                f"PWM硬件初始化失败: {e}。项目要求禁止使用模拟模式，必须确保PWM硬件正常工作。"
            ) from e
    ...
    def _init_serial_interface(self):
        ...
        try:
            ...
            raise RuntimeError(
                f"串口连接失败: {e}。项目要求禁止使用模拟模式，必须确保串口硬件可用。"
            ) from e
    ...
    def _init_i2c_interface(self):
        ...
        try:
            ...
            raise RuntimeError(
                f"I2C连接失败: {e}。项目要求禁止使用模拟模式，必须确保I2C硬件可用。"
            ) from e
    ...
    def _init_spi_interface(self):
        ...
        try:
            ...
            raise RuntimeError(
                f"SPI连接失败: {e}。项目要求禁止使用模拟模式，必须确保SPI硬件可用。"
            ) from e
    ...
    def _init_gpio_interface(self):
        ...
        try:
            ...
            try:
                ...
                raise ImportError(
                    "未找到GPIO库（gpiozero）。项目要求禁止使用模拟模式，必须安装硬件库。"
                )
    ...
    def _move_servo_to_position(self, position: float) -> bool:
        ...
        try:
            ...
            duty_cycle = self.pwm_min_duty + (position / 180.0) * (
                self.pwm_max_duty - self.pwm_min_duty
            )
    ...
    def _move_stepper_to_position(self, position: float) -> bool:
        ...
        try:
            ...
            if self.control_interface == ControlInterface.GPIO:
                ...
                logger.warning(
                    f"控制接口 {self.control_interface.value} 不支持步进电机移动"
                )
                return False
    ...
    def set_direction(self, direction: int) -> bool:
        ...
        if self.control_interface == ControlInterface.GPIO:
            if self._gpio_library == "RPi.GPIO":
                ...
                # 模拟模式已禁用
                raise RuntimeError(
                    "模拟GPIO模式已禁用。项目要求禁止使用虚拟数据，必须使用真实硬件。"
                )
            else:
                raise ValueError(
                    f"未知的GPIO库: {self._gpio_library}。项目要求禁止使用模拟模式，必须使用真实硬件库。"
                )
