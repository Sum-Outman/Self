...
def install_system_packages(platform_info):
    ...
    if system == "linux":
        if platform_info["is_raspberry_pi"]:
            ...
            for pkg in packages:
                ...
                success, stdout, stderr = run_command(
                    f"sudo apt-get install -y {pkg} 2>/dev/null || echo '跳过 {pkg}'"
                )
        ...
        if not brew_installed:
            ...
            print(
                '  建议安装Homebrew: /bin/bash -c "$(curl -fsSL https://raw.githubusercontent.com/Homebrew/install/HEAD/install.sh)"'
            )
            return
    ...
    if platform_info["is_raspberry_pi"]:
        ...
        if current_user:
            for group in groups:
                ...
                success, stdout, stderr = run_command(
                    f"sudo usermod -a -G {group} {current_user}"
                )
                print(f"  {'✓' if success else '✗'}")
        ...
        success, stdout, stderr = run_command(
            f"echo '{udev_rule}' | sudo tee /etc/udev/rules.d/99-usb-permissions.rules"
        )
        if success:
...
def print_summary(platform_info, installation_ok):
    ...
    if installation_ok:
        ...
        print(
            "  2. 运行测试: python -m models.system_control.real_hardware.test_real_hardware"
        )
        print(
            "  3. 参考文档: models/system_control/real_hardware/hardware_dependencies.md"
        )
    else:
        print("\n问题解决:")
        print("  1. 检查网络连接")
