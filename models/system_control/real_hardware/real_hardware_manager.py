...
from .base_interface import RealHardwareInterface, HardwareType
...
class RealHardwareManager:
    ...
    def register_interface_type(
        self, hardware_type: HardwareType, interface_class: Type[RealHardwareInterface]
    ):
        """注册新的接口类型"""
        if not issubclass(interface_class, RealHardwareInterface):
            raise TypeError("接口类必须继承自RealHardwareInterface")

        self.interface_types[hardware_type] = interface_class
        self.logger.info(
            f"注册接口类型: {hardware_type.value} -> {interface_class.__name__}"
        )

    def create_interface_from_device(
        self, device: HardwareDevice
    ) -> Optional[RealHardwareInterface]:
        """
        从硬件设备创建真实硬件接口
        ...
        try:
            ...
            if hardware_type not in self.interface_types:
                self.logger.warning(
                    f"不支持此设备类型的接口: {device.device_type.value}"
                )
                return None  # 返回None
            ...
            if interface:
                ...
                self.logger.info(
                    f"为设备创建接口成功: {device.name} ({device.device_id})"
                )
    ...
    def _map_device_type_to_hardware_type(
    ...
    def _create_interface_config(
        self, device: HardwareDevice, hardware_type: HardwareType
    ) -> Dict[str, Any]:
        """创建接口配置"""
        config = {
    ...
    def _create_interface_instance(
        self,
        ...
        try:
            if hardware_type == HardwareType.MOTOR:
                ...
                control_interface = interface_config.get(
    ...
    def _update_device_status_from_interface(self, interface: RealHardwareInterface):
        ...
        # 更新设备状态
        self.hardware_manager.update_device_status(device_id, new_status, error_message)
