...
class ImportanceModel(nn.Module):
    ...
    def compute_features(self, memory_data: Dict[str, Any]) -> torch.Tensor:
        ...
        max_count = 100
        access_feature = torch.log(
            torch.tensor(1.0 + min(accessed_count, max_count))
        ) / torch.log(torch.tensor(1.0 + max_count))
...
class MemoryGraphNeuralNetwork(nn.Module):
    ...
    def __init__(self, feature_dim: int, hidden_dim: int = 64, num_layers: int = 2):
        ...
        logger.info(
            f"初始化MemoryGraphNeuralNetwork: 特征维度={feature_dim}, 隐藏维度={hidden_dim}, 层数={num_layers}"
        )
    ...
    def forward(
        ...
        if num_nodes > 1:
            ...
            if edge_features:
                ...
                # 重构为矩阵形式
                edge_predictions = torch.zeros(
                    num_nodes, num_nodes, device=node_features.device
                )
                idx = 0
                for i in range(num_nodes):
                    for j in range(i + 1, num_nodes):
    ...
    def predict_edge_strength(
        self, node_embeddings: torch.Tensor, node_i: int, node_j: int
    ) -> float:
        """预测两个节点之间的边强度
    ...
    def detect_communities(
        self, node_embeddings: torch.Tensor, threshold: float = 0.7
    ) -> List[List[int]]:
        """检测社区/聚类
...
class AutonomousMemoryManager:
    ...
    def _calculate_performance_score(self) -> float:
        ...
        memory_score = (
            1.0 - self.system_state["memory_usage"] * 0.5
        )  # 内存使用越高，分数越低
        ...
        for param_name in self.adaptive_params:
            if param_name in policy:
                ...
                weighted_value = (
                    ...
                    + user_policy.get(param_name, self.adaptive_params[param_name])
                    * user_weight
                )
                ...
                new_value = (
                    current_value * (1 - learning_rate) + weighted_value * learning_rate
                )
    ...
    def record_decision(
        self, decision_type: str, context: Dict[str, Any], outcome: Dict[str, Any]
    ):
        """记录记忆管理决策
        ...
        if len(self.meta_memory["decision_history"]) > max_history:
            self.meta_memory["decision_history"] = self.meta_memory["decision_history"][
                -max_history:
            ]
    ...
    def _update_strategy_effectiveness(
        self, decision_type: str, outcome: Dict[str, Any]
    ):
        """更新策略有效性评估"""
    ...
    def _learn_from_decision(self, decision_record: Dict[str, Any]):
        ...
        try:
            ...
            logger.debug(
                f"Q-learning更新: 状态={state_key}, 动作={action_key}, 奖励={reward}, Q值={new_q}"
            )

        except Exception as e:
            logger.error(
                f"从决策学习失败: {e}, 上下文: 状态={state_key}, 动作={action_key}, 奖励={reward}, Q表大小={                     len(                         self.q_table)}"
            )
            # 恢复措施：重置Q表条目为初始值
    ...
    def _get_state_key(self, system_state: Dict[str, Any]) -> str:
        ...
        load_category = (
            "low" if load_level < 0.3 else "medium" if load_level < 0.7 else "high"
        )
        memory_category = (
            "low" if memory_usage < 0.3 else "medium" if memory_usage < 0.7 else "high"
        )
    ...
    def _get_action_key(self, context: Dict[str, Any]) -> str:
        ...
        if decision_type == "forget":
            ...
            rate_category = (
                "low"
                if forgetting_rate < 0.03
                else "medium" if forgetting_rate < 0.07 else "high"
            )
            ...
            ratio_category = (
                ...
                else "medium" if compression_ratio < 0.7 else "high"
            )
    ...
    def get_optimal_action(self, state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if state_key in self.q_table and self.q_table[state_key]:
            # 选择最高Q值的动作
            best_action_key = max(self.q_table[state_key].items(), key=lambda x: x[1])[
                0
            ]
...
class MemorySystem:
    ...
    def __init__(
        ...
        agi_model: Optional[nn.Module] = None,
        ...
        # 多模态支持配置
        self.enable_multimodal_memory = (
            self.config.get("enable_multimodal_memory", True) and MULTIMODAL_AVAILABLE
        )
        if self.enable_multimodal_memory and self.multimodal_processor is None:
            ...
            try:
                ...
                logger.warning(
                    "多模态处理器初始化失败，已禁用多模态记忆功能。请检查依赖项是否安装完整。"
                )
        ...
        self.embedding_dim = self.config.get(
            "embedding_dim", 768
        )  # 嵌入维度（增加到768以匹配Transformer hidden_size）
        ...
        if self.embedding_model is None and self.agi_model is not None:
            try:
                ...
                logger.error(
                    f"创建AGI文本嵌入器失败: {e}, AGI模型类型: {                         type(                             self.agi_model).__name__ if self.agi_model else 'None'}"
                )
        ...
        if self.embedding_model is not None:
            ...
            elif hasattr(self.embedding_model, "config") and hasattr(
        ...
        self.working_memory_capacity = self.config.get(
            "working_memory_capacity", 100
        )  # 工作记忆容量增加到100
        self.rag_top_k = self.config.get("rag_top_k", 10)  # RAG检索结果增加到10
        self.enable_importance_learning = self.config.get(
        ...
        self.faiss_index_type = self.config.get(
            "faiss_index_type", "IVF1024,PQ64"
        )  # FAISS索引类型
        self.hnsw_m = self.config.get("hnsw_m", 32)  # HNSW M参数
        self.hnsw_ef_construction = self.config.get(
            "hnsw_ef_construction", 200
        )  # HNSW构建参数
        self.hnsw_ef_search = self.config.get("hnsw_ef_search", 100)  # HNSW搜索参数
        self.faiss_gpu_enabled = self.config.get(
            "faiss_gpu_enabled", False
        )  # 是否启用GPU加速
        self.index_rebuild_threshold = self.config.get(
            "index_rebuild_threshold", 1000
        )  # 索引重建阈值
        self.index_rebuild_interval_hours = self.config.get(
        ...
        # 动态关联图增强配置
        self.enable_gnn_learning = self.config.get(
            "enable_gnn_learning", True
        )  # 是否启用图神经网络学习
        self.enable_community_detection = self.config.get(
            "enable_community_detection", True
        )  # 是否启用社区检测
        self.enable_temporal_dynamics = self.config.get(
        ...
        self.community_min_size = self.config.get(
        ...
        self.graph_update_frequency = self.config.get(
            "graph_update_frequency", 100
        )  # 图更新频率（每N个记忆）

        # 知识库集成配置
        self.enable_knowledge_base = (
            self.config.get("enable_knowledge_base", True) and KNOWLEDGE_BASE_AVAILABLE
        )  # 是否启用知识库集成
        self.knowledge_base_config = self.config.get(
        ...
        # 第四阶段：自主记忆管理配置
        self.enable_autonomous_memory = self.config.get(
            "enable_autonomous_memory", True
        )  # 是否启用自主记忆管理
        if self.enable_autonomous_memory:
            # 创建自主记忆管理器
            autonomous_config = {
                "enable_autonomous_learning": self.config.get(
        ...
        if self.enable_knowledge_base:
            try:
                self.knowledge_manager = KnowledgeManager(
                    self.knowledge_base_config, self
                )
        ...
        self.short_term_cache_max_size = self.config.get(
            "short_term_cache_max_size", 200
        )  # 缓存最大容量
        ...
        if self.enable_gnn_learning:
            ...
            logger.info(
                f"图神经网络模型已初始化: 特征维度={gnn_feature_dim}, 隐藏维度={self.gnn_hidden_dim}"
            )
        else:
            self.gnn_model = None
    ...
    def update_autonomous_system_state(self, metrics: Optional[Dict[str, Any]] = None):
        ...
        logger.debug(
            f"自主记忆管理器系统状态已更新: {self.autonomous_manager.system_state}"
        )
    ...
    def record_memory_decision(
        ...
        logger.debug(
            f"记忆决策已记录: {decision_type}, 奖励={outcome.get('reward', 0.0)}"
        )
        ...
        try:
            ...
            if self.embedding_model is not None:
                ...
                if not hasattr(self.embedding_model, "encode"):
                    logger.warning(
                        f"嵌入模型 {model_type} 缺少encode方法，尝试使用forward方法"
                    )
                    # 为简单模型添加encode方法适配
                ...
                self.embedding_model = FromScratchTextEmbedder(
                    embedding_dim=self.embedding_dim, vocab_size=10000, max_length=512
                )
            ...
            logger.error(
                f"记忆系统初始化失败: {e}, 配置: 多模态={                     self.enable_multimodal_memory}, 知识库={                     self.enable_knowledge_base}, FAISS={                     self.enable_faiss_retrieval}, 图={                     self.enable_memory_graph}"
            )
            logger.error(
                "初始化失败，请检查数据库连接和依赖项。系统将无法提供记忆服务。"
            )
            raise
    ...
    def _initialize_faiss_index(self, db: Optional[Session] = None):
        ...
        try:
            ...
            try:
                ...
                logger.warning(
                    "FAISS库未安装，将回退到线性检索。请安装: pip install faiss-cpu 或 faiss-gpu"
                )
            ...
            try:
                ...
                logger.warning(
                    "HNSW库未安装，将仅使用FAISS或线性检索。请安装: pip install hnswlib"
                )
            ...
            self.faiss_index_save_path = self.config.get(
                "faiss_index_save_path", "./data/faiss_index.bin"
            )
            self.faiss_mapping_save_path = self.config.get(
                "faiss_mapping_save_path", "./data/faiss_index_mapping.json"
            )
            ...
            if (
                faiss_available
                and os.path.exists(self.faiss_index_save_path)
                and os.path.exists(self.faiss_mapping_save_path)
            ):
                try:
                    ...
                    if self.faiss_index.ntotal == len(self.faiss_index_mapping):
                        ...
                        index_loaded = True
                        logger.info(
                            f"FAISS索引加载成功: 向量数量={                                 self.faiss_index.ntotal}, 映射大小={                                 len(                                     self.faiss_index_mapping)}"
                        )
                    else:
                        logger.warning(
                            f"索引与映射大小不一致: 索引={                                 self.faiss_index.ntotal}, 映射={                                 len(                                     self.faiss_index_mapping)}，将重建索引"
                        )
                        index_loaded = False
                except Exception as e:
            ...
            self.index_initialized = True
            logger.info(
                f"FAISS/HNSW索引初始化完成: FAISS={faiss_available}, HNSW={hnsw_available}, 已加载={index_loaded}"
            )
    ...
    def _build_memory_graph(self, db: Session, force_rebuild: bool = False):
        ...
        try:
            ...
            # 获取所有长期记忆
            memories = db.query(Memory).filter(Memory.memory_type == "long_term").all()
            ...
            if (
                not force_rebuild
                and self.graph_initialized
                and len(memories) <= len(self.memory_to_node_map) * 1.5
            ):
                # 如果记忆数量变化不大，跳过重建
                logger.info(
                    f"图已存在且记忆数量变化不大（图节点: {                         len(                             self.memory_to_node_map)}, 当前记忆: {                         len(memories)}），跳过重建"
                )
                return
            ...
            for idx, memory in enumerate(memories):
                ...
                try:
                    ...
                    if len(embedding) != self.embedding_dim:
                        logger.warning(
                            f"记忆 {memory_id} 的嵌入维度不匹配: 期望 {                                 self.embedding_dim}, 实际 {                                 len(embedding)}"
                        )
                        # 使用零向量填充
            ...
            for association in associations:
                ...
                if (
                    source_id in self.memory_to_node_map
                    and target_id in self.memory_to_node_map
                ):
            ...
            if self.enable_gnn_learning and self.gnn_model is not None:
                with torch.no_grad():
                    ...
                    self.memory_graph = {
                        ...
                        "community_embeddings": graph_output.get(
                    ...
                    if (
                        ...
                        and graph_output["node_embeddings"] is not None
                    ):
                        communities = self.gnn_model.detect_communities(
                            graph_output["node_embeddings"], threshold=0.7
                        )
            ...
            logger.info(
                f"记忆关联图构建完成: 节点数={num_nodes}, 边数={np.count_nonzero(adjacency)}"
            )
    ...
    def _build_initial_adjacency(
        self, node_features: List[List[float]], memory_ids: List[int], db: Session
    ) -> np.ndarray:
        ...
        try:
            ...
            for mem in memories:
                memory_times[mem.id] = (
                    mem.created_at.timestamp() if mem.created_at else 0
                )
            ...
            for j in range(i + 1, num_nodes):
                ...
                if norm_i > 0 and norm_j > 0:
                    ...
                    if memory_ids[i] in memory_times and memory_ids[j] in memory_times:
                        time_diff = abs(
                            memory_times[memory_ids[i]] - memory_times[memory_ids[j]]
                        )
                        # 时间差越小，相似度越高（指数衰减）
                    ...
                    combined_weight = (
                        0.6 * semantic_sim + 0.25 * time_sim + 0.15 * type_sim
                    )

                    # 只添加强关联
                    if (
                        combined_weight > self.similarity_threshold * 0.6
                    ):  # 降低阈值以获取更多连接
    ...
    def _update_memory_graph_with_new_memory(
        self, memory_id: int, embedding: List[float], db: Session
    ):
        """使用新记忆更新图
    ...
    def _add_node_to_graph(self, memory_id: int, embedding: List[float], db: Session):
        ...
        new_node_features = torch.zeros(
            (new_idx + 1, node_features.shape[1]), dtype=node_features.dtype
        )
        if new_idx > 0:
            new_node_features[:new_idx] = node_features
        new_node_features[new_idx] = torch.tensor(
            feature_vector, dtype=node_features.dtype
        )
    ...
    def _build_faiss_index(self, db: Session, force_rebuild: bool = False):
        ...
        try:
            ...
            if (
                not force_rebuild
                and self.index_initialized
                and self.index_size > 0
                and len(memories) <= self.index_size * 1.5
            ):
                # 如果记忆数量变化不大，跳过重建
                logger.info(
                    f"索引已存在且记忆数量变化不大（索引大小: {                         self.index_size}, 当前记忆: {                         len(memories)}），跳过重建"
                )
                return
            ...
            for memory in memories:
                try:
                    ...
                    if len(embedding) == self.embedding_dim:
                        ...
                        logger.warning(
                            f"记忆 {                                 memory.id} 的嵌入维度不匹配: 期望 {                                 self.embedding_dim}, 实际 {                                 len(embedding)}"
                        )
                except Exception as e:
            ...
            if self.faiss_available:
                ...
                if self.faiss_index_type == "Flat":
                    ...
                    if len(embeddings_np) >= nlist * 10:  # 确保有足够的数据训练
                        ...
                        logger.warning("数据量不足训练IVF索引，使用平面索引替代")
                ...
                logger.info(
                    f"FAISS索引构建完成: 类型={                         self.faiss_index_type}, 向量数量={                         len(embeddings_np)}"
                )
            ...
            if self.hnsw_available and not self.faiss_available:
                ...
                logger.info(
                    f"HNSW索引构建完成: M={                         self.hnsw_m}, ef_construction={                         self.hnsw_ef_construction}, 向量数量={                         len(embeddings_np)}"
                )
            ...
            if self.faiss_available and self.faiss_index is not None:
                try:
                    ...
                    # 确保保存目录存在
                    os.makedirs(
                        os.path.dirname(self.faiss_index_save_path), exist_ok=True
                    )
                    os.makedirs(
                        os.path.dirname(self.faiss_mapping_save_path), exist_ok=True
                    )
                    ...
                    with open(self.faiss_mapping_save_path, "w", encoding="utf-8") as f:
                        json.dump(
                            self.faiss_index_mapping, f, ensure_ascii=False, indent=2
                        )

                    logger.info(
                        f"FAISS索引已保存: 索引文件={                             self.faiss_index_save_path}, 映射文件={                             self.faiss_mapping_save_path}"
                    )
                except Exception as e:
                    logger.error(f"保存FAISS索引失败: {e}")

            logger.info(
                f"向量索引构建完成: 总记忆数={len(memories)}, 有效嵌入数={len(embeddings_np)}"
            )
    ...
    def _update_faiss_index(
        self, memory_id: int, embedding: List[float], action: str = "add"
    ):
        """更新FAISS/HNSW索引
        ...
        """
        if not self.index_initialized or (
            self.faiss_index is None and self.hnsw_index is None
        ):
            return
        ...
        try:
            ...
            if action == "add":
                ...
                if self.faiss_index is not None:
                    ...
                    try:
                        ...
                        if hasattr(self, "faiss_index_save_path"):
                            os.makedirs(
                                os.path.dirname(self.faiss_index_save_path),
                                exist_ok=True,
                            )
                            os.makedirs(
                                os.path.dirname(self.faiss_mapping_save_path),
                                exist_ok=True,
                            )

                            # 保存FAISS索引
                            faiss.write_index(
                                self.faiss_index, self.faiss_index_save_path
                            )

                            # 保存内存ID映射
                            if hasattr(self, "faiss_index_mapping"):
                                with open(
                                    self.faiss_mapping_save_path, "w", encoding="utf-8"
                                ) as f:
                                    json.dump(
                            ...
                            logger.debug(
                                f"FAISS索引已更新并保存: 记忆ID={memory_id}, 索引大小={                                     self.index_size}"
                            )
                ...
                if memory_id in self.id_to_index_map:
                    ...
                    # 3. 更新FAISS索引映射（如果存在）
                    if (
                        hasattr(self, "faiss_index_mapping")
                        and idx_to_remove in self.faiss_index_mapping
                    ):
                    ...
                    if self._pending_rebuild_count >= self.index_rebuild_threshold:
                        logger.info(
                            f"达到索引重建阈值（{                                 self._pending_rebuild_count} >= {                                 self.index_rebuild_threshold}），标记为急需重建"
                        )
                        # 设置紧急重建标志
                        self.index_needs_urgent_rebuild = True

                    logger.debug(
                        f"记忆 {memory_id} 已从映射中移除，索引标记为需要重建（待重建计数: {                             self._pending_rebuild_count}）"
                    )
    ...
    def _linear_search_with_embedding(
        self, db: Session, query_builder, query_embedding: List[float], limit: int
            ...
            def encode_adapter(text: str, convert_to_tensor: bool = False):
                ...
                with torch.no_grad():
                    ...
                    if hasattr(self.embedding_model, "forward"):
                        ...
                        raise RuntimeError(
                            f"嵌入模型 {                                 self.embedding_model.__class__.__name__} 既没有encode方法也没有forward方法"
                        )

            # 将适配器绑定到嵌入模型
            self.embedding_model.encode = encode_adapter.__get__(
                self.embedding_model, self.embedding_model.__class__
            )
    ...
    def set_embedding_model(self, embedding_model: nn.Module):
        ...
        logger.info(
            f"嵌入模型已设置: {embedding_model.__class__.__name__}, 嵌入维度: {self.embedding_dim}"
        )
    ...
    def _get_embedding(
        ...
        # 多模态内容：使用多模态处理器
            ...
            try:
                ...
                if content_type == "image":
                    ...
                    if (
                        ...
                        # 假设是文件路径
                        processed_result = self.multimodal_processor.process_image(
                            image_path=content
                        )
                    else:
                        # 假设是base64
                    ...
                    if (
                        ...
                        processed_result = self.multimodal_processor.process_audio(
                            audio_path=content
                        )
                    else:
                        processed_result = self.multimodal_processor.process_audio(
                    ...
                    if (
                        ...
                        processed_result = self.multimodal_processor.process_video(
                            video_path=content
                        )
                    else:
                        processed_result = self.multimodal_processor.process_video(
                            video_base64=content
                        )
                    ...
                    if self.embedding_model:
                        ...
                        raise RuntimeError(
                            f"不支持的内容类型: {content_type}，且文本嵌入模型不可用"
                        )
                ...
                if self.embedding_model:
                    ...
                    try:
                        processed_result = self.multimodal_processor.process_text(
                            content
                        )
                        if processed_result and hasattr(processed_result, "embeddings"):
                            return processed_result.embeddings
            ...
            if self.embedding_model:
                ...
                logger.warning(
                    f"多模态不可用但处理器可用，尝试将内容类型 {content_type} 作为文本处理"
                )
                try:
                    processed_result = self.multimodal_processor.process_text(content)
                    if processed_result and hasattr(processed_result, "embeddings"):
                    ...
                    raise RuntimeError(
                        f"不支持的内容类型: {content_type}，且文本嵌入失败: {e}"
                    )
            else:
                raise RuntimeError(
                    f"不支持的内容类型: {content_type}，且多模态和文本嵌入均不可用"
                )
    ...
    def store_memory(
        ...
        if (
            ...
            and self.enable_faiss_retrieval
            and self.index_initialized
        ):
    ...
    def store_image_memory(
        ...
        if description:
            # 可以将描述与图像关联存储
            content = (
                f"图像: {description}\n数据: {image_content[:100]}..."
                if len(image_content) > 100
                else f"图像: {description}"
            )
    ...
    def store_audio_memory(
        ...
        if transcription:
            content = (
                f"音频转录: {transcription}\n数据: {audio_content[:100]}..."
                if len(audio_content) > 100
                else f"音频: {transcription}"
            )
    ...
    def _search_working_memory(
        ...
        for item in self.working_memory:
            ...
            final_score = score * (
                0.7 + 0.3 * importance_score
            )  # 70%相似度 + 30%重要性

            if final_score > 0.1:  # 只保留相关结果
                scored_items.append(
                    (final_score, item.copy())
                )  # 使用副本避免修改原始数据
    ...
    def get_cache_stats(self) -> Dict[str, Any]:
        ...
        if self.faiss_search_count > 0:
            faiss_avg_time = (
                self.faiss_search_time_total / self.faiss_search_count * 1000
            )  # 转换为毫秒
        ...
        if self.hnsw_search_count > 0:
            hnsw_avg_time = (
                self.hnsw_search_time_total / self.hnsw_search_count * 1000
            )  # 转换为毫秒
        ...
        if self.linear_search_count > 0:
            linear_avg_time = (
                self.linear_search_time_total / self.linear_search_count * 1000
            )  # 转换为毫秒
        ...
        return {
            ...
            "hierarchical_performance_summary": {
                ...
                "layer3_long_term_index": (
                    f"{self.index_size} items in index"
    ...
    def warmup_short_term_cache(
        ...
        try:
            ...
            # 按最近访问和重要性排序
            memories = (
                query.order_by(Memory.last_accessed.desc(), Memory.importance.desc())
                .limit(limit)
                .all()
            )
    ...
    def retrieve_memory(
        ...
        # 1. 检查是否需要紧急重建（达到重建阈值）
        if (
            hasattr(self, "index_needs_urgent_rebuild")
            and self.index_needs_urgent_rebuild
        ):
            # 紧急重建：达到删除阈值，立即重建
            pending_count = getattr(self, "_pending_rebuild_count", 0)
            logger.info(
                f"检测到索引急需重建（待重建计数: {pending_count}），立即触发强制重建"
            )
        ...
        if hasattr(self, "index_needs_rebuild") and self.index_needs_rebuild:
            ...
            pending_count = getattr(self, "_pending_rebuild_count", 0)
            logger.info(
                f"检测到索引需要重建，触发索引重建（待重建计数: {pending_count}）"
            )
            self._build_faiss_index(db, force_rebuild=True)
            self.index_needs_rebuild = False
            if hasattr(self, "_pending_rebuild_count"):
        ...
        if self.index_last_updated is not None:
            hours_since_last_rebuild = (
                current_time - self.index_last_updated
            ).total_seconds() / 3600
            hours_since_last_check = (
                current_time - self.last_index_rebuild_check
            ).total_seconds() / 3600

            # 如果索引超过指定小时数未更新，并且距离上次检查至少1小时，考虑重建
            if (
                hours_since_last_rebuild >= self.index_rebuild_interval_hours
                and hours_since_last_check >= 1
            ):
                logger.info(
                    f"索引已超过{                         hours_since_last_rebuild:.1f}小时未更新（阈值: {                         self.index_rebuild_interval_hours}小时），触发定期索引重建"
                )
        ...
        if query and not memory_type:  # 只有当没有指定记忆类型时才检查工作记忆
            ...
            if working_memory_results:
                ...
                if len(working_memory_results) >= min_required:
                    logger.debug(
                        f"从工作记忆中检索到 {len(working_memory_results)} 个结果，直接返回"
                    )
                    # 转换为标准记忆格式
                    formatted_results = []
                    ...
                    logger.debug(
                        f"从工作记忆中检索到 {                             len(working_memory_results)} 个结果，不足 {min_required} 个，继续下层检索"
                    )

        # 第二层：短期记忆缓存（内存中，秒级响应）
        if query and (
            memory_type in [None, "short_term"]
        ):  # 只检查短期记忆或不指定类型
            ...
            if short_term_cache_results:
                ...
                if len(short_term_cache_results) >= min_required:
                    logger.debug(
                        f"从短期记忆缓存中检索到 {len(short_term_cache_results)} 个结果，直接返回"
                    )
                    # 确保记忆格式正确
                    formatted_results = []
                    ...
                    logger.debug(
                        f"从短期记忆缓存中检索到 {                             len(short_term_cache_results)} 个结果，不足 {min_required} 个，继续下层检索"
                    )
        ...
        if (
            self.enable_faiss_retrieval
            and self.index_initialized
            and memory_type in index_search_types
        ):
            # 使用索引搜索
            faiss_results = self._search_with_faiss(
            ...
            if len(scored_memories) < min(limit, 3):
                logger.debug(
                    f"FAISS索引搜索结果不足 ({len(scored_memories)}个)，回退到线性搜索"
                )
            ...
            for memory in expired_memories:
                ...
                if memory.importance >= self.importance_threshold:
                    ...
                    if self.enable_memory_graph and self.graph_initialized:
                        try:
                            embedding = json.loads(memory.embedding)
                            self._update_memory_graph_with_new_memory(
                                memory.id, embedding, db
                            )
                            logger.debug(f"记忆 {memory.id} 已添加到动态关联图")
                        except Exception as e:
                            logger.error(
                                f"将记忆 {memory.id} 添加到动态关联图失败: {e}"
                            )
                else:
                    # 删除不重要的记忆
                    db.delete(memory)
                ...
                if (
                    ...
                    try:
                        ...
                        if memory_id in self.memory_to_node_map:
                            ...
                            logger.debug(
                                f"记忆 {memory_id} 标记为需要从图中移除，将在下次重建时处理"
                            )
                    except Exception as e:
    ...
    def update_memory_importance(self, db: Session, memory_id: int, importance: float):
        ...
        if memory:
            ...
            # 更新缓存中的记忆（如果存在）
            if (
                memory_id in self.short_term_memory_cache
                and memory.memory_type == "short_term"
            ):
                memory_dict = self._memory_to_dict(memory)
    ...
    def retrieve_relevant_memories(
        ...
        for memory in memories:
            try:
                memory_embedding = (
                    json.loads(memory.embedding) if memory.embedding else []
                )
    ...
    def update_working_memory(self, context: Dict[str, Any], db: Session, user_id: int):
        ...
        if len(self.working_memory) >= self.working_memory_capacity:
            ...
            self.working_memory = self.working_memory[
                -self.working_memory_capacity // 2:
            ]
    ...
    def add_to_working_memory(
        ...
        完整接口，用于快速添加记忆到工作缓冲区
        """
        context = {"content": content, "importance": importance, "type": context_type}
    ...
    def _update_memory_graph(self, context_item: Dict, db: Session, user_id: int):
        ...
        try:
            ...
            # 实际应基于语义相似度和时间接近性
            logger.debug(
                f"更新记忆关联图，用户 {user_id}，最近记忆数: {len(recent_memories)}"
            )
    ...
    def get_working_memory_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "usage_percentage": len(self.working_memory)
            / self.working_memory_capacity
            * 100,
        }

    def learn_importance(
        self, memory_id: int, feedback_score: float, db: Session, user_id: int
    ):
        """学习记忆重要性（基于反馈的强化学习）
        ...
        try:
            ...
            learning_rate = 0.1
            new_importance = memory.importance + learning_rate * (
                feedback_score - memory.importance
            )
            ...
            logger.info(
                f"记忆 {memory_id} 重要性更新: {memory.importance:.3f} -> {new_importance:.3f}"
            )
    ...
    def build_memory_association_graph(
    ...
    def _build_graph_from_memory_graph(
        self, db: Session, user_id: int
    ) -> Dict[str, Any]:
        """从内存图结构构建关联图
        ...
        try:
            ...
            memories = (
                db.query(Memory)
                .filter(Memory.user_id == user_id, Memory.memory_type == "long_term")
                .all()
            )
            ...
            for memory in memories:
                ...
                if (
                    ...
                    and self.memory_graph is not None
                    and "node_embeddings" in self.memory_graph
                    and self.memory_graph["node_embeddings"] is not None
                ):

                    node_embedding = (
                        self.memory_graph["node_embeddings"][node_idx].tolist()
                        if node_idx < len(self.memory_graph["node_embeddings"])
                        else None
                    )

                    # 社区信息
                    if (
                        "communities" in self.memory_graph
                        and self.memory_graph["communities"]
                    ):
                        for comm_id, community in enumerate(
                            self.memory_graph["communities"]
                        ):
            ...
            # 创建边（使用图结构中的邻接矩阵）
            adjacency = (
                self.memory_graph.get("adjacency") if self.memory_graph else None
            )
            edge_predictions = (
                self.memory_graph.get("edge_predictions") if self.memory_graph else None
            )
                    ...
                    for j in range(i + 1, num_nodes):
                        ...
                        if strength > 0.1:  # 阈值
                            ...
                            if (
                                ...
                                and hasattr(edge_predictions, "shape")
                                and i < edge_predictions.shape[0]
                                and j < edge_predictions.shape[1]
                            ):
                                pred_strength = (
                                    edge_predictions[i, j].item()
                                    if hasattr(edge_predictions[i, j], "item")
                                    else edge_predictions[i, j]
                                )
    ...
    def apply_selective_memory(self, db: Session, user_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.enable_autonomous_memory and self.autonomous_manager:
                ...
                importance_threshold = autonomous_params.get(
                    "importance_threshold", 0.7
                )
                ...
                if hasattr(self, "config"):
                    selective_memory_enabled = self.config.get(
                        "selective_memory_enabled", False
                    )
                    forgetting_rate = self.config.get("forgetting_rate", 0.05)
                    importance_threshold = self.config.get("importance_threshold", 0.7)
            ...
            logger.info(
                f"应用选择性记忆机制: 遗忘率={forgetting_rate}, 重要性阈值={importance_threshold}"
            )
            ...
            for memory in short_term_memories:
                if memory.importance >= importance_threshold:
                    ...
                    upgraded_count += 1
                    logger.debug(
                        f"升级短期记忆 {memory.id} 为长期记忆: 重要性={memory.importance}"
                    )
                else:
                    # 遗忘不重要短期记忆
                    # 基于遗忘率随机决定是否遗忘
                    ...
                    if random.random() < forgetting_rate:
                        ...
                        logger.debug(
                            f"遗忘短期记忆 {memory.id}: 重要性={memory.importance}"
                        )
            ...
            for memory in long_term_memories:
                if memory.importance < importance_threshold:
                    ...
                    if random.random() < forgetting_rate:
                        ...
                        logger.debug(
                            f"遗忘长期记忆 {memory.id}: 重要性={memory.importance}"
                        )
                    else:
            ...
            total_memories_after = (
                total_memories_before
                - forgotten_short_term_count
                - forgotten_long_term_count
            )
            ...
            stats = {
                ...
                "importance_threshold": importance_threshold,
                "compression_rate": (
                    forgotten_short_term_count + forgotten_long_term_count
                )
                / max(1, total_memories_before),
            }

            logger.info(
                f"选择性记忆处理完成: 升级 {upgraded_count} 个记忆, "
    ...
    def _build_edges_from_database(self, memories: List) -> List[Dict[str, Any]]:
        ...
        for i in range(len(memories)):
            ...
            for j in range(i + 1, len(memories)):
                ...
                try:
                    emb_i = (
                        json.loads(memories[i].embedding)
                        if memories[i].embedding
                        else []
                    )
                    emb_j = (
                        json.loads(memories[j].embedding)
                    ...
                    logger.debug(
                        f"计算记忆 {memories[i].id}-{memories[j].id} 相似度失败: {e}"
                    )
                    continue
    ...
    def initialize_cognitive_reasoning(
        ...
        try:
            # 检查是否启用认知推理
            enable_cognitive_reasoning = self.config.get(
    ...
    def reason_from_memory(
        ...
        # 确保推理集成器已初始化
        if (
            not hasattr(self, "cognitive_reasoning_integrator")
            or self.cognitive_reasoning_integrator is None
        ):
        ...
        # 使用推理集成器
        return self.cognitive_reasoning_integrator.reason_from_memory(
            query, db, user_id, context
        )
    ...
    def initialize_context_aware_enhancer(
        self, config: Optional[Dict[str, Any]] = None
    ) -> "ContextAwareMemoryEnhancer":
        """初始化情境感知记忆增强器
    ...
    def update_context(self, context_updates: Dict[str, Any]):
        ...
        # 确保情境感知增强器已初始化
        if (
            not hasattr(self, "context_aware_enhancer")
            or self.context_aware_enhancer is None
        ):
    ...
    def get_scene_stats(self) -> Dict[str, Any]:
        ...
        if (
            ...
            return {"scene_enhancement_enabled": False, "current_scene": "general"}
    ...
    def update_scene_context(self, scene_updates: Dict[str, Any]):
        ...
        if "scene_type" in scene_updates:
            self.context_aware_enhancer.current_scene["scene_type"] = scene_updates[
                "scene_type"
            ]

        if "scene_id" in scene_updates:
            self.context_aware_enhancer.current_scene["scene_id"] = scene_updates[
                "scene_id"
            ]
...
class CognitiveReasoningIntegrator:
    ...
    def __init__(
        ...
        if memory_system is None:
            logger.info(
                "CognitiveReasoningIntegrator 初始化时 memory_system 为 None，部分记忆相关功能将不可用（测试环境中为预期行为）"
            )
        self.config = config or {}

        # 推理配置参数
        self.enable_symbolic_reasoning = self.config.get(
            "enable_symbolic_reasoning", True
        )
        self.enable_neural_reasoning = self.config.get("enable_neural_reasoning", True)
        self.enable_analogical_reasoning = self.config.get(
            "enable_analogical_reasoning", True
        )
        self.enable_abductive_reasoning = self.config.get(
    ...
    def reason_from_memory(
        ...
        try:
            ...
            reasoning_result = self._execute_reasoning(
                reasoning_strategy, reasoning_context
            )

            # 5. 验证推理结果
            validated_result = self._validate_reasoning_result(
    ...
    def _retrieve_relevant_memories(
        self, query: str, db: Session, user_id: int
    ) -> List[Dict[str, Any]]:
        """检索与查询相关的记忆"""
        # 使用记忆系统的检索功能
        ...
        for memory in relevant_memories:
            ...
            # 提取关键概念
            enriched_memory["concepts"] = self._extract_concepts(
                memory.get("content", "")
            )

            # 计算推理相关性
            enriched_memory["reasoning_relevance"] = (
                self._calculate_reasoning_relevance(query, memory)
            )
        ...
        enriched_memories.sort(
            key=lambda x: x.get("reasoning_relevance", 0.0), reverse=True
        )
    ...
    def _calculate_reasoning_relevance(
        self, query: str, memory: Dict[str, Any]
    ) -> float:
        """计算记忆对推理查询的相关性"""
        ...
        # 综合相关性评分
        relevance_score = (
            concept_similarity * 0.4 + importance * 0.3 + time_relevance * 0.3
        )
    ...
    def _categorize_memory(self, memory: Dict[str, Any]) -> str:
        ...
        # 基于内容的简单分类
        if any(
            word in content_lower for word in ["因为", "所以", "因此", "由于", "导致"]
        ):
    ...
    def _causal_reasoning(
        ...
        if supporting_evidence:
            ...
            answer += "可以推断：" + self._generate_causal_inference(
                query, supporting_evidence
            )
            confidence = 0.8
        else:
            answer = "未找到明确的因果关系记忆"
    ...
    def _abductive_reasoning(
        ...
        if problems:
            # 尝试构建最佳解释
            best_explanation = self._generate_best_explanation(
                query, problems, memories
            )
    ...
    def _generate_causal_inference(
    ...
    def _validate_reasoning_result(
        ...
        if confidence < self.confidence_threshold:
            ...
            result["validation_note"] = (
                f"置信度过低 ({confidence:.2f} < {self.confidence_threshold})"
            )
        else:
            result["success"] = True
    ...
    def _record_reasoning_history(
        self, query: str, context: Dict[str, Any], result: Dict[str, Any]
    ):
        """记录推理历史"""
        history_entry = {
            "timestamp": time.time(),
    ...
    def get_reasoning_stats(self) -> Dict[str, Any]:
        ...
        successful = sum(
            1 for entry in self.reasoning_history if entry["result"]["success"]
        )
        avg_confidence = (
            sum(entry["result"]["confidence"] for entry in self.reasoning_history)
            / total
        )
...
class NeuralReasoningNetwork:
    ...
    def __init__(self, input_dim: int = 20, hidden_dim: int = 64, output_dim: int = 10):
        ...
        logger.info(
            f"神经网络推理模型已初始化: 输入维度={input_dim}, 隐藏维度={hidden_dim}, 输出维度={output_dim}"
        )
    ...
    def train(
        self, training_data: List[Dict[str, Any]], labels: List[int], epochs: int = 50
    ):
        """训练神经网络
        ...
        try:
            ...
            X = (
                torch.stack(X)
                if X
                else torch.zeros(0, self.input_dim, device=self.device)
            )
    ...
    def predict(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if isinstance(input_data, dict):
                # 单个记忆
                features = self.extract_features(input_data).unsqueeze(
                    0
                )  # [1, input_dim]
            ...
            with torch.no_grad():
                ...
                if len(pred_indices) > 0:
                    ...
                    # 所有类别的概率
                    prob_list = (
                        probabilities[0].cpu().numpy().tolist()
...
class ContextAwareMemoryEnhancer:
    ...
    def __init__(
        self, memory_system: "MemorySystem", config: Optional[Dict[str, Any]] = None
    ):
        self.memory_system = memory_system
        self.config = config or {}
        ...
        self.context_similarity_threshold = self.config.get(
            "context_similarity_threshold", 0.7
        )
        self.enable_context_prediction = self.config.get(
        ...
        # 增强：情景场景记忆配置
        self.enable_scene_classification = self.config.get(
            "enable_scene_classification", True
        )  # 是否启用场景分类
        self.enable_scene_transition_detection = self.config.get(
            "enable_scene_transition_detection", True
        )  # 是否启用场景切换检测
        self.scene_classification_threshold = self.config.get(
            "scene_classification_threshold", 0.7
        )  # 场景分类阈值
        self.scene_memory_window = self.config.get(
    ...
    def update_context(self, context_updates: Dict[str, Any]):
        ...
        if "conversation_history" in context_updates:
            ...
            if len(conversation_history) > self.context_window_size:
                self.current_context["conversation_history"] = conversation_history[
    ...
    def _get_context_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "conversation_length": len(
                self.current_context.get("conversation_history", [])
            ),
    ...
    def retrieve_context_aware_memories(
        ...
        if not use_context:
            # 不使用上下文，直接使用标准检索
            return self.memory_system.retrieve_relevant_memories(
                query, db, user_id, top_k
            )
        ...
        try:
            ...
            # 3. 按综合相关性排序
            context_enhanced_memories.sort(
                key=lambda x: x.get("combined_relevance", 0.0), reverse=True
            )
    ...
    def _calculate_context_relevance(self, memory: Dict[str, Any]) -> float:
        ...
        # 综合相关性评分
        weights = {"topic": 0.5, "intent": 0.4, "conversation": 0.1}
    ...
    def _record_context_retrieval_stats(
        self, query: str, base_count: int, result_count: int
    ):
        """记录上下文检索统计
        ...
        try:
            ...
            if len(self.retrieval_stats["recent_queries"]) > 100:
                self.retrieval_stats["recent_queries"] = self.retrieval_stats[
                    "recent_queries"
                ][-100:]
            ...
            new_avg = (
                current_avg * (self.retrieval_stats["total_queries"] - 1) + result_ratio
            ...
            if len(self.retrieval_stats["query_length_distribution"]) > 1000:
                self.retrieval_stats["query_length_distribution"] = (
                    self.retrieval_stats["query_length_distribution"][-1000:]
                )

            # 记录日志（可选，避免过多日志）
            if self.retrieval_stats["total_queries"] % 50 == 0:
                logger.info(
                    f"上下文检索统计: 总查询数={self.retrieval_stats['total_queries']}, "
    ...
    def learn_context_patterns(self, db: Session, user_id: int):
        ...
        try:
            ...
            context_features = {
                ...
                "recent_keywords": self._extract_keywords_from_conversation(
                ...
                for mem_type, pattern_data in memory_patterns.items():
                    if mem_type not in existing_pattern["memory_patterns"]:
                        ...
                        existing_data["avg_importance"] = (
                            existing_data["avg_importance"]
                            * (existing_data["count"] - pattern_data["count"])
                            + pattern_data["avg_importance"] * pattern_data["count"]
                        ) / existing_data["count"]
            ...
            logger.info(
                f"上下文模式学习完成: 模式键={pattern_key}, 模式总数={len(self.context_patterns)}"
            )

        except Exception as e:
            logger.error(f"学习上下文模式失败: {e}")

    def _extract_keywords_from_conversation(
    ...
    def _get_recently_accessed_memories(
        ...
        try:
            ...
            memories = (
                db.query(Memory)
                .filter(Memory.user_id == user_id, Memory.last_accessed.isnot(None))
    ...
    def _classify_scene_and_update(self):
        ...
        try:
            ...
            if scene_scores:
                # 找到得分最高的场景
                primary_scene = max(scene_scores.items(), key=lambda x: x[1]["score"])[
                    0
                ]
                ...
                if scene_data["score"] >= self.scene_classification_threshold:
                    ...
                    logger.debug(
                        f"场景分类: {previous_scene} -> {primary_scene} (分数: {                             scene_data['score']:.2f})"
                    )
    ...
    def _detect_scene_transition(self):
        ...
        try:
            ...
            if current_time - transition_time < 30:
                logger.info(
                    f"最近检测到场景切换: {                         recent_transition['from_scene']} -> {                         recent_transition['to_scene']}"
                )
    ...
    def retrieve_from_knowledge_base(
        ...
        try:
            ...
            knowledge_results = self.knowledge_manager.query_knowledge(
                query=query, limit=top_k, similarity_threshold=similarity_threshold
            )
    ...
    def retrieve_hybrid(
        ...
        # 从知识库检索
        knowledge_results = self.retrieve_from_knowledge_base(query=query, top_k=top_k)
        ...
        for result in memory_results:
            ...
            result_with_weight["weighted_score"] = (
                result.get("similarity_score", 0.5) * memory_weight
            )
...
class MemorySystemPerformanceMonitor:
    ...
    def __init__(
        ...
        self.performance_history_size = self.config.get(
    ...
    def collect_performance_metrics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            metrics.update(
                {
                    "cache_hit_rate": float(
                        cache_stats.get("short_term_cache_hit_rate", "0%").strip("%")
                    )
                    / 100,
                    "cache_size": cache_stats.get("short_term_cache_size", 0),
                    "working_memory_size": cache_stats.get("working_memory_size", 0),
                    "faiss_avg_search_time": float(
                        cache_stats.get("faiss_avg_search_time_ms", "0.0").strip("ms")
                    ),
                    "hnsw_avg_search_time": float(
                        cache_stats.get("hnsw_avg_search_time_ms", "0.0").strip("ms")
                    ),
                }
            )
            ...
            metrics.update(
                {
                    "reasoning_enabled": reasoning_stats.get(
                        "cognitive_reasoning_enabled", False
                    ),
                    "reasoning_success_rate": reasoning_stats.get("success_rate", 0.0),
                    "reasoning_attempts": reasoning_stats.get(
                        "total_reasoning_attempts", 0
                    ),
                }
            )
            ...
            metrics.update(
                {
                    "context_awareness_enabled": context_stats.get(
            ...
            try:
                ...
                metrics.update(
                    {"memory_usage_mb": 0, "cpu_percent": 0, "thread_count": 0}
                )
    ...
    def analyze_performance_trends(
        ...
        analysis = {
            ...
            or (recent_metrics[-1]["timestamp"] - recent_metrics[0]["timestamp"]),
            ...
            alerts.append(
                {
                    ...
                    "suggestions": self._get_optimization_suggestions(
                        "retrieval_optimization", "检索响应时间慢"
                    ),
                }
            )
    ...
    def generate_performance_report(
        self, time_window: Optional[int] = 3600
    ) -> Dict[str, Any]:
        """生成性能报告
...
class AdvancedMemoryForgetting:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.forgetting_params = {
            ...
            "ebbinghaus_decay_factor": self.config.get(
                "ebbinghaus_decay_factor", 0.56
            ),  # 艾宾浩斯衰减因子
            "association_forgetting_weight": self.config.get(
                "association_forgetting_weight", 0.3
            ),
            "entropy_forgetting_weight": self.config.get(
                "entropy_forgetting_weight", 0.2
            ),
            "quality_forgetting_weight": self.config.get(
                "quality_forgetting_weight", 0.2
            ),
            "min_retention_threshold": self.config.get(
                "min_retention_threshold", 0.1
            ),  # 最小保留阈值
            "max_forgetting_rate": self.config.get(
                "max_forgetting_rate", 0.3
            ),  # 最大遗忘率
        }
    ...
    def _create_effectiveness_evaluator(self):
        ...
        def evaluate_effectiveness(
            forgetting_stats: Dict[str, Any], system_state: Dict[str, Any]
        ) -> Dict[str, Any]:
            """评估遗忘效果"""
            ...
            importance_preservation = avg_importance_after / max(
                avg_importance_before, 0.01
            )
            ...
            return {
                ...
                "recommendation": self._generate_recommendation(
                    effectiveness_score, forgetting_stats
                ),
            }
    ...
    def _generate_recommendation(
        self, effectiveness_score: float, stats: Dict[str, Any]
    ) -> str:
        """生成优化建议"""
    ...
    def calculate_ebbinghaus_decay(
        self, hours_since_encoding: float, access_count: int = 1
    ) -> float:
        """计算艾宾浩斯遗忘曲线衰减
        ...
        retention = math.exp(
            -hours_since_encoding / (24.0 * decay_factor)
        )  # 24小时为单位

        return max(0.0, min(1.0, retention))

    def calculate_association_based_forgetting(
        self, memory_id: int, memory_graph: Dict[str, Any], db: Optional[Any] = None
    ) -> float:
        """计算基于关联性的遗忘概率
    ...
    def calculate_content_based_forgetting(
    ...
    def calculate_comprehensive_forgetting_probability(
        ...
        """
        # 获取策略配置
        strategy_config = self.forgetting_strategies.get(
            strategy, self.forgetting_strategies["selective"]
        )
        ...
        memory_data.get("last_accessed", created_at)
        ...
        time_decay_factor = 1.0 - self.calculate_ebbinghaus_decay(
            hours_since_encoding, access_count
        )
        ...
        if memory_graph:
            ...
            if memory_id:
                association_factor = self.calculate_association_based_forgetting(
                    memory_id, memory_graph
                )
    ...
    def apply_forgetting_strategy(
        ...
        if not memories:
            return {"success": False, "message": "没有需要处理的记忆", "stats": {}}
        ...
        if target_compression_rate is not None:
            ...
            if target_forget_count > 0:
                threshold = memory_analyses[target_forget_count - 1][
                    "forgetting_probability"
                ]
            else:
            ...
            # 使用自适应阈值
            avg_probability = sum(
                a["forgetting_probability"] for a in memory_analyses
            ) / len(memory_analyses)
        ...
        compression_rate = (
            forgotten_count / total_memories if total_memories > 0 else 0.0
        )

        # 计算平均重要性变化
        avg_importance_before = (
            sum(m.get("importance", 0.5) for m in memories) / total_memories
            if total_memories > 0
            else 0.5
        )
        avg_importance_after = (
            sum(m.get("importance", 0.5) for m in retained_memories) / retained_count
            if retained_count > 0
            else 0.5
        )
        ...
        stats = {
            ...
            "forgotten_memory_ids": [
                m["memory_id"] for m in forgotten_memories if m.get("memory_id")
            ],
            "avg_forgetting_probability": (
                sum(a["forgetting_probability"] for a in memory_analyses)
                / total_memories
                if total_memories > 0
                else 0.0
            ),
    ...
    def _execute_forgetting_in_db(
        self, db: Any, forgotten_analyses: List[Dict[str, Any]]
    ):
        """在数据库中执行遗忘操作"""
        try:
            from backend.db_models.memory import Memory, MemoryAssociation
    ...
    def get_strategy_recommendation(
        ...
        return {
            ...
            "system_state": {"load_level": load_level, "memory_usage": memory_usage},
        }
        ...
        if param_effects:
            ...
            if best_param_key:
                ...
                try:
                    ...
                    self.logger.info(
                        f"参数优化完成: 最佳阈值={                             optimal_threshold:.3f}, 基础遗忘率={                             optimal_base_rate:.3f}"
                    )
    ...
    def _calculate_recent_effectiveness(self) -> Dict[str, Any]:
        ...
        return {"avg_score": avg_score, "trend": trend, "recent_scores": scores}
...
class CrossSessionMemoryPersistence:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.persistence_config = {
            ...
            "auto_save_interval": self.config.get(
    ...
    def create_session(
        ...
        if len(user_session_ids) >= max_sessions:
            ...
            self.logger.info(
                f"用户 {user_id} 会话数达到限制，删除最旧会话: {oldest_session_id}"
            )
    ...
    def save_system_state(
        self, memory_system, db: Optional[Any] = None, force: bool = False
    ) -> bool:
        """保存系统状态
        ...
        if not force and last_save:
            ...
            if current_time - last_save < save_interval:
                self.logger.debug(
                    f"跳过自动保存，距上次保存仅 {int(current_time - last_save)} 秒"
                )
                return False
        ...
        try:
            ...
            self.logger.info(
                f"系统状态已保存: {filepath} (大小: {os.path.getsize(filepath)} 字节)"
            )
    ...
    def _collect_system_state(
        self, memory_system, db: Optional[Any] = None
    ) -> Dict[str, Any]:
        """收集系统状态"""
        state = {
            "timestamp": time.time(),
        ...
        if memory_system.enable_autonomous_memory and memory_system.autonomous_manager:
            state["autonomous_manager_state"] = (
                memory_system.autonomous_manager.get_system_state()
            )
    ...
    def _collect_memory_stats(
        ...
        stats = {
            ...
            "working_memory_count": (
                len(memory_system.working_memory)
        ...
        if db:
            try:
                ...
                # 统计长期记忆
                long_term_count = (
                    db.query(Memory).filter(Memory.memory_type == "long_term").count()
                )
    ...
    def _collect_cache_state(self, memory_system) -> Dict[str, Any]:
        ...
        state = {
            ...
            "short_term_cache_max_size": getattr(
                memory_system, "short_term_cache_max_size", 200
            ),
            "working_memory_size": len(getattr(memory_system, "working_memory", [])),
            "working_memory_capacity": getattr(
                memory_system, "working_memory_capacity", 100
            ),
        }
    ...
    def load_system_state(
        self, memory_system, db: Optional[Any] = None, state_file: Optional[str] = None
    ) -> bool:
        """加载系统状态
    ...
    def _validate_state(self, state: Dict[str, Any]) -> bool:
        ...
        current_time = time.time()
        if (
            timestamp < current_time - 315360000 or timestamp > current_time + 3600
        ):  # 10年前到1小时后
    ...
    def _apply_system_state(
        self, memory_system, state: Dict[str, Any], db: Optional[Any]
    ) -> bool:
        """应用系统状态"""
        try:
            ...
            # 恢复自主记忆管理器状态
            if (
                memory_system.enable_autonomous_memory
                and memory_system.autonomous_manager
                and "autonomous_manager_state" in state
            ):
                self._restore_autonomous_manager_state(
                    memory_system.autonomous_manager, state["autonomous_manager_state"]
                )
    ...
    def _check_system_compatibility(
        self, memory_system, system_info: Dict[str, Any]
    ) -> bool:
        """检查系统兼容性"""
        compatible = True
        ...
        if saved_dim != current_dim:
            self.logger.warning(
                f"嵌入维度不兼容: 保存的={saved_dim}, 当前的={current_dim}"
            )
            compatible = False

        # 检查功能启用状态
        features = [
            "enable_faiss_retrieval",
            "enable_memory_graph",
        ...
        for feature in features:
            ...
            if saved_value != current_value:
                self.logger.warning(
                    f"{feature} 状态不兼容: 保存的={saved_value}, 当前的={current_value}"
                )
                # 不标记为不兼容，因为功能状态可能已更改
    ...
    def _restore_cache_state(self, memory_system, cache_state: Dict[str, Any]):
        ...
        self.logger.info(
            f"缓存状态恢复: 命中={                 cache_state.get(                     'cache_hits',                     0)}, 未命中={                 cache_state.get(                     'cache_misses',                     0)}"
        )

    def _restore_autonomous_manager_state(
        self, autonomous_manager, manager_state: Dict[str, Any]
    ):
        """恢复自主记忆管理器状态"""
        try:
    ...
    def load_session_state(self, session_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            metadata_file = os.path.join(
                self.state_dir, f"session_{session_id}_metadata.json"
            )
            ...
            # 加载会话记忆
            memories_file = os.path.join(
                self.state_dir, f"session_{session_id}_memories.json"
            )
            memories_data = None
            ...
            session = {
                ...
                "state": (
                    memories_data.get("state_snapshot", {}) if memories_data else {}
                ),
                "memory_ids": (
                    memories_data.get("memory_ids", []) if memories_data else []
                ),
            ...
            self.logger.info(
                f"会话状态已加载: {session_id} ({len(session['memory_ids'])} 个记忆)"
            )
    ...
    def start_auto_save(self, memory_system, db: Optional[Any] = None):
        ...
        self.logger.info(
            f"自动保存已启动，间隔: {self.persistence_config['auto_save_interval']} 秒"
        )
...
class MemoryConflictResolver:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.detection_config = {
            ...
            "content_conflict_threshold": self.config.get(
                "content_conflict_threshold", 0.7
            ),
            "time_conflict_threshold": self.config.get(
                "time_conflict_threshold", 3600
            ),  # 1小时（秒）
            "fact_conflict_penalty": self.config.get("fact_conflict_penalty", 0.3),
            "max_conflict_age": self.config.get(
                "max_conflict_age", 2592000
            ),  # 30天（秒）
            "min_confidence_for_resolution": self.config.get(
                "min_confidence_for_resolution", 0.6
            ),
        }
    ...
    def detect_conflicts(
        self, memory_system, db: Optional[Any] = None, limit: int = 100
    ) -> List[Dict[str, Any]]:
        """检测记忆冲突
        ...
        try:
            ...
            conflicts.extend(
                self._detect_association_conflicts(memories, memory_system)
            )
    ...
    def _get_memories_from_system(
        self, memory_system, limit: int
    ) -> List[Dict[str, Any]]:
        """从内存系统获取记忆"""
        memories = []
        ...
        if hasattr(memory_system, "working_memory"):
            for memory in memory_system.working_memory[
                : min(limit, len(memory_system.working_memory))
            ]:
        ...
        if hasattr(memory_system, "short_term_memory_cache"):
            cache_size = min(
                limit - len(memories), len(memory_system.short_term_memory_cache)
            )
            for i, (key, memory) in enumerate(
                list(memory_system.short_term_memory_cache.items())[:cache_size]
            ):
                if hasattr(memory, "to_dict"):
                    memories.append(memory.to_dict())
                elif isinstance(memory, dict):
    ...
    def _group_memories_by_content_similarity(
        self, memories: List[Dict[str, Any]]
    ...
    def _analyze_content_group_for_conflicts(
    ...
    def _calculate_content_difference_ratio(
    ...
    def _analyze_time_conflicts_in_group(
        self, group: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
        """分析组内的时间冲突"""
        conflicts = []
    ...
    def _check_fact_conflict(
        ...
        for word in negation_words:
            if (word in content1 and word not in content2) or (
                word in content2 and word not in content1
            ):
                has_negation = True
                break
    ...
    def _analyze_priority_conflicts_in_group(
        self, group: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
        """分析组内的优先级冲突"""
        conflicts = []
    ...
    def _detect_association_conflicts(
        self, memories: List[Dict[str, Any]], memory_system
    ) -> List[Dict[str, Any]]:
        """检测关联冲突"""
        conflicts = []
    ...
    def _find_contradictory_edges(
        self, edges: List[Dict[str, Any]], memories: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
        """查找矛盾的关联边"""
        conflicts = []
            ...
            for j, edge2 in enumerate(edges[i + 1:], start=i + 1):
                ...
                if source1 == source2 and target1 != target2:
                    ...
                    for edge3 in edges:
                        ...
                        if (source3 == target1 and target3 == target2) or (
                            source3 == target2 and target3 == target1
                        ):
                            # 检查关联方向是否矛盾
    ...
    def resolve_conflicts(
        ...
        for conflict in conflicts:
            ...
            result = self._apply_resolution_strategy(
                conflict, selected_strategy, memory_system, db
            )
        ...
        total_severity_reduction = sum(
            r.get("severity_reduction", 0.0) for r in resolution_results
        )
        ...
        overall_result = {
            ...
            "avg_severity_reduction": (
                total_severity_reduction / len(conflicts) if conflicts else 0.0
            ),
    ...
    def _apply_resolution_strategy(
        ...
        """应用解决策略"""
        strategy_config = self.resolution_strategies.get(
            strategy, self.resolution_strategies["select"]
        )
    ...
    def _apply_merge_strategy(
        self, conflict: Dict[str, Any], memory_system, db: Optional[Any] = None
    ) -> Dict[str, Any]:
        """应用合并策略"""
        ...
        # 标记原记忆为已解决
        resolution_mark = self._mark_memories_as_resolved(
            memories, "merged", new_memory_id
        )
        ...
        return {
            ...
            "merged_content_preview": (
                merged_content[:100] + "..."
                if len(merged_content) > 100
                else merged_content
            ),
            "severity_reduction": conflict.get("severity", 0.5)
            * 0.8,  # 假设严重性降低80%
        ...
        if len(unique_contents) > 1:
            ...
            for memory in memories:
                ...
                if content in content_importance:
                    content_importance[content] = max(
            ...
            unique_contents.sort(
                key=lambda x: content_importance.get(x, 0.5), reverse=True
            )
    ...
    def _save_merged_memory(
        self, db: Any, memory_data: Dict[str, Any], conflict: Dict[str, Any]
    ) -> Optional[int]:
        """保存合并后的记忆到数据库"""
        try:
            from backend.db_models.memory import Memory
    ...
    def _mark_memories_as_resolved(
        self,
        memories: List[Dict[str, Any]],
        resolution_type: str,
    ...
    def _apply_select_strategy(
        ...
        other_memories = [
            m for m in memories if m.get("id") != selected_memory.get("id")
        ]
        ...
        update_results = None
        if db:
            update_results = self._update_memory_priorities(
                db,
                selected_memory,
                updated_importance,
                other_memories,
        ...
        return {
            ...
            "selected_memory_content_preview": (
                selected_memory.get("content", "")[:100] + "..."
                if len(selected_memory.get("content", "")) > 100
                else selected_memory.get("content", "")
            ),
    ...
    def _apply_both_strategy(
        ...
        for memory in memories:
            ...
            if memory_id:
                marker = {
                    ...
                    "other_memory_ids": [
                        m.get("id") for m in memories if m.get("id") != memory_id
                    ],
                    "marked_at": time.time(),
                }
        ...
        update_results = None
        if db:
            update_results = self._mark_conflicting_memories(
                db, conflict_markers, conflict
            )
    ...
    def _mark_conflicting_memories(
        self, db: Any, conflict_markers: List[Dict[str, Any]], conflict: Dict[str, Any]
    ) -> Dict[str, Any]:
        """标记冲突记忆"""
        try:
            from backend.db_models.memory import Memory

            update_info = {"marked_count": 0, "errors": []}
            ...
            db.rollback()
            return {"marked_count": 0, "errors": [str(e)]}

    def _apply_contextual_strategy(
        self, conflict: Dict[str, Any], memory_system, db: Optional[Any] = None
    ) -> Dict[str, Any]:
        """应用上下文策略"""
        # 上下文策略需要更多的外部信息和推理
        ...
        evidence = self.evidence_collector.collect_evidence(
            memories, conflict_type, memory_system
        )
    ...
    def _resolve_based_on_evidence(
        self,
        memories: List[Dict[str, Any]],
        ...
        for memory in memories:
            memory_id = memory.get("id")
            if memory_id:
                memory_scores[memory_id] = self._calculate_memory_evidence_score(
                    memory, evidence_items
                )
        ...
        # 找到最佳记忆
        best_memory = next(
            (m for m in memories if m.get("id") == best_memory_id), memories[0]
        )
    ...
    def _calculate_memory_evidence_score(
        self, memory: Dict[str, Any], evidence_items: List[Dict[str, Any]]
    ) -> float:
        """计算记忆的证据评分"""
        score = 0.0
        content = memory.get("content", "")
        ...
        for evidence in evidence_items:
            ...
            # 简单的关键词匹配
            common_words = set(content.lower().split()) & set(
                evidence_content.lower().split()
            )
            if common_words:
                score += (
                    len(common_words) / max(len(content.split()), 1)
                ) * evidence_confidence

        return min(1.0, score)

    def _record_conflict_resolution(
        self, conflict: Dict[str, Any], result: Dict[str, Any], strategy: str
    ):
        """记录冲突解决结果"""
        resolution_record = {
            "timestamp": time.time(),
            "conflict_id": hash(
                str(conflict.get("memory_ids", [])) + conflict.get("type", "")
            ),
            "conflict_type": conflict.get("type"),
    ...
    def _update_system_after_resolution(
        self, memory_system, db: Any, resolution_results: List[Dict[str, Any]]
    ):
        """解决冲突后更新系统"""
        # 在实际实现中，可能需要更新索引、缓存等
        # 这里仅记录更新信息
    ...
    def get_conflict_statistics(self) -> Dict[str, Any]:
        ...
        successful_resolutions = sum(
            1 for record in self.conflict_history if record.get("success", False)
        )
        resolution_rate = (
            successful_resolutions / total_conflicts if total_conflicts > 0 else 0.0
        )

        # 平均严重性
        total_severity = sum(
            record.get("conflict_severity", 0.0) for record in self.conflict_history
        )
        ...
        for record in self.conflict_history:
            ...
            type_distribution[conflict_type] = (
                type_distribution.get(conflict_type, 0) + 1
            )
    ...
    def get_resolution_recommendations(
        ...
        if len(recommendations) < 2:
            recommendations.append(
                {"strategy": "select", "confidence": 0.5, "reason": "通用选择策略"}
            )
...
class ConflictPatternAnalyzer:
    ...
    def _analyze_frequent_users(
        ...
        # 找到最频繁的用户
        frequent_users = sorted(user_counts.items(), key=lambda x: x[1], reverse=True)[
            :5
        ]
    ...
    def _analyze_time_patterns(
        ...
        time_patterns = {"hourly_distribution": [0] * 24, "daily_distribution": [0] * 7}
    ...
    def _analyze_content_patterns(
        ...
        """分析内容模式"""
        content_patterns = {"common_keywords": {}, "frequent_topics": {}}
    ...
    def _analyze_escalation_patterns(
        ...
        """分析冲突升级模式"""
        escalation_patterns = {"severity_trend": "stable", "frequency_trend": "stable"}
        ...
        if old_conflicts and recent_conflicts:
            avg_severity_old = sum(
                c.get("conflict_severity", 0.0) for c in old_conflicts
            ) / len(old_conflicts)
            avg_severity_recent = sum(
                c.get("conflict_severity", 0.0) for c in recent_conflicts
        ...
        if len(conflict_history) >= 20:
            old_count = len(
                [c for c in conflict_history[-20:-10] if c.get("timestamp", 0) > 0]
            )
            recent_count = len(
                [c for c in conflict_history[-10:] if c.get("timestamp", 0) > 0]
            )
...
class EvidenceCollector:
    ...
    def collect_evidence(
        self, memories: List[Dict[str, Any]], conflict_type: str, memory_system
    ) -> Dict[str, Any]:
        """收集证据"""
        evidence = {"evidence_items": [], "source_types": [], "total_confidence": 0.0}
        ...
        # 统计源类型
        source_types = set(
            item.get("source_type", "unknown") for item in evidence["evidence_items"]
        )
        ...
        if evidence["evidence_items"]:
            total_confidence = sum(
                item.get("confidence", 0.0) for item in evidence["evidence_items"]
            )
            evidence["total_confidence"] = total_confidence / len(
    ...
    def _collect_system_evidence(
        self, memory_system, memories: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
        """收集系统证据"""
        evidence_items = []
        ...
        # 收集缓存命中率证据
        if hasattr(memory_system, "short_term_cache_hits") and hasattr(
            memory_system, "short_term_cache_misses"
        ):
    ...
    def _collect_content_evidence(
        ...
        for memory in memories:
            ...
            length_category = (
                ...
                else "medium" if content_length < 200 else "long"
            )
        ...
        for memory in memories:
            ...
            if created_at:
                try:
                    ...
                    logging.getLogger(__name__).warning(
                        f"dateutil导入失败，跳过时间解析: {e}"
                    )
                except (ValueError, TypeError) as e:
                    # 根据项目要求"不采用任何降级处理，直接报错"，记录警告而不是静默忽略
                    logging.getLogger(__name__).warning(
                        f"时间解析失败 {created_at}，跳过: {e}"
                    )
