...
from models.quaternion_nn import (
    ...
    QuaternionFeedForward, QuaternionConv1D, QuaternionEmbedding
)
from models.quaternion_optimizer import (
    QuaternionAdam, QuaternionSGD,
    QuaternionAngleLoss, QuaternionDotLoss, QuaternionDoubleCoverLoss,
    QuaternionMixedLoss
)
...
class QuaternionMultiplicationTask(QuaternionValidationTask):
    ...
    def generate_data(self, num_samples: int) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        for i in range(num_samples):
            quat1 = Quaternion(q1[i, 0].item(), q1[i, 1].item(), q1[i, 2].item(), q1[i, 3].item())
            quat2 = Quaternion(q2[i, 0].item(), q2[i, 1].item(), q2[i, 2].item(), q2[i, 3].item())
            result = quat1 * quat2
    ...
    def evaluate(self, predictions: torch.Tensor, targets: torch.Tensor) -> Dict[str, float]:
        """评估乘法任务"""
        # 归一化预测和目标
        pred_norm = predictions / torch.norm(predictions, dim=1, keepdim=True).clamp(min=1e-8)
...
class QuaternionRotationTask(QuaternionValidationTask):
    ...
    def generate_data(self, num_samples: int) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        for i in range(num_samples):
            quat = Quaternion(quats[i, 0].item(), quats[i, 1].item(), 
                             quats[i, 2].item(), quats[i, 3].item())
            vec = vectors[i].numpy()
...
class QuaternionSlerpTask(QuaternionValidationTask):
    ...
    def __init__(self):
        super().__init__("四元数SLERP", input_dim=9, output_dim=4)  # 输入: 两个四元数 + 插值参数
    ...
    def generate_data(self, num_samples: int) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        for i in range(num_samples):
            quat1 = Quaternion(q1[i, 0].item(), q1[i, 1].item(), 
                              q1[i, 2].item(), q1[i, 3].item())
            quat2 = Quaternion(q2[i, 0].item(), q2[i, 1].item(), 
                              q2[i, 2].item(), q2[i, 3].item())
...
class QuaternionValidator:
    ...
    def run_validation(self, task_idx: int = None, num_samples: int = 1000, 
                       num_epochs: int = 50, batch_size: int = 32) -> Dict[str, Any]:
        """运行验证"""
        results = {}
        ...
        for task in tasks_to_run:
            ...
            for epoch in range(num_epochs):
                ...
                # 打印进度
                if (epoch + 1) % 10 == 0:
                    print(f"  周期 {epoch+1}/{num_epochs}, 训练损失: {avg_train_loss:.6f}, 测试损失: {test_loss:.6f}")
    ...
    def run_comprehensive_validation(self) -> Dict[str, Any]:
        ...
        for task_name, task_results in all_results.items():
            if "error" in task_results:
                ...
                best_metric = list(metrics.values())[0] if metrics else 0
                print(f"  {task_name}: 成功 - 最终测试损失: {task_results['final_test_loss']:.6f}")
