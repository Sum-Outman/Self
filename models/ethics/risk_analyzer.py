...
class RiskAnalyzer:
    ...
    def _initialize_default_factors(self):
        ...
        # 因子3: 操作风险 - 系统故障
        def operational_risk_calculation(
        ...
        # 因子5: 声誉风险 - 公众影响
        def reputational_risk_calculation(
            context: Dict[str, Any],
        ) -> Tuple[float, float]:
    ...
    def assess_risk(self, action: str, context: Dict[str, Any]) -> RiskAssessment:
        ...
        recommendations = self._generate_recommendations(
            factor_details, overall_risk_level
        )
    ...
    def _generate_recommendations(
        self, factor_details: List[Dict[str, Any]], overall_level: RiskLevel
    ) -> List[str]:
        """生成风险建议"""
        recommendations = []
        ...
        # 根据高风险因子生成具体建议
        high_risk_factors = [
            f for f in factor_details if f["risk_level"] in ["critical", "high"]
        ]
    ...
    def _generate_mitigation_strategies(
        self, factor_details: List[Dict[str, Any]]
    ) -> List[str]:
        """生成风险缓解策略"""
        strategies = []
        ...
        for factor in high_risk_factors:
            if factor["category"] == "safety":
                strategies.append(
                    f"针对{factor['name']}: 实施多重安全防护，增加紧急停止机制"
                )
            elif factor["category"] == "security":
    ...
    def _record_risk_assessment(self, assessment: RiskAssessment):
        ...
        with self._state_lock:
            ...
            if len(self.risk_history) > self.config["max_risk_history"]:
                self.risk_history = self.risk_history[
                    -self.config["max_risk_history"]:
                ]

    def _update_risk_trends(self, assessment: RiskAssessment):
        """更新风险趋势数据"""
        with self._state_lock:
            timestamp_key = assessment.timestamp.strftime(
                "%Y-%m-%d %H:00"
            )  # 按小时聚合
            ...
            if total_assessments > 0:
                ...
                level_counts = {
                    "critical": sum(
                        ...
                        if a.overall_risk_level == RiskLevel.CRITICAL
                    ),
                    "high": sum(
                        1
                        for a in self.risk_history
                        if a.overall_risk_level == RiskLevel.HIGH
                    ),
                    "medium": sum(
                        1
                        for a in self.risk_history
                        if a.overall_risk_level == RiskLevel.MEDIUM
                    ),
                    "low": sum(
                        1
                        for a in self.risk_history
                        if a.overall_risk_level == RiskLevel.LOW
                    ),
                    "negligible": sum(
                        1
                        for a in self.risk_history
                        if a.overall_risk_level == RiskLevel.NEGLIGIBLE
                    ),
                }
            else:
                avg_risk_score = 0.0
