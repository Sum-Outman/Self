            ...
            for rule_id, rule in self.ethics_rules.items():
                ...
                try:
                    ...
                    if rule.condition(check_context):
                        ...
                        # 更新最高伦理等级
                        if (
                            self._compare_ethics_levels(
                                rule.ethics_level, max_ethics_level
                            )
                            > 0
                        ):
    ...
    def _record_ethics_check(self, result: EthicsCheckResult):
        ...
        with self._state_lock:
            ...
            if len(self.ethics_history) > self.config["max_ethics_history"]:
                self.ethics_history = self.ethics_history[
                    -self.config["max_ethics_history"]:
                ]
    ...
    def get_ethics_status(self) -> Dict[str, Any]:
        ...
        with self._state_lock:
            ...
            return {
                ...
                "enabled_rules": sum(
                    1 for r in self.ethics_rules.values() if r.enabled
                ),
                "ethics_history_count": total_checks,
                "ethical_percentage": (
                    ethical_checks / total_checks * 100 if total_checks > 0 else 0.0
                ),
