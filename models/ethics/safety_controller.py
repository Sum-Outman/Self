            ...
            for rule_id, rule in self.safety_rules.items():
                ...
                try:
                    ...
                    if rule.condition(check_context):
                        ...
                        # 更新最高安全等级
                        if self._compare_safety_levels(rule.safety_level, max_safety_level) > 0:
                        ...
                        # 生成建议
                        recommendation = self._generate_recommendation(rule, check_context)
                        if recommendation:
    ...
    def _record_safety_check(self, result: SafetyCheckResult):
        ...
        with self._state_lock:
            ...
            if len(self.safety_history) > self.config["max_safety_history"]:
                self.safety_history = self.safety_history[-self.config["max_safety_history"]:]
            
            # 更新当前安全等级
            self.current_safety_level = result.safety_level
    
    def _generate_recommendation(self, rule: SafetyRule, context: Dict[str, Any]) -> str:
        """生成安全建议"""
    ...
    def get_safety_status(self) -> Dict[str, Any]:
        ...
        with self._state_lock:
            return {
                ...
                "enabled_rules": sum(1 for r in self.safety_rules.values() if r.enabled),
                "safety_history_count": len(self.safety_history),
                "emergency_stop_count": len(self.emergency_stop_history),
                "last_check": self.safety_history[-1].timestamp.isoformat() if self.safety_history else None
            }
