            ...
            if self.aggregation_type in ["mean", "sum"]:
                ...
                if self.aggregation_type == "mean":
                    ...
                    weighted_degrees = torch.where(
                        weighted_degrees == 0, torch.ones_like(weighted_degrees), weighted_degrees
                    )
...
class GraphAttentionLayer(nn.Module):
    ...
    def _compute_attention_scores(
        ...
        # 计算注意力分数
        # 对于每个头，计算节点i和j之间的注意力
        attention_input = torch.cat([h_i.expand(-1, num_nodes, -1, -1),
                                   h_j.expand(num_nodes, -1, -1, -1)], dim=-1)
        ...
        # 计算原始注意力分数
        raw_scores = self.a(attention_input).view(
            num_nodes, num_nodes, num_heads
        )
    ...
    def _attention_aggregation(
        ...
        # 堆叠所有头
        h_aggregated_all = torch.stack(aggregated_heads, dim=1)  # [num_nodes, num_heads, head_dim]
...
class GraphNeuralNetwork(nn.Module):
    ...
    def _create_pooling_layer(self):
        ...
        if self.config.pooling_type == "topk":
            return TopKPooling(
                ...
                ratio=self.config.pooling_ratio
            )
    ...
    def forward(
        ...
        for layer_idx, layer in enumerate(self.layers):
            ...
            # 应用池化 (在特定层)
            if (self.pooling_layer is not None and 
                layer_idx == self.config.num_layers // 2):
...
class DiffPoolLayer(nn.Module):
    ...
    def forward(
        ...
        # 计算池化后的特征
        pooled_features = assignment.T @ self.feature_net(node_features)  # [num_clusters, in_channels]
        
        # 计算池化后的邻接矩阵
        pooled_adjacency = assignment.T @ adjacency_matrix @ assignment  # [num_clusters, num_clusters]
