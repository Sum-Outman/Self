...
class GraphStructure:
    ...
    def to(self, device: torch.device) -> "GraphStructure":
        ...
        return GraphStructure(
            ...
            edge_weights=self.edge_weights.to(device) if self.edge_weights is not None else None,
...
class GraphLaplacian(nn.Module):
    ...
    def __init__(
        ...
        logger.info(f"图拉普拉斯计算器初始化完成: device={self.device}, normalization={normalization}, dtype={dtype}")
    ...
    def compute_laplacian(
        ...
        logger.debug(f"拉普拉斯矩阵计算完成: nodes={graph.num_nodes}, edges={graph.num_edges}, "
    ...
    def _compute_eigen_decomposition(
        ...
        if n > 1000 and self.use_sparse:
            ...
            try:
                ...
                # 转换回PyTorch张量
                eigenvalues = torch.from_numpy(eigenvalues_np).to(self.device).to(self.dtype)
                eigenvectors = torch.from_numpy(eigenvectors_np).to(self.device).to(self.dtype)
    ...
    def __str__(self) -> str:
        """字符串表示"""
        return (f"GraphLaplacian(device={self.device}, normalization={self.normalization}, "
