...
async def process_text(
    ...
    task: str = Form(
        "analysis", description="处理任务类型: analysis, summary, extract"
    ),
...
@router.post("/process/image", response_model=SuccessResponse)
async def process_image(
    file: UploadFile = File(..., description="图像文件"),
    task: str = Form(
        "analysis", description="处理任务类型: analysis, detect, classify, segment"
    ),
    db: Session = Depends(get_db),
    user: User = Depends(get_current_user),
):
...
@router.post("/process/audio", response_model=SuccessResponse)
async def process_audio(
    file: UploadFile = File(..., description="音频文件"),
    task: str = Form(
        "analysis", description="处理任务类型: analysis, transcribe, features"
    ),
    db: Session = Depends(get_db),
    user: User = Depends(get_current_user),
):
    ...
    try:
        ...
        if not file.content_type.startswith("audio/"):
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="文件必须是音频类型"
            )
        ...
        try:
            ...
            if hasattr(multimodal_service, "process_audio"):
                result = multimodal_service.process_audio(
                    file_content,
                    task,
                    filename=file.filename,
                    content_type=file.content_type,
                ...
                result = multimodal_service.process_audio_file(
                    file_content,
                    task,
                    filename=file.filename,
                    content_type=file.content_type,
...
@router.post("/process/video", response_model=SuccessResponse)
async def process_video(
    file: UploadFile = File(..., description="视频文件"),
    task: str = Form(
        "analysis", description="处理任务类型: analysis, detect, summarize, extract"
    ),
    db: Session = Depends(get_db),
    user: User = Depends(get_current_user),
):
    ...
    try:
        ...
        if not file.content_type.startswith("video/"):
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="文件必须是视频类型"
            )
        ...
        try:
            ...
            if hasattr(multimodal_service, "process_video"):
                result = multimodal_service.process_video(
                    file_content,
                    task,
                    filename=file.filename,
                    content_type=file.content_type,
                ...
                result = multimodal_service.process_video_file(
                    file_content,
                    task,
                    filename=file.filename,
                    content_type=file.content_type,
...
@router.post("/process/fusion", response_model=SuccessResponse)
    ...
    try:
        ...
        if image_file:
            ...
            modalities.append(
                {
                    "type": "image",
                    "content": f"图像文件: {image_file.filename if image_file else 'N/A'}, 大小: {image_size}字节",
                }
            )
        ...
        if audio_file:
            ...
            modalities.append(
                {
                    "type": "audio",
                    "content": f"音频文件: {audio_file.filename if audio_file else 'N/A'}, 大小: {audio_size}字节",
                }
            )
        ...
        if video_file:
            ...
            modalities.append(
                {
                    "type": "video",
                    "content": f"视频文件: {video_file.filename if video_file else 'N/A'}, 大小: {video_size}字节",
                }
            )
        ...
        try:
            ...
            if hasattr(multimodal_service, "fuse_modalities"):
                result = multimodal_service.fuse_modalities(
                    multimodal_inputs, fusion_type
                )
            elif hasattr(multimodal_service, "multimodal_fusion"):
                result = multimodal_service.multimodal_fusion(
                    multimodal_inputs, fusion_type
                )
            elif hasattr(multimodal_service, "process_fusion"):
                result = multimodal_service.process_fusion(
                    multimodal_inputs, fusion_type
                )
            else:
...
async def get_multimodal_capabilities(
    ...
    try:
        ...
        try:
            ...
            logger = logging.getLogger(__name__)
            logger.warning(
                f"从多模态服务获取能力信息失败: {service_error}，使用默认配置"
            )
            capabilities = _get_default_capabilities()

        return SuccessResponse.create(
            data={"capabilities": capabilities}, message="获取多模态处理能力成功"
        )
    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
def _get_default_capabilities() -> Dict[str, Any]:
    ...
    return {
        "text_processing": {
            "supported_tasks": ["analysis", "summary", "extract"],
            "max_length": 100000,
        ...
        "audio_processing": {
            ...
            "supported_tasks": ["analysis", "transcribe", "features"],
        },
        "video_processing": {
