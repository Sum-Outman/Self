...
async def learn_from_manual(
    ...
    equipment_type: str = Form("unknown", description="设备类型：printer, scanner, copier, coffee_machine, microwave, robot_arm, cnc_machine, lab_equipment, medical_device, unknown"),
    ...
    try:
        ...
        return SuccessResponse.create(
            message=f"设备 '{equipment_name}' 说明书学习完成",
            data=result
        ).dict()
...
async def get_equipment_knowledge(
    equipment_id: Optional[str] = Query(None, description="设备ID（如果为None则返回所有知识）"),
    db: Session = Depends(get_db),
    user: User = Depends(get_current_user),
):
    """获取设备知识API"""
...
async def get_operation_procedure(
    ...
    procedure_name: Optional[str] = Query(None, description="流程名称（如果为None则返回所有流程）"),
    db: Session = Depends(get_db),
    user: User = Depends(get_current_user),
):
    """获取设备操作流程API"""
...
async def execute_operation_step(
    ...
    try:
        ...
        return SuccessResponse.create(
            message=f"{mode_text}执行操作步骤 {step_number} 完成",
            data=result
        ).dict()
...
async def get_available_equipment_types(
    ...
    try:
        ...
        equipment_types = [
            ...
            {"value": "printer", "label": "打印机", "description": "打印文档和图像的设备"},
            {"value": "scanner", "label": "扫描仪", "description": "扫描纸质文档为电子文件的设备"},
            {"value": "copier", "label": "复印机", "description": "复印文档的设备"},
            {"value": "coffee_machine", "label": "咖啡机", "description": "制作咖啡的设备"},
            {"value": "microwave", "label": "微波炉", "description": "加热食物的设备"},
            {"value": "robot_arm", "label": "机器人手臂", "description": "工业或服务机器人手臂"},
            {"value": "cnc_machine", "label": "数控机床", "description": "计算机数控加工设备"},
            {"value": "lab_equipment", "label": "实验室设备", "description": "科学实验设备"},
            {"value": "medical_device", "label": "医疗设备", "description": "医疗诊断和治疗设备"}
        ]
...
async def get_available_operation_types(
    ...
    try:
        # 返回所有操作类型
        operation_types = [
            {"value": "press_button", "label": "按下按钮", "description": "按下设备按钮"},
            {"value": "turn_knob", "label": "旋转旋钮", "description": "旋转设备旋钮"},
            {"value": "slide_switch", "label": "滑动开关", "description": "滑动设备开关"},
            {"value": "open_door", "label": "打开门/盖", "description": "打开设备门或盖子"},
            {"value": "close_door", "label": "关闭门/盖", "description": "关闭设备门或盖子"},
            {"value": "insert_item", "label": "插入物品", "description": "向设备插入物品"},
            {"value": "remove_item", "label": "取出物品", "description": "从设备取出物品"},
            {"value": "adjust_setting", "label": "调整设置", "description": "调整设备设置"},
            {"value": "monitor_display", "label": "监控显示", "description": "监控设备显示屏"},
            {"value": "check_indicator", "label": "检查指示灯", "description": "检查设备指示灯状态"}
        ]
...
async def get_learning_methods(
    ...
    try:
        # 返回所有学习方法
        learning_methods = [
            {"value": "manual_learning", "label": "说明书学习", "description": "通过阅读设备说明书学习操作"},
            {"value": "demonstration_learning", "label": "示范学习", "description": "通过观察人类示范学习操作"},
            {"value": "imitation_learning", "label": "模仿学习", "description": "通过模仿人类动作学习操作"},
            {"value": "reinforcement_learning", "label": "强化学习", "description": "通过试错和反馈学习操作"}
        ]
...
async def analyze_manual(
    manual_text: str = Form(..., description="说明书文本内容"),
    analysis_type: str = Form("full", description="分析类型：full（完整）, components（部件）, procedures（流程）, safety（安全）"),
    db: Session = Depends(get_db),
    user: User = Depends(get_current_user),
):
    """分析说明书文本API（不保存学习结果）"""
    try:
        ...
        if analysis_type == "components":
            analysis_result["components_details"] = [
                ...
                {"name": "控制面板", "type": "display", "description": "显示状态和设置"},
...
async def websocket_learning_guidance(
    ...
    try:
        ...
        await websocket.send_json({
            ...
            "supported_operations": ["manual_analysis", "procedure_guidance", "step_execution", "safety_check"],
        ...
        while True:
            ...
            if message_type == "analyze_manual":
                ...
                if equipment_id and procedure_name:
                    # 获取实际流程
                    procedure_result = service.get_operation_procedure(equipment_id, procedure_name)
                    ...
                    if procedure_result.get("success", False):
                        ...
                        await websocket.send_json({
                            "type": "error",
                            "message": procedure_result.get("error", "获取流程失败")
                        })
                else:
                    await websocket.send_json({
