...
async def analyze_screen(
    screenshot_file: Optional[UploadFile] = File(None, description="屏幕截图文件（可选）"),
...
async def mouse_operation(
    action: str = Form(..., description="鼠标操作：click, double_click, right_click, drag, move, scroll"),
    ...
    try:
        # 验证操作类型
        valid_actions = ["click", "double_click", "right_click", "drag", "move", "scroll"]
...
async def translate_natural_language(
    natural_language: str = Form(..., description="自然语言描述，如'列出当前目录的文件'"),
    db: Session = Depends(get_db),
    user: User = Depends(get_current_user),
):
    """自然语言到命令行翻译API"""
...
async def set_operation_mode(
    mode: str = Form(..., description="操作模式：physical_robot, virtual_control, command_line"),
    db: Session = Depends(get_db),
    user: User = Depends(get_current_user),
):
    """设置操作模式API"""
...
async def websocket_real_time_operation(
    ...
    try:
        ...
        await websocket.send_json({
            ...
            "supported_operations": ["keyboard", "mouse", "command", "screen_analysis"],
