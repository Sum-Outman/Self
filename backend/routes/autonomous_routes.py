    ...
    try:
        if request.target_mode.lower() == "autonomous":
            ...
            if manager.running:
                ...
                return {"success": True, "message": "自主模式已在运行中", "current_mode": "autonomous"}
            ...
            if not manager.running:
                ...
                return {"success": True, "message": "系统未在自主模式中运行", "current_mode": "task"}
...
@router.get("/decision/statistics", response_model=DecisionStatisticsResponse, summary="获取决策统计")
...
async def mode_health_check(
    ...
    try:
        ...
        manager_status = {
            ...
            "manager_running": manager.running if hasattr(manager, 'running') else False,
            "current_state": manager.current_state.name if hasattr(manager, 'current_state') else "unknown",
        }
        
        # 检查决策引擎
        decision_engine_status = {
            "decision_engine_available": DECISION_ENGINE_AVAILABLE,
            "memory_size": len(decision_engine.memory) if hasattr(decision_engine, 'memory') else 0,
            "decision_history_size": len(decision_engine.decision_history) if hasattr(decision_engine, 'decision_history') else 0,
        }
        
        # 综合健康状态
        healthy = (
            AUTONOMOUS_MODE_AVAILABLE and 
            DECISION_ENGINE_AVAILABLE and
            (not manager.running or manager.current_state != AutonomousState.ERROR)
        )
...
async def start_autonomous_running(
    manager: Any = Depends(get_autonomous_manager)
):
    """启动自主模式运行"""
    try:
...
async def stop_autonomous_running(
    manager: Any = Depends(get_autonomous_manager)
):
    """停止自主模式运行"""
    try:
...
async def export_autonomous_data(
    manager: Any = Depends(get_autonomous_manager)
):
    """导出自主模式数据"""
    try:
...
async def startup_event():
    ...
    if AUTONOMOUS_MODE_AVAILABLE:
        try:
            manager = get_autonomous_mode_manager()
