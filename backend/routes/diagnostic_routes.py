...
async def get_diagnostic_service_status(
    ...
    try:
        ...
        running_count = sum(
            ...
            if report.get("status") == DiagnosticStatus.RUNNING.value
        )
        completed_count = sum(
            1
            for report in history
            if report.get("status") == DiagnosticStatus.COMPLETED.value
        )
        failed_count = sum(
            1
            for report in history
            if report.get("status") == DiagnosticStatus.FAILED.value
        )

        # 检查是否正在运行诊断
        diagnostic_service_instance = get_system_diagnostic_service()
        diagnosis_in_progress = getattr(
            diagnostic_service_instance, "diagnosis_in_progress", False
        )
        ...
        return {
            ...
            "status": {
                ...
                "auto_diagnosis_enabled": getattr(
                    diagnostic_service_instance, "running", False
                ),
...
async def start_auto_diagnosis(
    ...
    try:
        ...
        return {
            ...
            "message": "自动诊断已启动",
            "timestamp": datetime.now(timezone.utc).isoformat(),
            "interval_seconds": diagnostic_service.config.get(
                "diagnosis_interval", 300.0
            ),
        }
...
async def export_diagnostic_report_html(
    ...
    try:
        ...
        # 导出为HTML
        diagnostic_service.export_report_to_html(report, filepath)
...
async def export_diagnostic_report_text(
    ...
    try:
        ...
        # 导出为文本
        diagnostic_service.export_report_to_text(report, filepath)
...
async def download_diagnostic_report(
    ...
    try:
        ...
        if not os.path.exists(filepath):
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail=f"文件不存在: {filename}"
            )
