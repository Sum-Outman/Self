...
async def understand_concept(
    ...
    taste_sensor_data_json: Optional[str] = Form(
        None, description="味觉传感器数据JSON数组：[甜度, 酸度, 苦度, 咸度, 鲜味]"
    ),
    spatial_3d_data_json: Optional[str] = Form(
        None, description="3D空间数据JSON数组：[x,y,z坐标...]"
    ),
    ...
    scenario_type: Optional[str] = Form(
        None,
        description="场景类型：concept_understanding, computer_operation, equipment_learning, visual_imitation, teaching",
    ),
    user_intent: Optional[str] = Form(None, description="用户意图描述"),
    target_device: Optional[str] = Form(None, description="目标设备（用于设备学习）"),
    action_to_imitate: Optional[str] = Form(
        None, description="要模仿的动作（用于视觉模仿）"
    ),
    ...
    try:
        ...
        if taste_sensor_data_json:
            try:
                ...
                if isinstance(taste_data, list) and len(taste_data) == 5:
                    ...
                    logger.warning(
                        f"味觉传感器数据格式无效，期望长度为5的列表，实际: {taste_data}"
                    )
            except Exception as e:
                logger.warning(
                    f"味觉传感器数据JSON解析错误: {e}, 原始数据: {taste_sensor_data_json}"
                )
                # 跳过无效的味觉数据
        ...
        if spatial_3d_data_json:
            try:
                ...
                if isinstance(spatial_data, list):
                    ...
                    logger.warning(
                        f"3D空间数据格式无效，期望列表，实际: {spatial_data}"
                    )
            except Exception as e:
                logger.warning(
                    f"3D空间数据JSON解析错误: {e}, 原始数据: {spatial_3d_data_json}"
                )
                # 跳过无效的空间数据
        ...
        if sensor_data_json:
            try:
                ...
                logger.warning(
                    f"传感器数据JSON解析错误: {e}, 原始数据: {sensor_data_json}"
                )
                # 跳过无效的传感器数据
...
async def start_robot_teaching(
    ...
    teaching_method: str = Form(
        "实物教学", description="教学方法：实物教学、概念教学、交互教学"
    ),
    ...
    try:
        ...
        try:
            ...
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="多模态输入数据格式错误"
            )
        ...
        response_data = {
            ...
            "teaching_session": {
                ...
                "concept_learned": result.concept_understanding.get(
                    "object_name", object_name
                ),
    ...
    try:
        ...
        response_data = {
            ...
            "apple_cognition": {
                "text_understanding": {
                    ...
                    "understood_meaning": concept_info.get(
                        "understood_meaning", f"{quantity}个水果苹果"
                    ),
                    "semantic_analysis": {
                        "object_type": concept_attrs.get("object_type", "水果"),
                        "category": concept_info.get("category", "蔷薇科苹果属"),
                        "common_names": concept_info.get(
                            "common_names", ["苹果", "Apple"]
                        ),
                        "characteristics": concept_info.get(
                            "characteristics", ["可食用", "圆形", "通常红色或绿色"]
                        ),
                    },
                },
                "pronunciation": {
                    "chinese_pronunciation": concept_info.get(
                        "chinese_pronunciation", "píng guǒ"
                    ),
                    "english_pronunciation": concept_info.get(
                        "english_pronunciation", "apple"
                    ),
                    "phonetic_features": concept_info.get(
                        "phonetic_features", ["双音节", "第二声+第三声"]
                    ),
                },
                "visual_recognition": {
                    "shape": spatial_shape,
                ...
                "taste_perception": {
                    ...
                    "taste_profile": concept_info.get(
                        "taste_profile", "甜中带酸，典型苹果味"
                    ),
                },
                "spatial_perception": {
                    ...
                    "spatial_understanding": concept_info.get(
                        "spatial_understanding", "三维物体，可握在手中"
                    ),
                },
                "object_identification": {
                    ...
                    "alternative_interpretations": concept_info.get(
            ...
            "multimodal_integration": {
                ...
                "unified_representation": (
                    result.unified_representation[:10]
        ...
        if teaching_mode:
            ...
            concept_mastery = (
                ...
                else "基本掌握" if result.confidence > 0.6 else "学习中"
            )
            ...
            if result.confidence > 0.7:
                ...
                robot_response += (
                    f"味道{concept_info.get('taste_profile', '甜中带酸')}。"
                )
            else:
