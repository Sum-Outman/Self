...
# 导入真实传感器读取器
try:
    from backend.services.real_sensor_reader import get_real_sensor_reader, RealSensorReader
...
@router.get("/sensor-data", response_model=SuccessResponse[List[Dict[str, Any]]])
...
@router.get("/metrics", response_model=SuccessResponse[List[Dict[str, Any]]])
...
@router.post("/motor/command", response_model=SuccessResponse[Dict[str, Any]])
async def send_motor_command(
    ...
    try:
        ...
        try:
            ...
            if not hasattr(hardware_service, '_hardware_manager') or hardware_service._hardware_manager is None:
                raise HTTPException(
                    status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
                    detail="硬件管理器未初始化"
                )
            ...
            response = {
                ...
                "message": result.get("message", f"电机 {motor_id} 命令 '{motor_command}' 已执行"),
...
@router.get("/serial/ports", response_model=SuccessResponse[Dict[str, Any]])
...
@router.post("/serial/command", response_model=SuccessResponse[Dict[str, Any]])
async def send_serial_command(
    ...
    try:
        ...
        protocol_enum = SerialProtocol(protocol) if protocol in [p.value for p in SerialProtocol] else SerialProtocol.RAW
...
@router.post("/serial/receive", response_model=SuccessResponse[Dict[str, Any]])
async def receive_serial_data(
    ...
    try:
        ...
        if not SERIAL_DATA_SERVICE_AVAILABLE or not receive_serial_raw_data:
            # 回退到简单解码
            from backend.services.serial_decoder import get_serial_decoder, SerialProtocol
            
            decoder = get_serial_decoder()
            protocol = SerialProtocol(protocol_hint) if protocol_hint in [p.value for p in SerialProtocol] else None
...
@router.get("/sensors/data", response_model=SuccessResponse[Dict[str, Any]])
...
@router.post("/motors/command", response_model=SuccessResponse[Dict[str, Any]])
...
@router.get("/system/metrics", response_model=SuccessResponse[Dict[str, Any]])
...
class WebSocketConnectionManager:
    ...
    def add_sensor_data(self, sensor_id: str, data: Dict[str, Any]):
        ...
        if len(self.sensor_data_history[sensor_id]) > 1000:
            self.sensor_data_history[sensor_id] = self.sensor_data_history[sensor_id][-1000:]
        ...
        while True:
            ...
            if data.get("type") == "subscribe":
                ...
                try:
                    ...
                    if action == "motor_control" and hasattr(hardware_service._hardware_manager, 'send_motor_command'):
                        # 电机控制
                        control_result = hardware_service._hardware_manager.send_motor_command(
                            motor_id=device_id,
                            command=params.get("motor_command", "move"),
                            parameters=params
                        )
                    elif hasattr(hardware_service._hardware_manager, 'send_device_command'):
                        # 通用设备控制
                        control_result = hardware_service._hardware_manager.send_device_command(
                            device_id=device_id,
                            command=action,
                            parameters=params
                        )
                    ...
                    response = {
                        ...
                        "device_id": device_id,
                        "action": action,
                        "status": control_result.get("status", "executed") if control_result else "executed",
                        "message": control_result.get("message", f"设备 {device_id} 命令 '{action}' 已执行") if control_result else f"设备 {device_id} 命令 '{action}' 已执行",
...
async def generate_sensor_data():
    ...
    if REAL_SENSOR_READER_AVAILABLE and get_real_sensor_reader:
        try:
            ...
            print(f"传感器读取器初始化: 真实传感器{'可用' if sensor_reader.has_real_sensors() else '不可用'}")
        except Exception as e:
                ...
                try:
                    ...
                    if real_sensor_data:
                        for sensor_item in real_sensor_data:
                            ...
                            sensor_data = {
                                "type": "sensor_data",
                                "sensor_id": sensor_item.get("sensor_id", "unknown_001"),
                                "sensor_type": sensor_item.get("sensor_type", "unknown"),
                            ...
                            # 添加到历史记录
                            websocket_manager.add_sensor_data(sensor_data["sensor_id"], sensor_data)
...
@router.post("/shutdown", response_model=SuccessResponse[Dict[str, Any]])
@api_error_handler
async def shutdown_hardware(
    ...
    if not hardware_initialized:
        return SuccessResponse.create(
            data={"hardware_initialized": False},
            message="硬件组件未初始化，无需关闭"
        )
...
@router.get("/initialization-status", response_model=SuccessResponse[Dict[str, Any]])
@api_error_handler
async def get_hardware_initialization_status(
    ...
    return SuccessResponse.create(
        data={
            "hardware_initialized": hardware_initialized if HARDWARE_INIT_FUNCTIONS_AVAILABLE else False,
