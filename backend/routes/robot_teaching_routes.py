...
from backend.services.robot_teaching_service import (
...
async def teach_concept(
    ...
    teaching_method: str = Form(
        "实物教学", description="教学方法：实物教学、概念教学、交互教学、视频教学"
    ),
    ...
    try:
        ...
        return SuccessResponse.create(
            message=f"概念 '{concept_name}' 教学成功", data=result
        ).dict()
...
async def test_understanding(
    ...
    test_type: str = Form(
        "综合测试", description="测试类型：综合测试、模态测试、应用测试、快速测试"
    ),
    db: Session = Depends(get_db),
    user: User = Depends(get_current_user),
):
    ...
    try:
        ...
        result = teaching_system.test_understanding(
            concept_name=concept_name, test_type=test_type
        )
        ...
        return SuccessResponse.create(
            message=f"概念 '{concept_name}' 测试完成", data=result
        ).dict()
...
async def correct_misconception(
    ...
    try:
        ...
        result = teaching_system.correct_misconception(
            concept_name=concept_name, correction=correction
        )
        ...
        return SuccessResponse.create(
            message=f"概念 '{concept_name}' 误解纠正成功", data=result
        ).dict()
...
async def get_learning_progress(
    ...
    try:
        ...
        return SuccessResponse.create(message="学习进度获取成功", data=result).dict()
...
async def start_interactive_teaching(
    ...
    try:
        ...
        result = teaching_system.start_interactive_teaching(
            concept_name=concept_name, teacher_id=teacher_id
        )
        ...
        return SuccessResponse.create(
            message=f"开始交互式教学: {concept_name}", data=result
        ).dict()
...
async def process_interactive_response(
    ...
    try:
        ...
        return SuccessResponse.create(message="交互响应处理成功", data=result).dict()
        ...
        return SuccessResponse.create(
            ...
            data={
                ...
                "statistics": {
                    ...
                    "average_mastery": sum(c["mastery"] for c in concepts_data)
                    / len(concepts_data),
        ...
        return SuccessResponse.create(
            ...
            data={
                ...
                "statistics": {
                    ...
                    "success_rate": (
                        sum(1 for s in filtered_sessions if s["session_success"])
                        / len(filtered_sessions)
                        if filtered_sessions
                        else 0
                    ),
                    "average_duration": (
                        sum(s["duration_minutes"] for s in filtered_sessions)
                        / len(filtered_sessions)
                        if filtered_sessions
                        else 0
                    ),
...
async def websocket_interactive_teaching(
    ...
    try:
        ...
        if not session_id.startswith("interactive_"):
            await websocket.send_json({"type": "error", "message": "无效的会话ID"})
        ...
        while True:
            ...
            if message_type == "start_teaching":
                ...
                if not concept_name:
                    await websocket.send_json(
                        {"type": "error", "message": "需要概念名称"}
                    )
                    continue

                # 开始交互式教学
                result = teaching_system.start_interactive_teaching(
                    concept_name=concept_name, teacher_id="websocket_teacher"
                )

                await websocket.send_json({"type": "teaching_started", "data": result})
                ...
                await websocket.send_json({"type": "teacher_response", "data": result})
