...
async def get_speech_status():
    """获取语音功能状态"""
    "available" if SPEECH_RECOGNITION_AVAILABLE else "unavailable"
    "available" if SPEECH_SYNTHESIS_AVAILABLE else "unavailable"
    ...
    return {
        ...
        "overall_status": (
            "fully_operational"
            if (recognition_running and synthesis_running)
            else (
                "partially_operational"
                if (recognition_running or synthesis_running)
                else "unavailable"
            )
        ),
    }
...
async def recognize_speech(
    ...
    try:
        ...
        if file_extension == "wav":
            ...
            # 假设是16位单声道PCM
            audio_array = (
                np.frombuffer(audio_content, dtype=np.int16).astype(np.float32)
                / 32768.0
            )
            ...
            recognized_text = recognition_service.recognize_audio(
                audio_array, target_sample_rate
            )
...
async def recognize_speech_base64(request: Dict[str, Any]):
    """
    通过Base64编码识别语音
...
async def synthesize_speech(
    ...
    try:
        ...
        try:
            ...
            raise HTTPException(
                status_code=501, detail="语音合成服务未实现synthesize方法，无法合成语音"
            )
        ...
        if output_format == "wav":
            ...
            raise HTTPException(
                status_code=400, detail=f"不支持的输出格式: {output_format}"
            )
...
async def synthesize_speech_base64(request: Dict[str, Any]):
    """
    语音合成Base64 API
...
async def speech_conversation(request: Dict[str, Any]):
    """
    语音对话API
...
class SpeechServiceManager:
    ...
    def conversation(
        self, audio_input: np.ndarray, context: Dict[str, Any] = None
    ) -> Dict[str, Any]:
        """完整对话流程"""
        # 识别
