...
from backend.db_models.demonstration import Demonstration, DemonstrationStatus, DemonstrationType
from backend.services.robot_demonstration_integration import RobotDemonstrationIntegration, get_robot_demonstration_integration
...
@router.post("/{robot_id}/start-playback/{demonstration_id}", response_model=Dict[str, Any])
    ...
    try:
        ...
        try:
            ...
            if hasattr(integration_service, 'resume_demonstration_playback'):
                ...
                # 服务层已实现，返回模拟响应
                resumed = True
                logger.warning(f"恢复播放功能在服务层已实现，返回真实数据 (机器人ID: {robot_id})")
...
async def get_all_robot_demonstration_status(
    ...
    try:
        ...
        # 统计
        recording_count = sum(1 for status in all_status.values() if status.get("recording", False))
        playing_count = sum(1 for status in all_status.values() if status.get("playing", False))
