    ...
    try:
        ...
        if AGI_SERVICE_AVAILABLE:
            ...
            try:
                ...
                status_data = await asyncio.wait_for(
                    asyncio.to_thread(agi_service.get_status),
                    timeout=5.0
                )
        ...
        if SYSTEM_SERVICE_AVAILABLE:
            ...
            if system_status and "hardware" in system_status:
                status_data["hardwareConnected"] = system_status["hardware"].get("connected", False)
...
@router.post("/training/start", response_model=SuccessResponse[Dict[str, Any]])
...
@router.get("/training/progress", response_model=SuccessResponse[Dict[str, Any]])
...
@router.get("/logs", response_model=SuccessResponse[List[str]])
