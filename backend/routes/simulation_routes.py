...
async def get_simulation_interfaces(
    interface_name: Optional[str] = Query(None, description="特定接口名称")
):
    """获取仿真接口状态信息"""
    try:
        simulation_service = get_simulation_service()
...
async def execute_simulation_command(
    ...
    interface: Optional[str] = Query(
        None, description="指定仿真接口，默认为当前活动接口"
    ),
):
    """执行仿真环境命令"""
    try:
...
def _get_capability_recommendations(capabilities: Dict[str, Any]) -> List[str]:
    ...
    if not interfaces_available:
        recommendations.append(
            "未检测到可用的仿真接口，请安装PyBullet或启动Gazebo/ROS服务"
        )
        recommendations.append("PyBullet安装: pip install pybullet")
        recommendations.append(
            "Gazebo/ROS配置: 启动ROS服务 (roslaunch gazebo_ros empty_world.launch)"
        )
...
async def test_simulation_api():
    ...
    try:
        ...
        service_info = simulation_service.get_service_info()
        capabilities = simulation_service.get_simulation_capabilities()
        simulation_service.get_interface_status()
        ...
        test_results = {
            ...
            "capabilities_available": len(capabilities.get("interfaces_available", []))
            > 0,
            "timestamp": datetime.now(timezone.utc).isoformat(),
        }
        ...
        all_tests_passed = (
            ...
            and test_results["capabilities_available"]
        )
        ...
        return {
            ...
            "message": (
                "仿真环境API测试完成" if all_tests_passed else "仿真环境API测试失败"
            ),
            "recommendations": _get_capability_recommendations(capabilities),
            "timestamp": datetime.now(timezone.utc).isoformat(),
        }
