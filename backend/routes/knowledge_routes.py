...
async def get_tags(
    ...
    try:
        # 查询所有知识项中的标签
        items = (
            db.query(KnowledgeItem).filter(KnowledgeItem.uploaded_by == user.id).all()
        )
        ...
        return SuccessResponse.create(
            data={"tags": list(tags_set)}, message="获取标签列表成功"
        )
...
@router.get("/types/stats", response_model=SuccessResponse)
async def get_type_stats(
    ...
    try:
        ...
        return SuccessResponse.create(
            data={"type_stats": type_stats}, message="获取知识项类型统计成功"
        )
    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def get_knowledge_items(
    ...
    """获取知识项列表"""
    try:
        query = db.query(KnowledgeItem).filter(KnowledgeItem.uploaded_by == user.id)
        ...
        if tags:
            ...
            # 完整处理，实际可能需要更复杂的标签查询
            if tag_list:
                query = query.filter(KnowledgeItem.tags.contains(json.dumps(tag_list)))
            ...
            item_responses.append(
                {
                    ...
                    "size": item.size or 0,
                    "upload_date": (
                        item.upload_date.isoformat() if item.upload_date else ""
                    ),
                    "tags": item_tags,
...
async def search_knowledge(
    ...
    try:
        ...
        # 构建查询
        db_query = db.query(KnowledgeItem).filter(KnowledgeItem.uploaded_by == user.id)
        ...
        if tags and len(tags) > 0:
            db_query = db_query.filter(KnowledgeItem.tags.contains(json.dumps(tags)))
...
async def get_knowledge_item(
    ...
    try:
        ...
        if not item:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="知识项不存在"
            )
...
@router.post("/items/upload", response_model=SuccessResponse)
        ...
        return SuccessResponse.create(
            data={
                "item": {
                    ...
                    "upload_date": (
                        knowledge_item.upload_date.isoformat()
                        if knowledge_item.upload_date
                        else ""
                    ),
                    "tags": tag_list,
...
@router.get("/items/{item_id}/similar", response_model=SuccessResponse)
async def get_similar_items(
    ...
    try:
        ...
        similar_items = (
            ...
            .order_by(KnowledgeItem.upload_date.desc())
            .limit(limit)
            .all()
        )
...
@router.post("/items/{item_id}/access", response_model=SuccessResponse)
async def record_item_access(
    ...
    try:
        ...
        return SuccessResponse.create(data={}, message="访问记录成功")
    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(
...
async def delete_knowledge_item(
    ...
    try:
        ...
        return SuccessResponse.create(data={}, message="知识项删除成功")
    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(
...
# 管理员知识库管理功能
@router.get("/admin/items", response_model=SuccessResponse)
...
@router.get("/admin/items/{item_id}", response_model=SuccessResponse)
...
@router.delete("/admin/items/{item_id}", response_model=SuccessResponse)
async def admin_delete_knowledge_item(
    ...
    try:
        ...
        return SuccessResponse.create(data={}, message="管理员删除知识项成功")
    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(
...
# 批量操作
@router.post("/items/batch-delete", response_model=SuccessResponse)
async def batch_delete_knowledge_items(
    ...
    try:
        ...
        if not items_to_delete:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="未找到要删除的知识项"
            )
...
@router.post("/items/batch-update", response_model=SuccessResponse)
async def batch_update_knowledge_items(
    ...
    try:
        ...
        if not items_to_update:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="未找到要更新的知识项"
            )
                ...
                for item_data in import_data:
                    ...
                    knowledge_item = KnowledgeItem(
                        ...
                        file_path=item_data.get(
                            "file_path", f"imported_{imported_count + 1}"
                        ),
...
@router.get("/graph", response_model=SuccessResponse)
async def get_knowledge_graph(
    center_item_id: Optional[str] = Query(
        None, alias="center", description="中心知识项ID"
    ),
