...
from backend.services.robot_configuration_service import RobotConfigurationService, URDFParser
...
def get_configuration_service(db: Session = Depends(get_db)) -> RobotConfigurationService:
    """获取机器人配置服务"""
...
async def import_urdf(
    ...
    robot_name: Optional[str] = Form(None, description="机器人名称（可选，默认使用URDF中的名称）"),
...
async def import_config_file(
    ...
    try:
        ...
        if not (filename.endswith(('.json', '.jsonc', '.yaml', '.yml')) or format != "auto"):
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="仅支持JSON或YAML文件"
            )
...
async def export_urdf(
    ...
    try:
        # 验证用户权限
        robot = service.db.query(Robot).filter(
            Robot.id == robot_id,
