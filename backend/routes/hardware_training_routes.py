...
async def create_training_task(
    ...
    try:
        ...
        task = TrainingTask(
            ...
            phase_sequence=[TrainingPhase(phase) for phase in task_data.get("phase_sequence", [
                "initialization",
                "calibration",
                "warmup",
                "training",
            ...
            expected_duration=timedelta(seconds=task_data.get("expected_duration", 3600)),
...
async def websocket_progress(
    ...
    try:
        ...
        # 注册进度回调
        async def progress_callback(task_id_cb: str, phase: TrainingPhase, event: str, args: tuple):
        ...
        # 注册阶段完成回调
        async def phase_complete_callback(task_id_cb: str, phase: TrainingPhase, success: bool, result: Dict[str, Any]):
            if task_id_cb == task_id:
                await websocket.send_json({
                    "type": "phase_complete",
                    "task_id": task_id_cb,
