...
async def create_robot_joint(
    ...
    joint_type: str = Query("revolute", description="关节类型: revolute, prismatic, fixed"),
    ...
    try:
        ...
        # 检查关节是否已存在
        existing = db.query(RobotJoint).filter(
            RobotJoint.robot_id == robot_id,
            RobotJoint.name == name
        ).first()
        ...
        # 更新机器人关节计数
        robot.joint_count = db.query(RobotJoint).filter(RobotJoint.robot_id == robot_id).count()
