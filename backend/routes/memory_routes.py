    ...
    try:
        ...
        if ms is None:
            ...
            memory_stats = MemoryStats(
                ...
                average_retrieval_time_ms=stats_data.get(
                    "average_retrieval_time_ms", 0
                ),
        ...
        return SuccessResponse.create(
            data={"stats": memory_stats.dict(), "system_initialized": ms is not None},
...
async def search_memories(
    ...
    try:
        ...
        memories = ms.retrieve_relevant_memories(
            ...
            similarity_threshold=search_request.min_similarity
            or ms.similarity_threshold,
            ...
            memory_items.append(
                MemoryItem(
                    ...
                    created_at=datetime.fromtimestamp(
                        mem.get("created_at", time.time())
                    ),
                    accessed_at=datetime.fromtimestamp(
                        mem.get("accessed_at", time.time())
                    ),
...
@router.post("/knowledge/search", response_model=SuccessResponse)
...
@router.post("/hybrid/search", response_model=SuccessResponse)
            ...
            results.append(
                {
                    ...
                    "similarity_score": result.get(
                        "similarity_score", result.get("weighted_score", 0.5)
                    ),
    ...
    try:
        ...
        if not ms.initialized if hasattr(ms, "initialized") else False:
            ...
            health_status = {
                ...
                "features": {
                    "autonomous_memory": (
                        ms.enable_autonomous_memory
                        if hasattr(ms, "enable_autonomous_memory")
                        else False
                    ),
                    "scene_classification": (
                        ms.enable_scene_classification
                        if hasattr(ms, "enable_scene_classification")
                        else False
                    ),
                    "knowledge_integration": (
                        ms.enable_knowledge_base
                        if hasattr(ms, "enable_knowledge_base")
                        else False
                    ),
                    "multimodal_memory": (
                        ms.enable_multimodal_memory
                        if hasattr(ms, "enable_multimodal_memory")
                        else False
                    ),
                },
            }
        ...
        return SuccessResponse.create(
            data={"health": health_status}, message="获取记忆系统健康状态成功"
        )
    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
