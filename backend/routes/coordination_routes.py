...
async def create_cooperative_task(
    ...
    coordination_mode: CoordinationMode = Body(
        CoordinationMode.CENTRALIZED, description="协同模式"
    ),
...
async def add_robot_to_task(
    ...
    try:
        success, message = coordinator.add_robot_to_task(
            task_id=task_id, robot_id=robot_id, robot_name=robot_name, role=role
        )
...
async def assign_formation(
    ...
    try:
        success, message = coordinator.assign_formation(
            task_id=task_id, formation_type=formation_type, spacing=spacing
        )
...
async def assign_robot_role(
    ...
    try:
        success, message = coordinator.assign_robot_role(
            task_id=task_id, robot_id=robot_id, role=role
        )
...
async def get_all_tasks(
    ...
    try:
        ...
        return {"success": True, "tasks": task_list, "total_count": len(task_list)}
...
async def get_task_details(
    ...
    try:
                ...
                robot_states.append(
                    {
                        ...
                        "status": state.status,
                        "role": task.robot_roles.get(
                            robot_id, RobotRole.FOLLOWER
                        ).value,
                    }
                )
        ...
        return {
            ...
            "formation_type": (
                task.formation.formation_type.value if task.formation else None
            ),
        }
        ...
        formations = [
            {
                "type": "line",
                "name": "直线队形",
                "description": "机器人排列在一条直线上",
            },
            {"type": "column", "name": "纵队队形", "description": "机器人排列在一列上"},
            {
                "type": "v_shape",
                "name": "V字形队形",
                "description": "机器人排列成V字形",
            },
