...
async def start_learning_session(
    learning_type: str = Query(..., description="学习类型: demonstration, reinforcement, imitation"),
...
async def set_behavior_mode(
    mode: str = Query(..., description="行为模式: manual, autonomous, learning, adaptive, safety"),
    user: User = Depends(get_current_user),
):
    """设置机器人行为模式
...
async def get_multimodal_health(
    ...
    try:
        ...
        health_status = {
            ...
            "robot_control_available": status_info.get("robot_control_available", False),
            ...
            "overall_status": "healthy" if status_info.get("service_status") == "running" else "degraded",
            "timestamp": datetime.now(timezone.utc).isoformat(),
        }
