...
async def create_market_listing(
    ...
    category: RobotMarketCategory = Body(RobotMarketCategory.CUSTOM, description="分类"),
...
async def search_market_listings(
    ...
    sort_by: str = Query("relevance", description="排序字段 (relevance, rating, downloads, views, date)"),
...
async def get_popular_listings(
    ...
    """获取热门机器人市场列表"""
    try:
        listings = service.get_popular_listings(
            limit=limit,
            category=category
        )
