...
def generate_intelligent_response(message: str) -> str:
    ...
    try:
        ...
        # 检查响应是否包含错误信息
        if "错误" in response_text or "失败" in response_text or "出错" in response_text:
            # 模型生成失败，返回详细错误信息
            raise HTTPException(
                status_code=500,
                detail=f"模型生成失败: {response_text}"
            )
        ...
        raise HTTPException(
            status_code=500,
            detail=f"模型推理过程出错: {str(e)}"
        )


@router.get("/chat/models", response_model=SuccessResponse[Dict[str, Any]])
...
@router.get("/chat/mode", response_model=SuccessResponse[Dict[str, Any]])
...
@router.put("/chat/mode", response_model=SuccessResponse[Dict[str, Any]])
...
@router.post("/models/{model_name}/chat", response_model=SuccessResponse[Dict[str, Any]])
    ...
    try:
        ...
        return SuccessResponse.create(
            data={
                ...
                "tokens_used": result.get("tokens_used", len(message) // 4 + len(result.get("response", "")) // 4),
...
# 完整聊天系统功能
@router.post("/chat/sessions", response_model=SuccessResponse[Dict[str, Any]])
async def create_chat_session(
    ...
    """创建新聊天会话"""
    try:
        title = session_request.get("title", f"新会话 {datetime.now().strftime('%Y-%m-%d %H:%M')}")
...
@router.get("/chat/sessions/{session_id}", response_model=SuccessResponse[Dict[str, Any]])
...
@router.put("/chat/sessions/{session_id}", response_model=SuccessResponse[Dict[str, Any]])
...
@router.delete("/chat/sessions/{session_id}", response_model=SuccessResponse[Dict[str, Any]])
...
@router.get("/chat/sessions/{session_id}/messages", response_model=SuccessResponse[Dict[str, Any]])
...
@router.post("/chat/sessions/{session_id}/messages", response_model=SuccessResponse[Dict[str, Any]])
async def send_chat_message(
    ...
    try:
        ...
        ai_response = {
            ...
            "role": "assistant",
            "content": response_text,
            "timestamp": ai_msg.created_at.isoformat() if ai_msg.created_at else datetime.now(timezone.utc).isoformat(),
            "model_name": model_name,
...
@router.delete("/chat/sessions/{session_id}/messages/{message_id}", response_model=SuccessResponse[Dict[str, Any]])
...
@router.get("/chat/models/{model_name}/info", response_model=SuccessResponse[Dict[str, Any]])
...
async def get_chat_history(
    ...
    try:
        ...
        for session in db_sessions:
            ...
            last_message_text = ""
            if last_message:
                last_message_text = last_message.content[:100] + "..." if len(last_message.content) > 100 else last_message.content
            ...
            session_data = {
                ...
                "created_at": session.created_at.isoformat() if session.created_at else "2026-01-01T00:00:00",
...
@router.post("/uploads", response_model=SuccessResponse[Dict[str, Any]])
        ...
        while True:
            ...
            try:
                data = await asyncio.wait_for(
                    ...
                    timeout=Config.WEBSOCKET_PING_TIMEOUT
                )
            ...
            if message_type == "ping":
                ...
                try:
                    ...
                    # 发送AI响应
                    ai_response = result.get("response", f"我收到你的消息了：\"{message_content}\"。这是一个实时WebSocket响应。")
                    ...
                    await websocket.send_json({
                        ...
                        "model_name": model_name,
                        "processing_time": result.get("processing_time", 0.5),
                        "tokens_used": result.get("tokens_used", len(message_content) // 4 + len(ai_response) // 4),
                        "memories_retrieved": result.get("memories_retrieved", 0),
                        "timestamp": datetime.now(timezone.utc).isoformat()
                    })
