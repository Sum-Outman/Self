...
try:
    ...
    print(f"警告: 无法导入数据库优化模块: {e}")
    # 创建虚拟函数以避免导入错误
    def get_query_optimizer(db):
        raise ImportError("数据库优化模块不可用")
...
class TableAnalysisRequest(BaseModel):
    ...
    analyze_data_distribution: bool = Field(default=False, description="是否分析数据分布")
...
async def database_health_check(
    ...
    if not DATABASE_OPTIMIZATION_AVAILABLE:
        return DatabaseHealthResponse(
            ...
            suggestions=["数据库优化模块不可用，请检查依赖安装"]
        )
    ...
    try:
        ...
        try:
            ...
            try:
                logger.debug("尝试方法2: db.execute('SELECT COUNT(*) FROM sqlite_master')")
                    ...
                    try:
                        ...
                        if "sqlite" in Config.DATABASE_URL.lower():
                            ...
                            if os.path.exists(db_path):
                                logger.info(f"SQLite文件存在: {db_path}, 大小: {os.path.getsize(db_path)} 字节")
                                try:
                                    # 尝试直接使用sqlite3模块连接
                                    import sqlite3
                                    ...
                                    conn_sqlite.close()
                                    connection_status = True
                                    logger.info("数据库连接测试成功 (方法5: 直接sqlite3连接)")
                                except Exception as e5:
                                    logger.error(f"方法5失败 (直接sqlite3连接): {e5}")
                                    connection_error = f"所有方法都失败: {e1}, {e2}, {e3}, {e4}, {e5}"
                            else:
                                logger.error(f"SQLite文件不存在: {db_path}")
                                connection_error = f"SQLite文件不存在: {db_path}"
                        else:
                            connection_error = f"所有数据库连接测试方法都失败: {e1}, {e2}, {e3}, {e4}"
        ...
        try:
            if "sqlite" in Config.DATABASE_URL.lower():
                ...
                try:
                    query = text("SELECT COUNT(*) FROM sqlite_master WHERE type='table'")
                    ...
                    try:
                        ...
                        try:
                            conn = db.connection()
                            cursor = conn.execute(text("SELECT COUNT(*) FROM sqlite_master WHERE type='table'"))
                            row = cursor.fetchone()
                ...
                # PostgreSQL/MySQL
                query = text("SELECT COUNT(*) FROM information_schema.tables WHERE table_schema NOT IN ('pg_catalog', 'information_schema')")
                result = db.execute(query)
        ...
        status_value = "healthy" if connection_status and table_count > 0 else "unhealthy"
        if suggestions:
    ...
    try:
        ...
        if request.analyze_only:
            ...
            return QueryOptimizationResponse(
                ...
                optimization_rules=optimization_rules,
                estimated_improvement_percent=optimization_applied and 10.0 or 0.0,  # 估计值
                execution_plan=None,
                suggestions=suggestions,
...
async def get_slow_queries(
    ...
    try:
        ...
        return {
            ...
            "slow_query_threshold_ms": performance_report.get("slow_query_threshold_ms", 100),
...
async def get_database_configuration(
    ...
    try:
        return {
            ...
            "timestamp": datetime.now(timezone.utc).isoformat(),
            "configuration": {
                "database_type": "sqlite" if "sqlite" in Config.DATABASE_URL.lower() else "unknown",
...
@router.delete("/tables/{table_name}/indexes/{index_name}", response_model=Dict[str, Any])
