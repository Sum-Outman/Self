...
async def register_collision_shape(
    ...
    dimensions: Optional[List[float]] = Body(None, description="尺寸（对于BOX）[长, 宽, 高]"),
...
async def update_collision_shape(
    ...
    orientation: Optional[List[float]] = Body(None, description="方向四元数 [x, y, z, w]"),
    service: CollisionDetectionService = Depends(get_collision_service),
    user: User = Depends(get_current_user),
):
    """更新碰撞形状位置"""
