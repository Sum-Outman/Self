...
async def start_imitation_session(
    target_action: Optional[str] = Form(
        None,
        description="目标动作类型（wave_hand, walk, grasp_object, sit_down, stand_up, jump, bend_over, turn_around）",
    ),
    imitation_mode: str = Form(
        "record_and_replay",
        description="模仿模式：real_time, record_and_replay, keyframe_learning, style_transfer",
    ),
    ...
    try:
        ...
        return SuccessResponse.create(message="模仿会话开始成功", data=result).dict()
...
async def record_human_pose(
    ...
    try:
        ...
        return SuccessResponse.create(message="姿态录制成功", data=result).dict()
...
async def analyze_pose_sequence(
    ...
    try:
        ...
        return SuccessResponse.create(message="姿态序列分析完成", data=result).dict()
...
async def generate_robot_trajectory(
    session_id: str = Form(..., description="会话ID"),
    adaptation_level: float = Form(
        0.5, description="适应级别（0.0=完全复制，1.0=最大适应）"
    ),
    db: Session = Depends(get_db),
    user: User = Depends(get_current_user),
):
    ...
    try:
        ...
        adaptation_text = (
            ...
            else "部分适应" if adaptation_level < 0.7 else "高度适应"
        )
        return SuccessResponse.create(
            message=f"机器人轨迹生成完成（{adaptation_text}）", data=result
        ).dict()
...
async def evaluate_imitation_accuracy(
    ...
    try:
        ...
        return SuccessResponse.create(
            message=f"模仿评估完成：{feedback}", data=result
        ).dict()
...
async def get_available_actions(
    ...
    try:
        ...
        return SuccessResponse.create(
            message=f"获取到 {result.get('total_actions', 0)} 个可用动作", data=result
        ).dict()
...
async def get_imitation_sessions(
    ...
    try:
        ...
        return SuccessResponse.create(
            message=f"获取到 {result.get('total_sessions', 0)} 个模仿会话", data=result
        ).dict()
...
async def get_session_details(
    ...
    try:
        ...
        return SuccessResponse.create(
            message="会话详情获取成功",
            data={"session": target_session, "session_id": session_id},
        ).dict()
...
async def get_available_body_parts(
    ...
    try:
        ...
        return SuccessResponse.create(
            ...
            data={"body_parts": body_parts, "total_count": len(body_parts)},
        ).dict()
        ...
        imitation_modes = [
            {
                "value": "real_time",
                "label": "实时模仿",
                "description": "实时观察并立即模仿",
            },
            {
                "value": "record_and_replay",
                "label": "录制回放",
                "description": "先录制再回放模仿",
            },
            {
                "value": "keyframe_learning",
                "label": "关键帧学习",
                "description": "学习关键姿势帧",
            },
            {
...
async def batch_process_imitation(
    session_id: str = Form(..., description="会话ID"),
    steps_json: str = Form(
        "[]", description="处理步骤JSON数组（可选步骤：analyze, generate, evaluate）"
    ),
    db: Session = Depends(get_db),
    user: User = Depends(get_current_user),
):
    ...
    try:
        ...
        if not isinstance(steps, list):
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="处理步骤必须是JSON数组"
            )
        ...
        # 统计成功步骤
        successful_steps = [
            step for step, result in results.items() if result.get("success", False)
        ]
        ...
        while True:
            ...
            if message_type == "start_session":
                ...
                if result.get("success", False):
                    ...
                    await websocket.send_json(
                        {"type": "session_started", "data": result}
                    )
                else:
                    await websocket.send_json(
                        {
                ...
                await websocket.send_json({"type": "pose_recorded", "data": result})

            elif message_type == "analyze":
                # 分析姿态序列
                if not current_session_id:
                    await websocket.send_json(
                        {"type": "error", "message": "请先开始一个会话并录制姿态"}
                    )
                    continue
                ...
                result = service.generate_robot_trajectory(
                    current_session_id, adaptation_level
                )

                await websocket.send_json(
                    {"type": "trajectory_generated", "data": result}
                )
                ...
                await websocket.send_json({"type": "evaluation_result", "data": result})

            elif message_type == "get_actions":
                # 获取可用动作
                result = service.get_available_actions()

                await websocket.send_json({"type": "actions_list", "data": result})
