...
async def submit_vision_task(
    task_type: str = Body(
        ...
        examples=["object_detection", "object_tracking", "visual_navigation", "visual_manipulation"]
    ),
    target_object: Optional[str] = Body(
        None,
        description="目标对象（可选），如 'person', 'cup', 'table'"
    ),
    user: User = Depends(get_current_user),
):
    """提交视觉任务
...
async def test_vision_functionality(
    ...
    try:
        ...
        if service_status["success"]:
            status_data = service_status["status"]
            test_results["vision_controller_available"] = status_data["vision_controller_available"]
...
async def get_voice_commands_reference(
    ...
    voice_commands_reference = {
        "移动命令": [
            {"command": "向前走", "description": "向前移动", "参数": "direction=forward, distance=0.5m"},
            {"command": "向后退", "description": "向后移动", "参数": "direction=backward, distance=0.3m"},
            {"command": "向左转", "description": "向左转45度", "参数": "direction=left, angle=45°"},
            {"command": "向右转", "description": "向右转45度", "参数": "direction=right, angle=45°"},
        ],
        "手势命令": [
            {"command": "挥手", "description": "挥手打招呼", "参数": "gesture=wave, hand=right, duration=2s"},
            {"command": "站立", "description": "站立姿势", "参数": "gesture=stand, duration=3s"},
        ],
        "导航命令": [
            {"command": "去[目标]", "description": "导航到目标位置", "参数": "action=go_to, target=[目标名称]"},
            {"command": "跟随[对象]", "description": "跟随目标对象", "参数": "action=follow, target=[对象名称]"},
        ],
        "视觉任务": [
            {"command": "检测物体", "description": "开始对象检测", "参数": "task=object_detection"},
            {"command": "跟踪[对象]", "description": "跟踪特定对象", "参数": "task=object_tracking, target=[对象名称]"},
        ],
        "系统命令": [
