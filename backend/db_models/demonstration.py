...
class Demonstration(Base):
    ...
    demonstration_type = Column(SQLEnum(DemonstrationType), nullable=False, comment="示范类型")
    format_type = Column(SQLEnum(DemonstrationFormat), default=DemonstrationFormat.JSON_RAW, comment="数据格式")
    ...
    # 状态信息
    status = Column(SQLEnum(DemonstrationStatus), default=DemonstrationStatus.RECORDING, comment="示范状态")
    ...
    frames = relationship("DemonstrationFrame", back_populates="demonstration", cascade="all, delete-orphan")
...
class DemonstrationFrame(Base):
    ...
    # 关联信息
    demonstration_id = Column(Integer, ForeignKey("demonstrations.id"), nullable=False, comment="所属示范ID")
    ...
    camera_data_ref = Column(Integer, ForeignKey("camera_frames.id"), comment="关联的相机帧ID")
    ...
    # 元数据
    frame_type = Column(String(50), default="control", comment="帧类型: control, observation, mixed")
...
class CameraFrame(Base):
    ...
    image_format = Column(String(20), default="jpeg", comment="图像格式: jpeg, png, raw")
    ...
    demonstration_frames = relationship("DemonstrationFrame", back_populates="camera_frame")
...
class DemonstrationTask(Base):
    ...
    demonstrations = relationship("Demonstration", secondary="task_demonstrations", back_populates="tasks")
...
task_demonstrations = Table(
    ...
    Column("demonstration_id", Integer, ForeignKey("demonstrations.id"), primary_key=True),
