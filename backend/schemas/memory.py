...
class MemoryStats(BaseModel):
    ...
    average_retrieval_time_ms: float = Field(..., ge=0, description="平均检索响应时间(毫秒)")
...
class MemoryItem(BaseModel):
    ...
    scene_type: Optional[str] = Field(None, description="场景类型: task, learning, problem_solving, social, planning")
...
class MemorySearchRequest(BaseModel):
    ...
    min_importance: Optional[float] = Field(None, ge=0, le=1, description="最小重要性阈值")
    min_similarity: Optional[float] = Field(None, ge=0, le=1, description="最小相似度阈值")
...
class SystemConfigUpdate(BaseModel):
    """系统配置更新模型"""
    enable_autonomous_memory: Optional[bool] = Field(None, description="启用自主记忆管理")
    enable_scene_classification: Optional[bool] = Field(None, description="启用场景分类")
    enable_knowledge_integration: Optional[bool] = Field(None, description="启用知识库集成")
    working_memory_capacity: Optional[int] = Field(None, ge=10, le=1000, description="工作内存容量")
    cache_size_mb: Optional[int] = Field(None, ge=10, le=1024, description="缓存大小(MB)")
    similarity_threshold: Optional[float] = Field(None, ge=0.1, le=0.9, description="相似度阈值")
