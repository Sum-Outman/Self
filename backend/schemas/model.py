...
class ModelCreate(BaseModel):
    ...
    version: str = Field("1.0.0", pattern=r"^\d+\.\d+\.\d+$", description="语义化版本号")
...
class ModelEvaluate(BaseModel):
    ...
    metrics: List[str] = Field(["accuracy", "precision", "recall", "f1"], description="评估指标")
    sample_limit: int = Field(1000, ge=1, le=1000000, description="样本限制")
    
    @validator('metrics')
    def validate_metrics(cls, v):
        """验证评估指标"""
        valid_metrics = ["accuracy", "precision", "recall", "f1", "loss", "auc", "mse", "mae"]
        for metric in v:
            if metric not in valid_metrics:
                raise ValueError(f"无效的评估指标: {metric}。有效的指标包括: {', '.join(valid_metrics)}")
        return v
