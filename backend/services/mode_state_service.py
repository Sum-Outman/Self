...
class ModeStateService:
    ...
    def _init_database(self):
        ...
        # 创建索引
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_mode_states_type ON mode_states(mode_type)")
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_mode_states_created ON mode_states(created_at)")
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_mode_history_transition ON mode_history(transition_time)")
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_mode_history_from_to ON mode_history(from_mode, to_mode)")
    ...
    def save_mode_state(self,
        ...
        try:
            ...
            self.logger.info(
                f"模式状态保存成功: ID={state_id}, 模式={mode_type.value}, 状态={mode_state.value}")
    ...
    def restore_mode_state(self, state_id: int) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            tasks_data = self._decompress_data(
                row["tasks_data"]) if row["tasks_data"] else None
    ...
    def record_mode_transition(self,
        ...
        try:
            # 压缩任务快照
            compressed_tasks = self._compress_data(
                tasks_snapshot) if tasks_snapshot else None
    ...
    def get_transition_history(self,
        ...
        try:
            ...
            # 执行查询
            query = f"""                 SELECT id, from_mode, to_mode, transition_type, state_snapshot_id,                        duration_ms, success, error_message, transition_time, user_id, session_id, metadata                 FROM mode_history                 WHERE {where_clause}                 ORDER BY transition_time DESC                 LIMIT ? OFFSET ?             """
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # 获取模式分布
            cursor.execute(
                "SELECT mode_type, COUNT(*) as count FROM mode_states GROUP BY mode_type")
            mode_distribution = {row["mode_type"]: row["count"]
                                 for row in cursor.fetchall()}

            # 获取切换成功率
            cursor.execute(
                "SELECT success, COUNT(*) as count FROM mode_history GROUP BY success")
            success_counts = {bool(row["success"]): row["count"]
                              for row in cursor.fetchall()}

            total_transitions = sum(success_counts.values())
            success_rate = (success_counts.get(True, 0) /
                            total_transitions * 100) if total_transitions > 0 else 0
    ...
    def _cleanup_old_data(self):
        ...
        try:
            ...
            if deleted_states > 0 or deleted_history > 0:
                self.logger.info(
                    f"清理旧数据: 删除 {deleted_states} 个状态记录, {deleted_history} 个历史记录")
