...
class RobotMultimodalService:
    ...
    def process_multimodal_command(
        ...
        try:
            ...
            if robot_commands and self.robot_control_service:
                execution_result = self.multimodal_controller.execute_commands(
                    robot_commands, self.robot_control_service
                )
    ...
    def start_learning_session(
        self, learning_type: str, learning_config: Dict[str, Any]
    ) -> Dict[str, Any]:
        """开始学习会话
        ...
        try:
            ...
            if learning_type == "demonstration":
                ...
                if demonstration_data:
                    success = self.learning_controller.start_demonstration_learning(
                        demonstration_data
                    )
                    result["success"] = success
                    result["message"] = (
                        "示范学习已启动" if success else "示范学习启动失败"
                    )

            elif learning_type == "reinforcement":
                task_description = learning_config.get("task_description", "未指定任务")
                # 注意：实际实现中需要提供奖励函数
                success = self.learning_controller.start_reinforcement_learning(
                    task_description, lambda x: 0.0
                )
                result["success"] = success
                result["message"] = (
                    f"强化学习已启动: {task_description}"
                    if success
                    else "强化学习启动失败"
                )
                ...
                try:
                    ...
                    if hasattr(self.learning_controller, "start_imitation_learning"):
                        success = self.learning_controller.start_imitation_learning(
                            task_description, demonstration_data
                        )
                        result["success"] = success
                        result["message"] = (
                            f"模仿学习已启动: {task_description}"
                            if success
                            else "模仿学习启动失败"
                        )
                    else:
    ...
    def get_multimodal_capabilities(self) -> Dict[str, Any]:
        ...
        capabilities = {
            ...
            "behavior_modes": [
                mode.value for mode in RobotBehaviorMode
            ],  # 模块导入成功，行为模式可用
...
def test_robot_multimodal_service():
    ...
    try:
        ...
        # 测试多模态命令处理
        test_command = {"text_command": "向前走然后挥手", "behavior_mode": "autonomous"}
        ...
        if result.get("success"):
            ...
            for i, cmd in enumerate(result.get("commands", [])):
                print(
                    f"  命令 {                         i +                         1}: {                         cmd['command_type']} -> {                         cmd['target']} (置信度: {                         cmd['confidence']:.2f})"
                )
        else:
