    ...
    def connect_device(self, device_config: DeviceConfig) -> bool:
        ...
        with self.lock:
            ...
            try:
                # 模拟设备连接
                logger.info(
                    f"连接设备: {device_id} ({device_config.device_type.value})"
                )
        ...
        with self.lock:
            if device_id not in self.devices:
                raise HardwareTrainingError(
                    f"设备未连接: {device_id}", "DEVICE_NOT_CONNECTED", device_id
                )
            ...
            try:
                ...
                raise HardwareTrainingError(
                    f"命令执行失败: {str(e)}", "COMMAND_EXECUTION_FAILED", device_id
                )
        ...
        with self.lock:
            for device_id, device in self.devices.items():
                ...
                if time_since_heartbeat > timedelta(seconds=30):
                    ...
                    logger.warning(
                        f"设备心跳丢失: {device_id}, 最后心跳: {last_heartbeat}"
                    )
...
class TrainingPhaseExecutor:
    ...
    def _execute_initialization(
        self, task: TrainingTask, progress_callback: Optional[callable]
    ) -> Dict[str, Any]:
        """执行初始化阶段"""
        logger.info("执行初始化阶段")
        ...
        for device_config in task.device_configs:
            ...
            if progress_callback:
                progress_callback(
                    "device_initialized", device_config.device_id, success
                )
            ...
            try:
                ...
                for device_config in task.device_configs:
                    ...
                    response = self.device_manager.send_command(
                        ...
                        {
                            "iteration": iteration,
                            "parameters": task.training_parameters,
                        },
                    )
    ...
    def _execute_validation(
        ...
        if validation_results["device_validations"]:
            scores = [
                v.get("score", 0)
                for v in validation_results["device_validations"]
    ...
    def _execute_completion(
        ...
        for device_config in task.device_configs:
            ...
            if progress_callback:
                progress_callback(
                    "device_disconnected", device_config.device_id, success
                )
...
class HardwareTrainingManager:
    ...
    def create_task(self, task: TrainingTask) -> str:
        ...
        if task.task_id in self.tasks:
            raise HardwareTrainingError(
                f"任务已存在: {task.task_id}", "TASK_ALREADY_EXISTS"
            )
        ...
        try:
            ...
            for phase_index, phase in enumerate(task.phase_sequence):
                ...
                success, phase_result = self.phase_executor.execute_phase(
                    phase, task, self._create_progress_callback(task_id, phase)
                )
                ...
                self.progress[task_id].elapsed_time = (
                    datetime.now() - self.progress[task_id].start_time
                )

                # 触发阶段完成回调
                self._trigger_callback(
                    "phase_complete", task_id, phase, success, phase_result
                )
    ...
    def _create_progress_callback(self, task_id: str, phase: TrainingPhase):
        ...
        def callback(event: str, *args):
            ...
            if event == "training_progress":
                ...
                self.progress[task_id].overall_progress = base_progress + (
                    progress * phase_weight
                )
    ...
    def _complete_task(
        self, task_id: str, success: bool, error: Optional[HardwareTrainingError]
    ):
        """完成任务"""
