...
class SerialDataService:
    ...
    def start(self) -> bool:
        ...
        try:
            ...
            self.processing_thread = threading.Thread(
                target=self._processing_loop, daemon=True, name="SerialDataProcessor"
            )
    ...
    def register_destination_handler(
        self, destination: DataDestination, handler: Callable[[Dict[str, Any]], bool]
    ):
        """注册目的地处理器
    ...
    def send_serial_data(
        self, port: str, data: bytes, protocol: SerialProtocol = SerialProtocol.RAW
    ) -> bool:
        """发送串口数据
