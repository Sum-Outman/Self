...
import logging
import threading
from typing import Dict, Any, Optional, List, Type, Set, Callable
...
class ServiceInfo:
    ...
    def __init__(self, service_name: str, service_class: Type[BaseService], 
...
class ServiceRegistry(BaseService):
    ...
    def _initialize_service(self) -> bool:
        ...
        try:
            ...
            self.logger.info(f"服务注册中心初始化成功，已注册服务数: {len(self._services)}")
            return True
            ...
            if service_info.service_instance:
                try:
                    ...
                    if health_data.get("healthy", False):
                        ...
                        service_info.last_error = health_data.get("error", "健康检查失败")
                    ...
                    self.logger.warning(f"服务 {service_info.service_name} 健康检查失败: {e}")
    
    def register_service_class(self, service_name: str, service_class: Type[BaseService]) -> bool:
        """注册服务类
    ...
    def register_service_instance(self, service_name: str, service_instance: BaseService) -> bool:
        """注册服务实例
        ...
        with self._lock:
            ...
            if service_name in self._services:
                ...
                service_info = ServiceInfo(service_name, type(service_instance), service_instance)
            ...
            self.logger.info(f"服务实例 {service_name} 注册成功，状态: {service_info.status.value}")
    ...
    def get_all_service_info(self) -> Dict[str, Dict[str, Any]]:
        ...
        """
        with self._lock:
            return {name: service_info.to_dict() for name, service_info in self._services.items()}
    ...
    def add_dependency(self, service_name: str, dependency_name: str) -> bool:
        ...
        """
        with self._lock:
            if service_name not in self._services or dependency_name not in self._services:
                self.logger.error(f"服务 {service_name} 或依赖 {dependency_name} 未注册")
                return False
    ...
    def get_registry_status(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            return {
                ...
                "auto_discover_enabled": self.config.extra_config.get("auto_discover_services", True),
                "health_check_interval": self.config.extra_config.get("health_check_interval_seconds", 30),
    ...
    def shutdown_service(self, service_name: str, force: bool = False) -> Dict[str, Any]:
        """关闭服务
        ...
        try:
            ...
            if hasattr(service_instance, 'shutdown'):
                ...
                if isinstance(shutdown_result, dict) and not shutdown_result.get("success", True):
                    return {
                        "success": False,
