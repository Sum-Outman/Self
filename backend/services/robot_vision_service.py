...
class RobotVisionService:
    ...
    def _initialize_components(self):
        ...
        try:
            # 尝试导入视觉引导控制器
            try:
                from hardware.robot_vision_control import VisionGuidedController, create_vision_guided_controller
    ...
    def _process_task(self, task_id: str, task_data: Dict[str, Any]):
        ...
        try:
            ...
            logger.info(f"开始处理视觉任务: {task_id} (类型: {task_data.get('task_type')})")
            ...
            if task_type == "object_detection":
                ...
                # 对象跟踪任务
                success = self.vision_controller.execute_visual_task(
                    self.vision_controller.VisionTaskType.OBJECT_TRACKING,
                    target_object
                )
                ...
                # 视觉导航任务
                success = self.vision_controller.execute_visual_task(
                    self.vision_controller.VisionTaskType.NAVIGATION,
                    target_object
                )
                
                result_data = {
                    "target": target_object,
                    "navigation_started": success
                }
                
            elif task_type == "visual_manipulation":
                # 视觉操作任务
                success = self.vision_controller.execute_visual_task(
                    self.vision_controller.VisionTaskType.MANIPULATION,
                    target_object
                )
                
                result_data = {
                    "target": target_object,
                    "manipulation_started": success
                }
            ...
            task_data["end_time"] = time.time()
            task_data["execution_time"] = task_data["end_time"] - task_data["start_time"]
            
            logger.info(f"视觉任务完成: {task_id} (状态: {task_data['status']}, 用时: {task_data['execution_time']:.2f}s)")
        ...
        try:
            ...
            if "command_text" in command_data:
                ...
                if voice_command:
                    ...
                    # 执行语音命令
                    success = self.vision_controller.execute_voice_command(voice_command)
            ...
            command_data["end_time"] = time.time()
            command_data["execution_time"] = command_data["end_time"] - command_data["start_time"]
            
            logger.info(f"语音命令处理完成: {command_id} (状态: {command_data['status']}, 用时: {command_data['execution_time']:.2f}s)")
    ...
    def submit_vision_task(self, task_type: str, target_object: Optional[str] = None) -> Dict[str, Any]:
        """提交视觉任务
        ...
        try:
            ...
            logger.info(f"视觉任务已提交: {task_id} (类型: {task_type}, 目标: {target_object})")
    ...
    def get_service_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "status": {
                ...
                "pending_tasks": sum(1 for t in self.tasks.values() if t.get("status") == VisionTaskStatus.PENDING.value),
                "running_tasks": sum(1 for t in self.tasks.values() if t.get("status") == VisionTaskStatus.RUNNING.value),
                "completed_tasks": sum(1 for t in self.tasks.values() if t.get("status") == VisionTaskStatus.COMPLETED.value),
                "failed_tasks": sum(1 for t in self.tasks.values() if t.get("status") == VisionTaskStatus.FAILED.value),
                "pending_commands": sum(1 for c in self.voice_commands.values() if c.get("status") == VoiceCommandStatus.RECEIVED.value),
                "processing_commands": sum(1 for c in self.voice_commands.values() if c.get("status") == VoiceCommandStatus.PROCESSING.value),
                "completed_commands": sum(1 for c in self.voice_commands.values() if c.get("status") == VoiceCommandStatus.COMPLETED.value),
    ...
    def process_visual_frame(self, image_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"收到视觉帧处理请求 (base64: {bool(image_base64)}, path: {image_path})")
    ...
    def cleanup_old_data(self, max_age_hours: float = 24.0):
        ...
        try:
            ...
            if tasks_to_delete or commands_to_delete:
                logger.info(f"清理了 {len(tasks_to_delete)} 个旧任务和 {len(commands_to_delete)} 个旧语音命令")
