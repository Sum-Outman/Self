...
from ..db_models.robot import Robot, RobotJoint
...
class DemonstrationRecorder:
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "sampling_rate": 1.0 / self.sampling_interval if self.sampling_interval > 0 else 0
        }
...
class DemonstrationPlayer:
    ...
    def load_demonstration(self) -> bool:
        ...
        try:
            # 加载示范
            self.demonstration = self.db.query(Demonstration).filter(
            ...
            # 加载帧数据
            self.frames = self.db.query(DemonstrationFrame).filter(
                DemonstrationFrame.demonstration_id == self.demonstration_id
            ).order_by(DemonstrationFrame.frame_index).all()
            ...
            logger.info(f"加载示范成功: {self.demonstration.name}, 帧数: {self.total_frames}")
            return True
        ...
        if 0 <= frame_index < self.total_frames:
            ...
            if self.playing:
                ...
                if self.demonstration.recording_duration:
                    target_time = frame_index / self.total_frames * self.demonstration.recording_duration
...
class DemonstrationManager:
    ...
    def create_recorder(self, db: Session, robot_id: int, user_id: int) -> DemonstrationRecorder:
        """创建示范录制器"""
