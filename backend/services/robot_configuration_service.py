...
from ..db_models.robot import Robot, RobotJoint, RobotSensor, RobotType, RobotStatus, ControlMode
...
class RobotConfigurationService:
    ...
    def import_urdf(self, 
                   ...
                   description: Optional[str] = None) -> Tuple[bool, str, Optional[Robot]]:
        """
        导入URDF文件并创建机器人记录
        ...
        try:
            ...
            # 检查是否已存在同名机器人
            existing_robot = self.db.query(Robot).filter(
    ...
    def export_urdf(self, robot_id: int) -> Tuple[bool, str, Optional[str]]:
        ...
        try:
            ...
            # 获取关节
            joints = self.db.query(RobotJoint).filter(RobotJoint.robot_id == robot_id).all()
            
            # 获取传感器
            sensors = self.db.query(RobotSensor).filter(RobotSensor.robot_id == robot_id).all()
    ...
    def _generate_urdf(self, robot: Robot, joints: List[RobotJoint], sensors: List[RobotSensor]) -> str:
        """生成URDF内容"""
        lines = []
        ...
        lines.append('      <inertia ixx="0.1" ixy="0" ixz="0" iyy="0.1" iyz="0" izz="0.1"/>')
        lines.append('    </inertial>')
        lines.append('    <visual>')
        lines.append('      <origin xyz="0 0 0" rpy="0 0 0"/>')
        ...
        lines.append('  <!-- 关节定义 -->')
        for joint in joints:
            lines.append('  <joint name="{}" type="{}">'.format(joint.name, joint.joint_type))
            lines.append('    <parent link="{}"/>'.format(joint.parent_link if joint.parent_link else "base_link"))
            ...
            # 轴
            axis_str = " ".join([str(coord) for coord in eval(joint.axis)]) if joint.axis else "0 0 1"
            lines.append('    <axis xyz="{}"/>'.format(axis_str))
            
            # 限制
            lines.append('    <limit lower="{:.6f}" upper="{:.6f}" velocity="{:.6f}" effort="{:.6f}"/>'.format(
                joint.min_position, joint.max_position, joint.max_velocity, joint.max_torque
            ))
        ...
        if sensors:
            ...
            for sensor in sensors:
                lines.append('  <sensor name="{}" type="{}">'.format(sensor.name, sensor.sensor_type))
                lines.append('    <parent_link>{}</parent_link>'.format(sensor.parent_link if sensor.parent_link else "base_link"))
                lines.append('    <origin xyz="{:.6f} {:.6f} {:.6f}" rpy="0 0 0"/>'.format(
    ...
    def import_config_file(self, 
                          file_path: str, 
                          user_id: int,
                          format: str = "auto") -> Tuple[bool, str, Optional[Robot]]:
        """
        导入配置文件（JSON/YAML）
        ...
        try:
            ...
            # 创建机器人记录
            robot_name = config.get("name", f"config_import_{datetime.now().strftime('%Y%m%d_%H%M%S')}")
            ...
            robot = Robot(
                ...
                control_mode=ControlMode(config.get("control_mode", ControlMode.POSITION.value)),
    ...
    def export_config_file(self, 
                          robot_id: int, 
                          format: str = "json") -> Tuple[bool, str, Optional[Dict[str, Any]]]:
        """
