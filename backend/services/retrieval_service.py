...
class RetrievalService:
    ...
    def retrieve_text_by_image(
        self, image_bytes: bytes, text_candidates: List[str], top_k: int = 5
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            # 获取top-k结果
            top_scores, top_indices = similarities.topk(
                min(top_k, len(valid_candidates))
            )
    ...
    def retrieve_image_by_text(
        self, query_text: str, image_candidates: List[bytes], top_k: int = 5
    ) -> Dict[str, Any]:
