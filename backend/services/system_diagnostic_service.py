...
# 导入现有的监控模块
try:
    from models.system_control.system_monitor import SystemMonitor, SystemStatus, SystemAlert
...
class SystemDiagnosticService:
    ...
    def _start_periodic_diagnosis(self):
        ...
        diagnosis_thread = threading.Thread(
            target=periodic_diagnosis_loop,
            daemon=True,
            name="PeriodicDiagnosis"
        )
    ...
    def _diagnose_database(self) -> List[DiagnosticIssue]:
        ...
        try:
                ...
                with engine.connect() as conn:
                    ...
                    issues.append(DiagnosticIssue(
                        ...
                        suggestions=[
                            ...
                            "检查数据库负载",
                            "考虑数据库升级或优化"
                        ] if avg_query_time > 100 else None
                    ))
            except Exception as e:
            ...
            # 如果没有发现问题，添加成功诊断
            if not any(issue.level in [DiagnosticLevel.ERROR, DiagnosticLevel.WARNING] 
                      for issue in issues if issue.category == DiagnosticCategory.DATABASE):
                issues.append(DiagnosticIssue(
    ...
    def _diagnose_api(self) -> List[DiagnosticIssue]:
        ...
        try:
            ...
            try:
                ...
                API_DIAGNOSTIC_AVAILABLE = True
            except ImportError as e:
                logger.warning(f"API诊断模块导入失败: {e}")
                API_DIAGNOSTIC_AVAILABLE = False
                issues.append(DiagnosticIssue(
            ...
            for endpoint in api_endpoints:
                ...
                try:
                    ...
                    if response.status_code == expected_status:
                        ...
                        failed_tests += 1
                        level = DiagnosticLevel.ERROR if is_critical else DiagnosticLevel.WARNING
            ...
            # 如果没有发现严重问题，添加成功诊断
            if not any(issue.level in [DiagnosticLevel.ERROR] 
                      for issue in issues if issue.category == DiagnosticCategory.API):
                if successful_tests > 0:
                    issues.append(DiagnosticIssue(
                        id="api_health_ok",
                        category=DiagnosticCategory.API,
    ...
    def _diagnose_security(self) -> List[DiagnosticIssue]:
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    for package, info in critical_packages.items():
                        if package in installed_packages:
                            ...
                            try:
                                ...
                                if current_ver < min_ver and current_version not in info["vulnerable_versions"]:
                                    issues.append(DiagnosticIssue(
                    ...
                    if packages_checked > 0:
                        ...
                        elif any(issue.id.startswith("security_outdated_package_") for issue in issues):
                            security_status = "一般"
                        ...
                        issues.append(DiagnosticIssue(
                            ...
                            suggestions=[
                                ...
                                "使用安全扫描工具检查漏洞",
                                "订阅安全公告"
                            ] if vulnerable_packages_found > 0 else None
                        ))
            ...
            # 如果没有发现安全问题，添加成功诊断
            if not any(issue.level in [DiagnosticLevel.ERROR, DiagnosticLevel.WARNING, DiagnosticLevel.CRITICAL] 
                      for issue in issues if issue.category == DiagnosticCategory.SECURITY):
                issues.append(DiagnosticIssue(
    ...
    def _diagnose_hardware(self) -> List[DiagnosticIssue]:
        ...
        try:
                    ...
                    if battery:
                        issues.append(DiagnosticIssue(
                            ...
                            suggestions=[
                                "连接电源适配器",
                                "保存工作并准备关机"
                            ] if battery.percent < 20 and not battery.power_plugged else None
                        ))
            except Exception as e:
                # 电池信息不可用是正常的（桌面设备）
                pass  # 已实现
            ...
            # 如果没有发现硬件问题，添加成功诊断
            if not any(issue.level in [DiagnosticLevel.ERROR, DiagnosticLevel.WARNING] 
                      for issue in issues if issue.category == DiagnosticCategory.HARDWARE):
                issues.append(DiagnosticIssue(
    ...
    def _check_threshold(self, 
                        ...
                        category: DiagnosticCategory,
                        title: str,
                        description: str) -> List[DiagnosticIssue]:
        """检查指标是否超过阈值
        ...
        try:
            ...
            if level != DiagnosticLevel.INFO:
                ...
                # 根据问题级别提供建议
                suggestions = []
                if level in [DiagnosticLevel.WARNING, DiagnosticLevel.ERROR, DiagnosticLevel.CRITICAL]:
                    suggestions = [
                        f"监控{title}变化趋势",
                        f"分析{title}过高原因",
                        "考虑优化相关组件性能"
    ...
    def _analyze_diagnosis_results(self, 
                                  issues: List[DiagnosticIssue], 
                                  start_time: float) -> DiagnosticReport:
        """分析诊断结果
        ...
        if not issues:
            ...
            critical_count = len([i for i in issues if i.level == DiagnosticLevel.CRITICAL])
            error_count = len([i for i in issues if i.level == DiagnosticLevel.ERROR])
            warning_count = len([i for i in issues if i.level == DiagnosticLevel.WARNING])
        ...
        # 如果有资源问题，建议监控
        resource_issues = [i for i in issues if i.category == DiagnosticCategory.RESOURCE]
        if resource_issues:
    ...
    def get_diagnosis_history(self, 
                             hours: int = 24,
                             min_level: Optional[DiagnosticLevel] = None) -> List[DiagnosticReport]:
        """获取诊断历史
        ...
        for report in self.diagnostic_history:
            if report.timestamp >= cutoff_time:
                if min_level:
                    # 检查报告是否有至少一个指定级别的问题
                    if any(issue.level.value >= min_level.value for issue in report.issues):
    ...
    def run_diagnosis(self, categories: Optional[List[str]] = None) -> str:
        ...
        with self.reports_lock:
            ...
            thread = threading.Thread(
                target=_run_diagnosis_async,
                daemon=True,
                name=f"Diagnosis_{report_id}"
            )
            thread.start()
    ...
    def get_diagnostic_history(self, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        # 转换为API格式
        return [self._convert_report_to_api_format(report) for report in limited_reports]
    ...
    def _convert_report_to_api_format(self, report: DiagnosticReport) -> Dict[str, Any]:
        ...
        issues_by_category = {
            category.value: count for category, count in report.issues_by_category.items()
        }
    ...
    def export_report_to_json(self, report: DiagnosticReport, file_path: Optional[str] = None) -> str:
        """将诊断报告导出为JSON格式
...
<head>
    ...
    <title>系统诊断报告 - {report.report_id}</title>
    <style>
        body {{ font-family: Arial, sans-serif; margin: 20px; line-height: 1.6; }}
        .header {{ background-color: #f8f9fa; padding: 20px; border-radius: 5px; margin-bottom: 20px; }}
        .report-id {{ font-size: 24px; font-weight: bold; color: #333; }}
        .timestamp {{ color: #666; font-size: 14px; }}
        .status-badge {{ display: inline-block; padding: 5px 10px; border-radius: 3px; margin-left: 10px; }}
        .status-critical {{ background-color: #dc3545; color: white; }}
        .status-error {{ background-color: #fd7e14; color: white; }}
        ...
        .summary {{ background-color: #e8f4fd; border-left: 4px solid #007bff; padding: 15px; }}
        .issue {{ margin-bottom: 15px; padding: 10px; border-left: 4px solid; border-radius: 3px; }}
        .issue-critical {{ border-left-color: #dc3545; background-color: #f8d7da; }}
        .issue-error {{ border-left-color: #fd7e14; background-color: #ffe5d0; }}
        ...
        .issue-description {{ color: #6c757d; font-size: 14px; margin-bottom: 5px; }}
        .issue-suggestions {{ color: #28a745; font-size: 13px; }}
        ...
        .footer {{ margin-top: 30px; padding-top: 15px; border-top: 1px solid #dee2e6; color: #6c757d; font-size: 12px; text-align: center; }}
    </style>
</head>
<body>
    <div class="header">
        <div class="report-id">系统诊断报告: {report.report_id}</div>
            '''
            ...
            html_content += f'''
        <div class="timestamp">生成时间: {datetime.fromtimestamp(report.timestamp).strftime('%Y-%m-%d %H:%M:%S')}</div>
    ...
    <div class="section">
        ...
        <div class="summary">
            <div class="metric"><span class="metric-label">整体状态:</span> <span class="metric-value">{status_text}</span></div>
            <div class="metric"><span class="metric-label">问题总数:</span> <span class="metric-value">{report.issues_count}</span></div>
            <div class="metric"><span class="metric-label">执行时间:</span> <span class="metric-value">{report.execution_time_seconds:.2f}秒</span></div>
        </div>
    </div>
    
    <div class="section">
        <div class="section-title">问题统计</div>
        <table class="table">
            <thead>
                <tr>
                    <th>类别</th>
            ...
            # 按类别统计问题
            categories = sorted(set(issue.category for issue in report.issues), key=lambda c: c.value)
            for category in categories:
                issues_by_level = {}
                for level in DiagnosticLevel:
                    issues_by_level[level] = len([i for i in report.issues if i.category == category and i.level == level])
                
                html_content += f'''
                <tr>
                    <td>{category.value}</td>
                    <td>{issues_by_level.get(DiagnosticLevel.INFO, 0)}</td>
                    <td>{issues_by_level.get(DiagnosticLevel.WARNING, 0)}</td>
                    <td>{issues_by_level.get(DiagnosticLevel.ERROR, 0)}</td>
                    <td>{issues_by_level.get(DiagnosticLevel.CRITICAL, 0)}</td>
                </tr>'''
    ...
    <div class="section">
        ...
        <div class="issue {issue_class}">
            <div class="issue-title">{issue.title} ({issue.level.value})</div>
            <div class="issue-description">{issue.description}</div>
            '''
                
                if issue.metric_value is not None:
                    html_content += f'''
            <div class="issue-description">指标: {issue.metric_name} = {issue.metric_value:.2f}{f' (阈值: {issue.threshold:.2f})' if issue.threshold else ''}</div>
                    '''
                
                if issue.suggestions:
                    suggestions_html = "<br>".join(issue.suggestions)
                    html_content += f'''
            <div class="issue-suggestions">建议: {suggestions_html}</div>
                    '''
                
                html_content += f'''
            <div class="issue-description" style="font-size: 12px; color: #999;">
                时间: {datetime.fromtimestamp(issue.timestamp).strftime('%H:%M:%S')} | 组件: {issue.component if issue.component else 'N/A'}
            </div>
        </div>
                '''
    ...
    <div class="section">
        ...
        <div class="summary">
            <div class="metric"><span class="metric-label">平台:</span> <span class="metric-value">{platform.platform()}</span></div>
            <div class="metric"><span class="metric-label">Python版本:</span> <span class="metric-value">{platform.python_version()}</span></div>
            <div class="metric"><span class="metric-label">主机名:</span> <span class="metric-value">{platform.node()}</span></div>
            <div class="metric"><span class="metric-label">CPU核心数:</span> <span class="metric-value">{psutil.cpu_count(logical=True)}</span></div>
            <div class="metric"><span class="metric-label">总内存:</span> <span class="metric-value">{psutil.virtual_memory().total / (1024**3):.1f} GB</span></div>
        </div>
    </div>
    
    <div class="section">
        <div class="section-title">建议总结</div>
        <div class="summary">
            '''
    ...
    <div class="footer">
        本报告由 Self AGI 系统诊断服务生成 | 生成时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}
    </div>
</body>
</html>'''
    ...
    def export_report_to_text(self, report: DiagnosticReport, file_path: Optional[str] = None) -> str:
        """将诊断报告导出为文本格式
        ...
        try:
            ...
            text_lines.append("=" * 80)
            text_lines.append(f"生成时间: {datetime.fromtimestamp(report.timestamp).strftime('%Y-%m-%d %H:%M:%S')}")
            ...
            for i, issue in enumerate(sorted_issues, 1):
                ...
                if issue.metric_value is not None:
                    threshold_text = f" (阈值: {issue.threshold:.2f})" if issue.threshold else ""
                    text_lines.append(f"   指标: {issue.metric_name} = {issue.metric_value:.2f}{threshold_text}")
                ...
                text_lines.append(f"   时间: {datetime.fromtimestamp(issue.timestamp).strftime('%H:%M:%S')}")
                text_lines.append("")
            ...
            text_lines.append(f"总内存: {psutil.virtual_memory().total / (1024**3):.1f} GB")
            text_lines.append("")
            
            text_lines.append("=" * 80)
            text_lines.append(f"报告结束 - Self AGI 系统诊断服务")
...
def test_system_diagnostic_service():
    ...
    try:
        ...
        try:
            ...
            html_file.close()
            html_content = service.export_report_to_html(report, html_path)
            ...
            text_file.close()
            text_content = service.export_report_to_text(report, text_path)
