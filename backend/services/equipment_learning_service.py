...
class EquipmentKnowledge:
    ...
    operation_procedures: Dict[str, List[OperationStep]] = field(default_factory=dict)  # 操作流程
...
class LearningSession:
    ...
    learning_materials: List[str] = field(default_factory=list)  # 学习材料：说明书路径、视频路径等
...
class EquipmentLearningService:
    ...
    def _initialize_default_knowledge(self):
        ...
        printer_components = [
            EquipmentComponent(
                ...
                physical_properties={"color": "green", "size": "small", "shape": "circular"},
            ...
            EquipmentComponent(
                ...
                physical_properties={"color": "gray", "size": "large", "shape": "rectangular"},
            ...
            EquipmentComponent(
                ...
                physical_properties={"color": "black", "size": "medium", "shape": "rectangular"},
            ...
            EquipmentComponent(
                ...
                type="tray",
                location_3d=(0.0, 0.2, 0.5),
                physical_properties={"color": "gray", "size": "medium", "shape": "rectangular"},
    ...
    def learn_from_manual(self, manual_text: str, equipment_info: Dict[str, Any]) -> Dict[str, Any]:
        """从说明书学习设备操作
        ...
        try:
            with self._lock:
                # 提取设备信息
                equipment_id = equipment_info.get("equipment_id", f"equipment_{int(time.time())}")
                ...
                if equipment_id in self._knowledge_base:
                    ...
                    knowledge.confidence_score = min(1.0, knowledge.confidence_score + 0.2)
                    action = "updated"
                else:
                    # 创建新知识
    ...
    def _extract_components_from_manual(self, manual_text: str) -> List[EquipmentComponent]:
        """从说明书文本中提取部件信息（完整版）"""
        components = []
    ...
    def _extract_procedures_from_manual(self, manual_text: str) -> Dict[str, List[OperationStep]]:
        """从说明书文本中提取操作流程（完整版）"""
        procedures = {}
        ...
        for line in lines:
            if any(keyword in line for keyword in ["步骤", "操作", "方法", "procedure", "step"]):
            ...
            for i, line in enumerate(step_lines[:5]):  # 最多5个步骤
                steps.append(
                    OperationStep(
                        step_number=i+1,
                        description=line,
                        operation_type=OperationType.PRESS_BUTTON if i == 0 else OperationType.ADJUST_SETTING,
                        target_component="control_panel",
                        parameters={},
    ...
    def _extract_safety_guidelines(self, manual_text: str) -> List[str]:
        ...
        # 查找安全相关关键词
        safety_keywords = ["安全", "警告", "危险", "注意", "小心", "safety", "warning", "caution", "danger"]
    ...
    def get_operation_procedure(self, equipment_id: str, procedure_name: str = None) -> Dict[str, Any]:
        """获取设备操作流程
        ...
        with self._lock:
            ...
            if procedure_name:
                ...
                return {
                    ...
                    "estimated_total_time": sum(step.time_estimate for step in procedure)
                }
            else:
                # 返回所有流程
                procedures_info = {}
                for name, steps in knowledge.operation_procedures.items():
                    procedures_info[name] = {
                        ...
                        "estimated_total_time": sum(step.time_estimate for step in steps),
    ...
    def execute_operation_step(self, equipment_id: str, procedure_name: str, 
                              step_number: int, simulation_mode: bool = True) -> Dict[str, Any]:
        """执行操作步骤
        ...
        try:
            with self._lock:
                # 获取步骤信息
                procedure_result = self.get_operation_procedure(equipment_id, procedure_name)
    ...
    def _execute_single_step(self, step_info: Dict[str, Any], simulation_mode: bool) -> Dict[str, Any]:
        """执行单个步骤（模拟或实际）"""
        step_description = step_info.get("description", "")
    ...
    def _execute_real_operation(self, step_info: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """执行真实硬件操作
    ...
    def _generate_detailed_simulation(self, step_info: Dict[str, Any]) -> Dict[str, Any]:
        """生成详细模拟执行结果
        ...
        with self._lock:
            if equipment_id:
                ...
                return {
                    ...
                    "component_details": [comp.to_dict() for comp in knowledge.components],
                    "procedure_count": len(knowledge.operation_procedures),
    ...
    def get_learning_sessions(self, equipment_id: str = None, limit: int = 50) -> Dict[str, Any]:
        """获取学习会话记录
