...
class SystemService:
    ...
    def _calculate_response_time(self) -> float:
        ...
        try:
            ...
            if self._system_monitor is not None:
                ...
                recent_metrics = self._system_monitor.get_recent_metrics(
                    metric_type="response_time", limit=10
                )
                if recent_metrics:
                    total_time = sum(
                        metric.get("value", 0.0) for metric in recent_metrics
                    )
    ...
    def _calculate_accuracy(self) -> float:
        ...
        try:
            ...
            if self._system_monitor is not None:
                ...
                accuracy_metrics = self._system_monitor.get_recent_metrics(
                    metric_type="accuracy", limit=5
                )
                if accuracy_metrics:
                    total_accuracy = sum(
                        metric.get("value", 0.0) for metric in accuracy_metrics
                    )
    ...
    def _calculate_availability(self) -> float:
        ...
        try:
            ...
            if self._system_monitor is not None:
                ...
                uptime_metric = self._system_monitor.get_metric(
                    metric_id="system_uptime_seconds"
                )
                downtime_metric = self._system_monitor.get_metric(
                    metric_id="system_downtime_seconds"
                )
    ...
    def get_system_health(self) -> Dict[str, Any]:
        ...
        if self._system_monitor is not None:
            try:
                ...
                if health:
                    health_status["overall_health"] = health.get(
                ...
                if alerts:
                    health_status["alerts"] = [
                        alert.to_dict() for alert in alerts[:10]
                    ]  # 限制数量
    ...
    def get_mode_history(self, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        with self._mode_lock:
            return (
                self._mode_history[-limit:] if limit > 0 else self._mode_history.copy()
            )
