...
class Formation:
    ...
    center_position: List[float] = field(
        default_factory=lambda: [0.0, 0.0, 0.0]
    )  # 中心位置

    def get_robot_position(
        self, robot_index: int, leader_position: List[float] = None
    ) -> List[float]:
        """获取机器人在队形中的位置"""
...
class CooperativeTask:
    ...
    workspace_bounds: List[List[float]] = field(
        default_factory=lambda: [[-10, 10], [-10, 10], [0, 2]]
    )
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "formation": (
                self.formation.formation_type.value if self.formation else None
            ),
...
class MultiRobotCoordinator:
    ...
    def __init__(self):
        ...
        # 回调函数
        self.task_status_callbacks: Dict[
            str, List[Callable[[CooperativeTask], None]]
        ] = {}
    ...
    ) -> Tuple[bool, str, Optional[CooperativeTask]]:
        """创建协同任务"""
        try:
    ...
    def add_robot_to_task(
        ...
        try:
            with self.lock:
                ...
                logger.info(
                    f"添加机器人到任务: 机器人 {robot_name} ({robot_id}) 添加到任务 {task.name}"
                )
                return True, "机器人添加成功"
        ...
        if task.formation and task.robot_ids:
            ...
            for i, robot_id in enumerate(task.robot_ids):
                if robot_id in self.robot_states:
                    target_position = task.formation.get_robot_position(
                        i, leader_position
                    )
        ...
        try:
            if self.robot_service is not None:
                ...
                if robot_id in self.robot_states:
                    ...
                    # 例如：self.robot_service.send_motion_command("walk", {"direction":
                    # [dx, dy, dz], "distance": distance})

                    logger.debug(
                        f"发送机器人 {robot_id} 到位置 {target_position}, 距离: {distance:.3f}m"
                    )
    ...
    def update_robot_state(
        ...
        try:
            with self.lock:
                if robot_id not in self.robot_states:
                    self.robot_states[robot_id] = RobotState(
                        robot_id=robot_id, robot_name=f"robot_{robot_id}"
                    )
    ...
    def assign_formation(
        self, task_id: str, formation_type: str, spacing: float = 1.0
    ) -> Tuple[bool, str]:
        """分配队形给任务"""
        try:
            with self.lock:
    ...
    def assign_robot_role(
        self, task_id: str, robot_id: int, role: RobotRole
    ) -> Tuple[bool, str]:
        """分配角色给机器人"""
        try:
            with self.lock:
    ...
    def add_task_status_callback(
        self, task_id: str, callback: Callable[[CooperativeTask], None]
    ):
        """添加任务状态回调"""
        with self.lock:
    ...
    def add_robot_state_callback(
        self, robot_id: int, callback: Callable[[RobotState], None]
    ):
        """添加机器人状态回调"""
        with self.lock:
