...
class RobotMarketService:
    ...
    def create_listing(
    ...
    ) -> Tuple[bool, str, Optional[RobotMarketListing]]:
        """
        创建机器人市场列表
        ...
        try:
            ...
            robot = (
                self.db.query(Robot)
                .filter(Robot.id == robot_id, Robot.user_id == owner_id)
                .first()
            )
            ...
            # 检查是否已有相同标题的列表
            existing_listing = (
                self.db.query(RobotMarketListing)
                .filter(
            ...
            logger.info(
                f"创建机器人市场列表: {title} (机器人ID: {robot_id}, 所有者: {owner_id})"
            )
    ...
    def review_listing(
        ...
        try:
            ...
            listing = (
                self.db.query(RobotMarketListing)
                .filter(RobotMarketListing.id == listing_id)
                .first()
            )
            ...
            logger.info(
                f"审核机器人市场列表: {listing.title} - {status_text} (审核员: {reviewer_id})"
            )
    ...
    def get_listing(
        self, listing_id: int, increment_view: bool = True
    ) -> Optional[RobotMarketListing]:
        """
        获取列表详情
        ...
        try:
            listing = (
                ...
                .filter(
                    RobotMarketListing.id == listing_id,
                    RobotMarketListing.status
                    == RobotMarketStatus.APPROVED,  # 只返回已审核通过的
                )
                .first()
            )
    ...
    def search_listings(
        ...
        page: int = 1,
        page_size: int = 20,
    ) -> Tuple[List[RobotMarketListing], int]:
        """
        搜索机器人市场列表
        ...
        try:
            ...
            if min_rating > 0:
                db_query = db_query.filter(
                    RobotMarketListing.average_rating >= min_rating
                )
    ...
    def add_rating(
        ...
        try:
            ...
            # 检查用户是否已评分
            existing_rating = (
                self.db.query(RobotMarketRating)
                .filter(
            ...
            self.db.commit()
            logger.info(
                f"添加评分: 列表ID={listing_id}, 用户ID={user_id}, 评分={rating}"
            )
            return True, "评分提交成功"
    ...
    def _update_listing_rating(self, listing_id: int):
        ...
        try:
            ...
            result = (
                ...
                .filter(RobotMarketRating.listing_id == listing_id)
                .first()
            )
            ...
            if result and result.count > 0:
                ...
                if listing:
                    ...
                    listing.average_rating = (
                        float(result.average) if result.average else 0.0
                    )
    ...
    def add_comment(
    ...
    ) -> Tuple[bool, str, Optional[RobotMarketComment]]:
        """
        添加评论
        ...
        try:
            ...
            if parent_comment_id:
                parent_comment = (
                    self.db.query(RobotMarketComment)
                    .filter(RobotMarketComment.id == parent_comment_id)
                    .first()
                )
            ...
            logger.info(
                f"添加评论: 列表ID={listing_id}, 用户ID={user_id}, 内容长度={len(content)}"
            )
    ...
    def record_download(
        ...
        try:
            ...
            logger.info(
                f"记录下载: 列表ID={listing_id}, 用户ID={user_id}, 类型={download_type}"
            )
            return True, "下载记录成功"
    ...
    def toggle_favorite(
        self, listing_id: int, user_id: int, folder: str = "default", notes: str = ""
    ) -> Tuple[bool, str, bool]:
        """
        切换收藏状态
        ...
        try:
            ...
            # 检查是否已收藏
            favorite = (
                self.db.query(RobotMarketFavorite)
                .filter(
            ...
            if favorite:
                ...
                message = "已添加到收藏"
                logger.info(
                    f"添加收藏: 列表ID={listing_id}, 用户ID={user_id}, 收藏夹={folder}"
                )
    ...
    def get_popular_listings(
        self, limit: int = 10, category: Optional[RobotMarketCategory] = None
    ) -> List[RobotMarketListing]:
        """
        获取热门列表
    ...
    def get_listing_statistics(self, listing_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            rating_distribution = (
                ...
                .filter(RobotMarketRating.listing_id == listing_id)
                .group_by(RobotMarketRating.rating)
                .all()
            )
            ...
            downloads_by_day = (
                ...
                .group_by(func.date(RobotMarketDownload.created_at))
                .all()
            )
