...
class RobotDemonstrationIntegration:
    ...
    async def stop_demonstration_recording(
        self, robot_id: int, save: bool = True
    ) -> Optional[Demonstration]:
        """
        停止录制机器人示范
    ...
    async def apply_demonstration_frame(
        self, robot_id: int, frame_data: FrameData, control_mode: str = "position"
    ) -> bool:
        """
        应用示范帧数据到机器人控制
        ...
        try:
            ...
            if frame_data.joint_positions and control_mode == "position":
                # 这里应该调用机器人控制API
                # 例如: await robot_controller.set_joint_positions(robot_id,
                # frame_data.joint_positions)
                logger.debug(
                    f"应用关节位置控制: {len(frame_data.joint_positions)} 个关节"
                )
            ...
            # 应用关节速度控制
                ...
                # 这里应该调用机器人控制API
                logger.debug(
                    f"应用关节速度控制: {len(frame_data.joint_velocities)} 个关节"
                )
            ...
            # 应用关节扭矩控制
                ...
                # 这里应该调用机器人控制API
                logger.debug(
                    f"应用关节扭矩控制: {len(frame_data.joint_torques)} 个关节"
                )
