...
class SimulationService(BaseService):
    ...
    def _initialize_service(self) -> bool:
        ...
        try:
            ...
            self.logger.info(
                f"仿真环境服务初始化成功，活动接口: {self._active_interface}"
            )
            return True
        ...
        try:
            ...
            simulation = UnifiedSimulation(
                ...
                engine_config={
                    ...
                    "physics_timestep": extra_config.get(
                        "physics_timestep", 1.0 / 240.0
                    ),
                    "realtime_simulation": extra_config.get(
                        "realtime_simulation", False
                    ),
                },
                gui_enabled=extra_config.get("gui_enabled", False),
            ...
            if simulation.connect():
                if (
                    hasattr(simulation, "backend_type")
                    and simulation.backend_type == "pybullet"
                ):
                    ...
                    self.logger.warning(
                        f"统一仿真接口后端类型不是pybullet: {                             getattr(                                 simulation,                                 'backend_type',                                 'unknown')}"
                    )
                    return False
            else:
    ...
    def _initialize_unified_gazebo_interface(self) -> bool:
        ...
        try:
            ...
            simulation = UnifiedSimulation(
                engine="gazebo",
                engine_config={
                    "ros_master_uri": extra_config.get(
                        "ros_master_uri", "http://localhost:11311"
                    ),
            ...
            if simulation.connect():
                if (
                    hasattr(simulation, "backend_type")
                    and simulation.backend_type == "gazebo"
                ):
                    ...
                    self.logger.warning(
                        f"统一仿真接口后端类型不是gazebo: {                             getattr(                                 simulation,                                 'backend_type',                                 'unknown')}"
                    )
                    return False
            else:
    ...
    def _initialize_pybullet_interface(self) -> bool:
        ...
        try:
            ...
            if simulation.connect():
                if (
                    hasattr(simulation, "pybullet_available")
                    and simulation.pybullet_available
                ):
                    self._simulation_interfaces["pybullet"] = simulation
        ...
        if interface_name:
            if interface_name in self._simulation_interfaces:
                ...
                status = {
                    ...
                    "connected": True,
                    "backend_type": (
                        getattr(interface, "backend_type", "unknown")
                        if hasattr(interface, "backend_type")
                        else "unknown"
                    ),
                    "engine": (
                        getattr(interface, "engine", "unknown")
                        if hasattr(interface, "engine")
                        else "unknown"
                    ),
                    "available": True,
    ...
    def execute_simulation_command(
        ...
        try:
            ...
            for name, interface in self._simulation_interfaces.items():
                if (
                    name == self._active_interface
                    or name.replace("unified_", "") == self._active_interface
                ):
