...
class RealSensorReader:
    ...
    def __init__(self):
        ...
        self.logger.info(f"真实传感器读取器初始化完成，检测到 {len(self.detected_sensors)} 个传感器")
    ...
    def _check_hardware_libraries(self) -> Dict[str, bool]:
        ...
        for lib_name in libraries.keys():
            try:
                ...
                self.logger.debug(f"硬件库 {lib_name} 不可用")
            except Exception as e:
                self.logger.debug(f"检查硬件库 {lib_name} 时出错: {e}")
    ...
    def _detect_sensors(self):
        ...
        if self.detected_sensors:
            ...
            for sensor in self.detected_sensors:
                self.logger.info(f"  - {sensor['name']} ({sensor['type']}) 通过 {sensor['bus']}")
        else:
            ...
            for address in range(0x03, 0x78):
                try:
                    ...
                    if sensor_info:
                        ...
                        self.logger.debug(f"检测到I2C传感器: {sensor_info['name']} 地址 0x{address:02x}")
    ...
    def _identify_i2c_sensor(self, address: int) -> Optional[Dict[str, Any]]:
        ...
        # 常见I2C传感器地址映射
        i2c_address_map = {
            0x76: {"name": "BMP280/BME280", "type": "pressure", "bus": SensorBusType.I2C.value},
            0x77: {"name": "BMP280/BME280", "type": "pressure", "bus": SensorBusType.I2C.value},
    ...
    def _detect_spi_sensors(self):
        ...
        try:
            ...
            spi_sensors = [
                ...
                {"name": "MAX31855", "type": "temperature", "bus": SensorBusType.SPI.value},
            ]
        ...
        try:
            ...
            if "BMP280" in sensor_name or "BME280" in sensor_name:
                ...
                return {
                    ...
                    "unit": {"acceleration": "m/s²", "gyroscope": "rad/s", "temperature": "°C"},
                    "timestamp": time.time(),
                    "accuracy": 0.9,
                    "is_real": True,
                }
    ...
    def _read_spi_sensor(self, sensor: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        sensor_id = sensor["id"]
        sensor_name = sensor.get("name", "")
        sensor_address = sensor.get("address", 0)
        ...
        try:
            ...
            if not spi_available:
                ...
                if "MCP3008" in sensor_name:
                    ...
                    daily_variation = 5.0 * math.sin(hour_of_day * math.pi / 12 - math.pi/2)
        ...
        try:
            if "dht11" in sensor_name or "dht22" in sensor_name:
                ...
                if self.hardware_libraries.get("Adafruit_DHT", False):
                    ...
                    sensor_type = Adafruit_DHT.DHT11 if "dht11" in sensor_name else Adafruit_DHT.DHT22
