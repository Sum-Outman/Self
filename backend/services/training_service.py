...
from backend.db_models.agi import TrainingJob, AGIModel
...
class TrainingService:
    ...
    def __init__(self):
        if not self._initialized:
            ...
            # 训练服务器配置
            self.training_server_url = os.getenv("TRAINING_SERVER_URL", "http://localhost:8001")
            self.training_api_key = os.getenv("TRAINING_API_KEY", "test_key")
            self.use_training_server = os.getenv("USE_TRAINING_SERVER", "true").lower() == "true"
            ...
            # 记录训练服务器配置
            logger.info(f"训练服务器配置: URL={self.training_server_url}, 使用训练服务器={self.use_training_server}")
        ...
        try:
            ...
            if config:
                ...
                dataset_size = config.get("dataset_size", 
                                         config.get("training_data_size", 
                                                   config.get("num_samples", 0)))
                epochs = config.get("epochs", 
                                  config.get("num_epochs", 
                                            config.get("training_epochs", 0)))
                gpu_usage = config.get("gpu_usage", 
                                      config.get("gpu_utilization", 0.0))
                memory_usage = config.get("memory_usage", 
                                         config.get("memory_utilization", 0.0))
                batch_size = config.get("batch_size", 
                                       config.get("batch_size_per_gpu", 0))
            ...
            if epochs <= 0:
                # 根据常见实践设置默认epoch数
                if "pretrain" in str(config).lower() or "finetune" in str(config).lower():
                    epochs = 10
                else:
                    epochs = 50
            ...
            job_dict = {
                ...
                "start_time": db_job.started_at.isoformat() if db_job.started_at else None,
                "estimated_time": self._estimate_completion_time(db_job) if db_job.started_at else None,
                "gpu_usage": gpu_usage,
                "memory_usage": memory_usage,
                "config": config,
                "logs": self._get_training_logs(db_job),  # 获取训练日志
                "created_at": db_job.created_at.isoformat() if db_job.created_at else datetime.now(timezone.utc).isoformat(),
                "updated_at": db_job.updated_at.isoformat() if db_job.updated_at else db_job.created_at.isoformat() if db_job.created_at else datetime.now(timezone.utc).isoformat(),
    ...
    def _get_training_logs(self, db_job) -> List[str]:
        ...
        if hasattr(db_job, 'logs') and db_job.logs:
            ...
            try:
                import json
                job_logs = json.loads(db_job.logs) if isinstance(db_job.logs, str) else db_job.logs
    ...
    def get_training_jobs(self, status_filter: Optional[str] = None) -> List[Dict[str, Any]]:
        """获取训练任务列表"""
        try:
            # 如果数据库可用，从数据库获取
    ...
    def get_training_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "data_source": "database" if self._use_database and self._db else "no_database"
            }
        except Exception as e:
        ...
        try:
            ...
            if cuda_available and gpu_count > 0:
                ...
                return {
                    ...
                    "system_platform": psutil.OS_NAME if hasattr(psutil, 'OS_NAME') else "Unknown",
                    "python_version": psutil.PYTHON_VERSION if hasattr(psutil, 'PYTHON_VERSION') else "Unknown",
            ...
            if torch.cuda.is_available():
                ...
                for i in range(gpu_count):
                    try:
                        ...
                        info[f"device_{i}_memory_total"] = props.total_memory / (1024**3)
                        info[f"device_{i}_memory_allocated"] = memory_allocated
                        info[f"device_{i}_memory_reserved"] = memory_reserved
                        info[f"device_{i}_memory_free"] = (props.total_memory / (1024**3)) - memory_allocated
                        ...
                        try:
                            ...
                            device_temperature = pynvml.nvmlDeviceGetTemperature(handle, pynvml.NVML_TEMPERATURE_GPU)
                            pynvml.nvmlShutdown()
                ...
                if gpu_count > 0:
                    info["average_memory_allocated"] = total_memory_allocated / gpu_count
                    ...
                    if utilization_count > 0:
                        info["average_utilization"] = total_utilization / utilization_count
                    else:
    ...
    def _get_realtime_trainer_metrics(self) -> Dict[str, Any]:
        ...
        for port in possible_ports:
            try:
                # 尝试获取当前性能指标
                response = requests.get(f"http://localhost:{port}/api/performance/current", timeout=2.0)
                if response.status_code == 200:
                    ...
                    # 尝试获取系统信息
                    try:
                        sys_response = requests.get(f"http://localhost:{port}/api/performance/system", timeout=2.0)
            ...
            raw_model_types = [
                {
                    ...
                    "supported_tasks": ["文本生成", "图像描述", "多模态问答", "视频理解"],
                },
                {
                    "id": "transformer",
            ...
            if self._use_database and self._db:
                try:
                    ...
                    if db_id is not None:
                        # 查询数据库
                        db_job = self._db.query(TrainingJob).filter(TrainingJob.id == db_id).first()
                        if db_job:
    ...
    def update_training_job(self, job_id: str, updates: Dict[str, Any]) -> Dict[str, Any]:
        """更新训练任务"""
        try:
            # 如果数据库可用，更新数据库
            if self._use_database and self._db:
                        ...
                        if db_job:
                            ...
                            if db_job.config:
                                try:
                                    config = json.loads(db_job.config)
                                    if isinstance(config, dict):
                                        logs.append(f"学习率: {config.get('learning_rate', 'N/A')}")
                                        logs.append(f"批大小: {config.get('batch_size', 'N/A')}")
                                        logs.append(f"优化器: {config.get('optimizer', 'N/A')}")
                                except Exception:
                            ...
                            if db_job.result:
                                try:
                                    ...
                                    if isinstance(result, dict):
                                        logs.append(f"最终损失: {result.get('final_loss', 'N/A')}")
                                        logs.append(f"准确率: {result.get('accuracy', 'N/A')}")
                                except Exception:
    ...
    def start_training_job(self, job_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            training_server_message = "训练任务已启动（模拟模式）"
            ...
            if self.use_training_server and REQUESTS_AVAILABLE:
                try:
                    ...
                    if job_data and "config" in job_data:
                        ...
                        init_response = requests.post(init_url, json=init_data, headers=headers, timeout=5.0)
                        if init_response.status_code == 200:
                            logger.info(f"训练器初始化成功: {job_id}")
                        else:
                            logger.warning(f"训练器初始化失败（状态码 {init_response.status_code}），继续尝试启动训练")
                    
                    # 启动训练
                    start_url = f"{self.training_server_url}/train/start"
                    start_response = requests.post(start_url, headers=headers, timeout=5.0)
                    
                    if start_response.status_code == 200:
                        training_started = True
                        training_server_message = "训练任务已通过训练服务器启动"
                        logger.info(f"训练服务器启动成功: {job_id}")
                    else:
                        logger.warning(f"训练服务器启动失败（状态码 {start_response.status_code}），使用模拟模式")
                        
                except requests.exceptions.ConnectionError:
                    logger.warning(f"无法连接到训练服务器 {self.training_server_url}，使用模拟模式")
                except Exception as e:
                    logger.warning(f"调用训练服务器失败: {e}，使用模拟模式")
            
            # 记录训练启动状态
            if training_started:
                logger.info(f"训练任务已通过训练服务器启动: {job_id}")
            else:
                logger.info(f"训练任务已启动（模拟模式）: {job_id}")
            
            return {
                "success": True,
                "job_id": job_id,
                "message": training_server_message,
                "training_server_used": training_started,
                "training_server_url": self.training_server_url if training_started else None
            }
        except Exception as e:
    ...
    def upload_dataset(self, file_content: bytes, filename: str, metadata: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        """上传数据集
        ...
        try:
            ...
            logger.info(f"数据集上传成功: {filename} -> {unique_filename} ({len(file_content)} bytes)")
    ...
    def hyperparameter_optimization(self, 
                                  ...
                                  num_trials: int = 10,
                                  objective_metric: str = "accuracy") -> Dict[str, Any]:
        """执行超参数优化
        ...
        try:
            logger.info(f"开始超参数优化: 模型类型={model_type}, "
            ...
            def objective_function(**hyperparameters) -> float:
                ...
                try:
                    ...
                    # 根据模型类型创建模型配置
                    model_config = self._create_model_config(model_type, hyperparameters)
                    ...
                    try:
                        ...
                        if objective_metric == "accuracy":
                            score = metrics.get("val_accuracy", metrics.get("accuracy", 0.0))
                            ...
                            # 如果没有找到合适的指标，返回默认值
                            logger.warning(f"未找到适合目标指标 {objective_metric} 的评估结果")
                            return 0.5
            ...
            if objective_metric == "multi_objective":
                ...
                objective_functions = {
                    ...
                    "latency": lambda **params: self._estimate_latency(model_type, params),  # 基于超参数估计延迟
                    "model_size": lambda **params: self._estimate_model_size(model_type, params),  # 基于超参数估计模型大小
                }
                ...
                # 单目标优化
                result = optimizer.optimize(objective_function, hyperparameter_space, num_trials)
    ...
    def _create_model_config(self, model_type: str, hyperparameters: Dict[str, Any]) -> Dict[str, Any]:
        """创建模型配置
    ...
    def _estimate_latency(self, model_type: str, hyperparameters: Dict[str, Any]) -> float:
        """估计模型推理延迟
        ...
        batch_factor = min(1.0, 64.0 / batch_size)  # 批次越大，延迟越高但每个样本延迟可能更低
    ...
    def _estimate_model_size(self, model_type: str, hyperparameters: Dict[str, Any]) -> float:
        """估计模型大小
        ...
        if model_type == "multimodal":
            ...
            dim_feedforward = hyperparameters.get("dim_feedforward", 2048)
            # 近似Transformer参数数量
            size_factor = num_layers * (12 * d_model * d_model + 2 * d_model * dim_feedforward) / 10000000
        elif model_type == "cnn":
            channels = hyperparameters.get("channels", [32, 64, 128])
