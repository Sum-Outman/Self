...
class CollisionDetectionService:
    ...
    def check_collision(
        self, object_id1: str, object_id2: str
        ...
        try:
            ...
            # 根据形状类型调用相应的碰撞检测函数
            if (
                shape1.shape_type == CollisionShapeType.BOX
                and shape2.shape_type == CollisionShapeType.BOX
            ):
                return self._check_box_box_collision(shape1, shape2)
            elif (
                shape1.shape_type == CollisionShapeType.SPHERE
                and shape2.shape_type == CollisionShapeType.SPHERE
            ):
                ...
                collision, depth, normal = self._check_box_sphere_collision(
                    shape2, shape1
                )
                return (
                    collision,
    ...
    def _check_box_box_collision(
        self, box1: CollisionShape, box2: CollisionShape
    ) -> Tuple[bool, float, List[float]]:
        ...
        # 检查每个轴上的重叠
        overlap_x = abs(box1.position[0] - box2.position[0]) <= (
            half_size1[0] + half_size2[0]
        )
        overlap_y = abs(box1.position[1] - box2.position[1]) <= (
            half_size1[1] + half_size2[1]
        )
        overlap_z = abs(box1.position[2] - box2.position[2]) <= (
            half_size1[2] + half_size2[2]
        )

        if overlap_x and overlap_y and overlap_z:
            # 计算穿透深度和法线（完整）
            depth_x = (half_size1[0] + half_size2[0]) - abs(
                box1.position[0] - box2.position[0]
            )
            depth_y = (half_size1[1] + half_size2[1]) - abs(
                box1.position[1] - box2.position[1]
            )
            depth_z = (half_size1[2] + half_size2[2]) - abs(
                box1.position[2] - box2.position[2]
            )

            # 找到最小穿透深度的轴
            if depth_x < depth_y and depth_x < depth_z:
                normal = [
                    1.0 if box1.position[0] < box2.position[0] else -1.0,
                    0.0,
                    0.0,
                ]
                depth = depth_x
                ...
                normal = [
                    0.0,
                    1.0 if box1.position[1] < box2.position[1] else -1.0,
                    0.0,
                ]
                depth = depth_y
            else:
                normal = [
                    0.0,
                    0.0,
                    1.0 if box1.position[2] < box2.position[2] else -1.0,
                ]
                depth = depth_z
    ...
    def _check_sphere_sphere_collision(
        self, sphere1: CollisionShape, sphere2: CollisionShape
    ) -> Tuple[bool, float, List[float]]:
        """检测两个球体之间的碰撞"""
        # 计算中心距离
    ...
    def _check_box_sphere_collision(
        self, box: CollisionShape, sphere: CollisionShape
    ) -> Tuple[bool, float, List[float]]:
        ...
        closest_point = [
            max(
                box.position[0] - half_size[0],
                min(sphere.position[0], box.position[0] + half_size[0]),
            ),
            max(
                box.position[1] - half_size[1],
                min(sphere.position[1], box.position[1] + half_size[1]),
            ),
            max(
                box.position[2] - half_size[2],
                min(sphere.position[2], box.position[2] + half_size[2]),
            ),
        ]
    ...
    def _check_bounding_sphere_collision(
        self, shape1: CollisionShape, shape2: CollisionShape
    ) -> Tuple[bool, float, List[float]]:
        """使用边界球检测碰撞（通用但保守的方法）"""
        # 计算边界球半径
    ...
    def ray_cast(
        self, origin: List[float], direction: List[float], max_distance: float = 100.0
    ) -> List[Dict[str, Any]]:
        """射线检测
        ...
        for object_id, shape in self.shapes.items():
            hit_distance = self._ray_shape_intersection(
                origin, dir_norm, shape, max_distance
            )
