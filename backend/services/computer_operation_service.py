...
class ComputerOperationService:
        ...
        return {
            "file_management": {
                ...
                "delete_file": {
                    "description": "删除文件",
                    "steps": [
                        {"action": "command", "params": {"command": "del", "args": ["test.txt"]}}
                    ]
                }
            },
            "web_browsing": {
                ...
                "search_web": {
                    ...
                    "steps": [
                        ...
                        {"action": "keyboard", "params": {"text": "https://www.google.com"}},
                        {"action": "keyboard", "params": {"keys": ["enter"]}},
                        {"action": "wait", "params": {"seconds": 3}},
    ...
    def _initialize_command_patterns(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "系统信息": {
                ...
                "查看磁盘空间": "wmic logicaldisk get size,freespace,caption" if os.name == "nt" else "df -h",
                "查看内存使用": "wmic OS get FreePhysicalMemory,TotalVisibleMemorySize" if os.name == "nt" else "free -h",
            ...
            "进程管理": {
                "结束进程": "taskkill /PID {pid} /F" if os.name == "nt" else "kill -9 {pid}",
                "查找进程": "tasklist | findstr {name}" if os.name == "nt" else "ps aux | grep {name}"
            }
        }
        ...
        try:
            ...
            if screenshot_data:
                ...
                if PY_AUTOGUI_AVAILABLE:
                    ...
                    screen_image = cv2.cvtColor(np.array(screenshot), cv2.COLOR_RGB2BGR) if CV2_AVAILABLE else None
                else:
                    screen_image = None
    ...
    def perform_keyboard_operation(self, operation: KeyboardOperation) -> OperationResult:
        """执行键盘操作
    ...
    def _perform_virtual_keyboard_operation(self, operation: KeyboardOperation) -> Dict[str, Any]:
        """执行虚拟键盘操作（已禁用）
    ...
    def _perform_physical_keyboard_operation(self, operation: KeyboardOperation) -> Dict[str, Any]:
        """执行实体机器人键盘操作
    ...
    def _perform_simulated_keyboard_operation(self, operation: KeyboardOperation) -> Dict[str, Any]:
        """执行模拟键盘操作（已禁用）
    ...
    def _perform_command_line_keyboard_operation(self, operation: KeyboardOperation) -> Dict[str, Any]:
        """执行命令行键盘操作
        ...
        if operation.text:
            if system == "Windows":
                ...
                command = f'osascript -e \'tell application "System Events" to keystroke "{escaped_text}"\''
            else:  # Linux
    ...
    def _perform_virtual_mouse_operation(self, operation: MouseOperation) -> Dict[str, Any]:
        """执行虚拟鼠标操作（已禁用）
    ...
    def _perform_command_line_mouse_operation(self, operation: MouseOperation) -> Dict[str, Any]:
        """执行命令行鼠标操作
    ...
    def _perform_physical_mouse_operation(self, operation: MouseOperation) -> Dict[str, Any]:
        """执行实体机器人鼠标操作
    ...
    def _perform_simulated_mouse_operation(self, operation: MouseOperation) -> Dict[str, Any]:
        """执行模拟鼠标操作（已禁用）
    ...
    def _validate_mouse_operation(self, operation: MouseOperation) -> bool:
        ...
        # 检查位置是否在屏幕范围内
        x, y = operation.position
        if x < 0 or y < 0 or x > self.screen_resolution[0] or y > self.screen_resolution[1]:
    ...
    def _validate_command(self, operation: CommandOperation) -> bool:
        ...
        dangerous_commands = [
            ...
            "chmod 777", "chown", "dd if=", "shutdown", "halt", "poweroff",
            "wmic process call create", "wmic bios", "reg delete", "reg add"
        ]
    ...
    def translate_natural_language_to_command(self, natural_language: str) -> Dict[str, Any]:
        """将自然语言翻译为命令行
    ...
    def execute_task(self, task_name: str, params: Dict[str, Any] = None) -> OperationResult:
        """执行预定义任务
        ...
        try:
            # 查找任务
            category, task = task_name.split(".", 1) if "." in task_name else (None, task_name)
