...
class PathPlanningService:
    ...
    def _update_grid_with_obstacle(self, env_id: str, obstacle: Obstacle):
        ...
        y_end = min(height-1, int((obstacle.position[1] + obstacle.dimensions[1]/2)))
    ...
    def plan_path(self,
                 ...
                 step_size: float = 1.0) -> PlanningResult:
        """规划路径
        ...
        try:
            # 根据算法选择规划方法
            if algorithm in [PathPlanningAlgorithm.ASTAR, PathPlanningAlgorithm.DIJKSTRA]:
                result = self._plan_grid_path(env_id, start, goal, algorithm, max_iterations)
            elif algorithm in [PathPlanningAlgorithm.RRT, PathPlanningAlgorithm.RRT_STAR]:
                result = self._plan_rrt_path(env_id, start, goal, algorithm, max_iterations, step_size)
            else:
                # 默认使用A*算法
                result = self._plan_grid_path(env_id, start, goal, PathPlanningAlgorithm.ASTAR, max_iterations)
    ...
    def _plan_grid_path(self,
                       env_id: str,
                       start: List[float],
                       goal: List[float],
                       algorithm: PathPlanningAlgorithm,
                       max_iterations: int) -> PlanningResult:
        ...
        while open_list and nodes_explored < max_iterations:
            ...
            for neighbor_idx in neighbors:
                ...
                # 计算新的g成本
                tentative_g_cost = current_cell.g_cost + self._cost_between(current_idx, neighbor_idx)
                ...
                if tentative_g_cost < neighbor_cell.g_cost:
                    ...
                    # 添加到开放列表
                    heapq.heappush(open_list, (neighbor_cell.f_cost, id(neighbor_cell), neighbor_cell))
    ...
    def _plan_rrt_path(self,
                      ...
                      algorithm: PathPlanningAlgorithm,
                      max_iterations: int,
                      step_size: float) -> PlanningResult:
        """RRT路径规划（连续环境）"""
    ...
    def _is_valid_cell(self, grid: List[List[List[GridCell]]], idx: Tuple[int, int, int]) -> bool:
        """检查网格索引是否有效"""
        x, y, z = idx
        depth = len(grid)
        height = len(grid[0])
    ...
    def _get_neighbors(self, grid: List[List[List[GridCell]]], idx: Tuple[int, int, int]) -> List[Tuple[int, int, int]]:
        """获取邻居单元格（8方向或26方向）"""
        x, y, z = idx
        depth = len(grid)
        height = len(grid[0])
    ...
    def _reconstruct_path(self, grid: List[List[List[GridCell]]], goal_cell: GridCell) -> List[List[float]]:
        """重建路径"""
        path = []
        current_cell = goal_cell
        
        while current_cell:
            path.append([float(current_cell.x), float(current_cell.y), float(current_cell.z)])
    ...
    def _point_in_obstacle(self, point: List[float], obstacle: Obstacle) -> bool:
        ...
        if obstacle.shape_type == "box":
            ...
            return (
                obstacle.position[0] - half_dims[0] <= point[0] <= obstacle.position[0] + half_dims[0] and
                obstacle.position[1] - half_dims[1] <= point[1] <= obstacle.position[1] + half_dims[1] and
                obstacle.position[2] - half_dims[2] <= point[2] <= obstacle.position[2] + half_dims[2]
            )
