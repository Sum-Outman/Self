...
try:
    ...
    from training.gazebo_environment import (
        GazeboRobotEnvironment,
        GazeboEnvironmentConfig,
        RobotTaskType,
        GAZEBO_AVAILABLE,
    )
...
class TrainingCallback(BaseCallback):
    ...
    def _on_step(self) -> bool:
        ...
        # 获取当前奖励
        reward = (
            self.locals.get("rewards", [0.0])[0] if self.locals.get("rewards") else 0.0
        )
        done = (
            self.locals.get("dones", [False])[0] if self.locals.get("dones") else False
        )
...
class RobotReinforcementTrainingService:
    ...
    def create_training_task(
        self, config: RobotTrainingConfig, user: User
    ) -> Dict[str, Any]:
        """创建训练任务
        ...
        if not self.dependencies_available:
            return {"success": False, "error": "强化学习依赖库不可用", "task_id": None}

        # 验证机器人
        robot = (
            self.db.query(Robot)
            .filter(Robot.id == config.robot_id, Robot.user_id == user.id)
            .first()
        )
        ...
        logger.info(
            f"创建训练任务 {task_id}，机器人: {robot.name}，任务类型: {config.task_type.value}"
        )

        return {"success": True, "task_id": task_id, "message": "训练任务创建成功"}
    ...
    def _run_training(self, task_id: str):
        ...
        try:
            logger.info(
                f"开始训练任务 {task_id}: {                     config.task_type.value}，算法: {                     config.algorithm.value}"
            )
            ...
            model = model_class(
                ...
                policy_kwargs=(
                    {"net_arch": config.hidden_layers} if config.hidden_layers else None
                ),
            )
            ...
            with self.task_lock:
                if task_id in self.training_tasks:
                    ...
                    task["progress"].end_time = datetime.now(timezone.utc)
                    task["progress"].success_rate = task["progress"].episodes / max(
                        config.total_timesteps // config.max_steps, 1
                    )
    ...
    def get_task_status(self, task_id: str) -> Dict[str, Any]:
        ...
        with self.task_lock:
            ...
            return {
                ...
                "config": (
                    task["config"].to_dict()
                    if hasattr(task["config"], "to_dict")
                    else asdict(task["config"])
                ),
    ...
    def evaluate_model(self, task_id: str, num_episodes: int = 10) -> Dict[str, Any]:
        ...
        with self.task_lock:
            ...
            if model is None:
                return {"success": False, "error": "模型不存在或未训练"}
