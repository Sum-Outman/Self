...
class ImitationSession:
    ...
    robot_executed_poses: List[Dict[str, Any]] = field(
...
class VisualImitationService:
    ...
    def record_human_pose(
        self, session_id: str, pose_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        """录制人类姿态
    ...
    def analyze_pose_sequence(self, session_id: str) -> Dict[str, Any]:
        ...
        try:
            with self._lock:
                ...
                if not poses:
                    return {"success": False, "error": "没有录制的姿态数据"}
    ...
    def generate_robot_trajectory(
        self, session_id: str, adaptation_level: float = 0.5
    ) -> Dict[str, Any]:
        """生成机器人运动轨迹
        ...
        try:
            with self._lock:
                ...
                if not session.recorded_poses or not session.robot_executed_poses:
                    return {"success": False, "error": "缺少必要的数据进行评估"}
                    ...
                    sessions_list.append(
                        {
                            "session_id": session_id,
                            "target_action": (
                                session.target_action.value
                                if session.target_action
                                else None
                            ),
                            ...
                            "robot_executed_pose_count": len(
                                session.robot_executed_poses
                            ),
