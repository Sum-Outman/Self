...
from ..db_models.knowledge import AccessAuditLog, PermissionAuditLog, KnowledgeItem
...
class AccessAuditSystem:
    ...
    def _check_anomalies(self, 
        ...
        # 检查敏感操作
        if action in [AuditAction.DELETE, AuditAction.EXPORT, AuditAction.PERMISSION_CHANGE]:
    ...
    def _detect_failed_login_anomaly(self, user_id: str, ip_address: Optional[str]) -> List[AnomalyDetectionResult]:
        """检测失败登录异常"""
        ...
        failed_logins = self.db.query(AccessAuditLog).filter(
            and_(
    ...
    def _detect_access_rate_anomaly(self, user_id: str, ip_address: Optional[str]) -> List[AnomalyDetectionResult]:
        """检测访问频率异常"""
        ...
        access_count = self.db.query(AccessAuditLog).filter(
            and_(
                AccessAuditLog.user_id == user_id,
                AccessAuditLog.timestamp >= one_hour_ago,
                AccessAuditLog.action.in_([AuditAction.VIEW.value, AuditAction.SEARCH.value])
            )
        ).count()
    ...
    def _detect_unusual_hour_access_anomaly(self, user_id: str) -> List[AnomalyDetectionResult]:
        """检测非工作时间访问异常"""
        ...
        unusual_access = self.db.query(AccessAuditLog).filter(
            and_(
                AccessAuditLog.user_id == user_id,
    ...
    def _detect_sensitive_operation_anomaly(self, user_id: str, action: AuditAction) -> List[AnomalyDetectionResult]:
        """检测敏感操作异常"""
        ...
        sensitive_ops = self.db.query(AccessAuditLog).filter(
            and_(
                AccessAuditLog.user_id == user_id,
    ...
    def _log_anomaly(self, anomaly: AnomalyDetectionResult):
        ...
        try:
            ...
            self.logger.warning(f"检测到异常: {anomaly.anomaly_type} - {anomaly.description}")
    ...
    def get_statistics(self, days: int = 30) -> AuditStatistics:
        ...
        # 查询总记录数
        total_records = self.db.query(AccessAuditLog).filter(
        ...
        actions = self.db.query(
            AccessAuditLog.action,
            func.count(AccessAuditLog.id)
        ).filter(
            AccessAuditLog.timestamp >= start_date
        ).group_by(AccessAuditLog.action).all()
        ...
        users = self.db.query(
            ...
            AccessAuditLog.timestamp >= start_date
        ).group_by(AccessAuditLog.user_id).all()
        ...
        severities = self.db.query(
            AccessAuditLog.severity,
            func.count(AccessAuditLog.id)
        ).filter(
            AccessAuditLog.timestamp >= start_date
        ).group_by(AccessAuditLog.severity).all()
        
        for severity, count in severities:
            records_by_severity[severity] = count
        
        # 计算成功率
        success_count = self.db.query(AccessAuditLog).filter(
            and_(
                AccessAuditLog.timestamp >= start_date,
        ...
        peak_hour_query = self.db.query(
            ...
            AccessAuditLog.timestamp >= start_date
        ).group_by('hour').order_by(func.count(AccessAuditLog.id).desc()).first()
        
        peak_hour = int(peak_hour_query[0]) if peak_hour_query else None
        
        # 最活跃用户
        most_active_user = max(records_by_user.items(), key=lambda x: x[1])[0] if records_by_user else None
        
        # 最常访问的项目
        most_accessed_item_query = self.db.query(
            AccessAuditLog.target_id,
            func.count(AccessAuditLog.id).label('count')
        ).filter(
            and_(
                AccessAuditLog.timestamp >= start_date,
        ...
        ).group_by(AccessAuditLog.target_id).order_by(func.count(AccessAuditLog.id).desc()).first()
        
        most_accessed_item = most_accessed_item_query[0] if most_accessed_item_query else None
        
        # 可疑活动数量（高风险和中风险）
        suspicious_activities = self.db.query(AccessAuditLog).filter(
            and_(
                AccessAuditLog.timestamp >= start_date,
                AccessAuditLog.severity.in_([AuditSeverity.HIGH.value, AuditSeverity.CRITICAL.value])
            )
        ).count()
    ...
    def _detect_anomalies_for_period(self, start_date: datetime, end_date: datetime) -> List[AnomalyDetectionResult]:
        """检测指定时间段内的异常"""
        anomalies = []
    ...
    def _generate_recommendations(self, 
                                ...
                                anomalies: List[AnomalyDetectionResult]) -> List[str]:
        """生成建议"""
        recommendations = []
        
        # 基于统计数据的建议
        if stats.success_rate < 0.95:
            recommendations.append(f"操作成功率较低 ({stats.success_rate:.2%})，建议检查系统稳定性")
        
        if stats.suspicious_activities > 0:
            recommendations.append(f"检测到 {stats.suspicious_activities} 次可疑活动，建议进行安全审查")
        
        if stats.average_records_per_day > 1000:
            recommendations.append(f"日均审计记录数较高 ({stats.average_records_per_day:.0f})，考虑优化日志记录策略")
        
        # 基于异常的建议
        for anomaly in anomalies:
            if anomaly.severity in [AuditSeverity.HIGH, AuditSeverity.CRITICAL]:
                recommendations.append(f"严重异常检测: {anomaly.description}，建议立即处理")
    ...
    def cleanup_old_logs(self, days_to_keep: int = 90) -> int:
        ...
        try:
            # 删除旧日志
            deleted_count = self.db.query(AccessAuditLog).filter(
                AccessAuditLog.timestamp < cutoff_date
            ).delete()
            
            self.db.commit()
            
            self.logger.info(f"清理审计日志: 删除 {deleted_count} 条 {days_to_keep} 天前的记录")
...
# 审计装饰器
def audit_access(action: AuditAction, 
                target_type: str,
            ...
            try:
                ...
                if db:
                    ...
                    audit_system.log_access(
                        ...
                        ip_address=ip_address,
                        user_agent=user_agent,
                        details={"endpoint": func.__name__, "result": "success" if success else "error"},
                        success=success,
                        error_message=error_message
                    )
