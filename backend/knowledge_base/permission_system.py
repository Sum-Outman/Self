...
class KnowledgePermissionSystem:
    ...
    def __init__(self, db: Session):
        ...
        self.inheritance_hierarchy = {
            ...
            "knowledge:item:delete": {"includes": ["knowledge:item:view"]},
    ...
    def _map_base_permission_to_knowledge(
        self, base_perm: str
    ...
    def grant_permission(
        ...
        if success:
            ...
            self.logger.info(
                f"权限授予成功: {grantor_id} -> {grantee_id}, {permission.value}"
            )
    ...
    def revoke_permission(
        ...
        # 从数据库移除权限规则
        success = self._remove_permission_rule(
            user_id, permission, target_id, target_type
        )
        ...
        if success:
            ...
            self.logger.info(
                f"权限撤销成功: {revoker_id} -> {user_id}, {permission.value}"
            )
    ...
    def _check_ownership(self, user_id: str, target_id: str, target_type: str) -> bool:
        ...
        if target_type == "item":
            item = (
                ...
                .filter(
                    KnowledgeItem.id == target_id, KnowledgeItem.uploaded_by == user_id
                )
                .first()
            )
    ...
    def can_delete_item(self, user_id: str, item_id: str) -> bool:
        ...
        return self.check_permission(
            user_id,
            KnowledgePermission.ITEM_DELETE,
            target_id=item_id,
            target_type="item",
        )

    def filter_items_by_permission(
        self,
        user_id: str,
        items: List[KnowledgeItem],
        permission: KnowledgePermission = KnowledgePermission.ITEM_VIEW,
    ) -> List[KnowledgeItem]:
        """根据权限过滤知识条目"""
        filtered = []
        ...
        for item in items:
            if self.check_permission(
                user_id, permission, target_id=item.id, target_type="item"
            ):
                filtered.append(item)

        return filtered

    def audit_permission_access(
        self,
        user_id: str,
        permission: KnowledgePermission,
        target_id: Optional[str] = None,
