...
class PolicyRule:
    ...
    def evaluate(self, 
        ...
        # 评估条件
        if not self._evaluate_condition(user, permission, target, context, request_details):
            return None  # 返回None
...
class PermissionValidator:
    ...
    def validate(self, 
        ...
        self.logger.debug(f"权限验证完成: {request.permission.value}, 耗时: {processing_time:.3f}s")
    ...
    def _get_target(self, target_id: str, target_type: str) -> Optional[Any]:
        """获取目标信息"""
        if target_type == "item":
            return self.db.query(KnowledgeItem).filter(KnowledgeItem.id == target_id).first()
    ...
    def _assess_risk(self,
        ...
        risk_score += perm_sensitivity * self.risk_models["operation_risk"]["weights"]["permission_sensitivity"]
        risk_factors.append(f"权限敏感度: {perm_sensitivity:.2f}")
        
        # 用户信任度风险
        user_trust = self._calculate_user_trust_level(user)
        user_risk = 1.0 - user_trust
        risk_score += user_risk * self.risk_models["operation_risk"]["weights"]["user_trust_level"]
        risk_factors.append(f"用户信任度: {user_trust:.2f}")
        
        # 目标敏感度风险
        target_sensitivity = self._get_target_sensitivity(target)
        risk_score += target_sensitivity * self.risk_models["operation_risk"]["weights"]["target_sensitivity"]
        risk_factors.append(f"目标敏感度: {target_sensitivity:.2f}")
        
        # 上下文风险
        context_risk = self._get_context_risk(request.context)
        risk_score += context_risk * self.risk_models["operation_risk"]["weights"]["context_risk"]
    ...
    def _generate_final_response(self,
        ...
        # 检查是否有拒绝的规则
        deny_responses = [r for r in policy_responses if r.result == ValidationResult.DENY]
        if deny_responses:
            return deny_responses[0]
        
        # 检查是否需要审核
        review_responses = [r for r in policy_responses if r.result == ValidationResult.REVIEW]
        if review_responses:
            # 合并所有审核要求
            all_constraints = {}
            all_approvals = []
        ...
        # 检查条件允许
        conditional_responses = [r for r in policy_responses if r.result == ValidationResult.CONDITIONAL]
...
def _check_approval(self, user_id: str, permission: KnowledgePermission, target_id: Optional[str]) -> bool:
    """检查是否已批准"""
    # 实际项目中查询审批状态
    return False


def _check_conditions(self, constraints: Dict[str, Any], kwargs: Dict[str, Any]) -> bool:
    """检查条件是否满足"""
    # 实际项目中检查条件
    return True
