...
from ..core.permissions import PermissionManager, UserRole
...
class ModePermissionManager:
    ...
    def check_permission(
        ...
        try:
            ...
            # 检查角色权限
            role_has_permission = permission in self.role_permissions.get(
                user_role, set()
            )
    ...
    def check_mode_switch_permission(
        self,
        user: User,
        from_mode: str,
        to_mode: str,
        ...
        if transition_type == "graceful":
            ...
            if self.config["require_approval_for_emergency"]:
                has_approval = self._check_emergency_approval(
                    user.id, from_mode, to_mode, reason
                )
                if not has_approval:
    ...
    def _check_for_anomalies(
        ...
        if permission in sensitive_permissions:
            self.logger.warning(
                f"敏感权限访问尝试: user_id={user_id}, " f"权限={permission.value}"
            )
            self.stats["anomaly_detections"] += 1

    def _check_emergency_approval(
        self, user_id: int, from_mode: str, to_mode: str, reason: Optional[str] = None
    ) -> bool:
        """检查紧急切换审批
    ...
    def get_audit_log(
        ...
        if permission is not None:
            filtered_log = [
                entry for entry in filtered_log if entry["permission"] == permission
            ]
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            **self.stats,
            ...
            "locked_users": sum(
                1
                for data in self.failed_attempts.values()
                if data.get("lockout_until") and datetime.now() < data["lockout_until"]
            ),
...
def get_global_mode_permission_manager(
