...
class ModeTransitionProtocol:
    ...
    def initiate_transition(
        ...
        with self.transition_lock:
            # 检查是否已经有进行中的切换
            if self.current_transition and self.current_transition.get("state") not in [
                TransitionState.COMPLETED,
                TransitionState.FAILED,
            ]:

                msg = (
                    f"已有进行中的切换: {self.current_transition.get('id', 'unknown')}"
                )
            ...
            # 生成切换ID
            transition_id = (
                f"transition_{int(time.time())}_{from_mode[:3]}_{to_mode[:3]}"
            )
            ...
            try:
                ...
                if not check_result.safe:
                    if transition_type == TransitionType.EMERGENCY:
                        self.logger.warning(
                            f"紧急切换忽略安全检查: {check_result.errors}"
                        )
                    else:
                ...
                self.current_transition.update(
                    {
                        ...
                        "state": self.current_transition.get(
        ...
        try:
            ...
            if stability_score < self.config["min_system_stability_score"]:
                ...
                result.errors.append(
                    f"系统稳定性分数过低: {                         stability_score:.2f} < {                         self.config['min_system_stability_score']}"
                )
            else:
            ...
            if active_tasks > self.config["max_active_tasks"]:
                result.warnings.append(
                    f"活动任务数过多: {active_tasks} > {self.config['max_active_tasks']}"
                )
    ...
    def _perform_graceful_transition(self) -> bool:
        ...
        try:
            ...
            while self._has_active_tasks() and (time.time() - wait_start) < timeout:
                self.logger.info(
                    f"等待任务完成... ({self._get_active_task_count()} 个活动任务)"
                )
                time.sleep(1.0)

            if self._has_active_tasks():
                self.logger.warning(
                    f"优雅切换超时，仍有 {self._get_active_task_count()} 个活动任务"
                )
                return False
    ...
    def _update_statistics(
        self, success: bool, transition_type: TransitionType, duration_ms: int
    ):
        """更新统计信息"""
    ...
    def get_transition_history(self, limit: int = 50) -> List[Dict[str, Any]]:
        ...
        """
        return (
            self.transition_history[-limit:] if limit > 0 else self.transition_history
        )
        ...
        return {
            **self.stats,
            "current_transition_id": (
                self.current_transition.get("id") if self.current_transition else None
            ),
            "current_transition_state": (
                self.current_transition.get("state")
                if self.current_transition
                else None
            ),
    ...
    def reset(self):
        ...
        with self.transition_lock:
            ...
            self.transition_history = (
                self.transition_history[-10:] if self.transition_history else []
            )
...
def get_mode_transition_protocol(
