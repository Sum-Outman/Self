...
class DatabaseManager:
    ...
    def validate_database_schema(self) -> Dict[str, Any]:
        ...
        if missing_tables:
            validation_result["overall"] = False
            validation_result["recommendations"].append(f"创建缺失的表: {missing_tables}")
    ...
    def repair_database(self, auto_fix: bool = True) -> Dict[str, Any]:
        ...
        try:
            ...
            if not validation["connection"]["status"]:
                repair_result["errors"].append(f"数据库连接失败: {validation['connection']['message']}")
            ...
            if current_rev is None or current_rev != "head":
                if auto_fix:
                    ...
                    repair_result["warnings"].append("Alembic版本不是最新，建议运行迁移")
...
def main():
    ...
    parser.add_argument("--migrate", action="store_true", help="运行Alembic迁移到最新版本")
    parser.add_argument("--auto-fix", action="store_true", help="自动修复问题（配合--repair使用）")
    ...
    if args.check:
        ...
        print(f"数据库连接: {'✅ 成功' if validation['connection']['status'] else '❌ 失败'}")
        if not validation['connection']['status']:
