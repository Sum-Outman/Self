...
class Config:
    ...
    if _secret_key is None:
        ...
        if env == "production":
            ...
            logger.critical(
                f"安全警告：SECRET_KEY未设置，已生成随机值。"
                f"开发环境可以使用此值，但生产环境必须设置环境变量SECRET_KEY。"
    ...
    def validate_config(cls):
        ...
        if env == "production":
            ...
            if len(cls.SECRET_KEY) < 32:
                logger.warning("生产环境警告：SECRET_KEY长度小于32个字符，建议使用更长的密钥")
            
            if cls.DATABASE_URL.startswith("sqlite"):
                logger.warning("生产环境警告：使用SQLite数据库，建议使用PostgreSQL或MySQL")
            
            # 检查数据库URL中的默认密码
            if "your_mongodb_password" in cls.MONGODB_URL:
                logger.warning("生产环境警告：MongoDB URL中包含默认密码，请设置环境变量MONGODB_URL")
            
            if "your_rabbitmq_password" in cls.RABBITMQ_URL:
                logger.warning("生产环境警告：RabbitMQ URL中包含默认密码，请设置环境变量RABBITMQ_URL")
            
            # 检查Redis URL（开发环境默认没有密码）
            if cls.REDIS_URL == "redis://localhost:6379/0":
                logger.warning("生产环境警告：Redis使用默认URL且无密码，建议设置密码并使用环境变量REDIS_URL")
            ...
            sensitive_defaults = [
                ...
                ("ALIPAY_PRIVATE_KEY", cls.ALIPAY_PRIVATE_KEY, "your_alipay_private_key"),
            ]
            
            for var_name, value, default in sensitive_defaults:
                if value == default:
                    logger.warning(f"生产环境警告：{var_name}使用默认值，请设置环境变量{var_name}")
            
            # 记录安全配置状态
            logger.info("生产环境配置安全检查完成")
        else:
            # 开发环境：记录配置验证状态
    ...
    # 性能监控配置
    PERFORMANCE_MONITOR_INTERVAL = float(os.getenv("PERFORMANCE_MONITOR_INTERVAL", "5.0"))
    PERFORMANCE_ALERT_THRESHOLD_CPU = float(os.getenv("PERFORMANCE_ALERT_THRESHOLD_CPU", "80.0"))
    PERFORMANCE_ALERT_THRESHOLD_MEMORY = float(os.getenv("PERFORMANCE_ALERT_THRESHOLD_MEMORY", "85.0"))
    PERFORMANCE_ALERT_THRESHOLD_DISK = float(os.getenv("PERFORMANCE_ALERT_THRESHOLD_DISK", "90.0"))
    ...
    # 数据库查询优化配置
    SLOW_QUERY_THRESHOLD = float(os.getenv("SLOW_QUERY_THRESHOLD", "100.0"))  # 慢查询阈值（毫秒）
    MAX_QUERY_HISTORY = int(os.getenv("MAX_QUERY_HISTORY", "1000"))  # 最大查询历史记录数
    DATABASE_MONITOR_INTERVAL = float(os.getenv("DATABASE_MONITOR_INTERVAL", "60.0"))  # 数据库监控间隔（秒）
    QUERY_CACHE_ENABLED = os.getenv("QUERY_CACHE_ENABLED", "true").lower() == "true"  # 查询缓存是否启用
    QUERY_CACHE_TTL = int(os.getenv("QUERY_CACHE_TTL", "300"))  # 查询缓存TTL（秒）
    MAX_DATABASE_CONNECTIONS = int(os.getenv("MAX_DATABASE_CONNECTIONS", "100"))  # 最大数据库连接数