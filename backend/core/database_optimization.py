...
from sqlalchemy import event, text, inspect, Index, or_, and_
...
class QueryPerformanceMetrics:
    ...
    def record_query(self, query: str, execution_time_ms: float, params: Optional[Dict] = None):
        """记录查询执行时间"""
        # 生成查询指纹（移除参数差异）
        ...
        if execution_time_ms > self.slow_query_threshold:
            ...
            logger.warning(f"慢查询检测: {execution_time_ms:.2f}ms > {self.slow_query_threshold}ms")
            logger.debug(f"查询: {query[:100]}...")
    
    def record_optimization(self, original_query: str, optimized_query: str, 
                          improvement_percent: float, original_time_ms: float, 
                          optimized_time_ms: float):
        """记录查询优化结果"""
        optimization_info = {
            "original_query": original_query[:200] + ("..." if len(original_query) > 200 else ""),
            "optimized_query": optimized_query[:200] + ("..." if len(optimized_query) > 200 else ""),
        ...
        logger.info(f"查询优化完成: 性能提升 {improvement_percent:.1f}% "
    ...
    def get_performance_report(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "recent_optimizations": self.optimized_queries[-5:] if self.optimized_queries else [],
            "query_statistics": {
                "unique_queries": len(self.query_counts),
                "most_frequent_query": max(self.query_counts.items(), key=lambda x: x[1]) if self.query_counts else None,
                "average_execution_time_ms": sum(avg_times.values()) / len(avg_times) if avg_times else 0,
            }
        }
...
class QueryOptimizer:
    ...
    def _setup_sqlalchemy_events(self):
        ...
        def after_execute(conn, clauseelement, multiparams, params, result):
            ...
            if hasattr(self, '_current_query_start'):
                execution_time = (time.time() - self._current_query_start) * 1000  # 转换为毫秒
                query_str = self._current_query if hasattr(self, '_current_query') else str(clauseelement)
        ...
        try:
            ...
            for column in columns:
                column_info = {
                    ...
                    "is_primary_key": column["name"] in primary_keys.get("constrained_columns", []),
                    "has_index": any(idx["column_names"] == [column["name"]] for idx in indexes),
            ...
            return {
                ...
                "overall_suggestions": self._generate_table_suggestions(column_analysis, index_analysis)
            }
    ...
    def _analyze_column(self, column_info: Dict[str, Any], table_name: str) -> List[str]:
        """分析列并提供优化建议"""
        suggestions = []
        ...
        if not column_info["has_index"] and not column_info["is_primary_key"]:
            ...
            if column_info["name"].endswith("_id"):
                ...
                suggestions.append(f"考虑为时间戳列 {column_info['name']} 添加索引以优化时间范围查询")
        
        # 检查数据类型
        column_type = str(column_info["type"]).lower()
        if "text" in column_type and column_info["nullable"]:
            suggestions.append(f"Text类型列 {column_info['name']} 可为空，考虑添加非空约束或使用默认值")
    ...
    def _analyze_index(self, index_info: Dict[str, Any], table_name: str) -> List[str]:
        ...
        if len(index_info["columns"]) > 1:
            suggestions.append(f"复合索引 {index_info['name']} 包含 {len(index_info['columns'])} 列，确保最频繁查询的列在前")
        
        # 检查索引是否被过度使用
        if len(index_info["columns"]) > 3:
            suggestions.append(f"索引 {index_info['name']} 包含过多列（{len(index_info['columns'])}），考虑拆分为多个索引")
        
        return suggestions
    
    def _generate_table_suggestions(self, columns: List[Dict[str, Any]], indexes: List[Dict[str, Any]]) -> List[str]:
        """生成表的整体优化建议"""
        suggestions = []
        
        # 统计需要索引的列
        columns_needing_index = sum(1 for col in columns 
                                  if not col["has_index"] and not col["is_primary_key"] 
                                  and col["name"].endswith("_id"))
        
        if columns_needing_index > 0:
            suggestions.append(f"有 {columns_needing_index} 个外键列缺少索引，建议添加索引以优化关联查询")
...
class DatabaseMonitor:
    ...
    def _collect_metrics(self):
        ...
        try:
            with engine.connect() as conn:
                ...
                # 存储指标历史
                if connection_stats:
                    self.metrics_history["connection_count"].append((timestamp, connection_stats[0]))
                    self.metrics_history["active_connections"].append((timestamp, connection_stats[1]))
                    self.metrics_history["idle_connections"].append((timestamp, connection_stats[2]))
                ...
                logger.debug(f"数据库指标收集完成: {len(table_sizes)}个表, {len(index_stats)}个索引")
    ...
    def _check_alerts(self):
        ...
        try:
            # 检查连接数异常
            if "connection_count" in self.metrics_history and self.metrics_history["connection_count"]:
                recent_connections = [val for _, val in self.metrics_history["connection_count"][-5:]]
    ...
    def get_monitoring_report(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "active_alerts": [alert for alert in self.alerts if alert.get("acknowledged", False) == False],
